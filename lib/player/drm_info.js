 ...
 *
 * @fileoverview Defines the DrmInfo class.
 */
...
/**
 * Represents application provided DRM configuration options.
 * <br>
 *
 * The following options are supported:
 * <ul>
 * <li>
 *   keySystem: string (required) <br>
 *   The key system, e.g., "com.widevine.alpha".
 *
 * <li>
 *   licenseServerUrl: string (required) <br>
 ...
 * responses before they are passed to the key system.
 * </p>
 *
 * <p>
...
/**
 * A license request descriptor for license request pre-processing
 * (see {@link shaka.player.DrmInfo.LicensePreProcessor}).
 * <br>
 *
 * The following options are supported:
 * <ul>
...
shaka.player.DrmInfo.createFromConfig = function(config) {
  ...
  if (keySystem) {
    ...
    throw new Error('\'keySystem\' cannot be null or empty.');
  }
  ...
  if (licenseServerUrl) {
    ...
    throw new Error('\'licenseServerUrl\' cannot be null or empty.');
  }
...
/**
 * Creates a DrmInfo object from a DrmSchemeInfo object.
 *
 * @param {shaka.player.DrmSchemeInfo} drmSchemeInfo
 * @return {!shaka.player.DrmInfo}
 * TODO: Remove when DrmSchemeInfo is removed.
 */
shaka.player.DrmInfo.createFromDrmSchemeInfo = function(drmSchemeInfo) {
  var drmInfo = new shaka.player.DrmInfo();

  drmInfo.keySystem = drmSchemeInfo.keySystem;
  drmInfo.licenseServerUrl = drmSchemeInfo.licenseServerUrl;
  drmInfo.withCredentials = drmSchemeInfo.withCredentials;
  drmInfo.licensePostProcessor = drmSchemeInfo.licensePostProcessor;
  drmInfo.licensePreProcessor = drmSchemeInfo.licensePreProcessor;
  drmInfo.distinctiveIdentifierRequired =
      drmSchemeInfo.distinctiveIdentifierRequired;
  drmInfo.persistentStateRequired = drmSchemeInfo.persistentStateRequired;
  drmInfo.audioRobustness = drmSchemeInfo.audioRobustness;
  drmInfo.videoRobustness = drmSchemeInfo.videoRobustness;

  // Clone.
  drmInfo.serverCertificate =
      drmSchemeInfo.serverCertificate ?
      new Uint8Array(drmSchemeInfo.serverCertificate.buffer) :
      null;

  // Clone.
  drmInfo.initDatas = drmSchemeInfo.initDatas.map(
      function(initData) {
        return {
...
/**
 * Checks that this DrmInfo and |other| are compatible and combines the
 * initialization datas and key IDs of each into this DrmInfo.
 *
...
shaka.player.DrmInfo.prototype.combine = function(other) {
  if (!this.isCompatible(other)) {
    shaka.log.warning(
        'Incompatible DrmInfo objects:', 'this', this, 'other', other);
  }

  this.addInitDatas(other.initDatas);
  this.addKeyIds(other.keyIds);
};
...
shaka.player.DrmInfo.prototype.addInitDatas = function(
    otherInitDatas) {
  var unfilteredInitDatas = this.initDatas.concat(otherInitDatas);

  /**
   * @param {!shaka.player.DrmInfo.InitData} initData
   * @return {string}
   */
  var initDataKey = function(initData) {
    // FIXME: initData.initData may be very large; on some browsers using such
    // a large string as a key into an object may cause problems.
    return shaka.util.Uint8ArrayUtils.key(initData.initData) + ',' +
           initData.initDataType;
  };

  this.initDatas = shaka.util.ArrayUtils.removeDuplicates(
      unfilteredInitDatas, initDataKey);
};


/**
 * Adds the given key IDs, as hex strings (removing duplicates).
 *
 * @param {!Array.<string>} otherKeyIds
 */
shaka.player.DrmInfo.prototype.addKeyIds = function(otherKeyIds) {
  var unfilteredKeyIds = this.keyIds.concat(otherKeyIds);
  this.keyIds = shaka.util.ArrayUtils.removeDuplicates(unfilteredKeyIds);
};
