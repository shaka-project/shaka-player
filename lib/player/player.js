...
/**
 * @param {Object} restrictions An Object containing the restrictions
 *     on video tracks that you wish to set. For example, passing
 *     {'maxBandwidth': 700000, 'minBandwidth': 200000} will restrict
...
shaka.player.Player.prototype.setRestrictions = function(restrictions) {
  for (var key in restrictions) {
    if (this.restrictions_.hasOwnProperty(key)) {
      this.restrictions_[key] = restrictions[key];
    }
  }
  this.videoSource_.setRestrictions(this.restrictions_);
};
...
shaka.player.Player.prototype.getRestrictions = function() {
  var restrictions = {};
  for (var key in this.restrictions_) {
    restrictions[key] = this.restrictions_[key];
  }
  return restrictions;
};
