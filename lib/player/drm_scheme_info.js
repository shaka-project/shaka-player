 ...
 * @param {boolean} suppressMultipleEncryptedEvents If true, only the first
 *     'encrypted' event will be used.  This is desired for some applications.
...
shaka.player.DrmSchemeInfo =
    function(keySystem, suppressMultipleEncryptedEvents, licenseServerUrl,
             withCredentials, initData, licensePostProcessor) {
  /** @type {string} */
  this.keySystem = keySystem;

  /** @type {boolean} */
  this.suppressMultipleEncryptedEvents = suppressMultipleEncryptedEvents;
 ...
 *
 * Any restrictions on playback can be set by the callback on the Restrictions
 * object passed to it.
 *
 * Returns the raw license after application-specific headers have been removed.
 *
 * @typedef {function(!Uint8Array,
                      !shaka.player.DrmSchemeInfo.Restrictions):!Uint8Array}
 */
...
shaka.player.DrmSchemeInfo.createUnencrypted = function() {
  return new shaka.player.DrmSchemeInfo('', false, '', false, null, null);
};


/**
 * Combine two DrmSchemeInfos.  Their |keySystem|,
 * |suppressMultipleEncryptedEvents|, and |withCredentials| members must match.
 *
...
shaka.player.DrmSchemeInfo.combine = function(a, b) {
  ...
  shaka.asserts.assert(a.suppressMultipleEncryptedEvents ==
                       b.suppressMultipleEncryptedEvents,
                       'suppress mismatch');
  shaka.asserts.assert(a.withCredentials == b.withCredentials,
                       'credentials mismatch');

  var c = new shaka.player.DrmSchemeInfo(
      a.keySystem, a.suppressMultipleEncryptedEvents, a.licenseServerUrl,
      a.withCredentials, null, a.licensePostProcessor);
