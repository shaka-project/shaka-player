...
shaka.player.StreamVideoSource.prototype.attach = function(player, video) {
  ...
  this.video_.src = window.URL.createObjectURL(this.mediaSource);
...
shaka.player.StreamVideoSource.prototype.onMediaSourceOpen_ =
  ...
  // Keep track of the best start time, we will start each stream from this
  // time.
  var bestStartTime = 0;
  ...
  for (var i = 0; i < selectedStreamSetInfos.length; ++i) {
    ...
    var addStream = (function(contentType, streamInfo) {
      ...
      // Update |bestStartTime|.
      if (contentType != 'text') {
        // If the StreamInfo has a best start time then use it; otherwise,
        // fallback to the start time of the first segment.
        if (streamInfo.bestStartTime != null) {
          bestStartTime = Math.max(bestStartTime, streamInfo.bestStartTime);
        } else {
          shaka.asserts.assert(streamInfo.segmentIndex);
          var reference = streamInfo.segmentIndex.getReference(0);
          if (reference) {
            bestStartTime = Math.max(bestStartTime, reference.startTime);
          }
        }
      }
    ...
    // Get the stream's SegmentIndex before adding the stream since we might
    // need it to update |bestStartTime|.
  ...
  p.then(shaka.util.TypedBind(this,
      function() {
        this.startStreams_(selectedStreamInfosByType, bestStartTime);
 ...
 * @param {number} bestStartTime
 * @private
 */
shaka.player.StreamVideoSource.prototype.startStreams_ = function(
    selectedStreamInfosByType, bestStartTime) {
  shaka.asserts.assert(this.video_);

  shaka.log.debug('bestStartTime', bestStartTime);
  this.video_.currentTime = bestStartTime;

  // Start streams.
  for (var contentType in this.streamsByType_) {
    var stream = this.streamsByType_[contentType];
    shaka.asserts.assert(stream);
    this.eventManager.listen(stream, 'ended', this.onStreamEnded_.bind(this));
