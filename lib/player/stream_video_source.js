...
shaka.player.StreamVideoSource.prototype.computeStreamLimits = function() {
  ...
  for (var i = 0; i < streamSetInfos.length; ++i) {
    ...
    for (var j = 0; j < streamSetInfo.streamInfos.length; ++j) {
      ...
      if (!segmentIndex || segmentIndex.getNumReferences() == 0) continue;

      var firstReference = segmentIndex.getReference(0);
      shaka.asserts.assert(firstReference);
      startTime = Math.max(startTime, firstReference.startTime);

      if (streamInfo.currentSegmentStartTime != null) {
        endTime = Math.min(endTime, streamInfo.currentSegmentStartTime);
      } else {
        var lastReference =
            segmentIndex.getReference(segmentIndex.getNumReferences() - 1);
        shaka.asserts.assert(lastReference);
        if (lastReference.endTime != null) {
          endTime = Math.min(endTime, lastReference.endTime);
        }
      }
    }
  }
