...
goog.require('shaka.media.SegmentedTextStream');
...
shaka.player.StreamVideoSource.prototype.selectConfigurations = function(
  ...
  shaka.asserts.assert(types.length, 'No configs were selected!');
...
shaka.player.StreamVideoSource.prototype.createStreams_ = function(
  ...
  for (var type in streamInfosByType) {
    ...
    var isTextStream = type === 'text';
    var isSegmented = streamInfo.segmentIndexSource.representation_ ?
        !!streamInfo.segmentIndexSource.representation_.segmentTemplate : false;
    var isSegmentedTextStream = isTextStream && isSegmented;
    var stream;

    if (isSegmentedTextStream) {
     stream = this.createSegmentedTextStream_();
    } else if (isTextStream) {
     stream = this.createTextStream_();
    } else {
     stream = this.createStream_(streamInfo);
    }
...
/**
 * Creates a text stream that is build up from segments/vtt cue strings
 *
 * @return {!shaka.media.SegmentedTextStream}
 * @private
 */
shaka.player.StreamVideoSource.prototype.createSegmentedTextStream_ =
    function() {
  shaka.asserts.assert(this.video);
  var video = /** @type {!HTMLVideoElement} */ (this.video);
  var textTracks = /** @type {!TextTrackList} */ (video.textTracks);
  return new shaka.media.SegmentedTextStream(
      this,
      video,
      textTracks
  );
