 ...
 *   of the playhead, in seconds, after stream startup.
 *
 * <li>
...
shaka.player.StreamVideoSource.prototype.getBufferingGoal = function() {
  // Use the minimum buffering goal among all Streams, so none of the Streams
  // get stuck; ignore any Streams that don't need to buffer anything since
  // they can't get stuck.
  var minGoal = null;
  for (var type in this.streamsByType_) {
    var goal = this.streamsByType_[type].getBufferingGoal();
    if (goal > 0) {
      minGoal = minGoal != null ? Math.min(minGoal, goal) : goal;
    }
  }
  return minGoal || 0;
};
...
shaka.player.StreamVideoSource.prototype.onKeyStatusesChange = function(
  ...
  for (var keyId in keyStatusByKeyId) {
    ...
    if (!COMPILED && !streamInfos) {
      shaka.log.debug(
          'The key system is using an unknown key:',
          shaka.util.StringUtils.formatHexString(keyId));
      continue;
    }
    ...
    for (var i = 0; i < streamInfos.length; ++i) {
      ...
      streamInfo.allowedByKeySystem = message === undefined;
...
shaka.player.StreamVideoSource.prototype.selectStreamInfos_ = function(
  ...
  for (var i = 0; i < streamSetInfos.length; ++i) {
    ...
    shaka.asserts.assert(
        streamInfo,
        'Each StreamSetInfo should contain at least one usable StreamInfo.');
