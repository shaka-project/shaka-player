...
shaka.player.StreamVideoSource = function(manifestInfo, estimator) {
  ...
  /**
   * The initial stream start time, which does not include a timestamp
   * correction.
   * @private {number}
   */
...
shaka.player.StreamVideoSource.prototype.createAndStartStreams_ = function() {
  ...
  return this.startTask_.getPromise().then(shaka.util.TypedBind(this,
      ...
      function(error) {
        ...
        // Reset the video's playback rate; otherwise, when we try to start the
        // streams again we may erroneously set |originalPlaybackRate_| to 0.
...
shaka.player.StreamVideoSource.prototype.onStreamStarted_ = function(event) {
  ...
  if (this.minTimestampCorrection_ * this.maxTimestampCorrection_ >= 0) {
    this.correctSegmentIndexes_();
  } else {
    shaka.log.warning(
        'Some streams\' media timestamps are ahead of their SegmentIndexes,',
        'while other streams\' are behind.',
        'The content may have errors in it.');
  }

  this.onStreamsStarted();
};


/**
 * Correct the SegmentIndexes and currentSegmentStartTime of each usable (and
 * available) StreamInfo.
 * @private
 */
shaka.player.StreamVideoSource.prototype.correctSegmentIndexes_ = function() {
  ...
  for (var i = 0; i < streamSetInfos.length; ++i) {
    ...
    for (var j = 0; j < streamSetInfo.streamInfos.length; ++j) {
      ...
      if (streamInfo.currentSegmentStartTime != null) {
        streamInfo.currentSegmentStartTime += this.maxTimestampCorrection_;
      }
    }
  }
};
...
/**
 * Streams started hook. Called when each stream has started. This function is
 * only called once.
 *
 * The default implementation adjusts the video's current time using the
 * timestamp correction and then starts the video.
 *
 * @protected
 */
shaka.player.StreamVideoSource.prototype.onStreamsStarted = function() {
  shaka.asserts.assert(this.video);
