...
shaka.ads.InterstitialAdManager = class {
  ...
  /**
   * Returns a copy of |uri| with control characters and whitespace
   * removed, for scheme classification only. Browsers trim these
   * characters when resolving the scheme of a URL, so a URI whose scheme is
   * split or prefixed by them must be classified on the stripped form.
   * The outgoing URI itself is never modified.
   *
   * @param {string} uri
   * @return {string}
   * @private
   */
  static normalizeUri_(uri) {
  ...
  static isSafeClickThroughUri_(uri) {
    const scheme = shaka.util.URL.getScheme(
        shaka.ads.InterstitialAdManager.normalizeUri_(uri));
