...
shaka.ads.InterstitialAdManager = class {
   ...
   */
  constructor(adContainer, basePlayer, onEvent) {
    /** @private {HTMLElement} */
    ...
    this.baseVideo_ = this.basePlayer_.getMediaElement();

    /** @private {?HTMLMediaElement} */
    this.adVideo_ = null;
    if (this.adContainer_) {
      this.adVideo_ = this.createMediaElement_();
      this.adContainer_.appendChild(this.adVideo_);
    }

    /** @private {boolean} */
    this.usingBaseVideo_ = !this.adContainer_;

    /** @private {HTMLMediaElement} */
    this.video_ = this.usingBaseVideo_ ? this.baseVideo_ : this.adVideo_;
  ...
  configure(config) {
    this.config_ = config;
    if (!this.adContainer_ ||
        this.usingBaseVideo_ == this.config_.supportsMultipleMediaElements) {
      return;
    }
    this.usingBaseVideo_ = !this.config_.supportsMultipleMediaElements;
   ...
   */
  async addMetadata(interstitial, playerConfig) {
    this.player_.configure(playerConfig);
  ...
  async setupAd_(interstitial, sequenceLength, adPosition, initialTime) {
    ...
    const basicTask = async () => {
      ...
      if (nextCurrentInterstitial) {
        ...
        if (offset == null) {
          goog.asserts.assert(typeof(initialTime) == 'number',
              'Should be an number!');
        ...
        if (!this.usingBaseVideo_) {
          if (offset) {
            this.baseVideo_.currentTime += offset;
          }
