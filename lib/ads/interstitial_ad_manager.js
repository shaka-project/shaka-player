  ...
  async setupAd_(interstitial, sequenceLength, adPosition, initialTime,
    ...
    this.adEventManager_.listen(this.video_, 'timeupdate', () => {
      ...
      if (currentTime >= 0.25 * duration &&
          !eventsSent.includes('firstquartile')) {
        updateBaseVideoTime();
        this.onEvent_(
            new shaka.util.FakeEvent(shaka.ads.Utils.AD_FIRST_QUARTILE));
        eventsSent.push('firstquartile');
      } else if (currentTime >= 0.5 * duration &&
          !eventsSent.includes('midpoint')) {
        updateBaseVideoTime();
        this.onEvent_(
            new shaka.util.FakeEvent(shaka.ads.Utils.AD_MIDPOINT));
        eventsSent.push('midpoint');
      } else if (currentTime >= 0.75 * duration &&
          !eventsSent.includes('thirdquartile')) {
        updateBaseVideoTime();
        this.onEvent_(
            new shaka.util.FakeEvent(shaka.ads.Utils.AD_THIRD_QUARTILE));
        eventsSent.push('thirdquartile');
      }
    });
