...
shaka.ads.InterstitialAdManager = class {
  ...
  async setupVideoAd_(interstitial, sequenceLength, adPosition, initialTime,
      oncePlayed) {
    shaka.log.alwaysWarn('setupVideoAd_', interstitial);
    goog.asserts.assert(this.video_, 'Must have video');
    const startTime = Date.now();
    ...
    const basicTask = async (isSkip, isBadHttpStatus) => {
      shaka.log.alwaysWarn('basicTask', isSkip, isBadHttpStatus, interstitial);
      if (!isBadHttpStatus) {
        updateBaseVideoTime();
      }
      ...
      this.determineIfUsingBaseVideo_(this.isSmartTV_());
      if (nextCurrentInterstitial) {
        this.sendEvent_(shaka.ads.Utils.AD_STOPPED);
        this.adEventManager_.removeAll();
        this.setupAd_(nextCurrentInterstitial, sequenceLength,
    ...
    try {
      ...
      shaka.log.alwaysWarn('loaded', interstitial,
          playerStartTime, this.video_.currentTime, adPosition);
      if (!interstitial.overlay || !this.baseVideo_.paused || adPosition > 1) {
        this.video_.play();
      } else {
        this.video_.pause();
      }
