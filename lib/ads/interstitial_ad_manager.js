...
shaka.ads.InterstitialAdManager = class {
  ...
  addRegion(region) {
    ...
    // Apply RequestParam URL params (urlparam:2025, includeInRequests=altmpd)
    if (region.urlParams) {
      const queryParams = region.urlParams();
      if (queryParams) {
        const url = new URL(interstitialUri);
        new URLSearchParams(queryParams).forEach((value, key) => {
          url.searchParams.set(key, value);
        });
        interstitialUri = url.href;
      }
    }
