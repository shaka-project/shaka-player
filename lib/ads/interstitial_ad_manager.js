...
shaka.ads.InterstitialAdManager = class {
  ...
  constructor(adContainer, basePlayer, baseVideo, onEvent) {
    ...
    /** @private {!Set<string>} */
    this.hlsInterstitialIds_ = new Set();
  ...
  stop() {
    ...
    this.hlsInterstitialIds_.clear();
  ...
  async addMetadata(hlsInterstitial) {
    ...
    if (id) {
      if (this.hlsInterstitialIds_.has(id)) {
        return;
      }
      this.hlsInterstitialIds_.add(id);
    }
    this.updatePlayerConfig_();
    let adInterstitials = [];
    if (hlsInterstitial &&
  ...
  async getInterstitialsInfo_(hlsInterstitial) {
    ...
    if (resumeOffset != null && resumeOffset != 0 &&
        endTime != Infinity &&
        (startTime + resumeOffset == endTime)) {
      resumeOffset = null;
    }
    let playoutLimit = null;
    const playout =
              ...
              if (payloadSlot) {
                ...
                if (payloadSlot.tracking) {
                  interstitial.tracking =
                      this.createTracking_(payloadSlot.tracking);
                }
              }
            }
  ...
  sendTrackingEvent_(url) {
    const NetworkingEngine = shaka.net.NetworkingEngine;
    const context = {
      type: NetworkingEngine.AdvancedRequestType.INTERSTITIAL_TRACKING_EVENT,
    };
    const type = shaka.net.NetworkingEngine.RequestType.ADS;
    const request = shaka.net.NetworkingEngine.makeRequest(
        [url],
