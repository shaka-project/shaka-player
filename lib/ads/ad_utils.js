...
shaka.ads.Utils = class {
      ...
      for (const creative of TXml.findChildren(creatives, 'Creative')) {
        ...
        if (linear) {
          ...
          shaka.ads.Utils.processLinearAd_(
              interstitials, currentTime, linear, tracking);
        }
        ...
        if (nonLinearAds) {
          ...
          for (const nonLinear of nonLinears) {
            ...
            shaka.ads.Utils.processNonLinearAd_(
                interstitials, currentTime, nonLinear, tracking);
          }
        }
      }
    }
   ...
   * @private
   */
  static processLinearAd_(interstitials, currentTime, linear, tracking) {
    const TXml = shaka.util.TXml;
    let startTime = 0;
    if (currentTime != null) {
  ...
  static createTrackingFromInline(inline) {
    ...
    const error = TXml.findChild(inline, 'Error');
    if (error) {
    ...
    const impression = TXml.findChild(inline, 'Impression');
    if (impression) {
