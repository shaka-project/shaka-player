...
goog.require('shaka.ads.DashInterstitialParser');
...
/**
 * @event shaka.ads.AdManager.AdPlayingEvent
 * @description Fired when an ad actually starts playback.
 *   This event is triggered when the ad's media element enters the
 *   'playing' state, indicating that the ad is truly rendering and
 *   its playhead is advancing.
 *
 *   Unlike AdStartedEvent, which signals the intent to start an ad,
 *   this event guarantees that playback has begun successfully.
 *
 * @property {string} type
 *   'ad-playing'
 * @property {!shaka.extern.IAd} ad
 *   The ad that is now playing.
 * @exportDoc
 */
 ...
 *    Indicate at what position in the original content the break has started.
 * @exportDoc
 */
...
/**
 * @event shaka.ads.AdManager#AdInterstitialPreloadedEvent
 * @description Fired when the ad manager finish the preload of an interstitial.
 * @property {string} type
 *   'ad-interstitial-preloaded'
 * @property {shaka.extern.AdInterstitial} interstitial
 *    The interstitial preloaded.
 * @exportDoc
 */
...
shaka.ads.AdManager = class extends shaka.util.FakeEventTarget {
  ...
  initMediaTailor_() {
    ...
    this.mtAdManager_ = new shaka.ads.MediaTailorAdManager(
        ...
        (e) => this.processAndDispatchEvent_(e),
        (start, end) => this.player_.addSkipRange(start, end),
        (start, end) => this.player_.evictRange(start, end));
    goog.asserts.assert(this.config_, 'Config must not be null!');
    this.mtAdManager_.configure(this.config_);
  }
  ...
  async onHLSMetadata(metadata) {
    if (metadata.type == 'com.apple.quicktime.HLS') {
    ...
    } else if (metadata.type == 'com.apple.hls.preload') {
      if (this.config_ && this.config_.disableHLSInterstitial) {
        return;
      }
      if (!this.interstitialAdManager_) {
      ...
      if (this.interstitialAdManager_) {
        this.interstitialAdManager_.addPreloadMetadata(metadata);
      }
  ...
  onDASHMetadata(region) {
    ...
    if (schemeIdUri == 'urn:google:dai:2018') {
    ...
    } else if (shaka.ads.DashInterstitialParser.isInterstitialRegion(region)) {
      ...
      if (this.interstitialAdManager_) {
        const interstitial =
            shaka.ads.DashInterstitialParser.parseRegion(region);
        if (interstitial) {
          this.interstitialAdManager_.addInterstitials([interstitial]);
        }
      }
