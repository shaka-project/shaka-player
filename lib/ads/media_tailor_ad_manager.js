...
shaka.ads.MediaTailorAdManager = class {
   ...
   * @param {function(number, number)} addSkipRange
   *     Excludes [start, end) from streaming; see shaka.Player#addSkipRange.
   */
  constructor(adContainer, networkingEngine, video, onEvent, addSkipRange) {
    /** @private {HTMLElement} */
    ...
    this.addSkipRange_ = addSkipRange;

    /** @private {?Promise.PromiseWithResolvers<string>} */
    ...
    /** @private {boolean} */
    this.adBreakListenersAttached_ = false;
  }
  ...
  stop() {
    ...
    this.onEnded_();
    this.adListeners_ = [];
    this.eventsSent_.clear();
    this.trackingUrl_ = '';
    this.firstTrackingRequest_ = true;
    this.backupUrl_ = '';
    this.snapForwardTime_ = null;
    this.adBreaks_ = [];
    this.playedAds_.clear();
   ...
   */
  getCuePoints() {
    /** @type {!Map<string, !shaka.extern.AdCuePoint>} */
    const cuePointsMap = new Map();
    ...
    for (const adBreak of this.adBreaks_) {
      for (const ad of adBreak.ads) {
        ...
        if (!cuePointsMap.has(key)) {
          cuePointsMap.set(key, {start, end});
        }
      }
    }

    return Array.from(cuePointsMap.values()).sort(
        (a, b) => a.start - b.start);
  }
  ...
  async requestSessionInfo_(url, adsParams) {
    ...
    try {
      ...
      if (dataAsJson.manifestUrl && dataAsJson.trackingUrl) {
        this.trackingUrl_ = shaka.util.URL.resolve(url, dataAsJson.trackingUrl);
        const now = Date.now() / 1000;
        const loadTime = now - this.streamRequestStartTime_;
        this.onEvent_(new shaka.util.FakeEvent(shaka.ads.Utils.ADS_LOADED,
            (new Map()).set('loadTime', loadTime)));
        const resolvedManifestUrl =
            shaka.util.URL.resolve(url, dataAsJson.manifestUrl);
        this.streamPromise_.resolve(resolvedManifestUrl);
        this.streamPromise_ = null;
      } else {
      ...
      if (!this.backupUrl_.length) {
        this.streamPromise_.reject(new shaka.util.Error(
            shaka.util.Error.Severity.RECOVERABLE,
            shaka.util.Error.Category.ADS,
            shaka.util.Error.Code.MEDIATAILOR_REQUEST_FAILED));
        this.streamPromise_ = null;
        return;
      }
  ...
  async requestTrackingInfo_(trackingUrl, firstRequest) {
    ...
    try {
      ...
      if (dataAsJson.avails.length > 0) {
        ...
        if (ObjectUtils.alphabeticalKeyOrderStringify(this.adBreaks_) !=
            ObjectUtils.alphabeticalKeyOrderStringify(dataAsJson.avails)) {
      ...
      shaka.log.warning('Error fetching tracking info', e);
    }
  }
  ...
  async requestStaticResource_(nonLinearAd) {
    ...
    if (this.staticResources_.has(cacheKey)) {
      return;
    }
  ...
  setupAdBreakListeners_() {
    if (this.adBreakListenersAttached_) {
      return;
    }
    this.adBreakListenersAttached_ = true;
    this.onTimeupdate_();
    if (!this.isLive_) {
  ...
  checkForSnapback_() {
    ...
    if ((this.config_ && this.config_.disableSnapback) ||
      currentTime == 0 ||
      this.snapForwardTime_ != null) {
      return;
    }
    ...
    for (const adBreak of this.adBreaks_) {
      for (const ad of adBreak.ads) {
        ...
        } else if (previousAd.durationInSeconds > 0 &&
            ad.startTimeInSeconds < currentTime && ad.startTimeInSeconds >
    ...
    if (previousAdBreak && previousAd &&
        !this.playedAds_.has(previousAd.adId)) {
  ...
  sendInProgressEvents_(currentTime, ad) {
    ...
    if (currentTime >= firstQuartileTime &&
        !this.eventsSent_.has(MediaTailorAdManager.FIRSTQUARTILE_)) {
      this.eventsSent_.add(MediaTailorAdManager.FIRSTQUARTILE_);
      this.sendTrackingEvent_(MediaTailorAdManager.FIRSTQUARTILE_);
    } else if (currentTime >= midpointTime &&
        !this.eventsSent_.has(MediaTailorAdManager.MIDPOINT_)) {
      this.eventsSent_.add(MediaTailorAdManager.MIDPOINT_);
      this.sendTrackingEvent_(MediaTailorAdManager.MIDPOINT_);
    } else if (currentTime >= thirdQuartileTime &&
        !this.eventsSent_.has(MediaTailorAdManager.THIRDQUARTILE_)) {
      this.eventsSent_.add(MediaTailorAdManager.THIRDQUARTILE_);
    ...
    for (const adBreak of this.adBreaks_) {
      ...
      for (let i = 0; i < adBreak.ads.length; i++) {
        ...
        if (startTime <= currentTime && endTime > currentTime) {
          // Seek past a played ad.  A fully-watched break is also excluded from
          // streaming as a skip range in onEnded_; this handles partial breaks
          // and the window before that gap forms.
  ...
  /**
   * Drops a fully-watched ad break from the buffer and excludes it from future
   * streaming as one skip range spanning the whole break, so the individual
   * ads are neither re-fetched nor replayed on a seek back.
   *
   * @param {mediaTailor.AdBreak} adBreak
   * @private
   */
  skipPlayedAdBreak_(adBreak) {
    if (!adBreak.ads.length) {
      return;
    }
    const first = adBreak.ads[0];
    const last = adBreak.ads[adBreak.ads.length - 1];
    const start = first.startTimeInSeconds;
    const end = last.startTimeInSeconds + last.durationInSeconds;
    if (end <= start) {
      return;
    }
    // The break is behind the playhead now, so its content is not re-fetched.
    // The skip forms the gap once that buffered content is evicted; until then
    // a seek back plays it, which is acceptable.
    this.addSkipRange_(start, end);
  }
   ...
   */
  onEnded_() {
    if (this.ad_ && this.mediaTailorAd_) {
      // Remove all child nodes
      if (this.adContainer_) {
        shaka.util.Dom.removeAllChildren(this.adContainer_);
      }
      if (!this.isLive_) {
        this.playedAds_.add(this.mediaTailorAd_.adId);
      }
      ...
      if (position === totalAdsInBreak) {
        ...
        // The break is now fully watched and behind the playhead.  Skip it
        // unless the app opted to keep played ads playable.
        if (!this.isLive_ && this.mediaTailorAdBreak_ &&
            !(this.config_ && this.config_.disablePlayedLinearAdSkip)) {
          this.skipPlayedAdBreak_(this.mediaTailorAdBreak_);
        }
      }
      this.ad_ = null;
      this.mediaTailorAd_ = null;
  ...
  setupCurrentAdListeners_() {
    ...
    this.adListeners_.push({
      ...
      type: 'volumechange',
      listener: () => {
        this.sendTrackingEvent_(this.video_.muted ?
            MediaTailorAdManager.MUTE_ : MediaTailorAdManager.UNMUTE_);
      },
    });
    this.adListeners_.push({
      target: this.video_,
  ...
  sendTrackingEvent_(eventType) {
    if (!this.mediaTailorAd_ || !this.mediaTailorAdBreak_) {
      return;
    }
