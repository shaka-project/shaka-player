...
shaka.ads.MediaTailorAdManager = class {
   ...
   * @param {function(number, number)} evictRange
   *     Removes buffered media in [start, end); see shaka.Player#evictRange.
   */
    ...
    for (const adBreak of this.adBreaks_) {
      ...
      for (let i = 0; i < adBreak.ads.length; i++) {
        ...
        if (startTime <= currentTime && endTime > currentTime) {
          // Seek past a played ad.  Skipped when using skip ranges (the break
          // is already excluded from streaming in onEnded_).
          if (!(this.config_ && this.config_.disablePlayedLinearAdSkip) &&
              !(this.config_ && this.config_.skipPlayedAdsUsingSkipRanges) &&
  ...
  skipPlayedAdBreak_(adBreak) {
    ...
    // Evict before adding the skip range, so addSkipRange's buffered check
    // sees a cleared span and accepts the (now unbuffered) range.
    this.evictRange_(start, end).then(() => {
  ...
  onEnded_() {
    if (this.ad_ && this.mediaTailorAd_) {
      ...
      if (position === totalAdsInBreak) {
        ...
        // The break is now fully watched and behind the playhead.
