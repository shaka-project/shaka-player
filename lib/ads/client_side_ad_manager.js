...
shaka.ads.ClientSideAdManager = class {
   ...
   * @param {google.ima.AdsManager} imaAdsManager
   * @private
   */
  async startAds_(imaAdsManager) {
    const AutoPlay = new shaka.media.AutoPlay();
    ...
    const {result} = await AutoPlay.canAutoPlay();
    if (result === true) {
  ...
  onAdsManagerLoaded_(e) {
    ...
    try {
      ...
      this.startAds_(this.imaAdsManager_);
    } catch (adError) {
