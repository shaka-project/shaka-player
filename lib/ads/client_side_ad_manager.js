...
goog.require('shaka.media.AutoPlay');
...
shaka.ads.ClientSideAdManager = class {
  ...
  /**
   * Simple call to IMA's start() method
   */
  startAdManager(imaAdsManager) {
    if (imaAdsManager && imaAdsManager.start) {
      imaAdsManager.start();
    }
  }

  /**
   * Start ads after checking autoplay
   */
  startAds(imaAdsManager) {
    const canAutoPlay = new shaka.media.AutoPlay();

    // IMA won't crash if autoplay is allowed, so we
    // start the Ad Manager with no user interaction.
    // If playback restrictions are in place, we wait for the video to be played
    canAutoPlay.video().then(({result}) => {
      if (result[0] === true) {
        shaka.log.info('Autoplay allowed. Starting ads.');
        this.startAdManager(imaAdsManager);
      } else {
        shaka.log.warning(
            'Playback restrictions prevented ad breaks from starting.',
            'A user must interact with the document first.',
        );
  ...
  onAdsManagerLoaded_(e) {
    ...
    try {
      ...
      this.startAds(this.imaAdsManager_);
    } catch (adError) {
