...
shaka.drm.DrmEngine = class {
  ...
  static configureClearKey(configClearKeys, variants) {
    ...
    // ClearKey config overrides the manifest DrmInfo if present.
    // The variants are modified so that filtering in Player still works.
    // We need do this to detect that there is no need to use a persistent
    // license in this case.
    /** @type {?shaka.extern.DrmInfo} */
    const clearKeyDrmInfo = getClearKeyDrmInfo();
    if (clearKeyDrmInfo) {
      for (const variant of variants) {
        if (variant.video) {
