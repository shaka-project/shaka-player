...
shaka.drm.DrmEngine = class {
  ...
  initForPlayback(variants, offlineSessionIds, isLive = true) {
    ...
    for (const sessionId of offlineSessionIds) {
      this.storedPersistentSessions_.set(sessionId, {
        initData: null,
        initDataType: null,
      });
    }
    ...
    for (const metadata of this.config_.persistentSessionsMetadata) {
      this.storedPersistentSessions_.set(metadata.sessionId, {
  ...
  async init_(variants, isLive) {
    ...
    for (const variant of variants) {
      if (variant.video && !expandedStreams.has(variant.video)) {
        variant.video.drmInfos = expandRobustness(
      ...
      if (variant.audio && !expandedStreams.has(variant.audio)) {
        variant.audio.drmInfos = expandRobustness(
  ...
  async attach(video) {
    ...
    if (
      ...
      this.eventManager_.listen(this.video_, 'encrypted', (e) =>
        this.onEncryptedEvent_(e),
      );
    }
  }
    ...
    if (
      ...
      try {
        const operation = this.playerInterface_.netEngine.request(
            ...
            request,
            {isPreload: this.isPreload_()},
        );
        const response = await operation.promise;

        this.currentDrmInfo_.serverCertificate = shaka.util.BufferUtils.toUint8(
            response.data,
        );
      } catch (error) {
        // Request failed!
  ...
  async createOrLoad() {
    if (this.storedPersistentSessions_.size) {
      ...
      const keyIds =
        (this.currentDrmInfo_ && this.currentDrmInfo_.keyIds) || new Set([]);
  ...
  async queryMediaKeys_(variants) {
    ...
    const mediaKeySystemAccess = this.getKeySystemAccessFromVariants_(
        variants,
        drmInfosByKeySystem,
    );
    ...
    try {
      ...
      shaka.log.v2('Got MediaKeySystemAccess with configuration', realConfig);
      ...
      this.currentDrmInfo_ = this.createDrmInfoByInfos_(
          keySystem,
          drmInfosByKeySystem.get(keySystem),
      );
  ...
  setLoadSessionTimeoutTimer_(metadata) {
    ...
    timer.tickAfter(/* seconds= */ shaka.drm.DrmEngine.SESSION_LOAD_TIMEOUT_);
  }
  ...
  async loadOfflineSession_(sessionId, sessionMetadata) {
    ...
    this.eventManager_.listen(session, 'keystatuseschange', (event) =>
      this.onKeyStatusesChange_(event),
    );
  ...
  async sendLicenseRequest_(event) {
    ...
    if (
      event.messageType == 'individualization-request' &&
      advancedConfig &&
    ...
    if (shaka.drm.DrmUtils.isClearKeySystem(this.currentDrmInfo_.keySystem)) {
      this.fixClearKeyRequest_(request, this.currentDrmInfo_);
    }

    if (
      shaka.drm.DrmUtils.isPlayReadyKeySystem(this.currentDrmInfo_.keySystem)
    ) {
  ...
  unpackPlayReadyRequest_(request) {
    ...
    goog.asserts.assert(
        challenge.attributes['encoding'] == 'base64encoded',
  ...
  onKeyStatusesChange_(event) {
    ...
    keyStatusMap.forEach((status, keyId) => {
      ...
      const device = shaka.device.DeviceFactory.getDevice();
      if (
        shaka.drm.DrmUtils.isPlayReadyKeySystem(
            this.currentDrmInfo_.keySystem,
        ) &&
        keyId.byteLength == 16 &&
      ...
      if (!found) {
        ...
        goog.asserts.assert(
            status != 'usable',
            'Usable keys found in closed session',
        );
      }
  ...
  static async probeSupport() {
    ...
    const playreadyRobustness = ['150', '2000', '3000'];
    ...
    const testEncryptionSchemes = [null, 'cenc', 'cbcs'];
    ...
    const processMediaKeySystemAccess = async (keySystem, access, testHdcp) => {
      ...
      let persistentState = sessionTypes ?
        sessionTypes.includes('persistent-license') :
        false;
    ...
    const testSystemEme = async (
      ...
      try {
        ...
        if (device.getDeviceType() == shaka.device.IDevice.DeviceType.MOBILE) {
          access = await shaka.util.Functional.promiseWithTimeout(
   ...
   * @private
   */
  processSessionCloseReason_(closeReason) {
  ...
  handleSessionCloseReason_(session, closeReason) {
    ...
    // Ignore unknown closeReason (e.g. Safari doesn't return a reason).
    if (closeReason === 'unknown') {
      return;
    }

    if (closeReason === 'hardware-context-reset') {
  ...
  onSessionClosed_(session, metadata, closeReason) {
    ...
    shaka.log.info(
        'CDM closed session with reason:',
        closeReason,
        'sessionId:',
        session.sessionId,
    );
  ...
  recreateSessionAfterHardwareReset_(metadata) {
    shaka.log.info(
        'Recreating session after hardware context reset',
        'initDataType:',
        metadata.initDataType,
    );
    ...
    try {
      ...
      shaka.log.error(
          'Failed to recreate session after hardware reset:',
          error,
      );
      this.onError_(
          new shaka.util.Error(
  ...
  async closeOpenSessions_() {
    ...
    await Promise.all(
        openSessions.map(async ([session, metadata]) => {
          try {
          /**
           ...
           * We should remove the session to prevent it from being orphaned
           * after the playback session ends
           */
            if (
              !this.initializedForStorage_ &&
  ...
  createDrmInfoByInfos_(keySystem, drmInfos) {
    ...
    if (encryptionSchemes.length > 1) {
      shaka.log.warning(
          'Multiple unique encryption schemes found! ' +
    ...
    const defaultSessionType = this.usePersistentLicenses_ ?
      'persistent-license' :
      'temporary';
  ...
  static processDrmInfos_(
    ...
    for (const drmInfo of drmInfos) {
      ...
      if (
        drmInfo.keySystem == 'org.w3.clearkey' &&
      ...
      if (drmInfo.serverCertificate) {
        const found = serverCerts.some((cert) =>
          shaka.util.BufferUtils.equal(cert, drmInfo.serverCertificate),
        );
        if (!found) {
      ...
      if (drmInfo.initData) {
        for (const initDataOverride of drmInfo.initData) {
          const found = initDatas.some((initData) =>
