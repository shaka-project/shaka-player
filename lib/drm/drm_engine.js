...
shaka.drm.DrmEngine = class {
  ...
  handleSessionCloseReason_(session, closeReason) {
    shaka.log.info('Session closed with reason:', closeReason,
        'sessionId:', session.sessionId);
  ...
  onSessionClosed_(session, metadata, closeReason) {
    shaka.log.info('CDM closed session with reason:', closeReason,
        'sessionId:', session.sessionId);

    this.activeSessions_.delete(session);

    if (closeReason === 'hardware-context-reset') {
      shaka.log.warning('Hardware context reset detected. ' +
          'Recreating session...');
  ...
  recreateSessionAfterHardwareReset_(metadata) {
    ...
    if (!metadata.initDataType || !metadata.initData) {
      shaka.log.error('Cannot recreate session: missing initData or ' +
          'initDataType');
      this.onError_(new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.DRM,
    ...
    try {
      ...
      shaka.log.error('Failed to recreate session after hardware reset:',
          error);
      this.onError_(new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.DRM,
