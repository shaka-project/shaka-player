...
shaka.drm.DrmEngine = class {
  ...
  async attach(video) {
     ...
     *
     * For src= mode, MediaKeys must be attached before video.src is set.
     * The browser may error if the media parser encounters encrypted content
     * without keys attached.
     */
    if (!needWaitForEncryptedEvent &&
      ...
      await this.attachMediaKeys_();
    } else if (this.srcEquals_) {
      // Force early MediaKeys attachment for src= mode.
      await this.attachMediaKeys_();
    }
    ...
    // 'encrypted' events.
    // In src= mode, always listen for encrypted events — the license flow
