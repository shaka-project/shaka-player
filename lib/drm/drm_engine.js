...
shaka.drm.DrmEngine = class {
  ...
  async attach(video) {
    ...
    if (remote) {
      this.eventManager_.listen(remote, 'connect',
          () => this.closeOpenSessions_());
      this.eventManager_.listen(remote, 'connecting',
          () => this.closeOpenSessions_());
      this.eventManager_.listen(remote, 'disconnect',
          () => this.closeOpenSessions_());
  ...
  async retryLicensing(sessionMetadata, retryDelaySeconds) {
    const delay = retryDelaySeconds || 0.1;
    ...
    goog.asserts.assert(targetSession, 'targetSession should exist');
    goog.asserts.assert(targetMetadata, 'targetMetadata should exist');
    ...
    if (delay > 0) {
      await new Promise((resolve) => {
        new shaka.util.Timer(resolve).tickAfter(delay);
      });
    }
