...
shaka.drm.DrmEngine = class {
  ...
  async attach(video) {
     ...
     *
     * For FairPlay in src= mode on Safari, MediaKeys must be attached before
     * video.src is set. Safari errors with MEDIA_ERR_SRC_NOT_SUPPORTED if
     * the native HLS parser encounters #EXT-X-KEY without keys attached.
     */
    if (!needWaitForEncryptedEvent &&
      ...
      // Force early MediaKeys attachment for FairPlay src= mode on Safari.
      await this.attachMediaKeys_();
    }
    ...
    // 'encrypted' events.
    // Always listen for encrypted events for FairPlay — the license flow
    // depends on them even though MediaKeys are attached early.
    if (needWaitForEncryptedEvent ||
        this.currentDrmInfo_.keySystem === 'com.apple.fps' ||
