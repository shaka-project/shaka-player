...
shaka.drm.DrmEngine = class {
  ...
  constructor(playerInterface) {
    ...
    this.allSessionsLoaded_ = Promise.withResolvers();
    ...
    const noop = () => {};
    this.allSessionsLoaded_.promise.catch(noop);
  ...
  async destroyNow_() {
    ...
    if (this.video_) {
      // Only clear MediaKeys this engine actually set. With
      // config.drm.keepDrmEngineOnDetach, another (kept) DrmEngine may have
      // its own MediaKeys still bound to this same video element.
      if (this.mediaKeys_) {
        ...
        try {
          ...
          // Ignore any failures while removing media keys from the video
          // element.
  ...
  async attach(video) {
    ...
    const needWaitForEncryptedEvent =
        device.needWaitForEncryptedEvent(keySystem);
    /**
  ...
  async removeSession(sessionId) {
    ...
    if (found) {
      ...
      // handled.
      found.updatePromise = Promise.withResolvers();
      tasks.push(found.updatePromise.promise);
    }
  ...
  async createOrLoad() {
    if (this.storedPersistentSessions_.size) {
      ...
      await this.allSessionsLoaded_.promise;
      ...
      if (keyIds.size > 0 && this.areAllKeysUsable_()) {
        return this.allSessionsLoaded_.promise;
      }
      ...
      this.hasInitData_ = false;
      this.allSessionsLoaded_ = Promise.withResolvers();
      this.allSessionsLoaded_.promise.catch(() => {});
    }
  ...
  updateCurrentDrmInfo(drmInfo) {
    ...
    if (this.currentDrmInfo_.mediaTypes !== drmInfo.mediaTypes) {
      this.currentDrmInfo_.mediaTypes = drmInfo.mediaTypes;
    }
    // With raw keys, it is necessary to merge the information and build a new
    // license, which is why it is necessary to reprocess the DrmInfos.
    if (shaka.drm.DrmUtils.isClearKeyWihRawKey(drmInfo)) {
      this.currentDrmInfo_ = this.createDrmInfoByInfos_(
          this.currentDrmInfo_.keySystem, [this.currentDrmInfo_, drmInfo]);
    }
  }
  ...
  renewLicense(sessionId) {
    this.activeSessions_.forEach((_metadata, session) => {
      if (sessionId == undefined || session.sessionId ==
        sessionId) {
        this.triggerRenewal_(/** @type {!MediaKeySession} */(session));
      }
    });
  }

  /**
   * Retry licensing by closing and recreating sessions.
   * This is useful when license requests fail and need to be retried
   * from scratch.
   *
  ...
  async retryLicensing(sessionMetadata, retryDelaySeconds = 0.1) {
    this.destroyer_.ensureNotDestroyed();
    shaka.log.info('Retrying licensing for session', sessionMetadata.sessionId);

    /** @type {?MediaKeySession} */
    let targetSession = null;
    /** @type {?shaka.drm.DrmEngine.SessionMetaData} */
    let targetMetadata = null;

    this.activeSessions_.forEach((metadata, session) => {
      if (session.sessionId === sessionMetadata.sessionId) {
        targetSession = session;
        targetMetadata = metadata;
      }
    });

    if (!targetSession || !targetMetadata) {
      shaka.log.warning('Session not found for retry:',
          sessionMetadata.sessionId);
      return false;
    }

    if (!this.currentDrmInfo_) {
      shaka.log.warning('No DRM info available for retry');
      return false;
    }

    if (!targetMetadata.initData || !targetMetadata.initDataType) {
      shaka.log.error('Missing initData or initDataType for session retry');
      return false;
    }

    const initData = targetMetadata.initData;
    const initDataType = targetMetadata.initDataType;
    ...
    try {
      this.activeSessions_.delete(targetSession);
      await this.closeSession_(
          /** @type {!MediaKeySession} */ (targetSession));
      ...
      if (!newSession) {
        throw new shaka.util.Error(
            ...
            shaka.util.Error.Category.DRM,
            shaka.util.Error.Code.FAILED_TO_CREATE_SESSION,
            'Failed to create new session for license retry');
      }

      shaka.log.info('Successfully created new session for retry:',
          newSession.sessionId);
      return true;
    } catch (error) {
      shaka.log.error('Failed to retry licensing:', error);
      return false;
    }
  }
   ...
   */
  createSession(initDataType, initData, sessionType, isRenewal = false) {
    goog.asserts.assert(this.mediaKeys_,
    ...
    session.closed
        .then((closeReason) => {
          ...
          const metadata = this.activeSessions_.get(session);
          if (metadata) {
            this.onSessionClosed_(session, metadata, closeReason || 'unknown');
          }
        })
        .catch((error) => {
          shaka.log.warning('session.closed rejected:', error);
        });
  ...
  async sendLicenseRequest_(event) {
    ...
    if (metadata) {
      ...
      if (metadata.isRenewal) {
        request.licenseRequestType = 'license-renewal';
      }
    }
    ...
    try {
      ...
      // Call failureCallback if configured
      if (this.config_.failureCallback) {
        try {
          this.config_.failureCallback(shakaErr);
        } catch (callbackError) {
          shaka.log.error('Error in failureCallback:', callbackError);
        }
      }

      // Check if the error was handled by the callback
      if (shakaErr.handled) {
        shaka.log.info('License request failure handled by failureCallback');
    ...
    try {
      ...
      if (metadata) {
        metadata.lastRenewalTime = Date.now() / 1000;
      }
    } catch (error) {
  ...
  async closeSession_(session) {
    ...
    try {
      const results = await shaka.util.Functional.promiseWithTimeout(
          shaka.drm.DrmEngine.CLOSE_TIMEOUT_,
          Promise.all([session.close().catch(() => {}), session.closed]),
      );
      closeReason = results[1];
    } catch (e) {
      shaka.log.warning('Timeout waiting for session close');
    }

    if (closeReason) {
      this.handleSessionCloseReason_(session, closeReason);
    }
  }

  /**
   * Process close reason and handle logging/errors.
   * Common logic for all session close reason types.
   * See https://github.com/w3c/encrypted-media/pull/487
   *
   * @param {string} closeReason - The MediaKeySessionClosedReason value
   * @param {?shaka.drm.DrmEngine.SessionMetaData=} metadata
   * @private
   */
  processSessionCloseReason_(closeReason, metadata) {
    if (closeReason === 'internal-error') {
      shaka.log.error('CDM internal error. Cannot create new sessions.');
      this.onError_(new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.DRM,
          shaka.util.Error.Code.FAILED_TO_CREATE_SESSION,
          'CDM reported internal error'));
    } else if (closeReason === 'closed-by-application') {
      shaka.log.v1('Session closed by application');
    } else if (closeReason === 'release-acknowledged') {
      shaka.log.v1('Session closed: license release acknowledged');
    } else if (closeReason === 'resource-evicted') {
      shaka.log.warning('Session closed: resources evicted');
    } else if (closeReason === 'hardware-context-reset') {
      if (metadata) {
        shaka.log.warning(
            'Hardware context reset detected. Recreating session...');
        this.recreateSessionAfterHardwareReset_(metadata);
      } else {
        shaka.log.warning('Hardware context reset detected during app close');
      }
    } else {
      shaka.log.warning('Session closed with unknown reason:', closeReason);
    }
  }

  /**
   * Handle session close reason from MediaKeySessionClosedReason enum.
   * Called when application closes a session.
   * See https://github.com/w3c/encrypted-media/pull/487
   *
   * @param {!MediaKeySession} session
   * @param {string} closeReason - The MediaKeySessionClosedReason value
   * @private
   */
  handleSessionCloseReason_(session, closeReason) {
    shaka.log.info('Session closed with reason',
        closeReason, 'sessionId', session.sessionId);

    // Ignore unknown closeReason
    // (e.g. Safari and old browsers don't return a reason).
    if (closeReason === 'unknown') {
      return;
    }

    this.processSessionCloseReason_(closeReason);
  }

  /**
   * Called when session.closed Promise resolves (CDM-initiated closure).
   *
   * @param {!MediaKeySession} session
   * @param {!shaka.drm.DrmEngine.SessionMetaData} metadata
   * @param {string} closeReason - The MediaKeySessionClosedReason value
   * @private
   */
  onSessionClosed_(session, metadata, closeReason) {
    // Ignore unknown closeReason
    // (e.g. Safari and old browsers don't return a reason).
    if (closeReason === 'unknown') {
      return;
    ...
    shaka.log.info('CDM closed session with reason',
        closeReason, 'sessionId', session.sessionId);

    this.activeSessions_.delete(session);

    this.processSessionCloseReason_(closeReason, metadata);
  }

  /**
   * Recreate a session after hardware context reset.
   * This is called when the CDM closes a session due to hardware changes
   * (e.g., device sleep/wake, GPU switch).
   *
   * @param {!shaka.drm.DrmEngine.SessionMetaData} metadata
   * @private
   */
  recreateSessionAfterHardwareReset_(metadata) {
    shaka.log.info('Recreating session after hardware context reset',
        'initDataType', metadata.initDataType);

    if (!metadata.initDataType || !metadata.initData) {
      shaka.log.error(
          'Cannot recreate session: missing initData or initDataType');
      this.onError_(new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.DRM,
          shaka.util.Error.Code.FAILED_TO_CREATE_SESSION,
          'Cannot recreate session after hardware reset: missing metadata'));
      return;
    }

    try {
      this.createSession(
          metadata.initDataType, metadata.initData, metadata.type);
      shaka.log.info('Session successfully recreated after hardware reset');
    } catch (error) {
      shaka.log.error(
          'Failed to recreate session after hardware reset:', error);
      this.onError_(new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.DRM,
          shaka.util.Error.Code.FAILED_TO_CREATE_SESSION,
          'Failed to recreate session after hardware context reset: ' +
          error.message));
    }
  }
  ...
  pollExpiration_() {
    ...
    this.activeSessions_.forEach((metadata, session) => {
      ...
      if (this.config_.renewalIntervalSec > 0) {
        const timeSinceRenewal = now - metadata.lastRenewalTime;
        if (metadata.lastRenewalTime > 0 &&
            timeSinceRenewal >= this.config_.renewalIntervalSec) {
          this.triggerRenewal_(session);
        }
      }
    });
  }
   ...
   * @private
   */
  async triggerRenewal_(session) {
    shaka.log.info('Triggering license renewal for session', session.sessionId);

    // Update lastRenewalTime to prevent repeated triggers
    const metadata = this.activeSessions_.get(session);
    if (metadata) {
      metadata.lastRenewalTime = Date.now() / 1000;
    }
    ...
    /** @type {shaka.extern.DrmSessionMetadata} */
    const oldSessionMetadata = {
      sessionId: session.sessionId,
      sessionType: this.currentDrmInfo_.sessionType,
      initData: metadata ? metadata.initData : null,
      initDataType: metadata ? metadata.initDataType : null,
    };

    try {
      if (shaka.drm.DrmUtils.isFairPlayKeySystem(keySystem)) {
        const renewMessage = shaka.util.StringUtils.toUTF8('renew');
        await session.update(renewMessage);
      } else if (shaka.drm.DrmUtils.isPlayReadyKeySystem(keySystem) &&
          metadata && metadata.initData && metadata.initDataType) {
        newSession = await this.requestPlayReadyLicenseForRenewal_(
            session, metadata);
      } else {
        shaka.log.alwaysWarn('Manual license renewal is not supported for ' +
            keySystem);
        return;
      }
    } catch (error) {
      shaka.log.error('Failed to renew license:', error);
      return;
    }

    const newMetadata = this.activeSessions_.get(newSession);

    /** @type {shaka.extern.DrmSessionMetadata} */
    const newSessionMetadata = {
      sessionId: newSession.sessionId,
      sessionType: this.currentDrmInfo_.sessionType,
      initData: newMetadata ? newMetadata.initData : null,
      initDataType: newMetadata ? newMetadata.initDataType : null,
    };

    const event = new shaka.util.FakeEvent('licenserenewal', (new Map())
        .set('oldSessionMetadata', oldSessionMetadata)
        .set('newSessionMetadata', newSessionMetadata));
    this.playerInterface_.onEvent(event);
  }

  /**
   * Requests a new license for renewal for PlayReady.
   * @param {!MediaKeySession} session
   * @param {!shaka.drm.DrmEngine.SessionMetaData} metadata
   * @return {!Promise<!MediaKeySession>}
   * @private
   */
  async requestPlayReadyLicenseForRenewal_(session, metadata) {
    goog.asserts.assert(metadata.initDataType, 'initDataType must be defined');
    goog.asserts.assert(metadata.initData, 'initData must be defined');
    this.activeSessions_.delete(session);
    await this.closeSession_(session);
    const newSession = this.createSession(metadata.initDataType,
        metadata.initData, this.currentDrmInfo_.sessionType,
        /* isRenewal= */ true);
    if (!newSession) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.DRM,
          shaka.util.Error.Code.FAILED_TO_CREATE_SESSION,
          'Failed to create new session for renewal');
    }
    return newSession;
  }
  ...
  static fillInDrmInfoDefaults_(drmInfo, servers, advancedConfigs,
    ...
    if (shaka.drm.DrmUtils.isClearKeySystem(originalKeySystem) &&
 ...
 *   type: string,
 *   updatePromise: Promise.PromiseWithResolvers,
 ...
 *   The session type
 * @property {Promise.PromiseWithResolvers} updatePromise
 ...
 *   remove().
 * @property {number} lastRenewalTime
 *   The time in seconds when the session renewal was issued.
 * @property {boolean} isRenewal
 *   True if this session was created as part of a license renewal process.
 *   This is used to correctly set the licenseRequestType to 'license-renewal'
 *   instead of 'license-request' for PlayReady renewal sessions.
 */
shaka.drm.DrmEngine.SessionMetaData;
