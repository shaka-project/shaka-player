...
shaka.drm.DrmEngine = class {
  ...
  pollExpiration_() {
    ...
    this.activeSessions_.forEach((metadata, session) => {
      ...
      if (this.config_.enableAutoRenewal &&
          this.config_.renewalIntervalSec > 0) {
  ...
  triggerRenewal_(session) {
    ...
    if (this.currentDrmInfo_) {
      ...
      if (keySystem == 'com.apple.fps' || keySystem == 'com.apple.fps.1_0') {
        const renewMessage = shaka.util.StringUtils.toUTF8('renew');
        session.update(renewMessage).catch((error) => {
          shaka.log.error('Failed to trigger FairPlay renewal:', error);
        });
      }

      // PlayReady: re-request license using stored initData
      if (shaka.drm.DrmUtils.isPlayReadyKeySystem(keySystem) &&
          metadata && metadata.initData) {
        this.requestLicenseForRenewal_(session, metadata);
      }
    }

    const event = new shaka.util.FakeEvent('licenserenewal',
        (new Map()).set('sessionId', session.sessionId));
    this.playerInterface_.onEvent(event);
  }

  /**
   * Requests a new license for renewal (used for PlayReady).
   * @param {!MediaKeySession} session
   * @param {!shaka.drm.DrmEngine.SessionMetaData} metadata
   * @private
   */
  async requestLicenseForRenewal_(session, metadata) {
    const requestType = shaka.net.NetworkingEngine.RequestType.LICENSE;
    const url = this.currentDrmInfo_.licenseServerUri;

    const request = shaka.net.NetworkingEngine.makeRequest(
        [url], this.config_.retryParameters);
    request.body = metadata.initData;
    request.method = 'POST';
    ...
    try {
      const req = this.playerInterface_.netEngine.request(
          requestType, request, {isPreload: false});
      const response = await req.promise;
      await session.update(response.data);
      shaka.log.info('PlayReady license renewed for session',
          session.sessionId);
    } catch (error) {
      shaka.log.error('Failed to renew PlayReady license:', error);
    }
  }
 ...
 *   The time in seconds that the session was last renewed.
 */
shaka.drm.DrmEngine.SessionMetaData;
