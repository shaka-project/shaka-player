...
shaka.drm.DrmEngine = class {
  ...
  async triggerRenewal_(session) {
    ...
    try {
      // FairPlay specific: send 'renew' message.
      if (shaka.drm.DrmUtils.isFairPlayKeySystem(keySystem)) {
        ...
        // PlayReady: close session and create a new one
        await this.requestPlayReadyLicenseForRenewal_(session, metadata);
      } else {
        // Other key systems (Widevine, etc.) are not supported
    ...
    const event = new shaka.util.FakeEvent('licenserenewal',
        (new Map()).set('detail', sessionMetadata));
  ...
  async requestPlayReadyLicenseForRenewal_(session, metadata) {
    ...
    this.activeSessions_.delete(session);
    await this.closeSession_(session);
    this.createSession(metadata.initDataType, metadata.initData,
        this.currentDrmInfo_.sessionType);
  }
