...
shaka.drm.FairPlay = class {
  ...
  static defaultGetContentId(initData) {
    ...
    // sample.
    return shaka.util.URL.getDomain(uriString);
  }
  ...
  static defaultGetKeyId(uriString) {
    const contentId = shaka.util.URL.getDomain(uriString);
   ...
   * @export
   */
  static gumletFairPlayRequest(type, request, context) {
    if (type !== shaka.net.NetworkingEngine.RequestType.LICENSE ||
        request.attempt > 0) {
      return;
    }
    ...
    const bodyU8 = shaka.util.BufferUtils.toUint8(request.body);
    const spcB64 = shaka.util.Uint8ArrayUtils.toBase64(bodyU8);
    request.body = shaka.util.StringUtils.toUTF8(JSON.stringify({spc: spcB64}));
