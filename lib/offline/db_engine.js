...
shaka.offline.DBEngine.prototype.init = function() {
  ...
  return self.createConnection_()
      .then(function() {
        return self.createTransaction_(MANIFEST, READ_ONLY, function(store) {
          DBEngine.getLastId_(store, function(id) {
            self.nextManifestId_ = id;
          });
        });
      })
      .then(function() {
        return self.createTransaction_(SEGMENT, READ_ONLY, function(store) {
          DBEngine.getLastId_(store, function(id) {
            self.nextSegmentId_ = id;
          });
        });
      });
};
 ...
 * @param {function(number)} onId
 * @private
 */
shaka.offline.DBEngine.getLastId_ = function(store, onId) {
  /** @type {IDBRequest} */
