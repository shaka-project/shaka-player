...
shaka.offline.DBUpgradeFromVersion1.prototype.upgrade = function(
  ...
  /** @type {number } */
  var pendingManifests = 0;
  /** @type {number } */
  var completedManifests = 0;

  var onSegmentUpgraded = function() {
    completedSegments += 1;
    if (pendingSegments == completedSegments) {
      db.deleteObjectStore(oldSegments);
    }
  };

  var onManifestUpgraded = function() {
    completedManifests += 1;
    if (pendingManifests == completedManifests) {
      db.deleteObjectStore(oldManifests);
    }
  };

  forEach(oldSegmentStore, function(oldKey, oldValue) {
    ...
    // and segment datas.
    put(newSegmentStore, oldKey, newSegment, onSegmentUpgraded);
  });

  forEach(oldManifestStore, function(oldKey, oldValue) {
    ...
    put(newManifestStore, oldKey, newValue, onManifestUpgraded);
  });
};
