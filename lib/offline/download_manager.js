...
shaka.offline.DownloadManager = class {
   ...
   */
  constructor(networkingEngine, drmEngine, manifest, onProgress) {
  ...
  queue(groupId, request, estimatedByteLength, onDownloaded) {
    ...
    this.groups_.set(groupId, group.then(async () => {
      ...
      if (shaka.util.Pssh.containsPssh(new Uint8Array(response), true)) {
        this.updateInitData_(new Uint8Array(response));
      }
  ...
  updateInitData_(initData) {
    const normalisedInitData = shaka.util.Pssh.normaliseInitData(initData, true);
    this.drmEngine_.newInitData('cenc', normalisedInitData);
    ...
    const variants = shaka.util.Periods.getAllVariantsFrom(this.manifest_.periods);
    this.drmEngine_.init_(variants);
  }
