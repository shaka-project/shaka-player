...
shaka.offline.DownloadManager = class {
  ...
  constructor(onProgress) {
    /**
     * We sill store download requests in groups. The groups will be downloaded
     * in parallel but the segments in each group will be done serially.
     *
    ...
    /**
     * When a segment is downloaded, the actual size of the segment will be
     * added to this value so that we know exactly how many bytes we have
     * downloaded.
     *
     * @private {number}
     */
  ...
  download(net) {
    ...
    // Amend our new promise chain to our internal promise so that when we
    // destroy the download manger we will wait for all the downloads to stop.
