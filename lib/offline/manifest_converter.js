...
shaka.offline.ManifestConverter = class {
  ...
  fromPeriodDB(period, timeline) {
    ...
    const audioStreams =
        period.streams.filter((stream) => this.isAudio_(stream));
    /** @type {!Array.<shaka.extern.StreamDB>} */
    const videoStreams =
        period.streams.filter((stream) => this.isVideo_(stream));
    ...
    for (const stream of period.streams) {
      ...
      const refs = stream.segments.map((segment, index) => {
  ...
  createVariants(audios, videos) {
    ...
    for (const stream of audios) {
      for (const id of stream.variantIds) {
        variantIds.add(id);
      }
    }
  ...
  fromStreamDB_(streamDB) {
    /** @type {!Array.<!shaka.media.SegmentReference>} */
    const segments = streamDB.segments.map((segment, index) =>
