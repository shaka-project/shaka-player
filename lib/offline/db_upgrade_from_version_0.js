 ...
 * @override
 */
shaka.offline.DBUpgradeFromVersion0.prototype.upgrade = function(db) {
  /** @const */
  var Store = shaka.offline.DBUtils.Store;

  goog.asserts.assert(db.objectStoreNames.length == 0,
                      'Version 0 database should be empty');

  db.createObjectStore(Store.MANIFEST, {autoIncrement: true});
  db.createObjectStore(Store.SEGMENT, {autoIncrement: true});
};
