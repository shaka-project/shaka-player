...
shaka.offline.Storage.prototype.store = function(
  ...
  return this.initIfNeeded_()
      .then(function() {
        ...
        return this.loadInternal(
          ...
          function(data) {
            ...
            manifestId = this.storageEngine_.reserveManifestId();
      ...
      .then(function() {
        /** @type {string} */
        var uri = shaka.offline.OfflineScheme.manifestIdToUri(manifestId);
        return shaka.offline.OfflineUtils.createStoredContentFromManifestDB(
            uri,
            manifestDb);
      }.bind(this))
      .catch(function(err) {
...
shaka.offline.Storage.prototype.remove = function(content) {
  ...
  goog.asserts.assert(
      content.offlineUri,
      'Cannot remove content that has not finished downloading.');
  ...
  .bind(this)).then(function() {
    ...
    var segments = manifestDb.periods.map(function(period) {
      return period.streams.map(function(stream) {
        ...
        var segments = stream.segments.map(function(segment) {
          var segmentId = OfflineScheme.uriToSegmentId(segment.uri);
          goog.asserts.assert(segmentId != null, 'Invalid offline URI');
          return segmentId;
        });

        if (stream.initSegmentUri) {
          var segmentId = OfflineScheme.uriToSegmentId(stream.initSegmentUri);
          goog.asserts.assert(segmentId != null, 'Invalid offline URI');
    ...
    return this.storageEngine_.removeSegments(segments, function() {
      deleteCount++;
    ...
    return this.storageEngine_.removeManifests([manifestId], null);
  }.bind(this));
};
...
shaka.offline.Storage.prototype.list = function() {
  ...
  return this.initIfNeeded_()
      .then(function() {
        this.checkDestroyed_();
        return this.storageEngine_.forEachManifest(function(manifestDB) {
          var content = OfflineUtils.createStoredContentFromManifestDB(
              shaka.offline.OfflineScheme.manifestIdToUri(manifestDB.key),
              manifestDB);
...
shaka.offline.Storage.prototype.createOfflineManifest_ = function(
    originalManifestUri, manifestId, metadata) {
...
shaka.offline.Storage.prototype.createStream_ = function(
  ...
  shaka.offline.Storage.forEachSegment_(stream, startTime, function(segment) {
    ...
    /** @type {number} */
    var id = this.storageEngine_.reserveSegmentId();
