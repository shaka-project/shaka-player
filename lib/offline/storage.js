...
shaka.offline.Storage.prototype.remove = function(content) {
  ...
  return this.initIfNeeded_().then(function() {
  ...
  }.bind(this)).then(function(/** ?shakaExtern.ManifestDB */ manifest) {
    this.checkDestroyed_();
    var Functional = shaka.util.Functional;
    if (!manifest) {
      throw new shaka.util.Error(
    ...
    // TODO(modmaker): Delete offline EME sessions.

    // Get every segment for every stream in the manifest.
    /** @type {!Array.<number>} */
    var segments = manifest.periods.map(function(period) {
...
shaka.offline.Storage.prototype.createOfflineManifest_ = function(
  ...
  return {
    ...
    size: 0,
    periods: periods,
    sessionIds: this.drmEngine_.getSessionIds(),
    drmInfo: this.drmEngine_.getDrmInfo(),
...
shaka.offline.Storage.prototype.createStream_ = function(streamSet, stream) {
  ...
  while (ref) {
    ...
    this.downloadManager_.addSegment(
        ref, bandwidthSize, function(id, pos, streamId, data) {
  ...
  if (stream.initSegmentReference) {
    ...
    this.downloadManager_.addSegment(stream.initSegmentReference, 0,
