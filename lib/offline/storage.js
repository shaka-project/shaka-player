      ...
      .then(function() {
        ...
        return this.storageEngine_.forEach(
            'manifest', function(/** shakaExtern.ManifestDB */ manifest) {
              storedContents.push(
                  shaka.offline.OfflineUtils.createStoredContent(manifest));
            });
      }.bind(this))
...
shaka.offline.Storage.prototype.initIfNeeded_ = function() {
  ...
  if (this.storageEngine_) {
    goog.asserts.assert(
        this.storageEngine_.initialized(),
        'If storage engine has been created, it should be initialized.');
...
shaka.offline.Storage.prototype.createStream_ = function(
  ...
  while (ref) {
    ...
    segmentsDb.push({
      ...
      endTime: ref.endTime,
      uri: shaka.offline.OfflineScheme.segmentToUri(
          this.manifestId_, stream.id, id)
    });
  ...
  if (stream.initSegmentReference) {
    var id = this.storageEngine_.reserveId('segment');
    initUri = shaka.offline.OfflineScheme.segmentToUri(
        this.manifestId_, stream.id, id);
