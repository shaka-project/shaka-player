...
shaka.offline.Storage = class {
  ...
  store(uri, appMetadata, mimeType) {
    ...
    const storeOp = this.store_(uri, appMetadata || {}, getParser, config);
    return this.startOperation_(storeOp);
  }
   ...
   * @private
   */
  async store_(uri, appMetadata, getParser, config) {
   ...
   * @private
   */
  async downloadManifest_(storage, drmEngine, manifest, uri, metadata, config) {
    goog.asserts.assert(
        this.networkingEngine_,
    ...
    const downloader = new shaka.offline.DownloadManager(
        ...
        (progress, size) => {
          ...
          config.offline.progressCallback(pendingContent, progress);
        },
        (initData, systemId) => {
