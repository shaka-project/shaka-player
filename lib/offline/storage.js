...
shaka.offline.Storage.prototype.removeByUrl_ = function(offlineUri) {
  let uri = shaka.offline.OfflineUri.parse(offlineUri);
  ...
  return this.initIfNeeded_().then(() => {
    this.checkDestroyed_();
    return this.storageEngine_.getManifest(uri.key());
  }).then((data) => {
    this.checkDestroyed_();
    if (!data) {
      throw new shaka.util.Error(
    ...
    goog.asserts.assert(uri, 'Offline uri should never be null here.');
    this.checkDestroyed_();
    if (error) { throw error; }
    return this.removeManifest_(uri, manifestDB);
  });
};
...
shaka.offline.Storage.prototype.removeManifest_ = function(
  ...
  return Promise.resolve().then(() => {
    ...
    this.checkDestroyed_();
    return this.storageEngine_.removeManifests([uri.key()], onThingRemoved);
  });
};
...
shaka.offline.Storage.prototype.list = function() {
  ...
  return this.initIfNeeded_().then(() => {
    this.checkDestroyed_();
    return this.storageEngine_.forEachManifest((id, manifestDB) => {
      let uri =
