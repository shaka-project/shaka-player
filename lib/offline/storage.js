...
shaka.offline.Storage = class {
  ...
  createPeriod_(downloader, storage, estimator, drmEngine, manifest, period) {
    ...
    // for each of them.
    const streamSet = shaka.offline.Storage.getAllStreamsFromPeriod_(period);
  ...
  createStream_(downloader, storage, estimator, manifest, period, stream) {
    ...
    const firstMediaSegment = stream.segmentIndex.seek(startTime);
    const initSegment = firstMediaSegment ?
        firstMediaSegment.initSegmentReference : null;
    // The old presentationTimeOffset field was the inverse of the new
    // timestampOffset field, and didn't account for period start.
    streamDb.presentationTimeOffset = firstMediaSegment ?
        period.startTime - firstMediaSegment.timestampOffset : 0;
      ...
      downloader.queue(
          ...
          async (data) => {
            ...
            streamDb.segments.push({
              // The DB format still stores segment times relative to the
              // period.  The manifest format uses presentation-relative times.
              startTime: segment.startTime - period.startTime,
              endTime: segment.endTime - period.startTime,
              dataKey: ids[0],
            });
          });
    });
  ...
  static forEachSegment_(stream, startTime, callback) {
    ...
    let ref = stream.segmentIndex.seek(startTime);

    while (ref) {
      callback(ref);
      ref = stream.segmentIndex.next();
    }
  }
