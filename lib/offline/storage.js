...
shaka.offline.Storage = class {
  ...
  async store_(uri, appMetadata, externalThumbnails, externalText,
    ...
    try {
      ...
      const canDownload = !manifest.presentationTimeline.isDynamic();
      if (!canDownload) {
        throw new shaka.util.Error(
            shaka.util.Error.Severity.CRITICAL,
            shaka.util.Error.Category.STORAGE,
  ...
  async filterManifest_(manifest, drmEngine, config, usePersistentLicense) {
    ...
    const preferredVideoCodecs =
        config.preferredVideo.map((p) => p.codec).filter(Boolean);
    const preferredAudioCodecs =
        config.preferredAudio.map((p) => p.codec).filter(Boolean);
    const preferredTextFormats =
        config.preferredText.map((p) => p.format).filter(Boolean);
    ...
    for (const chapter of manifest.chapterStreams) {
      allTracks.push(shaka.util.StreamUtils.chapterStreamToTrack(chapter));
    }
    ...
    manifest.chapterStreams =
        manifest.chapterStreams.filter((stream) => chapterIds.has(stream.id));
  ...
  makeManifestDB_(drmEngine, manifest, uri, metadata, config, downloader,
    ...
    for (const stream of manifest.chapterStreams) {
      estimator.addImage(stream);
    }
    for (const variant of manifest.variants) {
  ...
  async createExternalImageStream_(uri, manifest) {
    ...
    for (const cue of cues) {
      ...
      const getUris = () => {
        if (uris == null) {
          uris = shaka.util.URL.resolveUris([uri], [cue.payload]);
        }
        return uris || [];
      };
  ...
  createStream_(downloader, estimator, manifest, stream, config, toDownload) {
    ...
    shaka.offline.Storage.forEachSegment_(stream, startTime, (segment, pos) => {
      ...
      if (!chapterTitle && !toDownload.has(pendingSegmentRefId)) {
