...
shaka.offline.Storage = function(player) {
   ...
   * @private {?shaka.extern.StoredContent}
   */
 ...
 * @export
 */
shaka.offline.Storage.prototype.destroy = async function() {
  // Make sure to only set the download manager and storage engine to null once
  // they have been destroyed to ensure that they are still usable for any
  // catch statements in the download manager chains.
  if (this.downloadManager_) {
    await this.downloadManager_.destroy();
  }

  if (this.storageEngine_) {
    await this.storageEngine_.destroy();
  }
...
shaka.offline.Storage.prototype.store = function(
  ...
  return this.initIfNeeded_().then(() => {
    ...
    return shaka.util.IDestroyable.with(data.drmEngine, () => {
      ...
      (content) => {
        ...
        this.storeInProgress_ = false;
        this.firstPeriodTracks_ = null;
        // Stop tracking the segments as they all got into storage successfully.
        this.segmentsFromStore_ = [];
      ...
      (err) => {
        ...
        return Promise.resolve().then(() => {
          return this.storageEngine_.removeSegments(segmentsToRemove, noop);
        }).then(() => {
          this.storeInProgress_ = false;
          this.firstPeriodTracks_ = null;
          this.segmentsFromStore_ = [];
...
shaka.offline.Storage.prototype.downloadAndStoreManifest_ = function(
  ...
  return this.downloadManager_.download(this.getNetEngine_()).then(() => {
    // Update the size before saving it.
    manifestDB.size = this.pendingContent_.size;
    return this.storageEngine_.addManifest(manifestDB);
  }).then((id) => {
    let uri = shaka.offline.OfflineUri.manifest('mechanism', 'cell', id);
    return shaka.offline.StoredContentUtils.fromManifestDB(uri, manifestDB);
  });
};
...
/**
 * Initializes the IStorageEngine if it is not initialized already.
 *
 * @return {!Promise}
 * @private
 */
shaka.offline.Storage.prototype.initIfNeeded_ = function() {
  ...
  let onProgress = (progress, size) => {
    goog.asserts.assert(
        this.pendingContent_,
    ...
    this.pendingContent_.size = size;

    this.config_.progressCallback(this.pendingContent_, progress);
  };

  const StorageEngineFactory = shaka.offline.StorageEngineFactory;
  return StorageEngineFactory.createStorageEngine().then((storageEngine) => {
    goog.asserts.assert(storageEngine, 'Need valid storage engine.');
    // Save this instance for later use in other methods.
    this.storageEngine_ = storageEngine;
    this.downloadManager_ = new shaka.offline.DownloadManager(onProgress);
  });
};
...
shaka.offline.Storage.prototype.createOfflineManifest_ = function(
  ...
  let periods = manifest.periods.map((period) => {
    return this.createPeriod_(estimator, drmEngine, manifest, period);
  });
...
shaka.offline.Storage.prototype.createPeriod_ = function(
    estimator, drmEngine, manifest, period) {
  const StreamUtils = shaka.util.StreamUtils;
  ...
  shaka.offline.Storage.getStreamSet_(manifest)
      ...
      .forEach((stream) => {
        streamDBs[stream.id] = this.createStream_(
            estimator, manifest, period, stream);
      });
...
shaka.offline.Storage.prototype.createStream_ = function(
  ...
  shaka.offline.Storage.forEachSegment_(stream, startTime, (segment) => {
    this.downloadManager_.queue(
        ...
        (data) => {
          return this.storageEngine_.addSegment({data: data}).then((id) => {
