...
shaka.offline.Storage = class {
  ...
  async store_(uri, appMetadata, getParser) {
    ...
    try {
      ...
      const manifest = await this.parseManifest(uri, parser);
   ...
   */
  async parseManifest(uri, parser) {
    let error = null;
  ...
  createOfflineManifest_(
    ...
    const streams = this.createStreams_(
        downloader, storage, estimator, drmEngine, manifest);
   ...
   * @private
   */
  createStreams_(downloader, storage, estimator, drmEngine, manifest) {
    ...
    for (const stream of streamSet) {
      const streamDB = this.createStream_(
          downloader, storage, estimator, manifest, stream);
   ...
   * @private
   */
  createStream_(downloader, storage, estimator, manifest, stream) {
