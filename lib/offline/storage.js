 ...
 * @param {!shaka.extern.ManifestParser.Factory=} manifestParserFactory
...
shaka.offline.Storage.prototype.store = async function(
    manifestUri, appMetadata, manifestParserFactory) {
  ...
  let data = await this.loadInternal(
      manifestUri, (e) => { error = e; }, manifestParserFactory);
    ...
    return shaka.util.IDestroyable.with(data.drmEngine, async () => {
      try {
        ...
        let manifestDB = await this.downloadManifest_(
              storage, data.drmEngine, data.manifest, manifestUri,
              appMetadata || {});
...
shaka.offline.Storage.prototype.loadInternal = function(
    manifestUri, onError, manifestParserFactory) {
