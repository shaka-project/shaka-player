...
shaka.offline.Storage.prototype.store_ = async function(
  ...
  const data = await this.loadInternal(uri, onError, getParser);
  const drmEngine = data.drmEngine;
  const manifest = data.manifest;

  let canDownload = !manifest.presentationTimeline.isLive() &&
  ...
  shaka.util.ManifestFilter.rollingFilter(manifest, (period) => {
    ...
    // TODO(vaage): Replaced this with a set once we support using ES6 sets.
    const variantIds = {};
    chosenTracks
        .filter((track) => track.type == 'variant')
        .forEach((track) => { variantIds[track.id] = true; });

    // TODO(vaage): Replaced this with a set once we support using ES6 sets.
    const textIds = {};
    chosenTracks
        .filter((track) => track.type == 'text')
        .forEach((track) => { textIds[track.id] = true; });

    period.variants = period.variants.filter((variant) => {
      return variantIds[variant.id];
    });

    period.textStreams = period.textStreams.filter((stream) => {
      return textIds[stream.id];
    });
  });
  ...
  return shaka.util.Destroyer.with([muxer, drmEngine], async () => {
    try {
      await muxer.init();
      this.checkDestroyed_();
...
shaka.offline.Storage.prototype.removeFromDRM_ = function(uri, manifestDB) {
  ...
  return shaka.util.Destroyer.with([drmEngine], async () => {
    ...
    await drmEngine.initForStorage(variants, this.config_.usePersistentLicense);
...
shaka.offline.Storage.deleteAll_ = function(networkingEngine, drmConfig) {
  ...
  return shaka.util.Destroyer.with([muxer], async () => {
    ...
    manifests
        ...
        .forEach((manifest) => {
          const variants = shaka.util.StreamUtils.getAllVariants(manifest);
          const sessions = manifest.offlineSessionIds;

          deleteLicenses = deleteLicenses.then(() => {
            return shaka.offline.Storage.deleteLicenseFor_(
                networkingEngine, drmConfig, variants, sessions);
          });
        });
