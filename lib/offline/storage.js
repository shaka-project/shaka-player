...
shaka.offline.Storage.deleteAll_ = function(networkingEngine, drmConfig) {
  ...
  return shaka.util.Destroyer.with([muxer], async () => {
    ...
    const manifests = await shaka.offline.Storage.getAllManifests_(muxer);
    // TODO: Change to for-of once the compiler no longer complains about
    // nothing here.
    for (let i = 0; i < manifests.length; i++) {
      const p = shaka.offline.Storage.deleteLicenseFor_(
          networkingEngine, drmConfig, muxer, manifests[i]);
