...
shaka.offline.indexeddb.V2StorageCell = class {
  ...
  updateManifests(manifests) {
    ...
    shaka.util.MapUtils.forEach(manifests, (key, value) => {
      // Need to make sure that the key we use is a number or else the update
      // won't replace the correct entry (|shaka.util.MapUtils.forEach| does
      // not preserve the type of the key).
  ...
  getAllManifests() {
    ...
    store.openCursor().onsuccess = (event) => {
      ...
      // Go to the next item in the store, this will cause |onsuccess| to be
      // called again.
      cursor.continue();
    };
  ...
  add_(storeName, values) {
    ...
    // Write each segment out. When each the request completes, the key will
