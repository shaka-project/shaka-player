...
shaka.queue.QueueManager = class extends shaka.util.FakeEventTarget {
  ...
  setupPreloadNext_(mediaElement) {
    ...
    const listener = async () => {
      // No preload if we don't automatically play the next one, that way we
      // save resources.
      if (!this.config_?.autoPlayNext) {
        return;
      }
  ...
  static parseM3uPlaylist_(text, baseUrl) {
    ...
    for (const line of lines) {
      ...
      const manifestUri = shaka.util.URL.resolve(baseUrl, trimmed);
