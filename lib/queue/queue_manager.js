...
goog.require('shaka.event.CurrentItemChanged');
goog.require('shaka.event.ItemsInserted');
goog.require('shaka.event.ItemsRemoved');
...
shaka.queue.QueueManager = class extends shaka.util.FakeEventTarget {
  ...
  insertItems(items) {
    ...
    this.dispatchEvent(new shaka.event.ItemsInserted());
  }
  ...
  async removeAllItems() {
    ...
    this.items_ = [];
    this.currentItemIndex_ = -1;
    this.dispatchEvent(new shaka.event.ItemsRemoved());
  }
  ...
  async playItem(itemIndex) {
    ...
    if (this.currentItemIndex_ !== itemIndex) {
      ...
      this.dispatchEvent(new shaka.event.CurrentItemChanged());
    }
