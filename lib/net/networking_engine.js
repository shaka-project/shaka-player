 ...
 *
 * @typedef {!function(shaka.net.NetworkingEngine.Request):
...
shaka.net.NetworkingEngine.prototype.destroy = function() {
  ...
  this.requestFilters_ = [];
  this.responseFilters_ = [];
  return Promise.all(this.requests_);
};
...
shaka.net.NetworkingEngine.prototype.send_ = function(type, request, index) {
  ...
  if (!plugin) {
    ...
    return Promise.reject('Unsupported scheme \'' + scheme + '\'');
  }

  return plugin(request).then(function(response) {
