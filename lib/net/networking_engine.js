...
shaka.net.NetworkingEngine = class extends shaka.util.FakeEventTarget {
  ...
  send_(type, request, backoff, index, lastError, numBytesRemainingObj) {
    let uriToRequest = request.uris[index];
    if (request.retryParameters.forceHTTPs) {
      uriToRequest = uriToRequest.replace('http://', 'https://');
    }

    const uri = new goog.Uri(uriToRequest);
