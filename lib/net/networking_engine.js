...
shaka.net.NetworkingEngine = class extends shaka.util.FakeEventTarget {
  ...
  constructor(onProgressUpdated, onHeadersReceived, onDownloadCompleted,
    ...
    /** @private {?shaka.extern.NetworkingConfiguration} */
    this.config_ = null;
  ...
  send_(type, request, context, backoff, lastError, numBytesRemainingObj) {
    ...
    filterRequestOperation.promise.then(() => {
      requestFilterTimeMs = Date.now() - requestFilterStartTimeMs;
    }, () => {});

    if (this.config_.forceHTTP) {
      request.uris[index] = request.uris[index].replace('https://', 'http://');
    }
    if (this.config_.forceHTTPS) {
    ...
    let scheme = shaka.util.URL.getScheme(request.uris[index]);
    ...
    if (!scheme) {
      ...
      request.uris[index] =
          shaka.util.URL.setScheme(request.uris[index], scheme);
    }
    shaka.log.warning('scheme', scheme,
        [...shaka.net.NetworkingEngine.schemes_.keys()]);
    ...
    const commonAccessToken = this.hostCommonAccessTokenMap_.get(
        shaka.util.URL.getDomain(request.uris[index]));
    ...
    const sendOperation = backoffOperation.chain(() => {
      ...
      const requestPlugin = plugin(
          ...
          {
            minBytesForProgressEvents: this.config_.minBytesForProgressEvents,
          });
      ...
      if (response.timeMs == undefined) {
        ...
        response.timeMs += requestFilterTimeMs;
      }
      ...
      if (commonAccessTokenHeader) {
        this.hostCommonAccessTokenMap_.set(
            shaka.util.URL.getDomain(response.uri), commonAccessTokenHeader);
      }
      ...
      if (this.onDownloadCompleted_) {
        this.onDownloadCompleted_(type, request, context, response);
      }
      ...
      if (this.onDownloadFailed_) {
        ...
        this.onDownloadFailed_(
            type, request, context, shakaError, httpResponseCode, aborted);
      }
      if (this.destroyed_) {
        return shaka.util.AbortableOperation.aborted();
      }
