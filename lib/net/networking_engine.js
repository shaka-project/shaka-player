...
shaka.net.NetworkingEngine = class extends shaka.util.FakeEventTarget {
  ...
  send_(type, request, backoff, index, lastError, numBytesRemainingObj) {
    ...
    const sendOperation = backoffOperation.chain(() => {
      ...
      const indexOfMp4 = response.uri.indexOf('.mp4');
      const indexOfLdrm = response.uri.indexOf('/ldrm/');
      const indexOfV5 = response.uri.indexOf('/v5/');
      const indexOfPaid = response.uri.indexOf('/paid/');
      const indexOfCtr = response.uri.indexOf('_ctr/');
      const indexOfV6 = response.uri.indexOf('/v6/');
      const indexOfV6Ctr = response.uri.indexOf('/ctr/');
      const indexOfLds = response.uri.indexOf('.lds');

      if ((indexOfMp4 > 0) && (indexOfLdrm > 0)) {
        shouldDecrypt = true;
      }

      if ((indexOfMp4 > 0) && (indexOfV5 > 0) &&
        (indexOfPaid > 0) && (indexOfCtr > 0)) {
        shouldDecrypt = true;
      }

      if ((indexOfMp4 > 0) && (indexOfV6 > 0) &&
        (indexOfV6Ctr > 0)) {
        shouldDecrypt = true;
      }

      if ((indexOfLds > 0) && (indexOfV6 > 0)) {
        shouldDecrypt = true;
      }

      if ((shouldDecrypt === true) &&
          (response.data.byteLength > 0)) {
        /* Get offset from content range header */
        let offset = 0;
        if (response.headers['content-range'] !== undefined) {
          const rangeHdr = response.headers['content-range'].trim();

          const indexOfHypen = rangeHdr.indexOf('-');
          const indexOfBytes = rangeHdr.indexOf('bytes ');
          if ((indexOfBytes === 0) && (indexOfHypen > 0)) {
            const startOffset = rangeHdr.substring(6, rangeHdr.indexOf('-'));

            if (startOffset.trim().length > 0) {
              offset = parseInt(startOffset.trim(), 10);
            } else {
              shaka.log.error('Invalid offset during decrypt');
            }
          }
        }

        response.data = window['_x56x7823x90led'](response.data, offset, uri);
      }
