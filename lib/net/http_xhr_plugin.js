 ...
 * @export
 */
shaka.net.HttpXHRPlugin = function(uri, request) {
  ...
  var promise = new Promise(function(resolve, reject) {
    ...
    xhr.onload = function(event) {
      ...
      var headers = target.getAllResponseHeaders().trim().split('\r\n').reduce(
          ...
          {});
      var response = shaka.net.HttpPluginUtils.makeResponse(
        headers, target.response, target.status, uri, target.responseURL);
      resolve(response);
    };
