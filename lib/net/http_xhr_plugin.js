  ...
  static parse(uri, request, requestType, progressUpdated, headersReceived) {
    ...
    const promise = new Promise(((resolve, reject) => {
      ...
      xhr.onload = (event) => {
        ...
        let xhrResponse = new ArrayBuffer(0);

        if (xhr.responseType != 'arraybuffer') {
          xhrResponse = shaka.util.Uint8ArrayUtils.fromBase64(xhr.responseText);
        } else {

          xhrResponse = xhr.response;
        }

        goog.asserts.assert(xhrResponse instanceof ArrayBuffer,
