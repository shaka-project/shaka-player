...
shaka.net.HttpFetchPlugin = class {
  ...
  static parse(uri, request, requestType, progressUpdated) {
    ...
    /** @type {shaka.extern.ProgressUpdated} */
    const onProgressUpdated = (time, bytes, numBytesRemaining) => {
      if (connectionTimer) {
      ...
      progressUpdated(time, bytes, numBytesRemaining);
    };

    const pendingRequest = shaka.net.HttpFetchPlugin.request_(
        uri, requestType, init, abortStatus, onProgressUpdated,
    ...
    if (timeoutMs) {
      timeoutTimer = new shaka.util.Timer(() => {
        abortStatus.timedOut = true;
        controller.abort();
      });

      timeoutTimer.tickAfter(timeoutMs / 1000);
    }

    // To avoid calling |abort| on the network request after it finished, we
    // will stop the timers when the requests resolves/rejects.
    op.finally(() => {
      if (timeoutTimer) {
        timeoutTimer.stop();
      }
