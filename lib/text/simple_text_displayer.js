...
shaka.text.SimpleTextDisplayer.prototype.append = function(cues) {
  ...
  const vttRegions = [];
  if (window.VTTRegion) {
    const regions = new Set(cues.map((cue) => cue.region));

    const videoWidth = this.video_.offsetWidth;
    const videoHeight = this.video_.offsetHeight;

    for (const region of regions) {
      vttRegions.push(convertToVttRegion(region, videoWidth, videoHeight));
    }
  }
