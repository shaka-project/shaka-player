...
shaka.text.VttTextParser = class {
  ...
  static parseCue_(text, timeOffset, regions) {
    ...
    VttTextParser.parseCueStyles(cue, payload);
   ...
   */
  static parseCueStyles(cue, payload) {
    const element = shaka.util.XmlUtils.parseXmlString(payload, '');
    if (element) {
      // TODO: Add support for nested elements at more than two levels of nesting
      const payloadString = element.textContent.trim();
      if (payloadString) {
        cue.payload = payloadString;
