...
shaka.text.VttTextParser = class {
  ...
  static parseCueStyles(payload, rootCue, styles) {
    payload = shaka.text.VttTextParser.replaceColorPayload(payload);
  ...
  /**
   * Convert color end tag to be valid for xml parsing
   * For example,
   ...
   */
  static replaceColorPayload(payload) {
    const names = [];
    let nameStart = -1;
    for (let i = 0; i < payload.length; i++) {
      if (payload[i] === 'c') {
        if (payload[i - 1] === '<') {
          ...
          const left = payload.substr(0, i);
          const right = payload.substr(i + 1);
          payload = left + names[names.length - 1] + right;
          names.pop();
        }
      } else if (payload[i] == '>') {
        if (nameStart > 0) {
  ...
  static generateCueFromElement_(element, rootCue, cues, styles) {
    ...
    if (element.nodeType === Node.ELEMENT_NODE && element.nodeName) {
      ...
      const textColorKeys = Object.keys(textColor);
      const bgColor = shaka.text.Cue.defaultTextBackgroundColor;
      const bgColorKeys = Object.keys(bgColor);
      ...
      for (const tag of tags) {
        ...
        switch (tag) {
          ...
          case 'u':
            ...
            break;
          default:
            if (textColorKeys.includes(tag)) {
              nestedCue.color = textColor[tag];
            } else if (bgColorKeys.includes(tag)) {
              nestedCue.backgroundColor = bgColor[tag];
            }
            break;
        }
      }
