...
shaka.text.VttTextParser = class {
  ...
  static parseCue_(text, timeOffset, regions, styles) {
    ...
    // If position is null (i.e. 'auto'), use text alignment to compute the
    // correct position
    if (cue.position === null) {
      VttTextParser.computeCuePosition_(cue);
    }

    // If position alignment is 'auto', use text alignment to compute the
    // correct position alignment
    if (cue.positionAlign === Cue.positionAlign.AUTO) {
      VttTextParser.computeCuePositionAlignment_(cue);
    }
   ...
   * @private
   */
  static computeCuePosition_(cue) {
    const Cue = shaka.text.Cue;
    const {textAlign} = cue;
    if (textAlign === Cue.textAlign.LEFT) {
      cue.position = 0;
    } else if (textAlign === Cue.textAlign.RIGHT) {
      cue.position = 100;
    } else {
      cue.position = 50;
    }
  ...
  /**
   * Compute cue position alignment in case position alignment is 'auto'
   * See https://www.w3.org/TR/webvtt1/#webvtt-cue-position-alignment
   *
   * @param {!shaka.text.Cue} cue
   * @private
   */
  static computeCuePositionAlignment_(cue) {
    const Cue = shaka.text.Cue;
    const {direction, textAlign} = cue;
    if (textAlign === Cue.textAlign.LEFT) {
      cue.positionAlign = Cue.positionAlign.LEFT;
    } else if (textAlign === Cue.textAlign.RIGHT) {
      cue.positionAlign = Cue.positionAlign.RIGHT;
    } else if (textAlign === Cue.textAlign.START) {
      if (direction === Cue.direction.HORIZONTAL_LEFT_TO_RIGHT) {
        cue.positionAlign = Cue.positionAlign.LEFT;
      } else {
        cue.positionAlign = Cue.positionAlign.RIGHT;
      }
    } else if (textAlign === Cue.textAlign.END) {
      if (direction === Cue.direction.HORIZONTAL_LEFT_TO_RIGHT) {
        cue.positionAlign = Cue.positionAlign.RIGHT;
      } else {
        cue.positionAlign = Cue.positionAlign.LEFT;
  ...
  static parseCueSetting(cue, word, regions) {
    ...
    if ((results = /^align:(start|middle|center|end|left|right)$/.exec(word))) {
      ...
      cue.size = Number(results[1]);
        ...
        /^position:([\d.]+)%(?:,(line-left|line-right|center|start|end|auto))?$/
            .exec(word))) {
