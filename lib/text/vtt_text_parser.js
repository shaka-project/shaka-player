...
shaka.text.VttTextParser = class {
  ...
  static parseCueStyles(payload, rootCue) {
    ...
    if (element) {
      ...
      for (const childElement of childNodes) {
        VttTextParser.generateCueFromElement_(childElement, rootCue, cues);
      }
  ...
  static generateCueFromElement_(element, rootCue, cues) {
    ...
    if (element.nodeType === Node.ELEMENT_NODE && element.nodeName) {
      ...
      const tags = element.nodeName.split(/[ .]+/).filter(Boolean);
    ...
    const contents = shaka.util.XmlUtils.getContents(element);
    if (contents) {
      nestedCue.payload = contents;
      cues.push(nestedCue);
    } else {
      const VttTextParser = shaka.text.VttTextParser;
      for (const childElement of [...element.childNodes]) {
        VttTextParser.generateCueFromElement_(childElement, nestedCue, cues);
      }
    }
  }
