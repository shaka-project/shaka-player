...
shaka.text.UITextDisplayer = class {
  ...
  constructor(video, videoContainer) {
    ...
    this.eventManager_.listen(this.video_, 'resize', () => {
      ...
      const aspectRatio = this.aspectRatio_;
      if (width && height) {
   ...
   */
  configure(config) {
    // The config param is updated by reference before, store the currently
    // applied pointer and clone the new config so we can compare previous
    // and new values later on.
    const oldConfig = this.config_;
    this.config_ = shaka.util.ObjectUtils.cloneObject(config);
    if (!oldConfig ||
      oldConfig.captionsUpdatePeriod !== this.config_.captionsUpdatePeriod) {
      this.configureCaptionsTimer_();
    }
    if (!oldConfig ||
      oldConfig.fontScaleFactor !== this.config_.fontScaleFactor
    ) {
      this.updateCaptions_(/* forceUpdate= */ true);
    }
    if (!oldConfig ||
      oldConfig.matchVideoAspectRatio !== this.config_.matchVideoAspectRatio ||
      !shaka.util.ObjectUtils.shallowCompare(oldConfig.margin,
          this.config_.margin)
    ) {
      this.updateContainer_();
    }
  ...
  updateContainer_() {
    ...
    const style = this.textContainer_.style;
    let offsetX = 0;
    let offsetY = 0;
    const matchVideoAspectRatio = this.config_ ?
      this.config_.matchVideoAspectRatio : false;
    if (matchVideoAspectRatio && this.aspectRatio_ !== null) {
      const width = this.videoContainer_.offsetWidth;
      const height = this.videoContainer_.offsetHeight;
      const scaledWidth = height * this.aspectRatio_ < width ?
        height * this.aspectRatio_ :
        width;
      const scaledHeight = scaledWidth / this.aspectRatio_;
      offsetX = (width - scaledWidth) / 2;
      offsetY = (height - scaledHeight) / 2;
    }
    const left = this.config_ ? this.config_.margin.left : 0;
    style.left = `${Math.max(left, offsetX)}px`;
    const right = this.config_ ? this.config_.margin.right : 0;
    style.right = `${Math.max(right, offsetX)}px`;
    const top = this.config_ ? this.config_.margin.top : 0;
    style.top = `${Math.max(top, offsetY)}px`;
    const bottom = this.config_ ? this.config_.margin.bottom : 0;
    style.bottom = `${Math.max(bottom, offsetY)}px`;
  }
