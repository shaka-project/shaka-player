...
shaka.text.UITextDisplayer = class {
  ...
  setCaptionStyles_(cueElement, cue, parents, hasWrapper) {
    ...
    } else if (cue.positionAlign == Cue.positionAlign.AUTO) {
      // No alignment given for the cue box, let's use the text alignment
      if (cue.textAlign == Cue.textAlign.START ||
        cue.textAlign == Cue.textAlign.LEFT) {
        style.alignItems = 'flex-start';
      } else if (cue.textAlign == Cue.textAlign.END ||
        cue.textAlign == Cue.textAlign.RIGHT) {
        style.alignItems = 'flex-end';
      }
    }
    ...
    if (cue.position != null) {
      if (cue.writingMode == Cue.writingMode.HORIZONTAL_TOP_TO_BOTTOM) {
        ...
        if (cue.positionAlign == Cue.positionAlign.LEFT ||
          ...
          (cue.positionAlign == Cue.positionAlign.AUTO &&
            style.alignItems == 'flex-end')) {
          style.paddingRight = (100 - cue.position) + '%';
        } else {
          // Position is 'center'
          if (cue.position > 50) {
            // Center of cue box is shifted toward the right of the viewport
            const width = (100 - cue.position) * 2;
            style.left = (100 - width) + '%';
            style.width = width + '%';
          } else if (cue.position < 50) {
            // Center of cue box is shifted toward the left of the viewport
            const width = cue.position * 2;
            style.left = '0%';
            style.width = width + '%';
          }
        }
        ...
        if (cue.positionAlign == Cue.positionAlign.LEFT ||
          ...
          (cue.positionAlign == Cue.positionAlign.AUTO &&
            style.alignItems == 'flex-end')) {
          style.paddingBottom = (100 - cue.position) + '%';
        } else {
          // Position is 'center'
          if (cue.position > 50) {
            // Center of cue box is shifted toward the bottom of the viewport
            const height = (100 - cue.position) * 2;
            style.top = (100 - height) + '%';
            style.height = height + '%';
          } else if (cue.position < 50) {
            // Center of cue box is shifted toward the top of the viewport
            const height = cue.position * 2;
            style.top = '0%';
            style.height = height + '%';
          }
        }
