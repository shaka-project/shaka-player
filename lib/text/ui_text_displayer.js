...
shaka.text.UITextDisplayer = class {
  ...
  updateCaptions_(forceUpdate = false) {
    ...
    for (const cue of currentCues) {
      const cueElement = this.displayCue_(
          this.textContainer_, cue, /* parentCue= */ null,
          /* isNested= */ false);
   ...
   * @param {?shaka.extern.Cue} parentCue
   * @param {boolean} isNested
   * @return {!Element} the created captions element
   * @private
   */
  displayCue_(container, cue, parentCue, isNested) {
    ...
    if (type != 'br') {
      this.setCaptionStyles_(cueElement, cue, parentCue, isNested);
    }
    ...
    for (const nestedCue of cue.nestedCues) {
      this.displayCue_(wrapper, nestedCue,
          /* parentCue= */ cue,
          /* isNested= */ true);
    }
   ...
   * @private
   */
  setCaptionStyles_(cueElement, cue, parentCue, isNested) {
    ...
    if (isLeaf && parentCue && cue.backgroundColor == '') {
      style.backgroundColor = parentCue.backgroundColor;
    }

    style.color = cue.color;
    if (isLeaf && parentCue && cue.color == '') {
      style.color = parentCue.color;
    }
