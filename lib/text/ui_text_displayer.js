...
shaka.text.UITextDisplayer = class {
  ...
  updateCaptions_(forceUpdate = false) {
    ...
    const shouldCueBeDisplayed = (cue) => {
      //console.log("currentCue: start=", cue.startTime, "end=", cue.endTime, "currentTime=", currentTime);
      return this.cues_.includes(cue) &&
             cue.startTime <= currentTime && cue.endTime + 10 > currentTime;
    };
