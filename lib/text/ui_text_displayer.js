...
shaka.text.UITextDisplayer = class {
  ...
  setCaptionStyles_(cueElement, cue, parents, hasWrapper) {
    ...
    if (cue.line != null) {
      if (cue.lineInterpretation == Cue.lineInterpretation.PERCENTAGE) {
      ...
      } else if (cue.lineInterpretation == Cue.lineInterpretation.LINE_NUMBER) {
        // The number is the height of the first line of the cue
        // as it appears on the video.
        // Positive numbers indicate top down
        // and negative numbers indicate bottom up.
        const lineNumberIsPositive = cue.line >= 0;
        style.position = 'absolute';
        if (cue.writingMode == Cue.writingMode.HORIZONTAL_TOP_TO_BOTTOM) {
          // When setting absolute positioning, we need to set left as well
          // to make sure subs are centered by default
          style.left = '50%';
          style.transform = 'translateX(-50%)';
          if (lineNumberIsPositive) {
            // As the lines naturally stack all we need to do is position
            // top down for a positive number, as the first line
            // always starts from the top.
            style.top = '0';
          } else {
            // As the lines naturally stack all we need to do is position
            // bottom up for a negative number, as the last line
            // always ends at the bottom.
            style.bottom = '0';
          }
        } else if (cue.writingMode == Cue.writingMode.VERTICAL_LEFT_TO_RIGHT) {
          style.top = '0';
        ...
        } else if (cue.writingMode == Cue.writingMode.VERTICAL_RIGHT_TO_LEFT) {
          style.top = '0';
          style.height = '100%';
          if (lineNumberIsPositive) {
            style.right = '0';
