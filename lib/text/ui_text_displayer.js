...
shaka.text.UITextDisplayer = class {
  ...
  constructor(video, videoContainer) {
    ...
    this.eventManager_.listen(this.video_, 'resize', () => {
      ...
      if (width && height) {
        const update = this.aspectRatio_ === null;
  ...
  updateContainer_() {
    ...
    let aspectX = width;
    let aspectY = width / this.aspectRatio_;
    if (aspectY > height) {
      aspectX = height * this.aspectRatio_;
      aspectY = height;
    }
    const minX = (width - aspectX) / 2;
    style.left = `${Math.max(this.config_.edges.left, minX)}px`;
    style.right = `${Math.max(this.config_.edges.right, minX)}px`;
    const minY = (height - aspectY) / 2;
    style.top = `${Math.max(this.config_.edges.top, minY)}px`;
    style.bottom = `${Math.max(this.config_.edges.bottom, minY)}px`;
  }
