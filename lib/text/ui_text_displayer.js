...
shaka.text.UITextDisplayer = class {
  ...
  _accessibilityFontSizeScaling(fontSize) {
    if (fontSize && fontSize !== '' && this.config_ &&
        this.config_.fontScaleFactor && this.config_.fontScaleFactor != 1) {
      const fontSizeInfo =
        shaka.text.UITextDisplayer.getLengthValueInfo_(fontSize);
      if (fontSizeInfo) {
        fontSize = String((fontSizeInfo.value * this.config_.fontScaleFactor) +
            fontSizeInfo.unit);
      }
    }
