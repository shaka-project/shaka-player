    ...
    if ('IntersectionObserver' in window) {
      ...
      this.visibilityObserver_ = new IntersectionObserver((entries) => {
        if (entries.length) {
          ...
          // Older Chromium can misreport intersectionRatio = 0 on transformed
          // or absolute-positioned ancestors; cross-check the container's rect.
          // See https://github.com/shaka-project/shaka-player/issues/10042
          if (!visible && this.videoContainer_) {
            const rect = this.videoContainer_.getBoundingClientRect();
            const vw =
          ...
          this.isContainerActuallyVisible_ = visible;
