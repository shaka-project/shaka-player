...
shaka.text.UITextDisplayer = class {
  ...
  updateContainer_() {
    ...
    style.left = `${Math.max(this.config_.margin.left, offsetX)}px`;
    style.right = `${Math.max(this.config_.margin.right, offsetX)}px`;
    style.top = `${Math.max(this.config_.margin.top, offsetY)}px`;
    style.bottom = `${Math.max(this.config_.margin.bottom, offsetY)}px`;
  }
