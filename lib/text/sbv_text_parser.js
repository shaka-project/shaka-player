...
shaka.text.SbvTextParser = class {
  ...
  parseMedia(data, time) {
    ...
    const vvtText = SbvTextParser.sbv2webvtt(str);
  ...
  static sbv2webvtt(data) {
    ...
    for (const cue of cuelist) {
      result += SbvTextParser.convertSbvCue_(cue);
    }
  ...
  /**
   * Convert a SBV cue into WebVTT cue
   *
   * @param {!string} caption
   * @return {!string}
   * @private
   */
  static convertSbvCue_(caption) {
