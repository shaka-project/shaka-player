...
shaka.text.TtmlTextParser.prototype.parseMedia = function(data, time) {
  ...
  if (xml) {
    ...
    var rateInfo = new TtmlTextParser.RateInfo_(
        frameRate, subFrameRate, frameRateMultiplier, tickRate);

    var styles = TtmlTextParser.getLeafNodes_(
        tt.getElementsByTagName('styling')[0]);
    var regionElements = TtmlTextParser.getLeafNodes_(
        tt.getElementsByTagName('layout')[0]);
    var cueRegions = [];
    for (var i = 0; i < regionElements.length; i++) {
      var cueRegion = TtmlTextParser.parseCueRegion_(
          regionElements[i], styles);
      if (cueRegion)
        cueRegions.push(cueRegion);
    }

    var textNodes = TtmlTextParser.getLeafNodes_(
        tt.getElementsByTagName('body')[0]);

    for (var i = 0; i < textNodes.length; i++) {
      var cue = TtmlTextParser.parseCue_(textNodes[i],
                                         time.periodStart,
                                         rateInfo,
                                         styles,
                                         regionElements,
...
shaka.text.TtmlTextParser.parseCue_ = function(
  ...
  var regionElement = shaka.text.TtmlTextParser.getElementFromCollection_(
      cueElement, 'region', regionElements);
  if (regionElement && regionElement.getAttribute('xml:id')) {
    var regionId = regionElement.getAttribute('xml:id');
    var regionsWithId = cueRegions.filter(function(region) {
...
shaka.text.TtmlTextParser.parseCueRegion_ = function(regionElement, styles) {
  ...
  var id = regionElement.getAttribute('xml:id');
  if (!id) {
  ...
  var results = null;
  var percentage = null;
  var extent = TtmlTextParser.getStyleAttributeFromRegion_(
  ...
  var origin = TtmlTextParser.getStyleAttributeFromRegion_(
