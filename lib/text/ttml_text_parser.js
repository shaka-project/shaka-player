...
shaka.text.TtmlTextParser = class {
  ...
  parseMedia(data, time) {
    ...
    if (xml) {
      ...
      const regionDivs = XmlUtils.findChildren(body, 'div');
      for (const div of regionDivs) {
        // if the body has a div without Attributes and has div children when we should
        // create cues based on the children atributes
            if (div.hasChildNodes() && !div.hasAttributes()) {
                const childrenNodes = XmlUtils.findChildren(div, 'div');
                this.createCuesFromBodyChildNodes(childrenNodes, time, rateInfo, metadataElements, styles,
                    regionElements, cueRegions, whitespaceTrim, cellResolutionInfo, cues);
            } else  {
                const bodyChildrenNodes = XmlUtils.findChildren(body, 'div');
                this.createCuesFromBodyChildNodes(bodyChildrenNodes, time, rateInfo, metadataElements, styles,
                    regionElements, cueRegions, whitespaceTrim, cellResolutionInfo, cues);
            }
        }
    }
    ...
    /**
     * Parses the body div nodes into cues
     * @param bodyChildrenNodes
     * @param time
     * @param rateInfo
     * @param metadataElements
     * @param styles
     * @param regionElements
     * @param cueRegions
     * @param whitespaceTrim
     * @param cellResolutionInfo
     * @param cues
     */
  createCuesFromBodyChildNodes(bodyChildrenNodes, time, rateInfo, metadataElements,
                                 styles, regionElements, cueRegions, whitespaceTrim,
                                 cellResolutionInfo, cues) {
    const XmlUtils = shaka.util.XmlUtils;
    const TtmlTextParser = shaka.text.TtmlTextParser;

    for (const div of bodyChildrenNodes) {
  ...
  static parseCue_(
    ...
    if (Array.from(cueElement.childNodes).every(isTextNode)) {
  ...
  static addStyle_(
    ...
    // See https://github.com/google/shaka-player/issues/2623
