...
shaka.text.TtmlTextParser = class {
      ...
      for (const div of XmlUtils.findChildren(body, 'div')) {
        ...
        for (const p of XmlUtils.findChildren(div, 'p')) {
          const cue = TtmlTextParser.parseCue_(
              ...
              regionElements, cueRegions, whitespaceTrim, /* isNested= */ false,
              cellResolutionInfo);
          if (cue) {
            cues.push(cue);
          }
  ...
  static parseCue_(
    ...
    let spaceStyle = null;
    spaceStyle = cueElement.getAttribute('xml:space') ||
    ...
    // In this regex, "\W+" means "one or more non-word characters",
    // which is only relevant when we are trimming whitespace.
