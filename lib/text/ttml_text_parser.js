...
shaka.text.TtmlTextParser.prototype.parseMedia = function(data, time) {
  ...
  if (xml) {
    ...
    let metadataElements = TtmlTextParser.getLeafNodes_(
...
shaka.text.TtmlTextParser.parseCue_ = function(
  ...
  let imageElement = shaka.text.TtmlTextParser.getElementFromCollection_(
      cueElement, 'smpte:backgroundImage', metadataElements, '#');
  shaka.text.TtmlTextParser.addStyle_(cue,
                                      cueElement,
                                      regionElement,
                                      imageElement,
                                      styles);
...
shaka.text.TtmlTextParser.parseCueRegion_ = function(regionElement, styles,
  ...
  if (globalExtent) {
    percentage = TtmlTextParser.percentValues_.exec(globalExtent);
    results = percentage || TtmlTextParser.pixelValues_.exec(globalExtent);
    if (results != null) {
      globalWidth = Number(results[1]);
      globalHeight = Number(results[2]);
    }
  }
...
shaka.text.TtmlTextParser.addStyle_ = function(
  ...
  if (imageElement) {
    let backgroundImageType = imageElement.getAttribute('imagetype');
    let backgroundImageEncoding = imageElement.getAttribute('encoding');
    let backgroundImageData = imageElement.textContent;
