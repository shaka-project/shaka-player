...
shaka.text.TtmlTextParser = class {
  ...
  parseCue_(
    ...
    if (backgroundImage && imsc1ImgUrnTester.test(backgroundImage)) {
      ...
      const newUri = shaka.util.URL.resolve(uri, backgroundImage);
      if (newUri) {
        imageUri = newUri;
      }
    }
  ...
  addTextDecoration_(cue, decoration) {
    const Cue = shaka.text.Cue;
    // Adds or removes a decoration from the cue, avoiding duplicates.
    const set = (value, add) => {
      const has = cue.textDecoration.includes(value);
      if (add && !has) {
        cue.textDecoration.push(value);
      } else if (!add && has) {
        shaka.util.ArrayUtils.remove(cue.textDecoration, value);
      }
    };
    for (const value of decoration.split(' ')) {
      switch (value) {
        case 'underline':
          set(Cue.textDecoration.UNDERLINE, true);
          break;
        case 'noUnderline':
          set(Cue.textDecoration.UNDERLINE, false);
          break;
        case 'lineThrough':
          set(Cue.textDecoration.LINE_THROUGH, true);
          break;
        case 'noLineThrough':
          set(Cue.textDecoration.LINE_THROUGH, false);
          break;
        case 'overline':
          set(Cue.textDecoration.OVERLINE, true);
          break;
        case 'noOverline':
          set(Cue.textDecoration.OVERLINE, false);
          break;
      }
    }
  }
