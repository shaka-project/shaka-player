...
shaka.text.TtmlTextParser = class {
  ...
  static getLeafNodes_(element) {
    ...
    for (const node of element.childNodes) {
      ...
      const isSpanChildOfP = node.nodeName == 'span' && element.nodeName == 'p';
      if (node.nodeType == Node.ELEMENT_NODE &&
          node.nodeName != 'br' && !isSpanChildOfP) {
        // Get the leaves the child might contain.
        goog.asserts.assert(
            node instanceof Element, 'Node should be Element!');
  ...
  static addNewLines_(element, whitespaceTrim) {
    let prevNode = null;
    for (const node of element.childNodes) {
      if (node.nodeName == 'br' && prevNode) {
        prevNode.textContent += '\n';
      } else if (node.childNodes.length > 0) {
        shaka.text.TtmlTextParser.addNewLines_(node, whitespaceTrim);
  ...
  static getElementFromCollection_(
    ...
    if (itemName) {
      for (const cur of collection) {
        if ((prefixName + cur.getAttribute('xml:id')) == itemName) {
          item = cur;
          break;
        }
      }
