...
shaka.text.TtmlTextParser = class {
  ...
  parseMedia(data, time) {
    ...
    if (xml) {
      ...
      for (const div of regionDivs) {
        // if the body has a div without Attributes and has div
        // children when we should create cues based on
        // the children atributes
        if (div.hasChildNodes() && !div.hasAttributes()) {
          const childrenNodes = XmlUtils.findChildren(div, 'div');
          this.createCuesFromBodyChildNodes(childrenNodes,
              time, rateInfo, metadataElements, styles,
              regionElements, cueRegions, whitespaceTrim,
              cellResolutionInfo, cues);
        } else {
          const bodyChildrenNodes = XmlUtils.findChildren(body, 'div');
          this.createCuesFromBodyChildNodes(bodyChildrenNodes,
              time, rateInfo, metadataElements, styles,
              regionElements, cueRegions, whitespaceTrim,
              cellResolutionInfo, cues);
        }
  ...
  /**
     ...
     * @param {!Array.<!Element>} bodyChildrenNodes
     ...
     * @param {?{columns: number, rows: number}} cellResolutionInfo
     * @param {!Array.<!shaka.text.Cue>} cues
     */
  createCuesFromBodyChildNodes(
      bodyChildrenNodes, time, rateInfo, metadataElements, styles,
