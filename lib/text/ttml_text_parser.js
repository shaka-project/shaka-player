...
shaka.text.TtmlTextParser = class {
  ...
  static parseCue_(
    ...
    let cueElement;
    /** @type {shaka.extern.xml.Node} */
    // TODO: need to sort out parent node
    let parentElement =
    /** @type {shaka.extern.xml.Node} */ (cueNode.parent);

    if (cueNode.innerText) {
      // if (!isContent) {
      ...
      const span = {
        tagName: 'span',
        innerText: cueNode.innerText,
        children: [],
        attributes: {},
        parent: parentElement,
      };
      for (const key in cueNode.attributes) {
        span.attributes[key] = cueNode.attributes[key];
      }
      cueElement = span;
    } else {
    ...
    // };
    // const isLeafNode = cueElement.children.every(isTextNode);
    const nestedCues = [];
    if (cueElement.children.length) {
    ...
    const hasTextContent = /\S/.test(cueElement.innerText);
  ...
  static addStyle_(
    ...
    if (imageElement) {
      ...
      const backgroundImageData = imageElement.innerText.trim();
