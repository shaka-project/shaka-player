...
shaka.text.NativeTextDisplayer = class {
        ...
        if (this.trackNodes_.has(track.id)) {
          ...
          if (track.language) {
            if (track.language in mozilla.LanguageMapping) {
              ...
              trackNode.srclang = base ?? 'und';
            }
          } else {
            trackNode.srclang = 'und';
          }
          // The built-in captions menu in Chrome may refuse to list invalid
          // subtitles.
          // In Safari, when a TextTrack's mode is set to "disabled" or "hidden"
          // and then "showing" again causes a blink if there is no src.
          // The data URL is just to avoid this.
  ...
  append(cues) {
    if (this.trackNodes_.has(this.trackId_)) {
      ...
      let adjustedCues = cues;
      if (delay != 0) {
        adjustedCues = cues.map((cue) => {
          const newCue = cue.clone();
          newCue.startTime = Math.max(0, newCue.startTime + delay);
          newCue.endTime = Math.max(0, newCue.endTime + delay);
          return newCue;
        });
      }
      shaka.text.Utils.appendCuesToTextTrack(
          this.trackNodes_.get(this.trackId_).track, adjustedCues);
    }
  }
