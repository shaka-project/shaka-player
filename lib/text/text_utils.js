...
shaka.text.Utils = class {
  ...
  static flattenPayload_(cue) {
    // Handle styles (currently colors and bold/italics/underline).
    const openStyleTags = [];
    const colors = cue.color || cue.backgroundColor;
    ...
    if (colors) {
      let extension = '';
      if (cue.color) {
        const textColorMap = shaka.text.Cue.defaultTextColor;
        const cueColorClass = Object.keys(textColorMap)
            .find((key) => textColorMap[key] === cue.color);
        if (cueColorClass) {
          extension += `.${cueColorClass}`;
        }
      }
      if (cue.backgroundColor) {
        const textBgColorMap = shaka.text.Cue.defaultTextBackgroundColor;
        const cueBgColorClass = Object.keys(textBgColorMap)
            .find((key) => textBgColorMap[key] === cue.backgroundColor);
        if (cueBgColorClass) {
          extension += `.${cueBgColorClass}`;
        }
      }
      openStyleTags.push({name: 'c', extension});
    }
    if (bold) {
      openStyleTags.push({name: 'b'});
    }
    if (italics) {
      openStyleTags.push({name: 'i'});
    }
    if (underline) {
      openStyleTags.push({name: 'u'});
    }

    // Prefix opens tags, suffix closes tags in reverse order of opening.
    const prefixStyleTags = openStyleTags.reduce((acc, tag) => {
      return `${acc}<${tag.name}${tag.extension || ''}>`;
    }, '');
