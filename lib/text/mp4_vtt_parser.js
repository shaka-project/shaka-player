...
shaka.text.Mp4VttParser = class {
  ...
  parseMedia(data, time) {
    ...
    // Each fragment - a moof and the mdat after it - is read as it comes, with
    // its own tfdt, tfhd and trun: a segment may be several of them, as
    // low-latency (chunked) packaging makes it, and reading only the last one
    // would lose the cues of all the others.
    const parseFragment = (/** !Uint8Array */ rawPayload) => {
      this.parseFragment_(rawPayload, presentations, baseTime,
          defaultDuration, time, cues);
      // A fragment without a tfdt continues where this one ends.
      for (const presentation of presentations) {
        baseTime += presentation.sampleDuration || defaultDuration || 0;
      }
      presentations = [];
    };
  ...
  /**
   * Reads the cues of one fragment: the samples its trun describes, from the
   * mdat that follows it.
   *
   * @param {!Uint8Array} rawPayload
   * @param {!Array<shaka.util.ParsedTRUNSample>} presentations
   * @param {number} baseTime in timescale units, from the tfdt
   * @param {?number} defaultDuration in timescale units, from the tfhd
   * @param {shaka.extern.TextParser.TimeContext} time
   * @param {!Array<shaka.text.Cue>} cues to add the cues to
   * @private
   */
  parseFragment_(rawPayload, presentations, baseTime, defaultDuration, time,
      cues) {
