...
shaka.text.SsaTextParser = class {
  ...
  static parseSsaColor_(colorString) {
    ...
    let abgr = -1;
    try {
      if (colorString.startsWith('&H')) {
        // See: https://bugs.chromium.org/p/v8/issues/detail?id=2869
        let abgrString = (' ' + colorString).slice(1);
        if (colorString.length < 10) {
          // Add leading zeros if necessary.
          while (abgrString.length != 10) {
            abgrString = [
              abgrString.slice(0, 2),
              '0',
              abgrString.slice(2),
            ].join('');
          }
        }
        abgr = parseInt(abgrString.substring(2), 16);
      } else {
        // Parse color from decimal format (bytes order AABBGGRR).
        abgr = parseInt(colorString, 16);
      }
    } catch (e) {
      shaka.log.warning('Error parsing SSA color.', colorString);
    }
    if (abgr >= 0) {
      const a = ((abgr >> 24) & 0xFF) ^ 0xFF; // Flip alpha.
      let alpha = Math.min(1, Math.max(0, (a / 255).toFixed(2)));
