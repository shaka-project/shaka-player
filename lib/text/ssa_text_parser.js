...
shaka.text.SsaTextParser = class {
      ...
      if (lineParts) {
        ...
        if (name == 'Dialogue') {
          ...
          const indexOfPayload =
            value.split(',', eventColumns.length - 1).join(',').length + 1;
  ...
  static addStyle_(cue, style) {
    ...
    if (color) {
      const match = SsaTextParser.colorFormat_.exec(color);
      if (match) {
        cue.color = '#' + match[4] + match[3] + match[2];
      }
    }
    const backgroundColor = style['BackColour'];
    if (backgroundColor) {
      const match = SsaTextParser.colorFormat_.exec(backgroundColor);
      if (match) {
        cue.backgroundColor = '#' + match[4] + match[3] + match[2];
      }
    }
 ...
 * @example &H00B4FCFC
 */
shaka.text.SsaTextParser.colorFormat_ =
