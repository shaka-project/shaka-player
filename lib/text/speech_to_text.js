...
shaka.text.SpeechToText = class {
  ...
  async initRecognition_(mediaStreamTrack, sourceLanguage, targetLanguage) {
    ...
    if (this.config_.processLocally &&
      ...
      try {
        ...
        shaka.log.debug('Speech to text: processLocally status',
            sourceLanguage, status);

        switch (status) {
          case 'available':
            this.recognition_.processLocally = true;
            break;
          case 'downloading':
          case 'downloadable':
            SpeechRecognition.install({
              ...
              this.initRecognition_(
                  mediaStreamTrack, sourceLanguage, targetLanguage);
            });
            return;
          case 'unavailable':
          default:
        ...
        shaka.log.error(
            'Speech to text: error getting status of processLocally',
            sourceLanguage, e);
      }
    }
    ...
    this.recognitionEventManager_.listen(this.recognition_, 'error', (e) => {
      ...
      if (event.error == 'network') {
        this.recognitionTimer_.tickAfter(/* seconds= */ 30);
      }
      shaka.log.debug('Speech to text: error', event);
      recognitionError = true;
    });
    this.recognitionEventManager_.listen(this.recognition_, 'end', (e) => {
