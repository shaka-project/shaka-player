      ...
      this.recognitionEventManager_.listen(this.recognition_, 'result',
          (e) => {
            ...
            for (let i = event.resultIndex; i < event.results.length; i++) {
              // The Web Speech API adds appropriate leading/trailing whitespace.
              text += event.results[i][0].transcript;
            }
            const words = text.split(' ');
            if (words.length > maxNumberOfWordsBeforeTruncate) {
              text = words.slice(-maxNumberOfWordsAfterTruncate).join(' ');
            }
