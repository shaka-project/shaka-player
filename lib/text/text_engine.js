...
/**
 * Get the size of the map which stores the closed captions.
 * This function is used only for unit tests.
 * @return {number}
 */
 ...
 * This function is used only for unit tests.
 *
 * @param {string} id Id of the closed captions.
 * @return {number}
 */
shaka.text.TextEngine.prototype.getNumberOfClosedCaptionsInChannel =
    function(id) {
  if (!this.closedCaptionsMap_ || ! this.closedCaptionsMap_.get(id)) {
    return 0;
  }
  return this.closedCaptionsMap_.get(id).size;
};
