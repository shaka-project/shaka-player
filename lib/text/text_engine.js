...
shaka.text.TextEngine = class {
  ...
  /**
   * @param {number} t A timestamp
   * @return {number} Number of seconds behind 't' we have buffered
   */
  bufferedBehindOf(t) {
    if (this.bufferStart_ == null || this.bufferStart_ > t) {
      return 0;
    }

    goog.asserts.assert(
        this.bufferEnd_ != null,
        'end should not be null if start is not null');

    return Math.min(t, this.bufferEnd_) - this.bufferStart_;
  }
