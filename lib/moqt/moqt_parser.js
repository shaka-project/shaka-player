...
goog.provide('shaka.moqt.MoqtParser');
...
goog.require('shaka.moqt.MoqtTransport');
...
/**
 * MoQT (Media over QUIC Transport) + WARP parser.
 *
...
shaka.moqt.MoqtParser = class {
  constructor() {
    ...
    this.moqtTransport_ = null;

    /** @private {?shaka.moqt.MoqtConnection} */
    this.connection_ = null;

    /** @private {!Array<Array<string>>} */
    this.announceNamespaces_ = [];

    /** @private {?function()} */
    this.unregisterAnnounceCallback_ = null;
  ...
  async start(uri, playerInterface) {
    ...
    this.moqtTransport_ = new shaka.moqt.MoqtTransport();

    /** @type {?Uint8Array} */
    let fingerprint = null;
    if (this.config_.moqt.fingerprintUri) {
      const requestType = shaka.net.NetworkingEngine.RequestType.FINGERPRINT;
      const request = shaka.net.NetworkingEngine.makeRequest(
          [this.config_.moqt.fingerprintUri], this.config_.retryParameters);
    ...
    try {
      this.connection_ = await this.moqtTransport_.connect(uri, fingerprint);
    } catch (error) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
    ...
    this.manifest_ = {
      ...
      type: shaka.media.ManifestParser.MOQT,
    ...
    if (!this.presentationTimeline_.isLive()) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.MANIFEST,
          shaka.util.Error.Code.MOQT_VOD_CONTENT_NOT_SUPPORTED);
    }
  ...
  async stop() {
    ...
    if (this.unregisterAnnounceCallback_) {
      this.unregisterAnnounceCallback_();
      this.unregisterAnnounceCallback_ = null;
    }
    ...
    if (this.moqtTransport_) {
      this.moqtTransport_.release();
      this.moqtTransport_ = null;
    }
    this.playerInterface_ = null;
    this.config_ = null;
  ...
  listenForAnnouncements_() {
    try {
      ...
      const unregister =
          this.moqtTransport_.registerAnnounceCallback((namespace) => {
            shaka.log.debug(`Received announcement callback with namespace:
                ${namespace.join('/')}`);

            const namespaceStr = namespace.join('/');
            const isAlreadyProcessed = this.announceNamespaces_.some((ns) =>
            ...
            // Store the namespace
            this.announceNamespaces_.push(namespace);

            // Subscribe to the catalog in this namespace
            this.subscribeToCatalog_(namespace).catch((error) => {
      ...
      this.unregisterAnnounceCallback_ = unregister;
  ...
  async subscribeToCatalog_(namespace) {
    try {
      ...
      const trackAlias = await this.moqtTransport_.subscribeTrack(
          namespaceStr, 'catalog', (obj) => {
            try {
              const text = shaka.util.StringUtils.fromUTF8(obj['data']);
              const catalog = /** @type {shaka.moqt.Utils.WarpCatalog} */(
                JSON.parse(text));
      ...
      const unregisterFunc = () => {
        ...
        shaka.log.debug(`Unsubscribing from catalog track with alias
            ${trackAlias}`);
        this.moqtTransport_.unsubscribeTrack(trackAlias).catch((err) => {
  ...
  /**
   * @param {shaka.moqt.Utils.WarpTrack} track
  ...
  /**
   * @param {shaka.moqt.Utils.WarpCatalog} catalog
   * @return {!Promise}
   * @private
   */
  processCatalog_(catalog) {
    shaka.log.debug('WARP Catalog:', catalog);
    const promises = [];
    let isLive = true;
...
shaka.media.ManifestParser.registerParserByMime(
    'application/moqt', () => new shaka.moqt.MoqtParser());
