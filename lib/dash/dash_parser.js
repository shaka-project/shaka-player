...
shaka.dash.DashParser = class {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    if (this.previousMpdType_ != mpdType) {
      this.previousMpdType_ = mpdType;
      const eventName = shaka.util.FakeEvent.EventName.StreamTypeChanged;
      const data = (new Map()).set('detail', {isLive: mpdType != 'static'});
