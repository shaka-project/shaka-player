...
shaka.dash.DashParser = class {
  ...
  async requestManifest_() {
    ...
    if (patchLocationUris.length) {
      manifestUris = patchLocationUris.map(
          (uri) => this.applyRequestParamFn_(uri, this.mpdpatchUrlParamsFn_));
      rootElement = 'Patch';
      ...
      if (this.mpdUrlParamsFn_) {
        manifestUris = manifestUris.map(
            (uri) => this.applyRequestParamFn_(uri, this.mpdUrlParamsFn_));
      }
    }
  ...
  parseMpdChaining_(mpd) {
    ...
    for (const prop of supplementalProperties) {
      ...
      if (schemeId == 'urn:mpeg:dash:chaining:2016') {
        ...
        return this.applyRequestParamFn_(
   ...
   * @private
   */
  applyRequestParamFn_(uri, paramFn) {
    if (!paramFn) {
      return uri;
    }
