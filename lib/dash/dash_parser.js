...
shaka.dash.DashParser = class {
  ...
  computePeriodSignature_(node) {
    /**
     * @param {(!shaka.extern.xml.Node|string)} element
    ...
    const sanitize = (element) => {
      ...
      const attributes = shaka.util.ObjectUtils.shallowCloneObject(
          element.attributes || {});
      const children = element.children ?
          element.children.map((child) => sanitize(child)) : [];
      return {
    ...
    return shaka.util.ObjectUtils.alphabeticalKeyOrderStringify(sanitized);
  }
