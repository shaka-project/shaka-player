...
shaka.dash.DashParser = class {
  ...
  async requestManifest_() {
    ...
    const mimeType =
        contentType ? contentType.toLowerCase().split(';').shift() : '';

    // This may throw, but it will result in a failed promise.
    await this.parseManifest_(
        response.data, response.uri, rootElement, mimeType);
   ...
   * @private
   */
  async parseManifest_(data, finalManifestUri, rootElement, mimeType) {
    let mpd;
    if (mimeType === 'application/dash+json') {
      const jsonString = shaka.util.StringUtils.fromBytesAutoDetect(data);
      try {
        const json = /** @type {!Object} */ (JSON.parse(jsonString));
        const xmlString = shaka.dash.MpdUtils.jsonToMpd(json);
      ...
      mpd = shaka.util.TXml.parseXml(data, rootElement);
    }
    if (!mpd) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
...
shaka.media.ManifestParser.registerParserByMime(
    'application/dash+json', () => new shaka.dash.DashParser());
