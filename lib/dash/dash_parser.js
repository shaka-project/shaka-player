...
shaka.dash.DashParser = class {
  ...
  async start(uri, playerInterface) {
    ...
    this.lowLatencyMode_ = this.config_.lowLatencyMode;
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    if (this.minTotalAvailabilityTimeOffset_) {
      if (this.lowLatencyMode_) {
        ...
        // manifest.lowLatencyMode value should be set to true to stream with
        // low latency mode.
        shaka.log.alwaysWarn('Low-latency DASH live stream detected, but ' +
          'low-latency mode is not enabled in Shaka Player. ' +
          'Set manifest.lowLatencyMode configuration to true, and see ' +
    ...
    if (!this.manifest_) {
      this.manifest_ = {
        ...
        lowLatency: this.lowLatencyMode_ &&
            this.minTotalAvailabilityTimeOffset_ ? true : false,
      };

      if (this.manifest_.lowLatency) {
        this.playerInterface_.configureLowLatency();
      }
