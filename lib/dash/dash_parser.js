...
shaka.dash.DashParser = class {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    if (!this.manifest_) {
      this.manifest_ = {
        presentationTimeline: presentationTimeline,
        variants: shaka.util.Periods.stitchVariants(
            periods.map((period) => period.variants)),
        textStreams: shaka.util.Periods.stitchTextStreams(
            periods.map((period) => period.textStreams)),
        offlineSessionIds: [],
   ...
   * @param {!Element} mpd
   * @return {{
   *   periods: !Array.<shaka.dash.DashParser.PeriodStreams>,
   *   duration: ?number,
   ...
   * @return {shaka.dash.DashParser.PeriodStreams}
   * @private
   */
  ...
  parseAdaptationSet_(context, elem) {
    ...
    for (const stream of streams) {
      ...
      for (const drmInfo of contentProtection.drmInfos) {
        drmInfo.keyIds.push(...stream.keyIds);
      }
    }
 ...
 *
 * @description
 * Contains the variants and text streams from one DASH period.
 *
 ...
 *   The variants from one Period.
 */
shaka.dash.DashParser.PeriodStreams;
