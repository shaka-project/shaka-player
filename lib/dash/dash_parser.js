...
shaka.dash.DashParser = class {
  ...
  parsePeriods_(context, getBaseUris, mpd, newPeriod) {
    ...
    const enablePeriodCaching = !!(this.config_ && this.config_.dash &&
    ...
    for (let i = 0; i < periodNodes.length; i++) {
      ...
      if (enablePeriodCaching) {
        ...
        if (reusePeriod) {
          ...
          console.log('Reusing period', period);
        }
      }
      if (!period) {
        ...
        if (enablePeriodCaching) {
          signature = signature || this.computePeriodSignature_(elem);
        }
      }
      periods.push(period);
   ...
   * @private
   */
  hashTXml(node, seed = 0x811c9dc5) {
    let hash = this.fnv1a(node.tagName, seed >>> 0);

    if (node.attributes) {
      const keys = Object.keys(node.attributes).sort();
      for (const key of keys) {
        hash = this.fnv1a('A', hash);
        hash = this.fnv1a(key, hash);
        hash = this.fnv1a('=', hash);
        hash = this.fnv1a(node.attributes[key], hash);
      }
    }
    ...
    if (node.children) {
      for (const child of node.children) {
        if (typeof child === 'string') {
          hash = this.fnv1a('S', hash);
          hash = this.fnv1a(child, hash);
        } else {
          hash = this.hashTXml(child, hash);
        }
      }
    }
