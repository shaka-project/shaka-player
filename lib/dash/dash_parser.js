...
shaka.dash.DashParser = class {
  ...
  async processPatchManifest_(mpd) {
    ...
    const additions = TXml.findChildren(mpd, 'add');
    ...
    for (const addition of additions) {
      const selector = addition.attributes['sel'];
      ...
      switch (paths[paths.length - 1].name) {
        case 'MPD':
          if (selector == '/MPD/@mediaPresentationDuration') {
            const content = TXml.getContents(addition) || '';
            this.parsePatchMediaPresentationDurationChange_(content);
          } else {
            periodAdditions.push(addition);
          }
          break;
        case 'SegmentTimeline':
          this.parsePatchSegment_(paths, TXml.findChildren(addition, 'S'));
          break;
        default:
          shaka.log.warning('Unhandled add', selector);
      }
    }

    // Add new periods after extending timelines, as new periods
    // remove context cache of previous periods.
    for (const addition of periodAdditions) {
      newPeriods.push(...this.parsePatchPeriod_(addition));
    }

    const replaces = TXml.findChildren(mpd, 'replace');
    for (const replace of replaces) {
      const selector = replace.attributes['sel'];
      const content = TXml.getContents(replace) || '';

      if (selector == '/MPD/@type') {
        this.parsePatchMpdTypeChange_(content);
      } else if (selector == '/MPD/@mediaPresentationDuration') {
        this.parsePatchMediaPresentationDurationChange_(content);
      } else if (selector.startsWith('/MPD/PatchLocation')) {
        this.updatePatchLocationUris_(replace);
      } else {
        shaka.log.warning('Unhandled replace', selector);
      }
    }

    const removes = TXml.findChildren(mpd, 'remove');
    for (const remove of removes) {
      const selector = remove.attributes['sel'];
      shaka.log.warning('Unhandled remove', selector);
    }
 ...
 * @property {Array<shaka.extern.xml.Node>} timepoints
