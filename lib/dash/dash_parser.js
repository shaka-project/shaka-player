...
shaka.dash.DashParser = class {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    console.log('***** dashparser', this.config_.dash.enablePatchMPDSupport,
        this.config_.dash.manifestPreprocessor);
    ...
    if (!this.manifest_) {
      ...
      await this.postPeriodProcessing_(periodsAndDuration.periods);
    }
  }
   ...
   * @private
   */
  async postPeriodProcessing_(periods) {
    await this.periodCombiner_.combinePeriods(periods, true);
  ...
  async processPatchManifest_(mpd) {
    ...
    if (newPeriods.length) {
      await this.postPeriodProcessing_(newPeriods);
    }
