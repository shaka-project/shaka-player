...
shaka.dash.DashParser = class {
  ...
  getURLParametersFunction_(elem) {
    ...
    if (urlQueryInfo && TXml.parseAttr(urlQueryInfo, 'useMPDUrlQuery',
      ...
      const queryTemplate = shaka.util.StringUtils.htmlUnescape(
          urlQueryInfo.attributes['queryTemplate'] || '');
      if (queryTemplate) {
        return () => {
          ...
          const parameters = queryTemplate.split('&').map((param) => {
            if (param == '$querypart$') {
              ...
              if (parts && parts.length == 2) {
                ...
                if (!queryData) {
                  queryData =
                      new URLSearchParams(this.lastManifestQueryParams_);
                }
  ...
  findRequestParamFn_(node, includeInType) {
    ...
    for (const param of TXml.findChildren(node, 'RequestParam')) {
      ...
      if (includeIn.includes(includeInType) || includeIn.includes('*')) {
        const fn = this.buildUrlParamsFn_(param);
        if (fn) {
          result = fn;
        }
      }
  ...
  /**
   * Builds a URL params function from a RequestParam element
   * (urn:mpeg:dash:urlparam:2025 scheme). Reads queryTemplate and
   * useMPDUrlQuery attributes directly from the element.
   *
   * @param {!shaka.extern.xml.Node} elem
   * @return {?function():string}
   * @private
   */
  buildUrlParamsFn_(elem) {
    const TXml = shaka.util.TXml;
    if (!TXml.parseAttr(elem, 'useMPDUrlQuery',
        TXml.parseBoolean, /* defaultValue= */ false)) {
      return null;
    }
    // Decode XML entities (&amp; → &) since TXml does not decode them in
    ...
    return () => {
      ...
      const parameters = queryTemplate.split('&').map((param) => {
        ...
        if (queryParamMatch) {
          ...
          if (!queryData) {
            queryData = new URLSearchParams(this.lastManifestQueryParams_);
          }
          const value = queryData.get(paramName);
          if (value) {
            return `${paramName}=${value}`;
        ...
        // Per DASH spec I.2.2.2: replace unknown $...$ identifiers with
