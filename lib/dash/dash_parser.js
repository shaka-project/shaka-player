...
shaka.dash.DashParser = class {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    if (uriObjs && uriObjs.length) {
      availabilityTimeOffset = TXml.parseAttr(
          uriObjs[0], 'availabilityTimeOffset',
    ...
    if (!ignoreMinBufferTime) {
      minBufferTime =
          TXml.parseAttr(mpd, 'minBufferTime', XmlUtils.parseDuration) || 0;
    }
  ...
  parsePeriods_(context, getBaseUris, mpd) {
    ...
    for (let i = 0; i < periodNodes.length; i++) {
      ...
      const start = /** @type {number} */ (
        TXml.parseAttr(elem, 'start', XmlUtils.parseDuration, prevEnd));
      const periodId = elem.attributes['id'];
      const givenDuration =
          TXml.parseAttr(elem, 'duration', XmlUtils.parseDuration);
      ...
      if (next) {
        ...
        const nextStart =
            TXml.parseAttr(next, 'start', XmlUtils.parseDuration);
        if (nextStart != null) {
  ...
  */
  ...
  parseAdaptationSet_(context, elem) {
    ...
    if (labelElements && labelElements.length) {
      ...
      if (firstLabelElement.innerText) {
        label = firstLabelElement.innerText;
      }
    }
  ...
  parseRepresentation_(context, contentProtection, kind, language, label,
    ...
    context.bandwidth =
        TXml.parseAttr(node, 'bandwidth', XmlUtils.parsePositiveInt) || 0;
  ...
  parseEventStream_(periodStart, periodDuration, elem, availabilityStart) {
    const TXml = shaka.util.TXml;
    const parseNumber = shaka.util.XmlUtils.parseNonNegativeInt;
