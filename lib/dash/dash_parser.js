...
shaka.dash.DashParser = class {
  ...
  constructor() {
     ...
     * @private {!Map<string, !shaka.extern.Period>}
     */
    this.parsedPeriodCache_ = new Map();
    ...
    /**
     * Encrypted streams whose key ID is not declared in the manifest, along
     * with the init segment we can read it from.
     * @private {!Array<{stream: !shaka.extern.Stream,
     *                   initSegmentReference: !shaka.media.InitSegmentReference
     *                  }>}
     */
    this.streamsWithUnknownKeyId_ = [];

    /**
     * A map of init segments to the default KID found inside them.  Keyed by
     * URI and byte range.
    ...
    this.mpdpatchUrlParamsFn_ = null;

    /** @private {?(function():string)} */
    this.mpdUrlParamsFn_ = null;
  ...
  async requestManifest_() {
    ...
    if (patchLocationUris.length) {
      manifestUris = patchLocationUris.map((uri) =>
        shaka.util.URL.applyUrlParams(uri, this.mpdpatchUrlParamsFn_));
      rootElement = 'Patch';
      ...
      if (this.mpdUrlParamsFn_) {
        manifestUris = manifestUris.map(
            (uri) => shaka.util.URL.applyUrlParams(uri, this.mpdUrlParamsFn_));
      }
    }
    ...
    this.lastManifestQueryParams_ = url.searchParams.toString();

    // This may throw, but it will result in a failed promise.
    await this.parseManifest(response.data, response.uri, rootElement);
   ...
   * @return {!Promise}
   */
  async processParsedMpd(mpd, finalManifestUri, rootElement) {
    if (!mpd) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
    ...
    // Resolve Linked Periods (ImportedMPD elements, DASH 6th ed. §5.3.2.6).
    let resolvedMpd = /** @type {!shaka.extern.xml.Node} */ (mpd);
    if (shaka.dash.MpdUtils.hasLinkedPeriods(resolvedMpd)) {
      const linkedPeriodOp = shaka.dash.MpdUtils.processLinkedPeriods(
          resolvedMpd, finalManifestUri, this.config_.retryParameters,
          this.playerInterface_.networkingEngine);
      this.operationManager_.manage(linkedPeriodOp);
      resolvedMpd = await linkedPeriodOp.promise;
    }

    if (!shaka.dash.MpdUtils.hasXlinks(resolvedMpd)) {
      return this.processManifest_(resolvedMpd, finalManifestUri);
    }
    ...
    const xlinkOperation = shaka.dash.MpdUtils.processXlinks(
        resolvedMpd, this.config_.retryParameters, failGracefully,
        finalManifestUri, this.playerInterface_.networkingEngine);
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    // MPD-level Location and BaseURL elements that carry a serviceLocation,
    // as absolute URIs, for the CMCD service-location filter
    // (ServiceDescription/ClientDataReporting@serviceLocations). Collected
    // here, from the same resolution steps that produce the request URIs,
    // so the filter's prefix match sees the URIs requests actually use.
    /** @type {!Array<shaka.extern.ServiceLocationBaseUri>} */
    const serviceLocationBaseUris = [];
    ...
    for (const locationsObj of locationsObjs) {
      ...
      const finalUri = shaka.util.URL.resolveUris(manifestBaseUris, [uri])[0];
      if (serviceLocation) {
        serviceLocationBaseUris.push(
            {serviceLocation: serviceLocation, uri: finalUri});
        if (this.contentSteeringManager_) {
    ...
    this.mpdpatchUrlParamsFn_ =
        this.findRequestParamFn_(mpd, 'mpdpatch') || null;
    this.mpdUrlParamsFn_ = this.findRequestParamFn_(mpd, 'mpd') || null;
    ...
    if (contentSteering && this.playerInterface_) {
      ...
      if (!this.contentSteeringManager_) {
        ...
        let uri = TXml.getContents(contentSteering);
        if (uri) {
          uri = shaka.util.URL.applyUrlParams(
              uri, this.findRequestParamFn_(mpd, 'steering'));
    ...
    for (const uriObj of uriObjs) {
      ...
      if (serviceLocation && uri) {
        ...
        for (const absolute of resolved) {
          serviceLocationBaseUris.push(
              {serviceLocation: serviceLocation, uri: absolute});
        }
      }
    }
    ...
    if (!someLocationValid || !this.contentSteeringManager_) {
      ...
      this.lastCalculatedBaseUris_.push(
          ...shaka.util.URL.resolveUris(manifestBaseUris, uris));
    }
    ...
    // cspell:words minifiers
    // Calling makeBaseUriGetter dynamically to prevent makeBaseUriGetter_ from
    // being inlined by downstream minifiers (e.g. Terser) when Shaka Player is
    // bundled into other projects. If the function gets inlined, it captures
    // `mpd` from the lexical scope and leaks memory.
    const makeBaseUriGetter_ = shaka.dash.DashParser.makeBaseUriGetter_;
    // eslint-disable-next-line no-restricted-syntax
    const getBaseUris = makeBaseUriGetter_.call(shaka.dash.DashParser,
        contentSteeringManager, someLocationValid, lastCalculatedBaseUris);
    ...
    // The ISO-BMFF List Profile (DASH 6th ed.) uses type="list", which is a
    // static/on-demand profile. Treat it as static for timeline purposes.
    const isStaticType = mpdType == 'static' || mpdType == 'list';

    if (this.manifest_ && this.manifest_.presentationTimeline) {
      this.isTransitionFromDynamicToStatic_ =
          this.manifest_.presentationTimeline.isLive() && isStaticType;
    }
    ...
    presentationTimeline.setStatic(isStaticType);
    ...
    const context = {
      ...
      roles: null,
      urlParams: shaka.dash.DashParser.defaultUrlParams_,
      name: '',
    };

    await contentSteeringPromise;

    // Parse MPD-level urlparam:2014 / urlparam:2016 descriptors (their
    // UrlQueryInfo / ExtUrlQueryInfo child, ISO/IEC 23009-1 Annex I).
    ...
    const isUrlParamDescriptor = (d) =>
      shaka.dash.DashParser.URL_PARAM_DESCRIPTOR_SCHEMES_.has(d.schemeIdUri);
    const mpdUrlParamDescriptor =
        essentials.find(isUrlParamDescriptor) ||
        supplementals.find(isUrlParamDescriptor);
    if (mpdUrlParamDescriptor) {
      const descriptorUrlParams =
          this.getURLParametersFunction_(mpdUrlParamDescriptor.element);
      if (descriptorUrlParams) {
        context.urlParams = descriptorUrlParams;
      }
    }

    // Parse MPD-level RequestParam elements (urlparam:2025, DASH 6th ed.)
    const mpdUrlParams = this.findRequestParamFn_(mpd, 'segment');
    if (mpdUrlParams) {
      context.urlParams = mpdUrlParams;
    }
    ...
    if ((isStaticType && !this.isTransitionFromDynamicToStatic_) ||
    ...
    if (!this.manifest_) {
      ...
      this.manifest_ = {
        ...
        serviceDescription:
            shaka.dash.ServiceDescriptionParser.parse(
                mpd, serviceLocationBaseUris),
      ...
      const programInformation =
          shaka.util.TXml.findChild(mpd, 'ProgramInformation');
      if (programInformation) {
        const eventName = shaka.util.FakeEvent.EventName.ProgramInformation;
        const data = (new Map()).set('detail', programInformation);
      ...
      // ServiceDescription can change between refreshes (Annex K), for
      // example to enable or reconfigure CMCD reporting.
  ...
  removePatchPeriod_(periodId) {
    ...
    this.parsedPeriodCache_.delete(periodId);
  }
  ...
  parsePeriods_(context, getBaseUris, mpd, newPeriod) {
    ...
    const usePeriodCache = context.dynamic;
    if (!usePeriodCache) {
      this.parsedPeriodCache_.clear();
    }
    let offset = 0;
    const oldInnerIds = (usePeriodCache && !newPeriod) ?
        new Set(this.lastManifestUpdatePeriodIds_.slice(1, -1)) : null;

    for (let i = 0; i < periodNodes.length; i++) {
      const elem = periodNodes[i];
      const next = periodNodes[i + 1];
      let start = /** @type {number|null} */ (
        TXml.parseAttr(elem, 'start', TXml.parseDuration, null));
      let periodId = elem.attributes['id'];
      const givenDuration =
          TXml.parseAttr(elem, 'duration', TXml.parseDuration);
      if (i == 0 && this.isTransitionFromDynamicToStatic_) {
        offset = seekRangeStart - (start || 0);
      }
      if (start == null) {
        start = prevEnd;
      } else {
        if (this.isTransitionFromDynamicToStatic_) {
          start = start + offset;
        }
      }
      ...
      let period = null;
      const isInnerId = i > 0 && i < periodNodes.length - 1;
      if (usePeriodCache &&
        oldInnerIds &&
        isInnerId &&
        !newPeriod) {
        periodId ??= this.periodIdFallback_(start);
        if (oldInnerIds.has(periodId)) {
          const parsedPeriod =
            this.parsedPeriodCache_.get(periodId);
          if (parsedPeriod) {
            period = parsedPeriod;
          }
        }
      ...
      if (!period) {
        period = this.parsePeriod_(context, getBaseUris, info);

        if (usePeriodCache && period.id) {
          this.parsedPeriodCache_.set(period.id, period);
        }
      }
      periods.push(period);

      if (period.id && periodDuration) {
        this.periodDurations_.set(period.id, periodDuration);
      }
    ...
    if (usePeriodCache && !newPeriod) {
      this.prunePeriodCache_(periods);
    }
   ...
   * @private
   */
  prunePeriodCache_(periods) {
    const activeIds = new Set(periods.map((period) => period.id));
    for (const id of Array.from(this.parsedPeriodCache_.keys())) {
      if (!activeIds.has(id)) {
        this.parsedPeriodCache_.delete(id);
      }
    }
  }
   ...
   * @private
   */
  periodIdFallback_(start) {
    return '__shaka_period_' + start;
  }
  ...
  parsePeriod_(context, getBaseUris, periodInfo) {
    ...
    if (!context.period.id) {
      shaka.log.info(
          ...
          ',  Assigning a default');
      context.period.id = this.periodIdFallback_(periodInfo.start);
    }
    ...
    for (const prop of supplementals) {
      if (shaka.dash.DashParser.URL_PARAM_DESCRIPTOR_SCHEMES_.has(
          prop.schemeIdUri)) {
    ...
    // Parse Period-level RequestParam elements (urlparam:2025 scheme)
    const periodUrlParams =
        this.findRequestParamFn_(periodInfo.node, 'segment');
    if (periodUrlParams) {
      context.urlParams = periodUrlParams;
    }
    ...
    adaptationSets =
        this.processPreselections_(context, periodInfo, adaptationSets);

    const [trickModeAdaptationSets, normalAdaptationSets] =
        shaka.util.ArrayUtils.partition(
            adaptationSets, (as) => !!as.trickModeFor);

    // Trick mode adaptation sets only contain I-frames.
    for (const trickModeSet of trickModeAdaptationSets) {
      for (const stream of trickModeSet.streams) {
        stream.isIframe = true;
      }
    }
  ...
  /**
   * Processes Preselections (ISO/IEC 23009-1 clause 5.3.11) signaled through
   * Period-level Preselection elements or through preselection descriptors on
   * AdaptationSets.
   *
   * Only Preselections whose components resolve to a single audio
   * AdaptationSet are supported for now: those correspond to media components
   * multiplexed at the elementary-stream level (for example Dolby AC-4 or
   * MPEG-H presentations), where each Preselection re-describes the same
   * segments as a different experience.  Each of them is materialized as a
   * copy of the AdaptationSet streams with the Preselection annotations
   * (language, label, codecs, channels, roles) applied, so every experience
   * surfaces as a regular audio track.  Preselections referencing multiple
   * AdaptationSets or ContentComponents are not supported yet and are
   * ignored.
   *
   * @param {shaka.dash.DashParser.Context} context
   * @param {shaka.dash.DashParser.PeriodInfo} periodInfo
   ...
   * @private
   */
  processPreselections_(context, periodInfo, adaptationSets) {
    const TXml = shaka.util.TXml;
    const ContentType = shaka.util.ManifestParserUtils.ContentType;

    const periodNode = periodInfo.node;
    goog.asserts.assert(periodNode, 'periodInfo.node should exist');

    // Normalized Preselection definitions gathered from both signaling forms.
    // Element-based definitions keep their node so the common attributes and
    // elements (clause 5.3.11.3) can be applied to the resulting streams;
    // descriptor-based definitions carry no overrides.
    /**
     * @type {!Array<{
     *   id: string,
     *   tag: ?string,
    ...
    for (const node of TXml.findChildren(periodNode, 'Preselection')) {
      const components = node.attributes['preselectionComponents'];
      if (!components) {
        shaka.log.warning('Preselection element is missing the required ' +
            '@preselectionComponents attribute. Ignoring it.');
        continue;
      }
      definitions.push({
        id: node.attributes['id'] || '1',
        tag: node.attributes['tag'] || null,
        componentIds: components.trim().split(/\s+/),
        node,
      });
    }

    for (const adaptationSet of adaptationSets) {
      for (const descriptor of adaptationSet.preselectionDescriptors) {
        if (descriptor.componentIds.length) {
          definitions.push({
            id: descriptor.tag || '1',
            tag: descriptor.tag,
            componentIds: descriptor.componentIds,
            node: null,
          });
        }
      }
    ...
    if (!definitions.length &&
        !adaptationSets.some((as) => as.preselectionDescriptors.length)) {
      return adaptationSets;
    }

    const adaptationSetNodes =
        TXml.findChildren(periodNode, 'AdaptationSet');

    // AdaptationSet ids referenced by at least one supported Preselection.
    /** @type {!Set<string>} */
    const referencedIds = new Set();
    // Per-Preselection stream copies, keyed by AdaptationSet id.
    /** @type {!Map<string, !Array<shaka.extern.Stream>>} */
    const streamCopies = new Map();

    for (const definition of definitions) {
      const componentIds = [...new Set(definition.componentIds)];
      const referenced = componentIds.map(
          (id) => adaptationSets.find((as) => as.id == id));
      if (referenced.some((as) => !as)) {
        shaka.log.warning('Preselection', definition.id, 'references ids',
            componentIds, 'that do not all match an AdaptationSet ' +
            '(ContentComponent references are not supported). Ignoring it.');
        continue;
      }
      if (referenced.length != 1) {
        shaka.log.warning('Preselection', definition.id, 'references ' +
            'multiple AdaptationSets, which is not supported yet. ' +
            'Ignoring it.');
        continue;
      }
      const adaptationSet = referenced[0];
      if (adaptationSet.trickModeFor ||
          adaptationSet.contentType != ContentType.AUDIO) {
        shaka.log.warning('Preselection', definition.id, 'does not reference ',
            'an audio AdaptationSet, which is not supported yet. ' +
            'Ignoring it.');
        continue;
      }

      referencedIds.add(adaptationSet.id);

      const preselectionNode = definition.node;
      if (!preselectionNode) {
        // A descriptor-based Preselection over a single AdaptationSet carries
        // no overrides: the AdaptationSet itself already describes the
        // experience, so just link its streams to the Preselection.
        for (const stream of adaptationSet.streams) {
          if (!stream.preselection) {
            stream.preselection = {id: definition.id, tag: definition.tag};
          }
        }
        continue;
      }

      const nodeIndex = adaptationSetNodes.findIndex(
          (node) => node.attributes['id'] == adaptationSet.id);
      if (nodeIndex < 0) {
        continue;
      }

      // Re-parse the AdaptationSet under a Preselection-specific context name
      // so the copies are distinct stream objects with stable identity across
      // manifest updates (same pattern as dependency streams).
      const originalContextName = context.name;
      context.name = 'preselection_' + definition.id;
      const duplicateSet = this.parseAdaptationSet_(
          context, nodeIndex, adaptationSetNodes[nodeIndex]);
      context.name = originalContextName;
      if (!duplicateSet) {
        continue;
      }

      for (const stream of duplicateSet.streams) {
        this.applyPreselectionToStream_(stream, definition.id,
            preselectionNode);
      }
      if (streamCopies.has(adaptationSet.id)) {
        streamCopies.get(adaptationSet.id).push(...duplicateSet.streams);
      } else {
        streamCopies.set(adaptationSet.id, duplicateSet.streams);
      }
    }

    return adaptationSets.filter((adaptationSet) => {
      const copies = streamCopies.get(adaptationSet.id);
      const essential = adaptationSet.preselectionDescriptors.some(
          (descriptor) => descriptor.essential);
      if (copies) {
        if (essential) {
          // Only consumable as part of a Preselection: expose the
          // per-Preselection streams instead of the plain ones.
          adaptationSet.streams = copies;
        } else {
          adaptationSet.streams = adaptationSet.streams.concat(copies);
        }
        return true;
      }
      if (essential && !referencedIds.has(adaptationSet.id)) {
        shaka.log.warning('AdaptationSet', adaptationSet.id, 'is only ' +
            'consumable through a Preselection that is not supported. ' +
            'Ignoring it.');
        return false;
      }
      return true;
  ...
  /**
   * Applies the annotations of a Preselection element (ISO/IEC 23009-1
   * clause 5.3.11.3) to one of its stream copies.  Per that clause, the
   * attributes of the Preselection re-describe the experience: in particular
   * its codecs attribute replaces the codecs of the Adaptation Set.
   *
   * @param {shaka.extern.Stream} stream
   * @param {string} preselectionId
   * @param {!shaka.extern.xml.Node} node The Preselection element.
   * @private
   */
  applyPreselectionToStream_(stream, preselectionId, node) {
    const TXml = shaka.util.TXml;

    // Streams may be reused from the stream map on manifest updates, so all
    // of these mutations must be idempotent.
    stream.preselection = {
      id: preselectionId,
      tag: node.attributes['tag'] || null,
    };

    const suffix = '_preselection_' + preselectionId;
    if (!stream.originalId || !stream.originalId.endsWith(suffix)) {
      stream.originalId = (stream.originalId || '') + suffix;
    }
    // Keep the copies out of the audio group of the original streams, so ABR
    // never hops between different Preselections of the same AdaptationSet.
    if (stream.groupId && !stream.groupId.endsWith(suffix)) {
      stream.groupId += suffix;
    }

    const lang = node.attributes['lang'];
    if (lang) {
      stream.language = shaka.util.LanguageUtils.normalize(lang);
      stream.originalLanguage = lang;
    }

    const codecs = node.attributes['codecs'];
    if (codecs) {
    ...
    const label = this.parseLabel_(node);
    if (label) {
      stream.label = label;
    }

    const channelsCount = this.parseAudioChannels_(
        TXml.findChildren(node, 'AudioChannelConfiguration'));
    if (channelsCount) {
      stream.channelsCount = channelsCount;
    }

    const parsedRoles = this.parseRoles_(node);
    if (parsedRoles.roleValues.length) {
      stream.roles = parsedRoles.roleValues;
      stream.primary = parsedRoles.main;
    }

    for (const accessibility of TXml.findChildren(node, 'Accessibility')) {
      const scheme = accessibility.attributes['schemeIdUri'];
      if (scheme == 'urn:tva:metadata:cs:AudioPurposeCS:2007') {
        const purpose = this.getAudioAccessibilityPurpose_(
            accessibility.attributes['value']);
        if (purpose) {
          stream.accessibilityPurpose = purpose;
        }
      }
    }
  }
  ...
  parseAdaptationSet_(context, position, elem) {
    ...
    const parsedRoles = this.parseRoles_(elem);
    const main = parsedRoles.main;
    const roleValues = parsedRoles.roleValues;

    // Default kind for text streams is 'subtitle' if unspecified in the
    // manifest.
    let kind = parsedRoles.kind;
    const isText = context.adaptationSet.contentType == ContentType.TEXT;
    if (isText && kind === undefined) {
    ...
    /** @type {!Array<!shaka.dash.DashParser.PreselectionDescriptor>} */
    const preselectionDescriptors = [];

    // Parses a preselection descriptor (ISO/IEC 23009-1 clause 5.3.11.2).
    // Its @value, when present, is "<tag>,<id list>": the Preselection Tag,
    // a comma, and a white-space-separated list of AdaptationSet or
    // ContentComponent ids (the first one being the Main AdaptationSet).
    // Without a value, the descriptor only marks the AdaptationSet as being
    // part of some Preselection defined elsewhere.
    const parsePreselectionDescriptor = (value, essential) => {
      let tag = null;
      let componentIds = [];
      if (value) {
        // The id list is made of digits and white space only, so the last
        // comma is the tag/id-list separator even if the tag contains commas.
        const separator = value.lastIndexOf(',');
        if (separator >= 0) {
          tag = value.substring(0, separator).trim();
          componentIds = value.substring(separator + 1).trim().split(/\s+/)
              .filter((id) => id.length > 0);
        }
      }
      preselectionDescriptors.push({tag, componentIds, essential});
    };
    ...
    for (const prop of essentialProperties) {
      ...
      } else if (schemeId == 'urn:mpeg:dash:preselection:2016') {
        // An essential preselection descriptor means this AdaptationSet is
        // only consumable as part of a Preselection (clause 5.3.11.2).
        parsePreselectionDescriptor(prop.value, /* essential= */ true);
      } else if (shaka.dash.DashParser.URL_PARAM_DESCRIPTOR_SCHEMES_.has(
          schemeId)) {
    ...
    for (const prop of supplementalProperties) {
      ...
      if (schemeId == 'http://dashif.org/guidelines/last-segment-number') {
        ...
        // A supplemental preselection descriptor: the AdaptationSet is part
        // of a Preselection but also consumable on its own.
        parsePreselectionDescriptor(prop.value, /* essential= */ false);
      } else if (shaka.dash.DashParser.URL_PARAM_DESCRIPTOR_SCHEMES_.has(
          schemeId)) {
        adaptationSetUrlParams = this.getURLParametersFunction_(prop.element);
      }
    ...
    const asUrlParams = this.findRequestParamFn_(elem, 'segment');
    if (asUrlParams) {
      adaptationSetUrlParams = asUrlParams;
    }
    ...
    for (const prop of accessibilities) {
      ...
      if (schemeId == 'urn:scte:dash:cc:cea-608:2015' &&
        ...
        const purpose =
            this.getAudioAccessibilityPurpose_(value);
        if (purpose) {
  ...
  getURLParametersFunction_(elem) {
    const TXml = shaka.util.TXml;
    const DashParser = shaka.dash.DashParser;
    const info =
        TXml.findChildNS(elem, DashParser.UP_NAMESPACE_2014_, 'UrlQueryInfo') ||
        TXml.findChildNS(
            elem, DashParser.UP_NAMESPACE_2016_, 'ExtUrlQueryInfo');
    if (!info) {
      return null;
    }
    return this.buildUrlQueryFn_(info);
  }
   ...
   * @param {string} includeInType
   * @return {(?function():string|undefined)}
   * @private
   */
  findRequestParamFn_(node, includeInType) {
    const TXml = shaka.util.TXml;
    let result;
    for (const param of TXml.findChildren(node, 'RequestParam')) {
      const includeIn = (param.attributes['includeInRequests'] || 'segment')
          .split(/\s+/);
      if (includeIn.includes(includeInType) || includeIn.includes('*')) {
        const fn = this.buildUrlQueryFn_(param);
        if (fn) {
          result = fn;
        }
      }
   ...
   * @private
   */
  buildUrlQueryFn_(elem) {
    const TXml = shaka.util.TXml;
    const StringUtils = shaka.util.StringUtils;
    const useMPDUrlQuery = TXml.parseAttr(
        elem, 'useMPDUrlQuery', TXml.parseBoolean, /* defaultValue= */ false);
    // Decode XML entities (&amp; → &) since TXml does not decode them in
    // attribute values, but attribute values in real MPDs use &amp; for &.
    const queryString = StringUtils.htmlUnescape(
        elem.attributes['queryString'] || '');
    const queryTemplate = StringUtils.htmlUnescape(
        elem.attributes['queryTemplate'] || '');
    // A template is required, and there must be at least one source of
    // parameters: the MPD URL query (@useMPDUrlQuery) or an explicit
    // @queryString.
    if (!queryTemplate || (!useMPDUrlQuery && !queryString)) {
      return null;
    }
    return () => {
      // initialQueryString (Annex I.2.3.2): the MPD URL query (when
      // @useMPDUrlQuery is set) concatenated with @queryString.
      const initialParts = [];
      if (useMPDUrlQuery && this.lastManifestQueryParams_) {
        initialParts.push(this.lastManifestQueryParams_);
      }
      if (queryString) {
        initialParts.push(queryString);
      }
      ...
      const parameters = queryTemplate.split('&').map((param) => {
        ...
        const queryParamMatch = /\$query:(.*?)\$/.exec(param);
        if (queryParamMatch) {
          const paramName = queryParamMatch[1];
          if (!queryData) {
            queryData = new URLSearchParams(initialQueryString);
          }
        ...
        // Per DASH spec I.2.2.2: replace unknown $...$ identifiers with empty
        // string; replace $$ with $.
        return param.replace(/\$\$|\$[^$]+\$/g, (m) => m === '$$' ? '$' : '');
      });
  ...
  parseRepresentation_(context, contentProtection, kind, language, label,
    ...
    const urlParamsDescriptor =
        essentialProperties.find(isUrlParamDescriptor) ||
        supplementalProperties.find(isUrlParamDescriptor);
    ...
    const repUrlParams = this.findRequestParamFn_(node, 'segment');
    if (repUrlParams) {
      representationUrlParams = repUrlParams;
    }
    ...
    let contextId = representationId ? periodId + ',' + representationId : '';

    if (contextId && context.name) {
      contextId += context.name;
    }

    if (this.patchLocationNodes_.length && contextId) {
      this.contextCache_.set(contextId, this.cloneContext_(context));
    }
    ...
    if (streamInfo.endTime != -1 &&
      ...
      const cache = this.continuityCache_.get(periodId);
      if (cache) {
        cache.endTime = streamInfo.endTime;
        if (!cache.reps.includes(representationId)) {
          cache.reps.push(representationId);
        }
        this.continuityCache_.set(periodId, cache);
      } else {
        const cache = {
          endTime: streamInfo.endTime,
          timeline: streamInfo.timeline,
          reps: [representationId],
        };
        this.continuityCache_.set(periodId, cache);
      }
    }
    ...
    if (contextId && this.streamMap_.has(contextId)) {
      ...
      stream = {
        ...
        codecs,
        supplementalCodecs: context.representation.supplementalCodecs,
    ...
    // Some packagers signal the encryption of a Representation without
    // declaring a @cenc:default_KID for it.  Without a key ID we cannot map
    // the EME key statuses onto this stream, so restrictions reported by the
    // CDM (such as HDCP output protection) would be silently ignored.  In that
    // case, read the default KID from the 'tenc' box of the init segment, the
    // same way the HLS parser does.
    const initSegmentReference = streamInfo.initSegmentReference;
    if (initSegmentReference && this.needsKeyIdFromInitSegment_(stream)) {
      this.streamsWithUnknownKeyId_.push({stream, initSegmentReference});
    }
  ...
  /**
   * Returns true if we should look for the key ID of this stream in its init
   * segment.
   *
   * @param {shaka.extern.Stream} stream
   * @return {boolean}
   * @private
   */
  needsKeyIdFromInitSegment_(stream) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    if (!stream.encrypted || stream.keyIds.size) {
      return false;
    }
    // Only audio and video streams are decrypted by the CDM, and the 'tenc'
    // box only exists in ISO BMFF content.
    if (stream.type != ContentType.AUDIO && stream.type != ContentType.VIDEO) {
      return false;
    }
    return stream.mimeType.includes('mp4');
  }

  /**
   * Reads the default KID from the init segment of every encrypted stream
   * whose key ID was not declared in the manifest, and assigns it to the
   * stream and to its DrmInfos.
   *
   * @return {!Promise}
   * @private
   */
  async fillMissingKeyIds_() {
    const streams = this.streamsWithUnknownKeyId_;
    this.streamsWithUnknownKeyId_ = [];
    if (!streams.length) {
      return;
    }
    await Promise.all(streams.map(async (info) => {
      const keyId = await this.getKeyIdFromInitSegment_(
          info.initSegmentReference);
      if (!keyId) {
        return;
      }
      this.addKeyIdToStream_(info.stream, keyId);

      // On a manifest update, the variants have already been built from copies
      // of this stream, so those copies need the key ID, too.  PeriodCombiner
      // keeps the ID of the stream it copied.
      if (this.manifest_) {
        for (const variant of this.manifest_.variants) {
          const streams = shaka.util.StreamUtils.getVariantStreams(variant);
          for (const stream of streams) {
            if (stream.id == info.stream.id) {
              this.addKeyIdToStream_(stream, keyId);
            }
          }
        }
      }
   ...
   * @private
   */
  addKeyIdToStream_(stream, keyId) {
    stream.keyIds.add(keyId);
  ...
  /**
   * Reads the default KID of an init segment, caching the result.
   *
   * A successful read is cached permanently, even when the init segment turns
   * out to carry no usable key ID.  A failed read is not cached, so that the
   * next manifest update can try again.
   *
   * @param {!shaka.media.InitSegmentReference} initSegmentReference
   * @return {!Promise<?string>}
   * @private
   */
  getKeyIdFromInitSegment_(initSegmentReference) {
    const cacheKey = initSegmentReference.getUris().join('') + '-' +
        initSegmentReference.getStartByte() + '-' +
        initSegmentReference.getEndByte();

    const cached = this.initSegmentToKidMap_.get(cacheKey);
    if (cached) {
      return cached;
    }

    const keyIdPromise = this.readKeyIdFromInitSegment_(initSegmentReference)
        .catch((error) => {
          // Not knowing the key ID is not fatal; the content may still be
          // playable.  Forget the failure so that we can try again later.
          this.initSegmentToKidMap_.delete(cacheKey);
          shaka.log.warning(
              'Unable to read the key ID from the init segment', error);
          return null;
        });
    this.initSegmentToKidMap_.set(cacheKey, keyIdPromise);
    return keyIdPromise;
  }
   ...
   * @private
   */
  async readKeyIdFromInitSegment_(initSegmentReference) {
    let data = await this.requestSegment_(
        initSegmentReference.getUris(),
        initSegmentReference.getStartByte(),
        initSegmentReference.getEndByte(), /* isInit= */ true);
    if (initSegmentReference.aesKey) {
      data = await shaka.media.SegmentUtils.aesDecrypt(
          data, initSegmentReference.aesKey, /* position= */ 0);
    }
    const keyId = shaka.media.SegmentUtils.getDefaultKID(data);
    // An all-zero KID is a placeholder, not a real key ID.
    if (keyId == shaka.dash.DashParser.DUMMY_KEY_ID_) {
      return null;
    }
    return keyId;
  }
  ...
  createFrame_(elem, parent, getBaseUris) {
    ...
    const allSupplementalCodecs = [
      TXml.getAttributeNS(
          elem, shaka.dash.DashParser.SCTE214_, 'supplementalCodecs') ||
        parent.supplementalCodecs,
    ];
    const supplementalCodecs =
        SegmentUtils.codecsFiltering(allSupplementalCodecs).join(',');
    const frameRate =
    ...
    const label = this.parseLabel_(elem);

    return {
      getBaseUris:
          () => shaka.util.URL.resolveUris(getBaseUris(), getFrameUris()),
      ...
      mimeType: mimeType,
      codecs: codecs,
      supplementalCodecs: supplementalCodecs.split(' ').join(','),
      frameRate: frameRate,
  ...
  /**
   * Parses the Role elements of an AdaptationSet or Preselection element.
   *
   * @param {!shaka.extern.xml.Node} elem
   * @return {{
   *   roleValues: !Array<string>,
   *   main: boolean,
  ...
  parseRoles_(elem) {
    ...
    for (const roleElement of roleElements) {
      ...
      if (scheme == null || scheme == 'urn:mpeg:dash:role:2011') {
        // These only apply for the given scheme, but allow them to be
        // specified if there is no scheme specified.
    ...
    // A Role value that was already consumed above to set "kind" (e.g.
    // value="caption") is conveyed via stream.kind and should not be
    // duplicated in stream.roles. Other Role values (e.g. "main") still
    // populate roles as before.
    const roleValues = roleElements.map((role) => {
      return role.attributes['value'];
    }).filter(shaka.util.Functional.isNotNull)
        .filter((value) => value !== kind);

    return {roleValues, main, kind};
  }

  /**
   * Gets the label of an element, from its label attribute (currently
   * non-standard, but supported by Kaltura) or from its first Label child
   * element (see DASH IOP 4.3,
   * https://dashif.org/docs/DASH-IF-IOP-v4.3.pdf, page 35).
   *
   * @param {!shaka.extern.xml.Node} elem
   * @return {?string}
   * @private
  ...
  /**
   * Maps the value of an Accessibility descriptor with the
   * urn:tva:metadata:cs:AudioPurposeCS:2007 scheme to an
   * AccessibilityPurpose.
   * See DASH DVB Document A168 Rev.6 Table 5.
   *
   * @param {?string} value
   * @return {?shaka.media.ManifestParser.AccessibilityPurpose}
   * @private
   */
  getAudioAccessibilityPurpose_(value) {
    const AccessibilityPurpose =
        shaka.media.ManifestParser.AccessibilityPurpose;
    switch (value) {
      case '1':
        return AccessibilityPurpose.VISUALLY_IMPAIRED;
      case '2':
        return AccessibilityPurpose.HARD_OF_HEARING;
      case '9':
        return AccessibilityPurpose.SPOKEN_SUBTITLES;
      default:
        return null;
    }
  }
  ...
  async requestForTiming_(getBaseUris, uri, method) {
    ...
    const requestUris = shaka.util.URL.resolveUris(getBaseUris(), uris);
    const request = shaka.net.NetworkingEngine.makeRequest(
  ...
  parseEventStream_(periodStart, periodDuration, elem, availabilityStart) {
    ...
    // Map schemeIdUri to the RequestParam includeInRequests type per DASH 6th
    // ed. Table I.4: 'altmpd' for alternativeMPD events, 'callback' for
    // callback events, undefined for all other schemes.
    ...
    if (schemeIdUri === 'urn:mpeg:dash:event:alternativeMPD:insert:2025' ||
      ...
      requestParamType = 'altmpd';
    } else if (schemeIdUri === 'urn:mpeg:dash:event:callback:2015') {
      requestParamType = 'callback';
    }
    const eventStreamUrlParams = requestParamType ?
        this.findRequestParamFn_(elem, requestParamType) : undefined;
    ...
    for (const eventNode of TXml.findChildren(elem, 'Event')) {
      ...
      const region = {
        ...
        urlParams: eventStreamUrlParams,
      };
  ...
  async requestSegment_(uris, startByte, endByte, isInit) {
    ...
    const request = shaka.net.NetworkingUtils.createSegmentRequest(
        ...
        startByte,
        endByte,
        this.playerInterface_.getStreamingRetryParameters());
  ...
  getPatchLocationUris_() {
    ...
    return shaka.util.URL.resolveUris(this.manifestUris_, patchLocations);
  }
   ...
   * @noinline
   */
  static defaultUrlParams_() {
    return '';
  }

  /**
   * @param {?shaka.util.ContentSteeringManager} contentSteeringManager
   * @param {boolean} someLocationValid
   * @param {Array<string>} lastCalculatedBaseUris
   * @return {function():!Array<string>}
   * @private
   * @noinline
   */
  static makeBaseUriGetter_(contentSteeringManager, someLocationValid,
 ...
 * @private
 */
shaka.dash.DashParser.UP_NAMESPACE_2014_ = 'urn:mpeg:dash:schema:urlparam:2014';
 ...
 * @private
 */
shaka.dash.DashParser.UP_NAMESPACE_2016_ = 'urn:mpeg:dash:schema:urlparam:2016';
 ...
 * @private
 */
shaka.dash.DashParser.URL_PARAM_DESCRIPTOR_SCHEMES_ = new Set([
  'urn:mpeg:dash:urlparam:2014',
  'urn:mpeg:dash:urlparam:2016',
]);
 ...
 *   The inherited supplemental codecs value.
 ...
 *   The name of the context if this is needed.
 *   Eg: clone streams for dependencyStream.
 */
 ...
 *   preselectionDescriptors:
 *       !Array<!shaka.dash.DashParser.PreselectionDescriptor>,
 * }}
 *
 * @description
 ...
 * @property {!Array<!shaka.dash.DashParser.PreselectionDescriptor>
 *          } preselectionDescriptors
 *   The preselection descriptors (urn:mpeg:dash:preselection:2016) found on
 *   the AdaptationSet.
 */
 ...
 *
 * @description
 * A parsed preselection descriptor (ISO/IEC 23009-1 clause 5.3.11.2).
 *
 * @property {?string} tag
 *   The Preselection Tag, or null if the descriptor had no value and only
 *   marks the AdaptationSet as being part of some Preselection.
 * @property {!Array<string>} componentIds
 *   The ids of the AdaptationSets or ContentComponents that form the
 *   Preselection, in processing order.  The first id references the Main
 *   AdaptationSet.  Empty if the descriptor had no value.
 * @property {boolean} essential
 *   True if the descriptor was an EssentialProperty, meaning the
 *   AdaptationSet is only consumable as part of a Preselection.
 */
shaka.dash.DashParser.PreselectionDescriptor;
 ...
 *   The init segment of the stream, if it has one.
 */
shaka.dash.DashParser.StreamInfo;


/**
 * A placeholder key ID used by some packagers in the 'tenc' box of clear
 * content.  It never identifies a real key.
 *
 * @private {string}
 * @const
 */
shaka.dash.DashParser.DUMMY_KEY_ID_ = '00000000000000000000000000000000';
