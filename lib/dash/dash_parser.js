...
shaka.dash.DashParser = class {
  ...
  constructor() {
    ...
    /** @private {shaka.extern.PresentationLatencyData} */
    this.presentationLatencyData_ = {
      prtMap: new Map(),
      periodIdMap: new Map(),
    };
    ...
    /** @private {string} */
    this.finalManifestUri_ = '';

    /** @private {?string} */
    this.patchLocation_ = null;
    ...
    /** @private {?shaka.extern.ServiceDescription} */
    this.serviceDescription_ = null;
  }
  ...
  async update() {
    try {
      if (this.patchLocation_) {
        await this.requestManifestPatch_();
      } else {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    if (patchLocation.length > 0) {
      this.mpd_ = mpd;
      this.finalManifestUri_ = finalManifestUri;
      const index = this.finalManifestUri_.lastIndexOf('/');
      if (index > 0) {
        this.patchLocation_ = this.finalManifestUri_.substr(0, index + 1)
          + patchLocation;
      }
    }
    ...
    this.serviceDescription_ = this.parseServiceDescription_(mpd);
  ...
  parseServiceDescription_(mpd) {
    const XmlUtils = shaka.util.XmlUtils;
    const node = XmlUtils.findChild(mpd, 'ServiceDescription');
    if (!node) {
      return null;
    }

    /** @type {?shaka.extern.ServiceDescription.Scope} */
    let scope = null;
    /** @type {?shaka.extern.ServiceDescription.Latency} */
    let latency = null;
    /** @type {?shaka.extern.ServiceDescription.PlaybackRate} */
    let playbackRate = null;

    const scopeNode = XmlUtils.findChild(node, 'Scope');
    if (scopeNode) {
      scope = {
        schemeIdUri: scopeNode.getAttribute('schemeIdUri'),
        value: scopeNode.getAttribute('value'),
      };
    }

    const latencyNode = XmlUtils.findChild(node, 'Latency');
    if (latencyNode) {
      latency = {
        target: (XmlUtils.parseAttr(latencyNode, 'target',
            XmlUtils.parseInt) || 0),
        max: (XmlUtils.parseAttr(latencyNode, 'max',
            XmlUtils.parseInt) || 0),
        min: (XmlUtils.parseAttr(latencyNode, 'min',
            XmlUtils.parseInt) || 0),
      };
    }

    const playbackRateNode = XmlUtils.findChild(node, 'PlaybackRate');
    if (playbackRateNode) {
      playbackRate = {
        max: (XmlUtils.parseAttr(playbackRateNode, 'max',
            XmlUtils.parseFloat) || 0),
        min: (XmlUtils.parseAttr(playbackRateNode, 'min',
            XmlUtils.parseFloat) || 0),
      };
    }
  ...
  parseAdaptationSet_(context, elem) {
    ...
    // Compute ProducerReferenceTime
    const periodId = context.periodInfo.node.id;
    this.presentationLatencyData_.periodIdMap
        .set(context.periodInfo.start, periodId);

    let ts = 1;
    let pto = 0;
    let segBaseOrTemplate = XmlUtils.findChild(elem, 'SegmentTemplate') ||
        XmlUtils.findChild(elem, 'SegmentBase') ||
        XmlUtils.findChild(elem, 'SegmentList') || null;

    if (segBaseOrTemplate == null) {
      const rep = XmlUtils.findChild(elem, 'Representation');
      if (rep != null) {
        segBaseOrTemplate = XmlUtils.findChild(rep, 'SegmentTemplate') ||
            XmlUtils.findChild(rep, 'SegmentBase') ||
            XmlUtils.findChild(rep, 'SegmentList') || null;
      }
    }

    if (segBaseOrTemplate) {
      ts = XmlUtils.parseAttr(segBaseOrTemplate, 'timescale',
          XmlUtils.parseInt) || ts;
      pto = XmlUtils.parseAttr(segBaseOrTemplate, 'presentationTimeOffset',
          XmlUtils.parseInt) || pto;
      pto = pto /ts * 1000;
    }

    const producerReferenceTimes =
        XmlUtils.findChildren(elem, 'ProducerReferenceTime');
    let type = 'encoder';
    if (producerReferenceTimes && producerReferenceTimes.length > 0) {
      for (const producerReferenceTime of producerReferenceTimes) {
        type = producerReferenceTime.getAttribute('type') || type;
        for (const stream of streams) {
          const key = `${periodId}_${stream.originalId}_${type}`;
          if (this.presentationLatencyData_.prtMap.has(key)) {
            continue;
          }

          const wallclockTime = XmlUtils.parseAttr(
              producerReferenceTime, 'wallclockTime', XmlUtils.parseDate) ||
              XmlUtils.parseAttr(
                  producerReferenceTime,
                  'wallClockTime', XmlUtils.parseDate) || 0;

          const wca = 1000 * wallclockTime;

          const pta = (XmlUtils.parseAttr(
              producerReferenceTime, 'presentationTime', XmlUtils.parseInt)
              || 0) - pto;

          const isInBand = producerReferenceTime.getAttribute('inband')
              === 'true';
          ...
          this.presentationLatencyData_.prtMap.set(key, data);
        }
      }
    } else {
      for (const stream of streams) {
        ...
        const wca = context.presentationTimeline.getPresentationStartTime()
            + context.periodInfo.start;
        const pta = pto;
        const isInBand = false;
  ...
  async onUpdate_() {
    ...
    try {
      if (this.patchLocation_) {
        shaka.log.info('Updating manifest with patch...');
        updateDelay = await this.requestManifestPatch_();
      } else {
  ...
  setUpdateTimer_(offset) {
    ...
    const finalDelay = this.lowLatencyMode_ ?
        Math.max(
  ...
  /**
   * @return {shaka.extern.PresentationLatencyData}
   */
  getPresentationLatencyData() {
    return this.presentationLatencyData_;
  }
  ...
  /**
   * Makes a network request for the manifest patch and
   * parses the resulting data.
   *
   * @return {!Promise.<number>} Resolves with the time it took, in seconds, to
   *   fulfill the request and parse the data.
   * @private
   */
  async requestManifestPatch_() {
    const requestType = shaka.net.NetworkingEngine.RequestType.MANIFEST;
    const request = shaka.net.NetworkingEngine.makeRequest(
        [this.patchLocation_], this.config_.retryParameters);
    const networkingEngine = this.playerInterface_.networkingEngine;
    ...
    await this.parseManifestPatch_(response.data);
    const endTime = Date.now();
    const updateDuration = (endTime - startTime) / 1000.0;
    this.averageUpdateDuration_.sample(1, updateDuration);
   ...
   * @private
   */
  parseManifestPatch_(data) {
    const Error = shaka.util.Error;
    const text = shaka.dash.DashParser.replaceNamespace_(
        shaka.util.StringUtils.fromUTF8(data), this.mpd_.namespaceURI);
    const patch = shaka.util.XmlUtils.parseXmlString(text, 'Patch');
    if (!patch) {
      throw new Error(
          Error.Severity.CRITICAL, Error.Category.MANIFEST,
          Error.Code.DASH_INVALID_XML);
    }
    return this.processManifestPatch_(patch);
  }

  /**
   * Replace XML namespace with provided namespace.
   *
   * @param {string} text
   ...
   * @private
   */
  static replaceNamespace_(text, namespace) {
    const prefix = 'xmlns="';
    const startIndex = text.indexOf(prefix);
    if (startIndex > 0) {
      const endIndex = text.indexOf('"', startIndex + prefix.length);
      if (endIndex > 0) {
        return text.substr(0, startIndex + prefix.length)
          + namespace + text.substr(endIndex);
      }
    }
    return text;
  }
  ...
  /**
   * Tries to apply patch. Request complete manifest if failed.
   *
   * @param {Element} patch
   * @return {!Promise}
   * @private
   */
  async processManifestPatch_(patch) {
    if (this.mpd_ && this.applyPatch_(this.mpd_, patch)) {
      await this.processManifest_(this.mpd_, this.finalManifestUri_);
      this.evictSegmentsFromCachesMpd_();
    } else {
      await this.requestManifest_();
    }
  }
  ...
  /**
   * Evicts segments that out of window from cached manifest
   *
   * @private
   */
  evictSegmentsFromCachesMpd_() {
    let child = this.mpd_.firstElementChild;
    const periodsToRemove = [];
    while (child) {
      if (child.nodeName === 'Period') {
        const periodId = child.getAttribute('id');
        let innerChild = child.firstElementChild;
        let removeCount = 0;
        let childCount = 0;
        while (innerChild) {
          if (innerChild.nodeName === 'AdaptationSet') {
            const segmentCount = this.getSegmentCountFromSegmentIndex_(
                innerChild, periodId);
            removeCount += this.evictSegmentsFromAdaptationSet_(innerChild,
                segmentCount);
            childCount += segmentCount;
          }
          innerChild = innerChild.nextElementSibling;
        }
        if (childCount == 0) {
          periodsToRemove.push(child);
    ...
    for (let i = 0; i < periodsToRemove.length; i++) {
      this.mpd_.removeChild(periodsToRemove[i]);
    }
  }

  /**
   * Get segment count from segment index
   *
   * @param {Element} node
   ...
   * @private
   */
  getSegmentCountFromSegmentIndex_(node, periodId) {
    const representations = node.getElementsByTagName('Representation');
    if (representations && representations.length > 0) {
      const id = periodId + ',' + representations[0].id;
      const segmentIndex = this.segmentIndexMap_[id];
      if (segmentIndex) {
        return segmentIndex.count();
      }
    }
    return 0;
  }
  ...
  /**
   * Get segment count from SegmentTimeline
   *
   * @param {Element} node
   * @return {number}
   * @private
   */
  getSegmentTimelineSegmentCount_(node) {
    let child = node.firstElementChild;
    let count = 0;
    while (child) {
      if (child.nodeName === 'S') {
        count++;
        if (child.hasAttribute('r')) {
          count += Number(child.getAttribute('r'));
        }
      }
      child = child.nextElementSibling;
    }
  ...
  /**
   * Evicts segments from AdaptationSet level
   *
   * @param {Element} node
   ...
   * @private
   */
  evictSegmentsFromAdaptationSet_(node, segmentCount) {
    const segmentTemplate = node.getElementsByTagName('SegmentTemplate');
    if (segmentTemplate && segmentTemplate.length > 0) {
      const segmentTimeline = segmentTemplate[0]
          .getElementsByTagName('SegmentTimeline');
      if (segmentTimeline && segmentTimeline.length > 0) {
        const removeCount =
            this.getSegmentTimelineSegmentCount_(segmentTimeline[0])
            - segmentCount;
        if (removeCount > 0) {
          this.evictSegmentsFromSegmentTimeline_(
              segmentTimeline[0], removeCount);
        }
        return removeCount;
      }
    }
  ...
  /**
   * Evicts segments from SegmentTimeline level
   *
   * @param {Element} node
   * @param {number} count
   * @private
   */
  evictSegmentsFromSegmentTimeline_(node, count) {
    let child = node.firstElementChild;
    while (child && count > 0) {
      let segmentCount = 1;
      if (child.hasAttribute('r')) {
        segmentCount += Number(child.getAttribute('r'));
      }
      ...
      if (segmentCount <= count) {
        ...
        const t = Number(child.getAttribute('t')) +
            count * Number(child.getAttribute('d'));
        const r = Number(child.getAttribute('r')) + 1 - count;

        child.setAttribute('t', t);
        if (r == 1) {
          child.removeAttribute('r');
        } else {
          child.setAttribute('r', r - 1);
        }
        count = 0;
      }
    }
  ...
  /**
   * Applies patch on a manifest.
   *
   * @param {Element} patch
   * @return {boolean}
   * @private
   */
  applyPatch_(mpd, patch) {
    const evaluator = new XPathEvaluator();
    let child = patch.firstElementChild;
    while (child) {
      let success = true;
      if (child.nodeName === 'add') {
        success = this.patchAdd_(evaluator, mpd, child);
      } else if (child.nodeName === 'replace') {
        success = this.patchReplace_(evaluator, mpd, child);
      } else if (child.nodeName === 'remove') {
        success = this.patchRemove_(evaluator, mpd, child);
      }
      if (!success) {
        shaka.log.warning('Patch operation failed', child);
        return false;
      }
      child = child.nextElementSibling;
    }
   ...
   *
   * @param {XPathEvaluator} evaluator
   * @param {Element} mpd
   * @param {string} xpath
   * @return {Element}
   */
  static getNodeByXPath(evaluator, mpd, xpath) {
    const arr = xpath.split('/');
    const customPrefix = 'xx';
    let finalXPath = '';
    for (let i = 0; i < arr.length; i++) {
      if (arr[i].length < 1) {
        continue;
      }
      finalXPath += '/' + customPrefix + ':' + arr[i];
    }

    /** @type {Object} */
    const resolver = (prefix) => {
      return prefix === customPrefix ? mpd.namespaceURI : null;
    };

    const result = evaluator.evaluate(finalXPath, mpd,
        /** @type {XPathNSResolver} */ (resolver),
        XPathResult.FIRST_ORDERED_NODE_TYPE, null);
    if (!result || !result.singleNodeValue) {
      return null;
    }
    return /** @type {Element} */ (result.singleNodeValue);
  }
   ...
   * @private
   */
  patchAdd_(evaluator, mpd, operation) {
    const xpath = operation.getAttribute('sel');
    const node = shaka.dash.DashParser.getNodeByXPath(evaluator, mpd, xpath);
    if (!node) {
      return false;
    }
    if (operation.hasAttribute('type')) {
      // Add attribute
      const attributeName = operation.getAttribute('type').substr(1);
      node.setAttribute(attributeName, operation.textContent);
    } else {
      // Add node
      for (let i = 0; i < operation.childNodes.length; i++) {
        node.appendChild(operation.childNodes[i].cloneNode(true));
      }
    }
    return true;
  }
   ...
   * @private
   */
  patchReplace_(evaluator, mpd, operation) {
    const xpath = operation.getAttribute('sel');
    const lastSlash = xpath.lastIndexOf('/');
    const lastElement = xpath.substr(lastSlash + 1);
    if (lastElement.lastIndexOf('@') >= 0) {
      // Replace attribute
      const node = shaka.dash.DashParser.getNodeByXPath(evaluator, mpd,
          xpath.substr(0, lastSlash));
      if (!node) {
        return false;
      }
      node.setAttribute(lastElement.substr(1), operation.textContent);
    } else {
      // Replace node
      const node = shaka.dash.DashParser.getNodeByXPath(evaluator, mpd, xpath);
      if (!node) {
      ...
      node.textContent = operation.firstElementChild.textContent;
    }
    return true;
  }
   ...
   * @private
   */
  patchRemove_(evaluator, mpd, operation) {
    const xpath = operation.getAttribute('sel');
    const lastSlash = xpath.lastIndexOf('/');
    const lastElement = xpath.substr(lastSlash + 1);
    if (lastElement.lastIndexOf('@') >= 0) {
      ...
      node.removeAttribute(lastElement.substr(1));
    } else {
      // Remove node
      const node = shaka.dash.DashParser.getNodeByXPath(evaluator, mpd, xpath);
      if (!node) {
