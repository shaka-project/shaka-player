...
shaka.dash.DashParser = class {
  ...
  constructor() {
     ...
     * @private {!Map<string, !shaka.dash.DashParser.PeriodCacheEntry>}
     */
    this.periodCache_ = new Map();
  ...
  removePatchPeriod_(periodId) {
    ...
    this.periodCache_.delete(periodId);
  }
  ...
  parsePeriods_(context, getBaseUris, mpd, newPeriod) {
    ...
    const enablePeriodCaching = context.dynamic && !!(this.config_ && this.config_.dash &&
        this.config_.dash.enablePeriodCaching);
    ...
    for (let i = 0; i < periodNodes.length; i++) {
      ...
      let signature = null;
      if (enablePeriodCaching) {
        const cacheKey = this.getPeriodCacheKey_(periodId, start);
        const cachedEntry = (!newPeriod && this.manifest_) ?
            this.periodCache_.get(cacheKey) : null;
        let reusePeriod = false;
        if (cachedEntry) {
          signature = this.computePeriodSignature_(elem);
          reusePeriod = cachedEntry.signature == signature;
        }
        if (reusePeriod) {
          period = cachedEntry.period;
        }
      }
      if (!period) {
        period = this.parsePeriod_(context, getBaseUris, info);
        if (enablePeriodCaching && !signature) {
          signature = this.computePeriodSignature_(elem);
        }
      }
      periods.push(period);
  ...
  prunePeriodCache_(periods) {
    ...
    for (const id of Array.from(this.periodCache_.keys())) {
      if (!activeIds.has(id)) {
        this.periodCache_.delete(id);
      }
    }
  }
   ...
   * @private
   */
  getPeriodCacheKey_(periodId, start) {
    if (periodId) {
      return periodId;
    }
    return '__shaka_period_' + (start != null ? start : 'null');
  }
  ...
  /**
   * Deterministic incremental hash
  ...
  hashTXml_(node, seed = 0x811c9dc5) {
    ...
    if (node.attributes) {
      ...
      for (const key of keys) {
        hash = this.fnv1a_('A', hash);
        hash = this.fnv1a_(key, hash);
        hash = this.fnv1a_('=', hash);
        hash = this.fnv1a_(node.attributes[key], hash);
      }
    }
    ...
    if (node.children) {
      for (const child of node.children) {
        if (typeof child === 'string') {
          hash = this.fnv1a_('S', hash);
          hash = this.fnv1a_(child, hash);
        } else {
          hash = this.hashTXml_(child, hash);
        }
      }
    }
   ...
   * @private
   */
  computePeriodSignature_(node) {
    return this.hashTXml_(node);
  }
 ...
 * @private
 */
shaka.dash.DashParser.PeriodCacheEntry;
