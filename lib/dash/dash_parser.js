...
shaka.dash.DashParser = class {
  ...
  constructor() {
    ...
    /**
     * A map of init segment URIs to the default KID found inside them.
  ...
  getKeyIdFromInitSegment_(initSegmentReference) {
    const uris = initSegmentReference.getUris();
    const cacheKey = uris.join('') + '-' +
        initSegmentReference.getStartByte() + '-' +
        initSegmentReference.getEndByte();
    return this.initSegmentToKidMap_.getOrInsertComputed(cacheKey, async () => {
      try {
        let data = await this.requestSegment_(
            uris, initSegmentReference.getStartByte(),
        ...
        // A missing key ID is not fatal; the content may still be playable.
        shaka.log.warning(
