...
shaka.dash.DashParser = class {
  ...
  parsePeriod_(context, getBaseUris, periodInfo) {
    ...
    const dependencyVideoMap = new Map();
    for (const adaptationSet of adaptationSets) {
      for (const [dependencyId, stream] of adaptationSet.dependencyVideoMap) {
        dependencyVideoMap.set(dependencyId, stream);
      }
    }

    if (dependencyVideoMap.size) {
      let duplicateAdaptationSets = null;
      for (const adaptationSet of adaptationSets) {
        const streamsWithDependencyVideo = [];
        for (const stream of adaptationSet.streams) {
          if (dependencyVideoMap.has(stream.originalId)) {
            ...
            for (const duplicateAdaptationSet of duplicateAdaptationSets) {
              ...
              if (newStream) {
                newStream.dependencyVideo =
                    dependencyVideoMap.get(newStream.originalId);
                streamsWithDependencyVideo.push(newStream);
              }
            }
          }
        }
        if (streamsWithDependencyVideo.length) {
          adaptationSet.streams.push(...streamsWithDependencyVideo);
        }
      }
    }
  ...
  parseAdaptationSet_(context, position, elem) {
    ...
    const streams = representations.map((representation) => {
      ...
      if (parsedRepresentation) {
        ...
        if (dependencyId) {
          ...
          dependencyVideoMap.set(dependencyId, parsedRepresentation);
          return null;
        }
      }
    ...
    if (streams.length == 0 && dependencyVideoMap.size == 0) {
 ...
 *   dependencyVideoMap: !Map<string, shaka.extern.Stream>
 * }}
 *
 * @description
 ...
 * @property {!Map<string, string>} dependencyVideoMap
 *   A map of dependencyVideo
 */
