...
shaka.dash.DashParser = class {
  ...
  async requestManifest_() {
    ...
    let manifestUris = this.patchLocationUris_.length ?
        this.patchLocationUris_ : this.manifestUris_;
  ...
  async parseManifest_(data, finalManifestUri) {
    ...
    const rootElement = this.patchLocationUris_.length ? 'Patch' : 'MPD';
    ...
    if (this.patchLocationUris_.length) {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    this.updatePatchLocationUris_(mpd);
  ...
  async processPatchManifest_(mpd) {
    ...
    for (const child of TXml.getChildNodes(mpd)) {
      let handled = true;
      const selector = StringUtils.htmlUnescape(child.attributes['sel']);
      const paths = TXml.parseXpath(selector);
      const node = paths[paths.length - 1];
      const position = child.attributes['pos'] || null;
      const content = TXml.getContents(child) || '';
      ...
      if (node.name === 'MPD') {
        ...
        } else if (node.attribute === null && child.tagName === 'add') {
          periodAdditions.push(child);
        } else {
          handled = false;
        }
      } else if (node.name === 'PatchLocation') {
        this.updatePatchLocationUris_(child);
      } else if (node.name === 'Period') {
        if (child.tagName === 'add') {
          periodAdditions.push(child);
        } else if (child.tagName === 'remove' && node.id) {
          this.removePatchPeriod_(node.id);
        }
      } else if (node.name === 'SegmentTemplate' && node.attribute !== null) {
        const timelines = this.modifySegmentTemplateAttribute_(paths, content);
        for (const timeline of timelines) {
          modifiedTimelines.add(timeline);
        }
      } else if (node.name === 'SegmentTimeline' || node.name === 'S') {
        const timelines = this.modifyTimepoints_(paths, child.tagName, position,
            TXml.findChildren(child, 'S'));
        for (const timeline of timelines) {
          modifiedTimelines.add(timeline);
        }
      } else {
      ...
      if (!handled) {
        shaka.log.warning('Unhandled ' + child.tagName + ' operation',
            selector);
      }
    }
  ...
  removePatchPeriod_(periodId) {
    ...
    const contextIds = Iterables.filter(this.contextCache_.keys(), (key) => {
      return key.split(',')[0] === periodId;
    });
  ...
  getContextIdsFromPath_(paths) {
    ...
    return representationIds.map((repId) => periodId + ',' + repId);
  }

  /**
   * Modifeis SegmentTemplate attribute based on MPD patch
   *
   * @param {!Array<shaka.util.TXml.PathNode>} paths
   ...
   * @private
   */
  modifySegmentTemplateAttribute_(paths, content) {
    const contextIds = this.getContextIdsFromPath_(paths);
    const lastPath = paths[paths.length - 1];
    const attribute = lastPath.attribute;
    const contentAsNumber = parseInt(content, 10);
    for (const contextId of contextIds) {
      /** @type {shaka.dash.DashParser.Context} */
      const context = this.contextCache_.get(contextId);
      const segmentInfo = context.segmentInfo;
      switch (attribute) {
        case 'timescale':
          segmentInfo.timescale = contentAsNumber;
          break;
        case 'duration':
          segmentInfo.duration = contentAsNumber;
          break;
        case 'startNumber':
          segmentInfo.startNumber = contentAsNumber;
          break;
        case 'presentationTimeOffset':
          segmentInfo.presentationTimeOffset = contentAsNumber;
          break;
        case 'media':
          segmentInfo.media = content;
          break;
        case 'index':
          segmentInfo.index = content;
          break;
      }
    }
    return contextIds;
   ...
   * @param {!Array<shaka.extern.xml.Node>} segments
   * @return {!Array<string>} context ids with updated timeline
   * @private
   */
  modifyTimepoints_(paths, action, position, segments) {
    const SegmentTemplate = shaka.dash.SegmentTemplate;

    let nodePosition = null;
    const lastPath = paths[paths.length - 1];
    if (lastPath.name === 'S') {
      nodePosition = lastPath.position;
    }
    ...
    for (const contextId of contextIds) {
      /** @type {shaka.dash.DashParser.Context} */
      const context = this.contextCache_.get(contextId);
      SegmentTemplate.modifyTimepoints(
          context, action, position, nodePosition, segments);
    }
    return contextIds;
  }
  ...
  */
  ...
  parseRepresentation_(context, contentProtection, kind, language, label,
    ...
    if (this.patchLocationUris_.length && context.periodInfo.isLastPeriod &&
          representationId) {
   ...
   * @private
   */
  updatePatchLocationUris_(node) {
    const TXml = shaka.util.TXml;
    this.patchLocationUris_ = [];
    const publishTime = this.manifestPatchContext_.publishTime;
    let patchLocations = TXml.findChildren(node, 'PatchLocation');
    if (!publishTime || !patchLocations.length) {
      return;
    }
    patchLocations = patchLocations.filter((patchLocation) => {
      const ttl = TXml.parseNonNegativeInt(patchLocation.attributes['ttl']);
      return !ttl || publishTime + ttl > Date.now() / 1000;
    })
        .map(TXml.getContents)
        .filter(shaka.util.Functional.isNotNull);
    if (patchLocations.length > 0) {
      // we are patching
      this.patchLocationUris_ = shaka.util.ManifestParserUtils.resolveUris(
