 ...
 * @private
 */
shaka.dash.DashParser.prototype.requestManifest_ = async function() {
...
shaka.dash.DashParser.prototype.parseManifest_ =
    async function(data, finalManifestUri) {
...
shaka.dash.DashParser.prototype.processManifest_ =
    async function(mpd, finalManifestUri) {
      ...
      const uris = XmlUtils.findChildren(mpd, 'BaseURL').map(XmlUtils.getContents);
      ...
      const suggestedPresentationDelay = XmlUtils.parseAttr(
...
shaka.dash.DashParser.prototype.parsePeriod_ = function(
  ...
  trickModeAdaptationSets.forEach((trickModeSet) => {
    ...
    normalAdaptationSets.forEach((normalSet) => {
      if (normalSet.id == targetId) {
        normalSet.streams.forEach((stream) => {
  ...
  for (let i = 0; i < audioSets.length; i++) {
    for (let j = 0; j < videoSets.length; j++) {
      const audioSet = audioSets[i];
      const videoSet = videoSets[j];
...
shaka.dash.DashParser.prototype.parseAdaptationSet_ = function(context, elem) {
  ...
  for (let i = 0; i < roleElements.length; i++) {
    const scheme = roleElements[i].getAttribute('schemeIdUri');
    if (scheme == null || scheme == 'urn:mpeg:dash:role:2011') {
      ...
      const value = roleElements[i].getAttribute('value');
      switch (value) {
        case 'main':
          main = true;
          break;
  ...
  for (const prop of accessibilities) {
    ...
    if (schemeId == 'urn:scte:dash:cc:cea-608:2015' ||
      ...
      let channelId = 1;
      const closedCaptionsValue = prop.getAttribute('value');
  ...
  const contentProtection = shaka.dash.ContentProtection.parseFromAdaptationSet(
  ...
  streams.forEach((stream) => {
    ...
    contentProtection.drmInfos.forEach((drmInfo) => {
      if (stream.keyId) {
...
shaka.dash.DashParser.prototype.createFrame_ = function(
  ...
  const uris = XmlUtils.findChildren(elem, 'BaseURL').map(XmlUtils.getContents);
...
shaka.dash.DashParser.prototype.requestForTiming_ =
    async function(baseUris, uri, method) {
      const requestUris =
...
shaka.dash.DashParser.prototype.parseUtcTiming_ = function(baseUris, elems) {
  ...
  const Functional = shaka.util.Functional;
  return Functional.createFallbackPromiseChain(schemesAndValues, (sv) => {
    ...
    switch (scheme) {
      ...
      case 'urn:mpeg:dash:utc:direct:2012': {
        ...
        return Promise.resolve(isNaN(date) ? 0 : (date - Date.now()));
      }
...
shaka.dash.DashParser.prototype.parseEventStream_ = function(
  ...
  XmlUtils.findChildren(elem, 'Event').forEach((eventNode) => {
 ...
 * @private
 */
shaka.dash.DashParser.prototype.requestInitSegment_ = async function(
