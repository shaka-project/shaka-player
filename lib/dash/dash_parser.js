...
shaka.dash.DashParser = class {
  ...
  parsePatchPeriod_(periods) {
    ...
    const periodsAndDuration = this.parsePeriods_(context,
        this.manifestPatchContext_.getBaseUris, periods, true);
  ...
  removePatchPeriod_(periodId) {
    ...
    const newPeriodList = [];
    for (const pID of this.lastManifestUpdatePeriodIds_) {
      if (pID !== periodId) {
        newPeriodList.push(pID);
      }
    }
    this.lastManifestUpdatePeriodIds_ = newPeriodList;
  }
  ...
  */
  ...
  parseRepresentation_(context, contentProtection, kind, language, label,
    ...
    if (contextId && context.dynamic && !this.streamMap_[contextId]) {
      const periodId = context.period.id ? context.period.id : '';
