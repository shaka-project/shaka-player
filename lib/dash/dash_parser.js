...
shaka.dash.DashParser = class {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    const patchLocation = TXml.findChildren(mpd, 'PatchLocation')
    ...
    if (patchLocation.length > 0) {
      ...
      this.patchLocationUris_ = shaka.util.ManifestParserUtils.resolveUris(
          manifestBaseUris, patchLocation);
    }
  ...
  async processPatchManifest_(mpd) {
    ...
    const additions = TXml.findChildren(mpd, 'p:add');
    ...
    const replaces = TXml.findChildren(mpd, 'p:replace');
