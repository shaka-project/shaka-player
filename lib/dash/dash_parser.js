  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    if (contentSteering && this.playerInterface_) {
      ...
      if (!hasPrevContentSteeringManager) {
        await this.contentSteeringManager_.requestInfo(
            contentSteering.textContent.trim());
      }
    }
    ...
    if (this.contentSteeringManager_) {
      for (const uriObj of uriObjs) {
        ...
        if (serviceLocation && uri) {
          this.contentSteeringManager_.addLocation(serviceLocation, uri);
    ...
    const baseUris = () => {
      if (this.contentSteeringManager_) {
        return this.contentSteeringManager_.getLocations();
      }
 ...
 *   baseUris: function():!Array.<string>,
 ...
 * @property {function():!Array.<string>} baseUris
 *   Function than rerutns an array of absolute base URIs for the frame.
