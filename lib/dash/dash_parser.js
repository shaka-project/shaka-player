...
shaka.dash.DashParser = class {
   ...
   * @param {number} sizeBytes size of the manifest payload, in bytes
   * @return {!Promise}
   * @private
   */
  async parseManifest_(data, finalManifestUri, rootElement, sizeBytes) {
    ...
    if (disableXlinkProcessing) {
      return this.processManifest_(mpd, finalManifestUri, sizeBytes);
    }
    ...
    return this.processManifest_(finalMpd, finalManifestUri, sizeBytes);
  }
   ...
   * @private
   */
  async processManifest_(mpd, finalManifestUri, sizeBytes) {
    ...
    this.manifest_.sizeBytes = sizeBytes;
