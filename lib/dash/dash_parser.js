...
shaka.dash.DashParser = class {
  ...
  async processManifest_(mpd, finalManifestUri) {
    ...
    if (this.config_.lowLatencyMode) {
    ...
    shaka.log.info('period combiner');
    shaka.log.info(this.periodCombiner_.getVariants());
    shaka.log.info(this.periodCombiner_.getTextStreams());
  ...
  parseAdaptationSet_(context, elem) {
    ...
    for (const prop of accessibilities) {
      ...
      if (schemeId == 'urn:scte:dash:cc:cea-608:2015' ) {
        ...
        if (value != null) {
          for (const captionStr of value.split(';')) {
            ...
            if (!captionStr.includes('=')) {
              // when the channel number is not provided, cycle through all the
              // channels arbitrarily (CC1 - CC4) in order of provided langs.
        ...
        if (value != null) {
          for (const captionStr of value.split(';')) {
            // CEA-708 caption values take the form "1=lang:eng;2=lang:deu" i.e.
            // serviceNumber=lang:3lettercode.
            const serviceAndLanguage = captionStr.split('=');

            const service = `svc${serviceAndLanguage[0]}`;

            // The language info can be different formats, lang:eng',
            // or 'lang:eng,war:1,er:1'. Extract the language info.
            const language =
              serviceAndLanguage[1].split(',')[0].split(':').pop();

            closedCaptions.set(service, LanguageUtils.normalize(language));
            shaka.log.info(`CEA-708 service ${service} seen`);
          }
        }
