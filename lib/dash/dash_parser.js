...
shaka.dash.DashParser = class {
  ...
  parsePeriods_(context, getBaseUris, mpd, newPeriod) {
    ...
    const enablePeriodCaching = context.dynamic && this.config_ &&
      this.config_.dash && this.config_.dash.enablePeriodCaching;
    if (!enablePeriodCaching) {
      this.parsedPeriodCache_.clear();
    }

    const oldInnerIds = (enablePeriodCaching && !newPeriod) ?
    ...
    for (let i = 0; i < periodNodes.length; i++) {
      ...
      let reasonNotCached = false;
      if (enablePeriodCaching &&
        oldInnerIds &&
        isInnerId &&
        !newPeriod) {
        const periodIdWithFallback = periodId ? periodId :
          this.periodIdFallback_(start);
        if (oldInnerIds.has(periodIdWithFallback)) {
          const parsedPeriod =
            this.parsedPeriodCache_.get(periodIdWithFallback);
          if (parsedPeriod) {
            shaka.log.debug('ZZ Reusing period from cache with key',
                periodIdWithFallback);
            period = parsedPeriod;
          }
        } else {
          reasonNotCached = true;
        }
      }
      if (!period) {
        // TODO: Remove this, just for testing for now
        const parseReason =
          !enablePeriodCaching ? 'caching disabled' :
          !oldInnerIds ? 'no previous inner IDs' :
          !isInnerId ? 'first/last period' :
          newPeriod ? 'new period from patch manifest' :
          reasonNotCached ? 'not found in cache' :
          'unknown reason';
        shaka.log.debug('ZZ Parsing period', i, 'with ID', periodId,
            '(' + parseReason + ')');
        period = this.parsePeriod_(context, getBaseUris, info);

        if (enablePeriodCaching && period.id) {
          shaka.log.debug('ZZ --> Caching period with ID', period.id);
    ...
    if (enablePeriodCaching && !newPeriod) {
