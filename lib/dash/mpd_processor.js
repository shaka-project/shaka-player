...
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromSegmentTimeline_ =
  ...
  for (var i = 0; i < timePoints.length; ++i) {
    ...
    for (var j = 0; j <= repeat; ++j) {
      ...
      // $Time$ placeholder.
      if ((references.length > 0) && (startTime != lastEndTime)) {
        ...
        lastReference.endTime = scaledStartTime;
      }

      lastEndTime = endTime;

      // Compute the media URL template placeholder replacements.
      var segmentReplacement =
...
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromSegmentDuration_ =
  ...
  // Compute a faux Period duration to use to compute a SegmentIndex.
  ...
  if (mpd.type == 'static') {
    ...
    // use to generate a SegmentIndex. We don't always want to use the real
    // Period's duration if the MPD is dynamic, since the SegmentIndex will
    // have to be re-generated anyways, and the entire Period's duration may be
    // very long.
  ...
  shaka.log.v1(
      'Generating SegmentIndex using faux Period duration',
      periodDuration);
  ...
  if (mpd.minUpdatePeriod) {
    ...
    var firstSegmentNumber =
        this.computeCurrentSegmentNumber_(mpd, period, segmentTemplate);
  ...
  if (segmentTemplate.presentationTimeOffset != null) {
    ...
    var pto = segmentNumber * segmentTemplate.segmentDuration;
