  ...
  if (mpd.minUpdatePeriod && (references.length > 0)) {
    ...
    for (var i = 0; i < references.length; ++i) {
      if (references[i].endTime >= bestAvailableTimestamp) {
        streamInfo.bestStartTime = references[i].startTime;
        break;
      }
    }
    shaka.asserts.assert(streamInfo.bestStartTime != null);
  }
...
shaka.dash.MpdProcessor.prototype.buildStreamInfoFromSegmentDuration_ =
  ...
  // Compute the optimal number of segment references to generate starting from
  // the current segment.
  var numSegments =
  ...
  // Find the earliest segment and the current segment. All segment numbers
  // are relative to the start of |period| unless marked otherwise.
  ...
  if (mpd.minUpdatePeriod) {
    ...
    if (!pair) {
      ...
      // TODO: Instead of returning an error, set a field on StreamInfo to
      // signal when the stream will be available.
      return false;
    }
    // Build the SegmentIndex starting from the earliest available segment.
    // Since |numSegments| is the optimal number of segment references to
    // generate starting from the current segment, we need to generate
    // additional segment references.
    earliestSegmentNumber = pair.earliest;
    currentSegmentNumber = pair.current;
    numSegments += currentSegmentNumber - earliestSegmentNumber;
  } else {
  ...
  for (var i = earliestSegmentNumber;
       i - earliestSegmentNumber < numSegments;
       ++i) {
    var startTime = (i - 1) * segmentTemplate.segmentDuration;
    var endTime = startTime + segmentTemplate.segmentDuration;

    var scaledStartTime = startTime / segmentTemplate.timescale;
    var scaledEndTime = endTime / segmentTemplate.timescale;

    // Note that |i| is a segment number, thus it is one-based.
    var absoluteSegmentNumber = (i - 1) + segmentTemplate.startNumber;

    // Compute the media URL template placeholder replacements.
    var segmentReplacement = absoluteSegmentNumber;
    var timeReplacement =
        ((i - 1) + (segmentTemplate.startNumber - 1)) *
 ...
 * MPD is static then N * segmentDuration equals |period|'s duration; if the
 * MPD is dynamic then N * segmentDuration is the smallest of
 * |period|'s duration, minimumUpdatePeriod, and MAX_SEGMENT_INDEX_SPAN.
 *
 * If the MPD is dynamic then N can be regarded as the number of segment
 * references that we can generate right now, such that the generated segment
 * references will all be valid when it's time to actually fetch those
 * segments.
 *
 * @param {!shaka.dash.mpd.Mpd} mpd
 * @param {!shaka.dash.mpd.Period} period
    ...
    for (var j = 0; j < period.adaptationSets.length; ++j) {
      ...
      for (var k = 0; k < adaptationSet.representations.length; ++k) {
        ...
        if (streamInfo.segmentIndex) {
          var n = streamInfo.segmentIndex.getNumReferences();
          var lastReference = streamInfo.segmentIndex.getReference(n - 1);
          maxLastEndTime = Math.max(maxLastEndTime, lastReference.endTime);
        }
      }
