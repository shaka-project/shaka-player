...
shaka.dash.MpdProcessor.prototype.createManifestInfo_ = function(
  ...
  for (var i = 0; i < mpd.periods.length; ++i) {
    ...
    for (var typeIdx = 0; typeIdx < setTypes.length; ++typeIdx) {
      ...
      // Now group AdaptationSets of the same type by group to squash multiple
      // AdaptationSets into a single StreamSetInfo.
...
/**
 * Creates a StreamSetInfo from AdaptationSets from the same group.
 *
 * @param {!shaka.dash.mpd.Mpd} mpd
...
shaka.dash.MpdProcessor.prototype.createStreamSetInfo_ = function(
  ...
  for (var i = 0; i < adaptationSets.length; ++i) {
    ...
    if ((adaptationSet.lang || '') != streamSetInfo.lang) {
      shaka.log.warning(
          'AdaptationSet groups should have the same language.',
          adaptationSets);
    }

    if ((adaptationSet.contentType || '') != streamSetInfo.contentType) {
      shaka.log.error(
          'AdaptationSet groups must have the same content type:',
          'ignoring AdaptationSet.',
          adaptationSet);
      continue;
    }
