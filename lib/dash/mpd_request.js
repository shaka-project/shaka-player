 ...
 * @param {?function=} opt_mpdPreProcessor Is invoked after the request
 *
 * @struct
 * @constructor
 */
shaka.dash.MpdRequest = function(url, opt_requestTimeout, opt_mpdPreProcessor) {
...
shaka.dash.MpdRequest.prototype.send = function() {
  var url = this.url_;
  var mpdPreProcessor = this.mpdPreProcessor_;
  return url.fetch(this.parameters_).then(
      /** @param {!ArrayBuffer|string} data */
      function(data) {
        var unprocessedMpd = data;
        if (data && typeof mpdPreProcessor === 'function') {
          unprocessedMpd = mpdPreProcessor(data) || data;
        }
        var mpd = shaka.dash.mpd.parseMpd(
            /** @type {string} */ (unprocessedMpd), url.urls, [url.currentUrl]);
        if (mpd) {
