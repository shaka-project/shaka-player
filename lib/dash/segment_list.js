...
shaka.dash.SegmentList.createStream = function(
  ...
  shaka.asserts.assert(context.representation.segmentList,
                       'Should only be called with SegmentList');
  var SegmentList = shaka.dash.SegmentList;

  var init = shaka.dash.SegmentBase.createInitSegment(context, 'segmentList');
  ...
  if (segmentIndex) {
    shaka.asserts.assert(manifest, 'This should be an update');
...
shaka.dash.SegmentList.parseSegmentListInfo_ = function(context) {
  ...
  var segmentInfo = MpdUtils.parseSegmentInfo(context, 'segmentList');
  ...
  for (var i = 0; i < max; i++) {
    ...
    references.push(
        new shaka.media.SegmentReference(
            i + startNumber, startTime, endTime, mediaUri, segment.start,
            segment.end));
    prevEndTime = endTime;
  }
