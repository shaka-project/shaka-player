...
shaka.dash.SegmentList = class {
  ...
  static createStreamInfo(context, streamMap) {
    ...
    const timescale = SegmentList.getTimescaleFromSegmentList_(context);

    const initSegmentReference = shaka.dash.SegmentBase.createInitSegment(
        context, SegmentList.fromInheritance_, timescale);
  ...
  /**
   * Gets timescale from SegmentList.
   *
   * @param {shaka.dash.DashParser.Context} context
   * @return {number}
   * @private
   */
  static getTimescaleFromSegmentList_(context) {
    ...
    const node = XmlUtils.findChildren(segmentLists[0], 'SegmentList');
    if (!node) {
      return 1;
    }

    const timescale = segmentLists[0].getAttribute('timescale');
    if (timescale) {
