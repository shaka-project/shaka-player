...
goog.provide('shaka.dash.TimelineSegmentIndex');
...
goog.requireType('shaka.media.PresentationTimeRange');
...
shaka.dash.SegmentTemplate = class {
  ...
  static createStreamInfo(
    ...
    if (info.indexTemplate) {
      ...
      if (!segmentIndex) {
        console.log(`Creating TSI with end ${periodEnd}`);
        segmentIndex = new shaka.dash.TimelineSegmentIndex(
            info,
        ...
        if (segmentIndex instanceof shaka.dash.TimelineSegmentIndex) {
          segmentIndex.appendTemplateInfo(info, periodEnd);
        }
        const availabilityStart =
          context.presentationTimeline.getSegmentAvailabilityStart();
        segmentIndex.evict(availabilityStart);
      }

      if (info.timeline) {
        /**
         * @type {!Array<shaka.media.PresentationTimeRange>}
         */
        const presentationRange = info.timeline;
...
shaka.dash.TimelineSegmentIndex = class extends shaka.media.SegmentIndex {
   ...
   * @param {shaka.media.InitSegmentReference} initSegmentReference
  */
  constructor(templateInfo, repId, bandwidth, baseUris,
    ...
    /** @private {number} */
    this.earliestTimeAfterRelease_ = 0;

    shaka.log.debug(`TSI ${this.repId_} ${periodStart}-${periodEnd}`);
  }
  ...
  release() {
    ...
    if (this.templateInfo_ && this.templateInfo_.timeline &&
      this.templateInfo_.timeline.length) {
      const timeline = this.templateInfo_.timeline;
      this.earliestTimeAfterRelease_ = timeline[0].start + this.periodStart_;
    }
  ...
  evict(time) {
    ...
    shaka.log.debug(`${this.repId_} Evicting at ${time}`);
    let numToEvict = 0;
    const timeline = this.templateInfo_.timeline;
    ...
    this.setNumEvicted(this.getNumEvicted() + numToEvict);
   ...
   * @export
   */
  appendTemplateInfo(info, periodEnd) {
    if (!this.templateInfo_) {
  ...
  isBeforeFirstEntry(time) {
    ...
    if (hasTimeline) {
      ...
      return time < this.earliestTimeAfterRelease_;
    }
  }
  ...
  find(time) {
    ...
    for (let i = 0; i < timeline.length; i++) {
      ...
      if (i === lastIndex && this.periodEnd_ !== Infinity) {
        end = this.periodEnd_;
      } else if (i < lastIndex) {
   ...
   */
  get(position) {
    const correctedPosition = position - this.getNumEvicted();
    ...
    if (!ref) {
      ...
      const createUrisCb = shaka.dash.TimelineSegmentIndex
          .createUris_.bind(null,
