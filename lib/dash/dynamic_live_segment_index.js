...
/**
 * Creates a DynamicLiveSegmentIndex. If the SegmentIndex's corresponding
 * stream is not available then the SegmentIndex will be inactive: it will not
 * contain any SegmentReferences nor will it generate new SegmentReferences.
 * However, an inactive SegmentIndex can be activated by integrating an active
 * SegmentIndex into it.
 *
...
shaka.dash.DynamicLiveSegmentIndex = function(
  ...
  /**
   * Either the time, in seconds, when the last segment became available, or
   * null if this SegmentIndex is inactive.
   * @private {?number}
   */
  this.lastSegmentAvailabilityStartTime_ =
      this.length() > 0 ?
      mpd.availabilityStartTime + period.start + this.last().endTime :
      null;
  shaka.asserts.assert(this.lastSegmentAvailabilityStartTime_ <=
                       manifestCreationTime);

  /**
   * Either the segment number of the next new SegmentReference, or null if
   * this SegmentIndex is inactive.
   * @private {?number}
   */
...
shaka.dash.DynamicLiveSegmentIndex.prototype.integrate = function(
    segmentIndex) {
  if (this.lastSegmentAvailabilityStartTime_ != null) {
  ...
  if (other.lastSegmentAvailabilityStartTime_ == null) {
  ...
  this.merge(other);
  this.lastSegmentAvailabilityStartTime_ =
      other.lastSegmentAvailabilityStartTime_;
...
/** @override */
shaka.dash.DynamicLiveSegmentIndex.prototype.onUpdate = function(wallTime) {
  this.generateSegmentReferences_(wallTime);
  shaka.dash.LiveSegmentIndex.prototype.onUpdate.call(this, wallTime);
};
...
shaka.dash.DynamicLiveSegmentIndex.prototype.generateSegmentReferences_ =
  ...
  // |lastSegmentAvailabilityStartTime_| takes into account
  // @suggestedPresentationDelay, so we must subtract the SPD from |wallTime|
  // to determine the correct change in time.
  var suggestedPresentationDelay = this.mpd.suggestedPresentationDelay || 0;
  var delta = wallTime -
              suggestedPresentationDelay -
              this.lastSegmentAvailabilityStartTime_;
  shaka.asserts.assert(delta >= 0);

  // Determine the number of new SegmentReferences to generate.
  var segmentTemplate = this.representation_.segmentTemplate;
  var scaledSegmentDuration =
      (segmentTemplate.segmentDuration / segmentTemplate.timescale);
  var numNewSegments = Math.floor(delta / scaledSegmentDuration);
  ...
  Array.prototype.push.apply(
      this.references,
      shaka.media.SegmentIndex.shift(newReferences, this.timestampCorrection));
