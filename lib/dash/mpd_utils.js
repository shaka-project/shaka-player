...
shaka.dash.MpdUtils = class {
  ...
  static expandSWithPattern_(sNode, patterns) {
    ...
    // --------------------------------------------------
    // 1. Build full pattern array with P.r
    // --------------------------------------------------
    let patternDurations = [];
    ...
    // --------------------------------------------------
    // 2. Rotate pattern by pE
    // --------------------------------------------------
    let entryPoint = Number(sNode.attributes['pE'] || 0);
    if (entryPoint >= patternDurations.length) {
      entryPoint = 0;
    }
    patternDurations = [
      ...patternDurations.slice(entryPoint),
      ...patternDurations.slice(0, entryPoint),
    ];

    // --------------------------------------------------
    // 3. Repeat pattern according to S.r
    // --------------------------------------------------
    const sR = Number(sNode.attributes['r'] || 0);
    const durations = [];
    for (let i = 0; i <= sR; i++) {
      durations.push(...patternDurations);
    }
