...
shaka.dash.MpdUtils.createFromTemplate = function(
    representation, number, time, startByte, endByte) {
  shaka.asserts.assert(representation.segmentTemplate);
  ...
  if (!urlTemplate) {
    ...
    return representation.baseUrl ?
        new shaka.util.FailoverUri(representation.baseUrl, startByte, endByte) :
        null;
  }
  ...
  return new shaka.util.FailoverUri(mediaUrl, startByte, endByte);
};
