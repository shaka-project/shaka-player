...
shaka.dash.MpdUtils = class {
  ...
  static createTimeline(
    ...
    for (let i = 0; i < timePoints.length; ++i) {
      ...
      let t = TXml.parseAttr(timePoint, 't', XmlUtils.parseNonNegativeInt);
      const d =
          TXml.parseAttr(timePoint, 'd', XmlUtils.parseNonNegativeInt);
      const r = TXml.parseAttr(timePoint, 'r', XmlUtils.parseInt);

      const k = TXml.parseAttr(timePoint, 'k', XmlUtils.parseInt);
      ...
      if (repeat < 0) {
        if (next) {
          const nextStartTime =
              TXml.parseAttr(next, 't', XmlUtils.parseNonNegativeInt);
    ...
    return networkOperation.chain(
        (response) => {
          ...
          return shaka.dash.MpdUtils.processXlinks(
              null, element, retryParameters, failGracefully, uris[0],
   ...
   *
   * @param {?shaka.extern.xml.Node} parent
   * @param {!shaka.extern.xml.Node} element
   * @param {!shaka.extern.RetryParameters} retryParameters
   * @param {boolean} failGracefully
   * @param {string} baseUri
  ...
  static processXlinks(
      parent, element, retryParameters,
      ...
      if (failGracefully) {
        ...
        handled = handled.chain(undefined, (error) => {
          ...
          return MpdUtils.processXlinks(
              parent, element, retryParameters, failGracefully, baseUri,
