...
shaka.dash.MpdUtils = class {
   ...
   * single-period MPD, merges its Period content (AdaptationSets, segment
   * info) into the Linked Period, and removes the ImportedMPD element.
   *
   * @param {!shaka.extern.xml.Node} mpd
  ...
  static processLinkedPeriods(mpd, baseUri, retryParameters, networkingEngine) {
    ...
    for (const period of TXml.findChildren(mpd, 'Period')) {
      const importedMpdElem = TXml.findChild(period, 'ImportedMPD');
      if (!importedMpdElem) {
        continue;
      }

      const href = TXml.getContents(importedMpdElem);
      if (!href) {
        period.children =
            period.children.filter((c) => c !== importedMpdElem);
        continue;
      }

      const importedUri =
          ManifestParserUtils.resolveUris([baseUri], [href.trim()])[0];
      const request =
      ...
      const op = networkOperation.chain((response) => {
        ...
        try {
          ...
          // Malformed XML — skip this import.
        }
        if (importedMpd) {
          const importedPeriod = TXml.findChild(importedMpd, 'Period');
          if (importedPeriod) {
            // Inject the imported MPD's URL as a BaseURL so that relative
            // segment URLs in the imported content resolve correctly.
            const baseUrlNode = /** @type {!shaka.extern.xml.Node} */ ({
              tagName: 'BaseURL',
              attributes: {},
              children: [response.uri],
              parent: period,
            });
            period.children.unshift(baseUrlNode);

            // Copy AdaptationSets and segment info into the Linked Period.
            for (const child of TXml.getChildNodes(importedPeriod)) {
              if (child.tagName === 'AdaptationSet' ||
                  child.tagName === 'SegmentBase' ||
                  child.tagName === 'SegmentList' ||
                  child.tagName === 'SegmentTemplate') {
                period.children.push(child);
              }
            }

            // Copy duration if the Linked Period doesn't define its own.
            if (!period.attributes['duration'] &&
                importedPeriod.attributes['duration']) {
              period.attributes['duration'] =
                  importedPeriod.attributes['duration'];
            }
          }
        }
