...
shaka.dash.ContentProtection.parseFromAdaptationSet = function(
  ...
  if (parsedNonCenc.length > 0) {
    ...
    if (drmInfos.length == 0) {
      var keySystem = shaka.media.DrmEngine.UnknownKeySystem;
      drmInfos = [ContentProtection.createDrmInfo_(keySystem, defaultInit)];
    }
 ...
 * @return {!Array.<string>} The parsed key IDs
 */
shaka.dash.ContentProtection.parseFromRepresentation = function(
  ...
  if (context.firstRepresentation) {
    ...
    var repUnknown = repContext.drmInfos.length == 1 &&
        !repContext.drmInfos[0].keySystem;
  ...
  if (repContext.defaultKeyId) {
    return [repContext.defaultKeyId];
  } else if (context.defaultKeyId) {
    return [context.defaultKeyId];
  } else {
    return [];
  }
};
