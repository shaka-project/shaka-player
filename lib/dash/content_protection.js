 ...
 * @param {?shakaExtern.ContentProtectionCallback} callback
...
shaka.dash.ContentProtection.parseFromAdaptationSet = function(
  ...
  if (parsedNonCenc.length > 0) {
    ...
    // If there are only CENC element(s), then assume all key-system are
    // supported.
...
/**
 * Creates a DrmInfo object from the given element.
 *
 ...
 * removes those elements.  This ignores elements where the scheme is
 * 'urn:mpeg:dash:mp4protection:2011' and |value| is not 'cenc'.
 *
 * @param {!Array.<!Node>} elems
...
shaka.dash.ContentProtection.parseElements_ = function(elems) {
  return elems
      .map(function(elem) {
        ...
        if (schemeUri == shaka.dash.ContentProtection.MP4Protection_) {
          var value = elem.getAttribute('value');
          if (value != 'cenc') {
            shaka.log.error('Unrecognized scheme type "' + value +
                            '" scheme type should be "cenc"');
            return null;
          }
        }
