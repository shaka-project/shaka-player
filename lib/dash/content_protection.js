...
shaka.dash.ContentProtection = class {
  ...
  static parseFromRepresentation(
    ...
    if (context.firstRepresentation) {
      ...
      // There is case when each Representation have unique key and
      // we need to merge Representation drmInfos with context drmInfos
      for (const repInfo of repContext.drmInfos) {
        let existInContext = false;
        for (const contextInfo of context.drmInfos) {
          // For each drm info need to generate keyIds
          // Check duplicates in context by keyIds and push if not exist
          repInfo.keyIds = new Set(
              repInfo.initData.map((element) => element.keyId),
          );
          repInfo.keyIds.delete(null);
          const repKeys = Array.from(repInfo.keyIds.values()).join(',');

          contextInfo.keyIds = new Set(
              contextInfo.initData.map((element) => element.keyId),
          );
          contextInfo.keyIds.delete(null);
          const contextKeys = Array.from(contextInfo.keyIds.values()).join(',');

          if (repKeys === contextKeys) {
            existInContext = true;
            break;
          }
        }
        if (!existInContext) {
          context.drmInfos.push(repInfo);
        }
      }
