...
shaka.dash.ContentProtection.parseElements_ = function(elems) {
  ...
  return elems.map(
      ...
      function(elem) {
        ...
        // if there is no pssh block and keySystem is Playready,
        // build Pssh block for Playready tags and, so MediaKeySession can be
        // launched without waiting encrypted event.
        var ContentProtection = shaka.dash.ContentProtection;
        if (!psshs.length && ContentProtection.defaultKeySystems_[schemeUri] ===
          'com.microsoft.playready') {
          var msProNode = shaka.util.XmlUtils.findChild(element, 'mspr:pro');
          var msProText = msProNode.textContent;
          var pssh = shaka.dash.ContentProtection.buildPssh(schemeUri, msProText);
          psshs.push(pssh);
        }
...
/**
 * Creates pssh data for content protection blocks. So, content protection
 * elements can get initData from pssh data.
 * |  4 Bytes  |   4 Bytes   |    4 Bytes   |   16 Bytes   |  4 Bytes  |   ?   |
 * | Pssh size | Type of box | Pssh version | KeySystem id | Data Size |  Data |
 *
 * @param {string} schemeUri
 * @param {string} dataBase64
 * @return {string}
 */
shaka.dash.ContentProtection.buildPssh = function(schemeUri, dataBase64) {
  var data = shaka.util.Uint8ArrayUtils.fromBase64(dataBase64);

  var schemeUriHexStr = schemeUri.match(/[\da-f]{2}/gi).join('');
  var schemeUriData = shaka.util.Uint8ArrayUtils.fromHex(schemeUriHexStr);

  var psshBoxData = new Uint8Array(shaka.util.Uint8ArrayUtils.toUTF8('pssh'));

  var psshBoxLength = 12 /* Full Header Size */ + 16 /* schemeUri */ +
      4 /* DataSize */ + data.length;

  var psshArrayBuffer = new ArrayBuffer(psshBoxLength);
  var writer = new shaka.util.DataViewWriter(
      new DataView(psshArrayBuffer),
      shaka.util.DataViewWriter.Endianness.BIG_ENDIAN);

  writer.writeUint32(psshBoxLength);
  writer.put(psshBoxData); // 0x70, 0x73, 0x73, 0x68
  writer.writeUint32(0); // 0x00, 0x00, 0x00, 0x00
  writer.put(schemeUriData);
  writer.writeUint32(data.length);
  writer.put(data);

  var psshByteArray = writer.getArray();

  return shaka.util.Uint8ArrayUtils.toBase64(psshByteArray);
};
