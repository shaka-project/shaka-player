...
shaka.dash.ContentProtection = class {
   ...
   */
  static getInitDataFromPro(element) {
    const proNode = shaka.util.XmlUtils.findChildNS(
        element.node, 'urn:microsoft:playready', 'pro');
    if (!proNode) {
      return null;
    }
    const fromBase64 = shaka.util.Uint8ArrayUtils.fromBase64;
    const type = fromBase64(btoa('pssh'));
    const systemId = fromBase64(btoa(element.schemeUri));
    const data = fromBase64(proNode.textContent);
    const dataLength = data.length;
    const psshSize = 0x4 + type.length + systemId.length + 0x4 + dataLength;
    /** @type {!ArrayBuffer} */
    ...
    byteCursor += 0x4;
    psshBox.set(type, byteCursor);
    byteCursor += type.length;
  ...
  static convertElements_(defaultInit, callback, elements) {
    ...
    for (const element of elements) {
      ...
      if (keySystem) {
        ...
        const proInitData = ContentProtection.getInitDataFromPro(element);
