...
shaka.routing.Walker = class {
  ...
  async takeNextStep_() {
    ...
    try {
      await this.implementation_.enterNode(
      ...
      this.currentlyAt_ =
          await this.implementation_.handleError(this.currentlyWith_);
    }
  }
 ...
 *   When an error has occured executing |enterNode|, |handleError| will be
 *   called. This will allow the walker to reset and be ready for use again.
 *   This method will be passed the current payload and will need to return the
 *   node that the walker should restart at. This method may modify the current
 *   payload.
 */
