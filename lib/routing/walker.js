...
shaka.routing.Walker = class {
  ...
  constructor(startingAt, startingWith, implementation) {
    shaka.log.v1(`[lgt] Init walker at ${startingAt.name}`, startingWith);
  ...
  doOneThing_() {
    ...
    shaka.log.v1('[lgt] idle, waiting for new route request...');
  ...
  tryNewRoute_() {
    ...
    if (this.currentRoute_) {
      shaka.log.v1(
          '[lgt] --- cancel current route: ',
          `${this.currentStart_.name} -> ${this.currentRoute_.node.name}`);
      this.currentRoute_.listeners.onCancel();
      this.currentRoute_ = null;
      this.currentStart_ = null;
    }
    ...
    if (newRoute) {
      shaka.log.v1(
          '[lgt] +++ starting new route: ',
          `from ${this.currentlyAt_.name} -> ${newRoute.node.name}`);
      request.listeners.onStart();

      // Convert the route created from the request's create method to an
      // active route.
      this.currentRoute_ = {
        // start: this.currentlyAt_,
        node: newRoute.node,
        payload: newRoute.payload,
        interruptible: newRoute.interruptible,
        listeners: request.listeners,
      };
      this.currentStart_ = this.currentlyAt_;
    } else {
      shaka.log.v1('[lgt] skip routing attempt.');
  ...
  async takeNextStep_() {
    ...
    shaka.log.v1(
        `[lgt] step: <${this.currentlyAt_.name}> `,
        `(active route: ${this.currentStart_.name} `,
        `-> ${this.currentRoute_.node.name})`);
    ...
    try {
      ...
      if (error.code == shaka.util.Error.Code.OPERATION_ABORTED) {
        ...
        shaka.log.v1(
            `[lgt] Routing error at ${this.currentlyAt_.name}: `,
            `${this.currentStart_.name} -> ${this.currentRoute_.node.name}`);
      ...
      this.currentlyAt_ = await this.implementation_.handleError(
          ...
          error);
      shaka.log.v1(
          '[lgt] Reset load graph after error. ',
          `Move to <${this.currentlyAt_.name}>`);
    }
  }
