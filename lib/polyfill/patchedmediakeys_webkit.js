...
class extends shaka.util.FakeEventTarget {
  ...
  generate_(initData, offlineSessionId) {
    ...
    this.generatePromise_ = Promise.withResolvers();
    ...
    return this.generatePromise_.promise;
  }
   ...
   * progress.
   *
   * @param {!Promise.PromiseWithResolvers} promise  The promise associated with
   *   this call.
  ...
  update_(promise, response) {
    ...
    if (this.updatePromise_) {
      ...
      // succeeds or fails.
      this.updatePromise_.promise.then(() => this.update_(promise, response))
  ...
  update(response) {
    ...
    const nextUpdatePromise = Promise.withResolvers();
    this.update_(nextUpdatePromise, response);
    return nextUpdatePromise.promise;
  }
  ...
  close() {
    ...
    if (this.type_ != 'persistent-license') {
      ...
      // implementation.
      if (!this.sessionId) {
        this.closed_.reject(new Error('The session is not callable.'));
        return this.closed_.promise;
      }
