...
shaka.polyfill.PatchedMediaKeysWebkit.getVideoElement_ = function() {
  ...
  const tmpVideo = videos.length ? videos[0] : document.createElement('video');
...
shaka.polyfill.PatchedMediaKeysWebkit.MediaKeySession.prototype.generate_ =
    function(initData, offlineSessionId) {
      ...
      // without a video tag, we can get ourselves into trouble.  It seems that
      // sometimes, the setting of media.src hasn't been processed by some other
      // thread, and GKR can throw an exception.  If this occurs, wait 10 ms and
      // try again at most once.  This situation should only occur when init
      // data is available ahead of the 'needkey' event.
...
shaka.polyfill.PatchedMediaKeysWebkit.MediaKeySession.prototype.update_ =
    function(promise, response) {
      ...
      if (this.keySystem_ == 'webkit-org.w3.clearkey') {
        ...
        if (kty != 'oct') {
          // Reject the promise.
          const error = new Error('Response is not a valid JSON Web Key Set.');
          this.updatePromise_.reject(error);
          this.updatePromise_ = null;
        }
