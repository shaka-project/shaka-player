...
shaka.polyfill.MediaCapabilities = class {
   ...
   * Three-way decision:
   *
   * 1. No navigator.mediaCapabilities at all → install the full custom
   *    implementation as before.
   *
   * 2. navigator.mediaCapabilities exists AND
   *    device.supportsMediaCapabilities() === true → the native MCap is
   *    known-good for all query types including EME; do nothing.
   *
   * 3. navigator.mediaCapabilities exists BUT
   *    device.supportsMediaCapabilities() === false → only two things are
   *    broken: (a) 'file' type queries are unreliable on many platforms,
   *    (b) keySystemAccess may be missing from encrypted 'media-source'
   *    results.  We install a lightweight wrapper that:
   *      – routes 'file' (and unrecognised types) through our own
   *        implementation, and
   *      – delegates 'media-source' to the native MCap but back-fills
   *        keySystemAccess when it is absent from an encrypted result.
   *
   * This avoids reimplementing Cast canDisplayType_, codec-rejection logic,
   * and Tizen AC-3 workarounds for browsers where native MCap already handles
   * 'media-source' correctly.
   *
   * @suppress {const}
   * @export
   */
  static install() {
    ...
    if (!navigator.mediaCapabilities) {
      // Case 1: No native MCap at all – install the full polyfill.
      shaka.log.info('MediaCapabilities: no native support, installing full ' +
          'polyfill.');

      navigator.mediaCapabilities = /** @type {!MediaCapabilities} */ ({});

      // Keep the patched MediaCapabilities object from being
      // garbage-collected in Safari.
    ...
    if (device.supportsMediaCapabilities()) {
      // Case 2: Native MCap is fully reliable – nothing to patch.
      shaka.log.info(
          'MediaCapabilities: native mediaCapabilities support found.');
      return;
    }

    // Case 3: Native MCap exists but has known partial issues.  Install a
    // lightweight wrapper instead of a full replacement.
    shaka.log.info('MediaCapabilities: partial patch – delegating ' +
        'media-source to native, polyfill file mode and keySystemAccess.');

    // Keep a reference so the wrapper can call through.
    shaka.polyfill.MediaCapabilities['originalMcap'] =
        navigator.mediaCapabilities;
    ...
    navigator.mediaCapabilities.decodingInfo = async (mediaDecodingConfig) => {
      const MimeUtils = shaka.util.MimeUtils;
      const videoConfig = mediaDecodingConfig.video;
      const audioConfig = mediaDecodingConfig.audio;

      const isAudioOnly = !!audioConfig && !videoConfig;

      const isMp4Video = !!videoConfig && videoConfig.contentType &&
          MimeUtils.getContainerType(videoConfig.contentType) == 'mp4';

      const useNative = isAudioOnly || isMp4Video ||
          mediaDecodingConfig.type !== 'media-source';

      if (useNative) {
        // Use the native MediaCapabilities implementation for audio-only
        // queries, MP4 video queries, and non-'media-source' types. Route
        // non-MP4 media-source video queries through the polyfill
        // implementation because native support is unreliable on some
        // platforms.
        return shaka.polyfill.MediaCapabilities.decodingInfo_(
            mediaDecodingConfig);
      }

      // For 'media-source', trust native MCap for codec/format/display checks
      // since it often has platform-specific knowledge we can't replicate
      // (e.g. Cast canDisplayType_, Tizen codec rejection, etc.).
      const result = await originalDecodingInfo(mediaDecodingConfig);

      // Native MCap on several platforms returns supported:true for encrypted
      // content but omits keySystemAccess from the result.  Back-fill it so
      // callers always get a usable object for DRM setup.
      if (result.supported &&
          mediaDecodingConfig.keySystemConfiguration &&
          !result.keySystemAccess) {
        const videoConfig = mediaDecodingConfig['video'];
        const audioConfig = mediaDecodingConfig['audio'];
        const mcapKeySystemConfig =
        mediaDecodingConfig.keySystemConfiguration;
        try {
          result.keySystemAccess =
          ...
          shaka.log.info('MediaCapabilities: DRM support check failed for ' +
          'encrypted media-source config; marking as unsupported.');
  ...
  static async decodingInfo_(mediaDecodingConfig) {
    ...
    if (mediaDecodingConfig.type == 'media-source') {
      ...
      if (audioConfig) {
        const isSupported =
            await shaka.polyfill.MediaCapabilities.checkAudioSupport_(
                audioConfig);
        if (!isSupported) {
          return res;
        }
   ...
   * @private
   */
  static async checkAudioSupport_(audioConfig) {
    const device = shaka.device.DeviceFactory.getDevice();
    const deviceType = device.getDeviceType();
    ...
    if (deviceType === shaka.device.IDevice.DeviceType.CAST &&
      ...
      if (window.cast &&
        ...
        if (memoizedCanDisplayTypeRequests.has(extendedType)) {
          result = memoizedCanDisplayTypeRequests.get(extendedType);
        } else {
          result = await cast.__platform__.canDisplayType(extendedType);
          memoizedCanDisplayTypeRequests.set(extendedType, result);
        }
        return result;
      }
    }
