  ...
  static async decodingInfo_(mediaDecodingConfig) {
    ...
    if (mediaDecodingConfig.type == 'media-source') {
      ...
      if (device.getDeviceType() !== shaka.device.IDevice.DeviceType.CAST &&
          originalMcapDecodingInfo) {
        try {
          const mediaDecodingConfigClear =
              shaka.util.ObjectUtils.cloneObject(mediaDecodingConfig);
          delete mediaDecodingConfigClear.keySystemConfiguration;
          const mcapRes =
              await originalMcapDecodingInfo(mediaDecodingConfigClear);
          if (!mcapRes.supported) {
            return res;
          }
          res.powerEfficient = mcapRes.powerEfficient;
          res.smooth = mcapRes.smooth;
        } catch (e) {
          return res;
        }
      } else {
...
/**
 * @type {?function(!MediaDecodingConfiguration):
