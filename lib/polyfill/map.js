...
shaka.polyfill.Map = class {
  ...
  /**
   * Install the WeakMap polyfill if needed.
   * @export
   */
  static installWeakMap() {
    ...
    if (!('getOrInsert' in WeakMap.prototype)) {
      ...
      // eslint-disable-next-line no-extend-native, no-restricted-syntax
      WeakMap.prototype.getOrInsert = shaka.polyfill.Map.weakMapGetOrInsert_;
    }
    ...
    if (!('getOrInsertComputed' in WeakMap.prototype)) {
      ...
      WeakMap.prototype.getOrInsertComputed =
        shaka.polyfill.Map.weakMapGetOrInsertComputed_;
    }
  }
   ...
   * @private
   */
  static weakMapGetOrInsert_(key, defaultValue) {
    if (!this.has(key)) {
      this.set(key, defaultValue);
    }
    return this.get(key);
   ...
   * @private
   */
  static weakMapGetOrInsertComputed_(key, callbackFunction) {
    if (!this.has(key)) {
      this.set(key, callbackFunction(key));
    }
    return this.get(key);
...
shaka.polyfill.register(shaka.polyfill.Map.installWeakMap);
