...
shaka.polyfill.PatchedMediaKeysApple = class {
   ...
   */
  static install() {
    if (!shaka.util.Platform.willMediaKeysAppleBePolyfilled()) {
      return;
    }
   ...
   * @private
   */
  static dispatchEncryptedEvent_(sourceEvent, initDataType, initData) {
    ...
    const encryptedEvent =
      /** @type {!MediaEncryptedEvent} */(/** @type {?} */(event));
    encryptedEvent.initDataType = initDataType;
    encryptedEvent.initData = initData;

    sourceEvent.target.dispatchEvent(event);
  }
  ...
  static onWebkitNeedKey_(event) {
    ...
    try {
      ...
      if (data) {
        PatchedMediaKeysApple.dispatchEncryptedEvent_(
            event,
            'sinf',
            shaka.util.BufferUtils.toArrayBuffer(event.initData));

        return;
      }
    } catch (error) {
      // Continue with SKD initDataType detection
    }
    ...
    PatchedMediaKeysApple.dispatchEncryptedEvent_(
        event,
        'skd',
        shaka.util.BufferUtils.toArrayBuffer(initData));
  }
};
