...
shaka.polyfill.PatchedMediaKeysApple = class {
  ...
  static install() {
    ...
    */
    ...
    // suggested in w3c/encrypted-media#32.
  ...
  /**
   * Rebuild FairPlay init data according to Apple's docs.  It's unclear why
   * this is not done by the browser, or if the unprefixed version will do it
   * for us.
   *
   ...
   * @return {BufferSource}
   */
  static rebuildInitData_(initData, certificate) {
    const intLength = 4;

    // TODO: Move this into DrmEngine if it is still needed with unprefixed
    // EME. FairPlay init data is in two parts and must be processed a bit.
    ...
    if (length + intLength != initDataArray.byteLength) {
    ...
    const uriString = shaka.util.StringUtils.fromUTF16(
        initDataArray.slice(intLength), /* littleEndian= */ true);
    ...
    const rebuiltInitData = new Uint8Array(
        ...
        intLength + contentIdArray.byteLength +
        intLength + certificate.byteLength);
