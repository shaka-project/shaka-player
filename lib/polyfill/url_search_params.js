...
shaka.polyfill.URLAndSearchParams = class {
  ...
  static installURLSearchParams_() {
    ...
    // Keep the polyfilled URLSearchParams from being garbage-collected in
    // Safari / Tizen.  String notation is required to prevent the Closure
    // Compiler from collapsing the property into a local variable.
    // See: https://github.com/shaka-project/shaka-player/issues/3696
    //      https://github.com/shaka-project/shaka-player/issues/8607
    shaka.polyfill.URLAndSearchParams['installedUSP'] =
        window.URLSearchParams;

    Object.defineProperty(window, 'URLSearchParams', {
      value: shaka.polyfill.URLAndSearchParams.URLSearchParamsImpl_,
    });
  ...
  static installURL_() {
    ...
    if (URLAndSearchParams.isNativeURLComplete_()) {
      ...
      // Always install a searchParams getter on the native URL prototype.
      //
      // Rationale: some platforms (e.g. Tizen 3 / Safari 3) have a native URL
      // constructor that passes all feature-detection tests yet return
      // undefined for searchParams on real https:// URLs at runtime.
      // Conditioning this patch only on isNativeURLSearchParamsComplete_() is
      // not sufficient: URLSearchParams and URL.prototype.searchParams are
      // independent subsystems — a platform can have a fully-featured
      // URLSearchParams (all methods, correct encoding) while its
      // URL.prototype.searchParams getter is broken or missing.
      //
      // The getter is lazy and cached per instance, so installing it
      // unconditionally is safe: on browsers where searchParams already works
      // correctly we merely shadow the native accessor with an equivalent one.
      ...
      try {
        ...
        // If the prototype is non-configurable (exotic runtime), log and
        // continue — the native searchParams, if present, will be used as-is.
        shaka.log.warning(
            'URLAndSearchParams: Could not patch URL.prototype.searchParams.',
            e);
      }
      return;
    }
    ...
    // Keep the polyfilled URL from being garbage-collected in Safari /
    // Tizen.  String notation is required to prevent the Closure Compiler
    // from collapsing the property into a local variable.
    // See: https://github.com/shaka-project/shaka-player/issues/3696
    //      https://github.com/shaka-project/shaka-player/issues/8607
    shaka.polyfill.URLAndSearchParams['installedURL'] = window.URL;

    Object.defineProperty(window, 'URL', {
      value: URLAndSearchParams.URLImpl_,
    });
...
/**
 * A strong reference to the polyfilled URLSearchParams constructor, held here
 * to prevent Safari's garbage collector from discarding it.  We use string
 * notation so the Closure Compiler does not collapse this into a local.
 * @type {?Function}
 */
shaka.polyfill.URLAndSearchParams['installedUSP'] = null;

/**
 * A strong reference to the polyfilled URL constructor, held here to prevent
 * Safari's garbage collector from discarding it.  We use string notation so
 * the Closure Compiler does not collapse this into a local.
 * @type {?Function}
 */
shaka.polyfill.URLAndSearchParams['installedURL'] = null;
