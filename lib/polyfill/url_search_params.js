...
shaka.polyfill.URLSearchParams = class {
   ...
   * absent or incomplete (missing the forEach/keys/values/entries iterators).
   * @private
   */
  static installURLSearchParams_() {
    if (window.URLSearchParams) {
      // Verify the native implementation exposes the iterable interface.
      // Some older browsers (e.g. Edge 14) shipped URLSearchParams without it.
      // eslint-disable-next-line no-restricted-syntax
      const proto = window.URLSearchParams.prototype;
      if (typeof proto[Symbol.iterator] === 'function') {
        shaka.log.debug(
            'URLSearchParams: Native URLSearchParams support found.');
        return;
      }
      shaka.log.info(
          'URLSearchParams: Native URLSearchParams found but iterator ' +
          'support is missing. Installing polyfill.');
    } else {
      shaka.log.info(
  ...
  /**
   * Installs URL.prototype.searchParams if it is missing.
  ...
  static installURLSearchParamsOnURL_() {
    ...
    shaka.log.info(
        'URLSearchParams: URL.prototype.searchParams not found. ' +
        'Installing polyfill.');
