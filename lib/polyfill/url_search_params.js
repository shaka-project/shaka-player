...
shaka.polyfill.URLSearchParams = class {
  ...
  static installURLSearchParams_() {
    // All methods and properties required by the WHATWG URL standard.
    // If any are missing we replace the entire implementation so that callers
    // can rely on a consistent, complete API.
    ...
    if (window.URLSearchParams) {
      ...
      const isComplete =
          typeof proto[Symbol.iterator] === 'function' &&
          requiredMethods.every((m) => typeof proto[m] === 'function');
