...
shaka.polyfill.URLAndSearchParams = class {
  ...
  static isNativeURLComplete_() {
    try {
      // Basic resolution of a relative URI against a custom scheme must work.
      const u = new URL('b', 'dummy://a');
      if (u.href !== 'dummy://a/b') {
        return false;
      }
      // pathname assignment must percent-encode spaces.
      u.pathname = 'c d';
      if (u.href !== 'dummy://a/c%20d') {
        return false;
      }
      // searchParams must be present AND functional.
      // Checking only existence is not enough — some browsers expose the
      // property but return undefined or an object without toString().
      if (!u.searchParams ||
          typeof u.searchParams.toString !== 'function') {
        return false;
      }

      // Resolving a relative URI against a base that carries query params must
      // drop the base's query string (WHATWG URL standard §5.1 "basic URL
      // parser").  We only require that the resolution doesn't throw and that
      // the result starts with the right origin/path — we do NOT pin the exact
      // href, because browsers differ in whether they keep or drop the base
      // query when the relative reference has no query of its own (both
      // behaviours have appeared in the wild).
      const customWithQuery = new URL('b', 'dummy://foo?a=1');
      if (!customWithQuery.href.startsWith('dummy://foo')) {
        return false;
      }
      // searchParams on the resolved URL must also be callable.
      if (typeof customWithQuery.searchParams.toString !== 'function') {
        return false;
      }
      return true;
    } catch (e) {
   ...
   */
  static installURL_() {
    if (shaka.polyfill.URLAndSearchParams.isNativeURLComplete_()) {
    ...
    window.URL = /** @type {?} */ (
      shaka.polyfill.URLAndSearchParams.URLImpl_);
  }
};
