...
shaka.media.ContentWorkarounds = class {
  ...
  static insertEncryptionMetadata_(
    ...
    // The data before the end of the source box will be copied to the same
    // location as before.  The data after that will be appended after the added
    // metadata box.
    const sourceBoxEnd = sourceBox.start + sourceBox.size;
    const beforeData = initSegment.subarray(0, sourceBoxEnd);
    const afterData = initSegment.subarray(sourceBoxEnd);

    newInitSegment.set(beforeData);
    newInitSegment.set(metadataBoxArray, sourceBoxEnd);
    newInitSegment.set(afterData, sourceBoxEnd + metadataBoxArray.byteLength);
    ...
    for (const box of ancestorBoxes) {
      goog.asserts.assert(box.start < sourceBoxEnd,
