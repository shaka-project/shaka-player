...
shaka.media.ContentWorkarounds = class {
  ...
  static freeDvvcBox(initSegmentBuffer) {
    ...
    new shaka.util.Mp4Parser()
        ...
        .fullBox('stsd', shaka.util.Mp4Parser.sampleDescription)
        .boxes(['encv', 'hvc1'], (box) => {
          parentDataView = box.reader.getDataView();
          parentStart = box.start;
          return shaka.util.Mp4Parser.visualSampleEntry(box);
        })
        .box('dvvC', (box) => {
          const ContentWorkarounds = shaka.media.ContentWorkarounds;
          box.parser.stop();
          // box start - parent box start - type size
          const byteOffset = box.start - parentStart - 4;
          parentDataView.setUint32(byteOffset,
              ContentWorkarounds.BOX_TYPE_FREE_);
        })
        .parse(initSegment);
