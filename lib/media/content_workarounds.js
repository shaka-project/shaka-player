...
shaka.media.ContentWorkarounds = class {
   ...
   */
  static fakeEncryption(initSegmentBuffer, encryptionScheme, uri) {
    ...
    if (isEncrypted) {
      shaka.log.warning('Init segment already indicates encryption.');
      return initSegment;
    }
    ...
    for (const workItem of boxesToModify) {
      ...
      modifiedInitSegment = ContentWorkarounds.insertEncryptionMetadata_(
          ...
          workItem.newType, encryptionScheme);
    }

    // Uncomment if you want to save the modified init segment to a file, i.e.
    // to inspect it in external tool.
    // ContentWorkarounds.saveFile(modifiedInitSegment);
  ...
  static fakeMediaEncryption(mediaSegmentBuffer) {
    ...
    let sampleDescriptionOffset = 0;
    let sampleDescriptionIndex = 0;
    const onTfhdBox = (box) => {
      goog.asserts.assert(box.flags !== null, 'fullBox should have flags');
      const TFHD_FLAG_BASE_DATA_OFFSET = 0x01;
      const TFHD_FLAG_SAMPLE_DESC = 0x02;
      if (box.flags & TFHD_FLAG_SAMPLE_DESC) {
        box.reader.skip(4); // track id
        if (box.flags & TFHD_FLAG_BASE_DATA_OFFSET) {
          box.reader.skip(8);
        }
        const headers = 12;
        sampleDescriptionOffset = box.start + headers +
          box.reader.getPosition();
        sampleDescriptionIndex = box.reader.readUint32();
      }
      box.parser.stop();
    };
    new shaka.util.Mp4Parser()
    ...
    if (sampleDescriptionIndex !== 2 && sampleDescriptionOffset) {
      const dataview = shaka.util.BufferUtils.toDataView(mediaSegment);
      dataview.setUint32(sampleDescriptionOffset, 2);
    }
  ...
  static insertEncryptionMetadata_(
    ...
    const metadataBoxArray = ContentWorkarounds.createEncryptionMetadata_(
        initSegment, sourceBox, metadataBoxType, encryptionScheme);
    ...
    const cutPoint = ((shaka.util.Platform.isApple() &&
      metadataBoxType === ContentWorkarounds.BOX_TYPE_ENCA_) ||
  ...
  static createEncryptionMetadata_(initSegment, sourceBox, metadataBoxType,
    ...
    metadataBoxView.setUint32(
        ...
        sourceBoxType);
    const encryptionSchemeCode = (encryptionScheme.charCodeAt(0) << 24) |
        (encryptionScheme.charCodeAt(1) << 16) |
        (encryptionScheme.charCodeAt(2) << 8) |
        (encryptionScheme.charCodeAt(3));
    metadataBoxView.setUint32(
        sourceBox.size +
          ContentWorkarounds.CANNED_SINF_BOX_ENCRYPTION_SCHEME_OFFSET_,
        encryptionSchemeCode);
  ...
  static saveFile(data) {
    ...
    downloadLink.download = 'modified-init.mp4';
    downloadLink.click();
...
shaka.media.ContentWorkarounds.CANNED_SINF_BOX_ =
    new shaka.util.Lazy(() => new Uint8Array([
      ...
      // Scheme: filled in later based on desired encryption scheme
      0x00, 0x00, 0x00, 0x00,
...
/**
 * The location of the encryption scheme field in the "schm" box inside
 * the canned "sinf" box above.
 *
 * @const {number}
 * @private
 */
shaka.media.ContentWorkarounds.CANNED_SINF_BOX_ENCRYPTION_SCHEME_OFFSET_ = 0x20;
