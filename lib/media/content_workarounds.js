...
shaka.media.ContentWorkarounds = class {
  ...
  static fakeMediaEncryption(mediaSegmentBuffer) {
    let mediaSegment = shaka.util.BufferUtils.toUint8(mediaSegmentBuffer);
    ...
    if (parsedTfhd && parsedTfhd.sampleDescriptionIndex !==
      ...
      if (parsedTfhd.sampleDescriptionIndex !== null) {
        ...
        shaka.media.ContentWorkarounds.updateBoxSize_(mediaSegment,
        ...
        if (parsedTrun && parsedTrun.dataOffset !== null) {
          ...
          dataview.setUint32(dataOffsetPosition, newDataOffset);
        }
        const beforeSdi = mediaSegment.subarray(0, sdiPosition);
        const afterSdi = mediaSegment.subarray(sdiPosition);
        mediaSegment = new Uint8Array(mediaSegment.byteLength + sdiSize);
        mediaSegment.set(beforeSdi);

        const bytes = [];
        for (let byte = sdiSize - 1; byte >= 0; byte--) {
          bytes.push((desiredSampleDescriptionIndex >> (8 * byte)) & 0xff);
        }
        mediaSegment.set(new Uint8Array(bytes), sdiPosition);
        mediaSegment.set(afterSdi, sdiPosition + sdiSize);
        for (const box of ancestorBoxes) {
          shaka.media.ContentWorkarounds.updateBoxSize_(mediaSegment, box.start,
              box.size + sdiSize);
        }
      }
    }
