...
shaka.media.ContentWorkarounds = class {
  ...
  static fakeEncryption(stream, initSegmentBuffer, uri) {
    ...
    modifiedInitSegment = ContentWorkarounds.insertPsshBox_(stream,
        modifiedInitSegment);
   ...
   * @private
   */
  static insertPsshBox_(stream, initSegment) {
    const pssh = new shaka.util.Mp4Generator().pssh(stream);
    if (pssh.byteLength === 0) {
      return initSegment;
    }
    ...
    const updatedMoovBoxSize = moovBoxSize + pssh.byteLength;
    ...
    newInitSegment.set(beforeData);
    newInitSegment.set(pssh, cutPoint);
    newInitSegment.set(afterData, cutPoint + pssh.byteLength);
