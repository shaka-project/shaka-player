 ...
 */
shaka.media.VttTextParser = function(data, segmentStartTime, segmentEndTime) {
...
shaka.media.VttTextParser.parseCue_ = function(text, segmentStartTime) {
  ...
  if (window.VTTCue) {
    ...
    while (word) {
      if (!shaka.media.VttTextParser.parseSetting(ret, word)) {
        throw new shaka.util.Error(
            shaka.util.Error.Category.TEXT,
...
shaka.media.VttTextParser.parseSetting = function(cue, word) {
  ...
  if ((results = /^align:(start|middle|end|left|right)$/.exec(word))) {
    ...
    cue.size = Number(results[1]);
      ...
      /^position:(\d{1,2}|100)%(?:,(line-left|line-right|center))?$/.exec(
      word))) {
