...
shaka.media.MediaSourceEngine = class {
  ...
  async codecSwitchIfNecessary_(contentType, mimeType, codecs, streamsByType) {
    ...
    let newCodecs = MimeUtils.getCodecs(newMimeType);
    const newBasicType = MimeUtils.getBasicType(newMimeType);

    // When looking at muxed content, we should only look at the first codec, to
    // avoid extraneous switches.
    if (this.needSplitMuxedContent_) {
      currentCodecs = MimeUtils.getCodecBase(currentCodecs);
      newCodecs = MimeUtils.getCodecBase(newCodecs);
    }

    // Current/new codecs base and basic type match then no need to switch
    if (currentCodecs === newCodecs && currentBasicType === newBasicType) {
      return false;
    }

    let allowChangeType = true;
    if (this.needSplitMuxedContent_ || (newCodecs.split(',').length &&
