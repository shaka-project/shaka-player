...
shaka.media.MediaSourceEngine = class {
   ...
   * @param {!shaka.media.MediaSourceEngine.PlayerInterface} playerInteface
   *   Interface for common player methods.
   * @param {?shaka.lcevc.Dec} [lcevcDec] Optional -  LCEVC Decoder Object
   */
  constructor(video, textDisplayer, playerInteface, lcevcDec) {
    /** @private {HTMLMediaElement} */
    this.video_ = video;

    /** @private {?shaka.media.MediaSourceEngine.PlayerInterface} */
    this.playerInteface_ = playerInteface;
  ...
  getTimestampAndDispatchMetadata_(contentType, data, reference, mimeType,
    ...
    if (shaka.util.MimeUtils.RAW_FORMATS.includes(mimeType)) {
      ...
      if (frames.length && reference) {
        ...
        this.playerInteface_.onMetadata(
      ...
      if (metadata.length) {
        this.playerInteface_.onMetadata(metadata, timestampOffset,
  ...
  workAroundBrokenPlatforms_(segment, startTime, contentType) {
    ...
    const keySystem = this.playerInteface_.getKeySystem();
