...
shaka.media.MediaSourceEngine = class {
  ...
  async appendBuffer(
    ...
    if (shaka.util.TsParser.probe(uint8ArrayData)) {
      ...
      let timestampOffset;
      if (startTime != null) {
        timestampOffset = reference.startTime - startTime;
      }
      if (!this.sequenceMode_ && timestampOffset != null) {
      ...
      if (metadata.length) {
        const metadataOffset = timestampOffset ||
            this.sourceBuffers_[contentType].timestampOffset;
        this.onMetadata_(metadata, metadataOffset,
            reference ? reference.endTime : null);
      }
    } else {
