  ...
  getTimestampAndDispatchMetadata(contentType, data, reference, stream,
    ...
    if (shaka.util.MimeUtils.RAW_FORMATS.includes(mimeType)) {
      ...
      if (!this.tsParsers_.has(contentType)) {
        parser = new shaka.util.TsParser();
        this.tsParsers_.set(contentType, parser);
      } else {
        parser = this.tsParsers_.get(contentType);
        parser.clearData();
      }
      parser.setDiscontinuitySequence(reference.discontinuitySequence);
      const tsParser = parser.parse(uint8ArrayData);
