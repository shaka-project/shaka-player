...
shaka.media.MediaSourceEngine = class {
  ...
  async appendBuffer(contentType, data, reference, hasClosedCaptions, seeked) {
    ...
    // Only process ID3 metadata in TS or when we are transmuxing.
    if (this.transmuxers_[contentType] || containerType == 'mp2t') {
      const uint8ArrayData = shaka.util.BufferUtils.toUint8(data);
      const id3Frames = shaka.util.Id3Utils.getID3Frames(uint8ArrayData);
      if (id3Frames.length) {
        // TODO...
