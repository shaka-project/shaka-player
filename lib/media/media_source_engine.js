...
shaka.media.MediaSourceEngine = class {
  ...
  async doDestroy_() {
    ...
    for (const contentType of this.queues_.keys()) {
      // Make a local copy of the queue and the first item.
      const q = this.queues_.get(contentType);
    ...
    if (goog.DEBUG) {
      for (const contentType of this.queues_.keys()) {
        goog.asserts.assert(
            this.queues_.get(contentType).length == 0,
  ...
  async enqueueBlockingOperation_(run) {
    ...
    try {
      ...
      if (goog.DEBUG) {
        for (const contentType of contentTypes) {
          if (this.queues_.get(contentType).length) {
            goog.asserts.assert(
                this.queues_.get(contentType).length == 1,
                'Should be at most one item in queue!');
            goog.asserts.assert(
                allWaiters.includes(this.queues_.get(contentType)[0].p),
  ...
  popFromQueue_(contentType) {
    goog.asserts.assert(this.queues_.get(contentType), 'Queue should exist');
