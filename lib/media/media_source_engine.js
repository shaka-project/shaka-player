...
shaka.media.MediaSourceEngine = class {
   ...
   * @private
   */
  workAroundBrokenPlatforms_(segment, reference, contentType, uri) {
    ...
    if (this.config_.insertFakeEncryptionInInit &&
        isInitSegment &&
        encryptionExpected &&
        Platform.requiresEncryptionInfoInAllInitSegments(keySystem) && isMp4) {
      shaka.log.debug('Forcing fake encryption information in init segment.');
      const encryptionScheme = this.playerInterface_.getEncryptionScheme();
      goog.asserts.assert(encryptionScheme, 'Should have an encryption scheme');
      segment = shaka.media.ContentWorkarounds.fakeEncryption(segment,
          encryptionScheme, uri);
    }
    ...
    if (this.config_.insertFakeEncryptionInInit && !isInitSegment &&
        encryptionExpected && Platform.requiresTfhdFix(contentType) &&
        !isEncrypted && isMp4) {
...
/**
 * @typedef {{
 *   getEncryptionScheme: function():?string,
 ...
 * @property {function():?string} getEncryptionScheme
 *   Gets currently used encryption scheme or null if not used.
