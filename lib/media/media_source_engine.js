...
shaka.media.MediaSourceEngine = class {
  ...
  async appendBuffer(
    ...
    if (reference && this.sequenceMode_ && contentType != ContentType.TEXT) {
      // In sequence mode the sourceBuffer places each appended segment
      // immediately after the previously appended one, ignoring its real
      // timestamps.  When playing in reverse we append segments in decreasing
      // time order, so consecutive appends are never contiguous; we must set a
      // fresh timestampOffset for every segment or they would be stacked
      // forward in append order and the timeline would be wrong.
 ...
 *   Gets the current playback rate.  A negative value indicates the content is
