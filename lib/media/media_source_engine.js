...
shaka.media.MediaSourceEngine = class {
   ...
   *
   * HACK a suppression due to missing SourceBuffer.changeType definition.
  ...
  */
  changeType(contentType, newFullMimeType) {
    return this.enqueueOperation_(
        ...
        this.change_.bind(this, contentType, newFullMimeType));
  }
  ...
  reset(contentType) {
    return this.enqueueOperation_(
        contentType,
        // eslint-disable-next-line no-restricted-syntax
        this.reset_.bind(this, contentType, this.video_.currentTime),
    );
  }
};
...
/**
 * Get whether or not the engine supports codec switching.
 *
 * @return {boolean}
 */
shaka.media.MediaSourceEngine.isChangeTypeSupported = () => {
  // eslint-disable-next-line no-restricted-syntax
