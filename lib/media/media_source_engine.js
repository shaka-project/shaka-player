...
shaka.media.MediaSourceEngine = class {
  ...
  /**
   * Detach a SourceBuffer from the MediaSource for trick play.
   *
   * Removes the SourceBuffer from both MediaSource.sourceBuffers and
   * activeSourceBuffers (per MSE spec), so that
   * HTMLMediaElement.buffered no longer includes this type in the
   * intersection.  Metadata (MIME type, encryption info, queue) is
   * preserved so the buffer can be reattached later.
   *
   * @param {shaka.util.ManifestParserUtils.ContentType} contentType
   */
  detachSourceBufferForTrickPlay(contentType) {
    const sb = this.sourceBuffers_.get(contentType);
    if (!sb) {
      return;
    }

    // Remove event listeners first so the queue does not advance when
    // abort() fires 'updateend'.
    this.eventManager_.unlisten(sb, 'error');
    this.eventManager_.unlisten(sb, 'updateend');
    ...
    try {
      ...
      // Ignore — the SourceBuffer may already be unusable.
    }

    // Remove from MediaSource (drops from activeSourceBuffers too).
    try {
      this.mediaSource_.removeSourceBuffer(sb);
    } catch (e) {
      shaka.log.debug('detachSourceBufferForTrickPlay: exception', e);
    }

    // Remove from our tracking.  sourceBufferTypes_ is intentionally kept
    // so reattach knows the correct MIME type.
    this.sourceBuffers_.delete(contentType);

    // Drain pending operations so awaiting code gets unblocked.
    const queue = this.queues_.get(contentType);
    if (queue) {
      for (const item of queue) {
        item.p.reject(new shaka.util.Error(
  ...
  /**
   * Reattach a previously detached SourceBuffer.
   *
   * Creates a new SourceBuffer of the same MIME type that was stored
   * during init and wires up the standard event listeners.  The caller
   * must ensure that the streaming engine re-appends the init segment
   * before media segments.
   *
   * @param {shaka.util.ManifestParserUtils.ContentType} contentType
   */
  reattachSourceBufferForTrickPlay(contentType) {
    if (this.sourceBuffers_.has(contentType)) {
      return;  // Already attached.
    }

    const mimeType = this.sourceBufferTypes_.get(contentType);
    if (!mimeType) {
      return;  // Was never initialised.
    }
    ...
    try {
      ...
      shaka.log.error(
          'reattachSourceBufferForTrickPlay failed', contentType, exception);
      return;
    }
  ...
  onError_(contentType) {
    if (!this.sourceBuffers_.has(contentType)) {
      return;  // SourceBuffer was detached (e.g. trick play).
    }
