...
shaka.media.MediaSourceEngine = class {
   ...
   * @private
   */
  enqueueOperation_(contentType, start, allowSplit = false) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    this.destroyer_.ensureNotDestroyed();

    if (allowSplit && this.needSplitMuxedContent_) {
      const audioOperation = {
        start: start,
        p: new shaka.util.PublicPromise(),
      };
      this.queues_[ContentType.AUDIO].push(audioOperation);

      if (this.queues_[ContentType.AUDIO].length == 1) {
        this.startOperation_(ContentType.AUDIO);
      }
      const videoOperation = {
        start: start,
        p: new shaka.util.PublicPromise(),
      };
      this.queues_[ContentType.VIDEO].push(videoOperation);

      if (this.queues_[ContentType.VIDEO].length == 1) {
        this.startOperation_(ContentType.VIDEO);
      }

      return Promise.all([audioOperation.p, videoOperation.p]);
    } else {
      const operation = {
        start: start,
        p: new shaka.util.PublicPromise(),
