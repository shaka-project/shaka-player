...
shaka.media.MediaSourceEngine = class {
   ...
   */
  static async isStreamSupported(stream) {
    ...
    for (const ref of stream.segmentIndex) {
      ...
      const codecs = ref.codecs || stream.codecs || '';
   ...
   * @private
   */
  async initSourceBuffer_(contentType, stream, codecs, forceTransmux = false) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;

    goog.asserts.assert(
        await shaka.media.MediaSourceEngine.isStreamSupported(stream),
