  ...
  async appendBuffer(contentType, data, reference, hasClosedCaptions, seeked) {
    ...
    if (contentType == ContentType.TEXT) {
      ...
      await this.textEngine_.appendBuffer(
          data, reference.startTime, reference.endTime);
      return;
    }
    ...
    if (this.transmuxers_[contentType]) {
      ...
      if (transmuxedData.metadata) {
        ...
        this.onMetadata_(transmuxedData.metadata, timestampOffset,
      ...
      if (!reference) {
        ...
        if (closedCaptions.length) {
          const videoOffset =
            this.sourceBuffers_[ContentType.VIDEO].timestampOffset;
          this.textEngine_.storeAndAppendClosedCaptions(
              closedCaptions, reference.startTime, reference.endTime,
              videoOffset);
        }
      }
    }
    ...
    if (goog.DEBUG && reference) {
      ...
      const newBuffered = shaka.media.TimeRangesUtils.computeAddedRanges(
