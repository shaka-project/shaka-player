...
shaka.media.MediaSourceEngine = class {
  ...
  static probeSupport() {
    ...
    const testMimeTypes = [
      ...
      // IAMF is only defined for CMAF/MP4 so far.
      'audio/mp4; codecs="iamf.000.000.Opus"',
      'audio/mp4; codecs="iamf.000.000.mp4a.40.2"',
      'audio/mp4; codecs="iamf.000.000.fLaC"',
      'audio/mp4; codecs="iamf.000.000.ipcm"',
      // WebM types
  ...
  /**
   * Computes how far behind the given timestamp is buffered for the given
   * content type.  This is used when playing in reverse, where the buffer is
   * filled behind the playhead instead of ahead of it.
   *
   * @param {shaka.util.ManifestParserUtils.ContentType} contentType
   * @param {number} time
   * @return {number} The amount of time buffered behind in seconds.
   */
  bufferedBehindOf(contentType, time) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    if (contentType == ContentType.TEXT) {
      return this.textEngine_.bufferedBehindOf(time);
    } else {
      const buffered = this.getBuffered_(contentType);
      return shaka.media.TimeRangesUtils.bufferedBehindOf(buffered, time);
    }
  }
  ...
  async appendBuffer(
    ...
    // If the stream belongs to a Preselection, give the application a chance
    // to rewrite the segment so the media engine decodes the selected
    // experience (for example, an AC-4 presentation).
    if (stream.preselection) {
      /** @type {shaka.extern.PreselectionSegmentInfo} */
      const preselectionSegmentInfo = {
        data,
        preselection: stream.preselection,
        stream,
        contentType,
        isInitSegment: reference == null,
      };
      const modifiedInfo = this.config_.modifyPreselectionSegmentCallback(
          preselectionSegmentInfo);
      if (modifiedInfo && modifiedInfo.data) {
        data = modifiedInfo.data;
      }
    }
    ...
    // into the 'ended' state (e.g. before switching to reverse playback).  In
    // that state abort() and the timestampOffset setter throw, so any append
    // that needs them (seeks, adaptations, reverse) must first transition the
    // MediaSource back to 'open'.  Enqueue the reopen ahead of them.
    if (this.ended()) {
      this.enqueueOperation_(
          contentType,
          () => this.reopenMediaSource_(contentType),
          null);
    }
   ...
   * This is a workaround for platforms that do not replace the old content
  ...
  async flush(contentType) {
    if (!shaka.device.DeviceFactory.getDevice().requiresPipelineFlush()) {
      // The pipeline is flushed by clearing the buffer on this platform, and
      // nudging the playhead would only cause a hiccup in playback.
      return;
    }
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    if (contentType == ContentType.TEXT) {
  ...
  /**
   * Transition the MediaSource from the 'ended' state back to 'open' so that
   * further appends (and the abort()/timestampOffset operations they rely on)
   * are allowed again.  appendBuffer() and remove() are the only operations
   * that reopen an ended MediaSource, so we remove an empty range just past the
   * end of the buffered content, which reopens it without discarding anything.
   * @param {shaka.util.ManifestParserUtils.ContentType} contentType
   * @private
   */
  reopenMediaSource_(contentType) {
    const sourceBuffer = this.sourceBuffers_.get(contentType);
    if (!this.ended()) {
      // Already reopened by a previous operation in the queue.
      this.onUpdateEnd_(contentType);
      return;
    }
    const buffered = sourceBuffer.buffered;
    const start = buffered.length ? buffered.end(buffered.length - 1) : 0;
    // This reopens the MediaSource and fires a real 'updateend' event, which
    // resolves the enqueued operation.
    sourceBuffer.remove(start, start + 0.001);
  }

  /**
   * Nudge the playhead to force the media pipeline to be flushed.
   * This is necessary on some platforms to get new content to replace old
   * content.
   * @param {shaka.util.ManifestParserUtils.ContentType} contentType
   * @private
   */
  ...
  setTimestampOffset_(contentType, timestampOffset) {
    // Remember what was asked for, before the workaround below may skew it.
    ...
    // Work around for
    // https://github.com/shaka-project/shaka-player/issues/1281: on affected
    // platforms, rounding the timestampOffset can push the first frame just
    // below appendWindowStart, which makes MSE drop everything up to the next
    // keyframe.  Nudge the offset forward so the first keyframe survives.
    if (timestampOffset < 0 &&
        shaka.device.DeviceFactory.getDevice().requiresTimestampOffsetFudge()) {
  ...
  workAroundBrokenPlatforms_(stream, segment, reference, contentType) {
    ...
    if (this.config_.insertFakeEncryptionInInit && encryptionExpected &&
      ...
      if (isInitSegment) {
        ...
        segment = shaka.media.ContentWorkarounds.fakeEncryption(
            stream, segment, uri, keySystem);
    ...
    // Some I-frame only playlists clip the media segment byte range without
    ...
    if (!isInitSegment && stream.isIframe && this.config_.repairIFrames) {
      segment = shaka.media.ContentWorkarounds.fixBrokenIframes(segment);
    }
