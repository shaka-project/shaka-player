...
shaka.media.MediaSourceEngine = class {
  ...
  constructor(video, textDisplayer, playerInterface, config, lcevcDec) {
    ...
    /**
     * @private {!Map<string, !ArrayBuffer|ArrayBufferView>}
     */
    this.initSegments_ = new Map();
  }
   ...
   * @param {?shaka.media.SegmentReference} reference 
   */
  dispatchBufferAppending(contentType, data, reference) {
    // make a bufferAppending event with init segment data
      ...
      const eventData = (new Map())
          ...
          .set('initSegment', this.initSegments_.get(contentType))
  ...
  /**
   * Update the init segment for audio and video chunks.
   * First delete the previous data to prevent memory leaks.
   * @param {string} contentType 
   * @param {!ArrayBuffer|ArrayBufferView} initSegment 
   */
  updateInitSegment(contentType, initSegment) {
    this.initSegments_.delete(contentType);
    this.initSegments_.set(contentType, initSegment);
  }
  ...
  async appendBuffer(
    ...
    if (this.transmuxers_.has(contentType)) {
      ...
      if (ArrayBuffer.isView(transmuxerOutput)) {
        ...
        if (output.init != null) {
          // store the init segment data for append events
          const initData = output.init;
          this.updateInitSegment(contentType, initData); 
    ...
    await this.enqueueOperation_(contentType, () => {
      ...
      if (!isInitData) this.dispatchBufferAppending(contentType, data, reference);
