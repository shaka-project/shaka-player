...
shaka.media.MediaSourceEngine = class {
  ...
  constructor(video, textDisplayer, playerInterface, config, lcevcDec) {
    ...
    this.textOffset_ = new shaka.util.PublicPromise();
   ...
   * @return {!Promise}
   */
  async init(streamsByType, manifestType=shaka.media.ManifestParser.UNKNOWN,
  ...
  async appendBuffer(
    ...
    if (contentType == ContentType.TEXT) {
      if (this.manifestType_ == shaka.media.ManifestParser.HLS) {
        ...
        const offset = await this.textOffset_;
    ...
    if (reference) {
      ...
      if (timestamp != null) {
        ...
        if (isBestSourceBufferForTimestamps) {
          this.textOffset_.resolve(timestampOffset);
        }
      }
      if (metadata.length) {
   ...
   */
  resync(contentType, timestampOffset) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
