...
shaka.media.MediaSourceEngine = class {
  ...
  /**
   * Reset the sequence mode text offset. Used when
   * discontinuities are encountered.
   *
   * @param {number} offset
   */
  resetTextSequenceModeOffset(offset) {
    this.textSequenceModeOffset_ = new shaka.util.PublicPromise();
    this.textSequenceModeOffset_.resolve(offset);
  }
   ...
   * @return {?number}
   */
  getTimestampAndDispatchMetadata(contentType, data, reference, mimeType,
      timestampOffset) {
    let timestamp = null;
  ...
  async appendBuffer(
    ...
    if (reference) {
      const timestamp = this.getTimestampAndDispatchMetadata(
