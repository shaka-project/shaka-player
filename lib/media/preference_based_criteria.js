...
shaka.media.PreferenceBasedCriteria = class {
  ...
  create(variants) {
    ...
    if (this.config_.languages && this.config_.languages.length) {
      for (const language of this.config_.languages) {
        const byLanguage = Class.filterByLanguage_(variants, language);
