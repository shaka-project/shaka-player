...
shaka.media.PreferenceBasedCriteria = class {
  ...
  create(variants) {
    ...
    const channelCount = this.config_.channelCount;
    const activeAudioChannelCount = this.config_.activeAudioChannelCount;
    let byChannel = [];
    if (channelCount) {
      byChannel = Class.filterVariantsByAudioChannelCount_(
          current, channelCount);
    }

    if (!byChannel.length &&
        activeAudioChannelCount &&
        activeAudioChannelCount != channelCount
    ) {
      byChannel = Class.filterVariantsByAudioChannelCount_(
          current, activeAudioChannelCount);
    }
