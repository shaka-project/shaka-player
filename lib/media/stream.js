...
shaka.media.Stream.prototype.start = function(streamInfo, initialBufferSize) {
  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      /** @param {!Array} results */
      function(results) {
        var index = /** @type {!shaka.media.SegmentIndex} */ (results[0]);
        var initData = /** @type {ArrayBuffer} */ (results[1]);

        var segmentRange = index.getRangeForInterval(
            this.video_.currentTime, initialBufferSize);
        if (!segmentRange) {
          var error = new Error('Cannot start stream: ' +
                                'there are no available SegmentReferences.');
          error.type = 'stream';
          return Promise.reject(error);
        }

        this.segmentIndex_ = index;
        expectedFirstTimestamp = segmentRange.references[0].startTime;

        shaka.log.v1('Fetching segment range', segmentRange);
...
shaka.media.Stream.prototype.switch = function(streamInfo, immediate) {
  ...
  Promise.all(async).then(shaka.util.TypedBind(this,
      ...
      function(results) {
        ...
        if (!segmentRange) {
          var error = new Error('Cannot switch stream: ' +
                                'there are no available SegmentReferences.');
          error.type = 'stream';
          return Promise.reject(error);
        }

        shaka.log.v1('Fetching segment range', segmentRange.references);
...
shaka.media.Stream.prototype.resync = function() {
  ...
  this.sbm_.abort().then(shaka.util.TypedBind(this,
      function() {
        ...
        var index = this.segmentIndex_.find(time);
        if (index < 0) {
        ...
        var reference = this.segmentIndex_.get(index);
...
shaka.media.Stream.prototype.onUpdate_ = function() {
  ...
  if (index < 0 || index >= this.segmentIndex_.length()) {
    this.handleEof_();
...
shaka.media.Stream.prototype.findNextNeededIndex_ = function(time) {
  ...
  var index = this.segmentIndex_.find(time);
  while ((index >= 0) && (index < this.segmentIndex_.length())) {
    var reference = this.segmentIndex_.get(index);
