  ...
  if (!this.proceeded_) {
    ...
    proceed.then(
        function() {
          ...
          shaka.log.debug(
              'Stream', this.streamInfo_.mimeType + ':', 'proceeding...');
  ...
  this.sbm_.abort().then(shaka.util.TypedBind(this,
      function() {
        ...
        if (forceClear ||
          ...
          shaka.log.debug('Clear required.');
          ...
          shaka.log.debug('No clear required.');
...
shaka.media.Stream.prototype.onUpdate_ = function() {
  ...
  if (this.started_ && !this.proceeded_) {
    shaka.log.v1(
        'Stream', this.streamInfo_.mimeType + ':', 'waiting to proceed...');
    this.updateTimer_ = null;
    return;
  }
  ...
  if (bufferedAhead >= bufferingGoal) {
    shaka.log.v1(
        'Stream', this.streamInfo_.mimeType + ':', 'buffering goal reached.');
    this.startIfNeeded_();
  ...
  if (!reference) {
    shaka.log.v1(
        'Stream', this.streamInfo_.mimeType + ':',
        'a new segment is not needed or is not available.');
  ...
  fetch.then(shaka.util.TypedBind(this,
      ...
      function(timestampCorrection) {
        ...
        if (this.needsNudge_ && this.sbm_.bufferedAheadOf(currentTime) > 0) {
          shaka.log.debug('Applying nudge...');
          this.needsNudge_ = false;
...
shaka.media.Stream.prototype.startIfNeeded_ = function() {
  ...
  shaka.log.info('Stream', this.streamInfo_.mimeType, 'has started.');
  this.started_ = true;
