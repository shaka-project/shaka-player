...
shaka.media.Stream.prototype.resync = function() {
  ...
  this.sbm_.abort().then(shaka.util.TypedBind(this,
      function() {
        ...
        // saner and fixes "dead-zone" issues such as #15.  If seeking within
        // the buffered range, we avoid clearing so that we don't re-download
        // content.
        if (!this.sbm_.isBuffered(this.video_.currentTime)) {
