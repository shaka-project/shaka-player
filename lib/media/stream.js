...
/**
 * @event shaka.media.Stream.PleaseBufferEvent
 * @description Fired when we should enter a buffering state.
 * @property {string} type 'pleasebuffer'
 * @property {boolean} bubbles false
 */
...
/**
 * The minimum amount of content we will try to buffer.
 * StreamInfo.minBufferTime, if greater, will override this.
 * This is used to sanitize tiny or missing minBufferTime values.
 *
 * @private {number}
 * @const
 */
shaka.media.Stream.BUFFER_SIZE_SECONDS_ = 3.0;
  ...
  this.sbm_.abort().then(shaka.util.TypedBind(this,
      function() {
        ...
        if (!this.sbm_.isBuffered(time) || !this.sbm_.isInserted(reference)) {
          ...
          // TODO: Fix errors caused by rapid seeking when the application
          // doesn't rate-limit seeks.
...
shaka.media.Stream.prototype.onUpdate_ = function() {
  ...
  fetch.then(shaka.util.TypedBind(this,
      ...
      function(error) {
        ...
        if (error.type == 'aborted') {
          // Ignore.
        } else if (error.type == 'truncated') {
          // Wait and expect to recover.  We got a little ahead of ourselves.
          this.updateTimerId_ =
              window.setTimeout(this.onUpdate_.bind(this), 10000);

          // Ask to go into a buffering state to help us recover.
          var event = shaka.util.FakeEvent.create({
            'type': 'pleasebuffer',
            'bubbles': true
          });
