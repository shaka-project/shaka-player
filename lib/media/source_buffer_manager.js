...
shaka.media.SourceBufferManager.prototype.fetch = function(
  ...
  if (opt_initSegment) {
    this.task_.append(function() {
      var p = this.append_(opt_initSegment);
  ...
  if (references.length) {
    ...
    if (singleLocation) {
      this.appendFetchStages_(references);
    } else {
      for (var i = 0; i < references.length; ++i) {
        this.appendFetchStages_([references[i]]);
      }
    }
  }
...
shaka.media.SourceBufferManager.prototype.appendFetchStages_ =
  ...
  this.task_.append(
      function() {
        ...
        request.acceptableErrors = [404, 410];
...
shaka.media.SourceBufferManager.prototype.append_ = function(data) {
  ...
  if (!data.byteLength) {
    // We got back a 404 or 410, so reject with a special error code.
    var error = new Error('Fetch truncated.');
