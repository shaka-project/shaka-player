...
shaka.media.StreamingEngine = class {
  ...
  */
  ...
  async loadNewTextStream(stream) {
    ...
    const presentationTime = this.playerInterface_.getPresentationTime();
    const needPeriodIndex = this.findPeriodForTime_(presentationTime);
    const state = await this.createMediaState_(
        stream,
        needPeriodIndex,
        /* resumeAt= */ 0);
    this.destroyer_.ensureNotDestroyed();
   ...
   */
  setTrickPlay(on) {
    this.handleAsyncErrors_(() => this.setTrickPlayInternal_(on));
  }

  /**
   * Private async implementation of setTrickPlay.
   * @param {boolean} on
   * @return {!Promise}
   * @private
   */
  async setTrickPlayInternal_(on) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    ...
    if (on) {
      ...
      await this.switchInternal_(trickModeVideo, /* clearBuffer= */ false,
      ...
      await this.switchInternal_(normalVideo, /* clearBuffer= */ true,
          /* safeMargin= */ 0, /* force= */ false);
      this.destroyer_.ensureNotDestroyed();
    }
  }
  ...
  switchVariant(variant, clearBuffer, safeMargin) {
    if (variant.video) {
      // Work around the loss of non-null status in the compiler by capturing
      // a reference here, after it's known to be non-null.
      const video = variant.video;
      this.handleAsyncErrors_(() => this.switchInternal_(
          video, /* clearBuffer= */ clearBuffer,
   ...
   * @private
   */
  async switchInternal_(stream, clearBuffer, safeMargin, force) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    ...
    // If another switch happened while we were waiting for createSegmentIndex,
    // we should not do anything more for this one.
  ...
  async initStreams_(audio, video, text, resumeAt) {
    ...
    const initPromises = Array.from(streamsByType.keys()).map(async (type) => {
   ...
   * @param {number} resumeAt
   * @return {!Promise.<shaka.media.StreamingEngine.MediaState_>}
   * @private
   */
  async createMediaState_(stream, needPeriodIndex, resumeAt) {
    // Make sure the segment index exists.
  ...
  async handlePeriodTransition_(mediaState) {
    ...
    try {
      ...
      const switchPromises = copyOfStateTypes.map(async (type) => {
        const state = this.mediaStates_.get(type);
        const stream = streamsByType.get(type);
        if (stream) {
          ...
          const mediaState = this.mediaStates_.get(type);
          // If another switch happened while we were waiting for the one
          // above, we should not do anything more for this one.
          if (mediaState.stream != stream &&
              mediaState.stream != stream.trickModeVideo) {
            return;
          }
  ...
  /**
   * Execute the given function and handle any async errors thrown by
   * dispatching them to the player.
   *
   * This allows some async methods to be executed from synchronous front-ends,
   * so long as the results of those methods are not needed and the
   * side-effects don't need to be waited on.
   *
   * @param {function():!Promise} fn
   * @return {!Promise}
   * @private
   */
  async handleAsyncErrors_(fn) {
    try {
      await fn();
    } catch (error) {
      if (this.playerInterface_) {
