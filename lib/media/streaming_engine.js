...
shaka.media.StreamingEngine = class {
  ...
  /**
   * @return {!Array<shaka.util.ManifestParserUtils.ContentType>} The content
   *   types currently being streamed.
   */
  getContentTypes() {
    return Array.from(this.mediaStates_.keys());
  }
  ...
  advanceRefPastSkipRanges_(mediaState, ref) {
    ...
    const region = this.skipRangeController_.getRangeAt(ref.startTime);
    if (!region) {
      return ref;
    }
  ...
  createSegmentPrefetch_(stream) {
    ...
    if (this.config_.segmentPrefetchLimit > 0) {
      ...
      return new shaka.media.SegmentPrefetch(
          ...
          (reference) => !this.skipRangeController_.getRangeAt(
              reference.startTime));
    }
    return null;
  }
  ...
  getTimeNeeded_(mediaState, presentationTime) {
    ...
    if (!mediaState.lastSegmentReference) {
      return this.skipRangeController_.getTimePast(presentationTime);
    }
    ...
    return this.skipRangeController_.getTimePast(
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference, adaptation) {
    ...
    try {
      ...
      if (lowLatencyMode && isReadableStreamSupported && isMP4 &&
        ...
        // range.
        if (this.skipRangeController_.getRangeAt(reference.startTime)) {
          shaka.log.info(
