...
shaka.media.StreamingEngine = class {
  ...
  /**
   * Reset the cached init-segment reference for a content type so the
   * next update cycle re-fetches and re-appends it.  Call this after
   * the MediaSourceEngine's SourceBuffer for that type has been
   * detached and reattached (the new SourceBuffer has no init data).
   *
   * @param {shaka.util.ManifestParserUtils.ContentType} contentType
   */
  resetInitSegmentState(contentType) {
    const mediaState = this.mediaStates_.get(contentType);
    if (mediaState) {
   ...
   * @param {boolean} on
   */
  setTrickPlay(on, clearBuffer = false) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    ...
    if (on) {
      ...
      this.switchInternal_(trickModeVideo, /* clearBuffer= */ clearBuffer,
  ...
  seeked() {
    ...
    const isSeekTrickPlay =
        this.playerInterface_.isSeekBasedTrickPlayActive();
    for (const type of this.mediaStates_.keys()) {
      const mediaState = this.mediaStates_.get(type);
      const logPrefix = shaka.media.StreamingEngine.logPrefix_(mediaState);

      // During seek-based trick play, only VIDEO needs to be
      // re-positioned.  Audio and text are effectively suspended (the
      // update_() early-return prevents new fetches), so touching their
      // state would abort in-flight operations, waste already-downloaded
      // bytes, and create unnecessary reschedule churn.  This mirrors
      // ExoPlayer where disabled renderers don't participate in seeks.
      //
      // Exception: when downloadAudioDuringTrickPlay is enabled, audio
      // must be repositioned alongside video so that the audio buffer
      // covers the seek target — required by platforms (e.g. WebOS) where
      // HTMLMediaElement.buffered is the intersection of all active SBs.
      if (isSeekTrickPlay && type !== ContentType.VIDEO) {
        if (type === ContentType.AUDIO &&
            this.playerInterface_.shouldDownloadAudioDuringTrickPlay()) {
          // Audio should be repositioned — fall through.
        } else {
          continue;
        }
      }

      const positionBuffered = newTimeIsBuffered(type);

      // During seek-based trick play we always need to reset the segment
      // iterator so that the fetch direction follows the seek direction,
      // but we only clear the buffer when the position is NOT already
      // buffered.  This allows rewind through previously-fetched I-frame
      // content to render instantly from the existing buffer.
      const needsReset = !this.crossBoundarySeek_ &&
          (isSeekTrickPlay || !positionBuffered);
      ...
      if (needsReset) {
        ...
        // During seek-based trick play, ALWAYS clear lastSegmentReference
        // so that getTimeNeeded_() returns the current presentation time
        // instead of a stale forward endTime.  This is essential for
        // rewind: without it, the SE creates the reverse iterator from
        // the old forward position and never fetches backward segments.
        if (isSeekTrickPlay) {
          mediaState.lastSegmentReference = null;
        }

        // Only clear the buffer if the new position is NOT buffered and
        // this is NOT seek-based trick play.  During trick play,
        // previously-fetched I-frame segments must remain so the
        // controller can render them instantly from the buffer frontier.
        if (!positionBuffered && !isSeekTrickPlay) {
          const bufferEnd =
              this.playerInterface_.mediaSourceEngine.bufferEnd(type);
          const somethingBuffered = bufferEnd != null;

          // Don't clear the buffer unless something is buffered.  This
          // extra check prevents extra, useless calls to clear the
          // buffer.
        ...
        // During seek-based trick play only VIDEO reaches this point
        // (non-video types are skipped above).  Aborting is still
        // necessary: the in-flight fetch has a stale iterator and its
        // completion would overwrite lastSegmentReference with the old
        // position, causing getSegmentReferenceNeeded_() to create an
        // iterator pointing to the wrong location.  ExoPlayer also
        // flushes the video pipeline on each trick-play seek
        // (flushForSingleIFrameSeek); this is the Shaka equivalent.
        // Starvation is prevented by the controller's render-pending
        // gate + SE_RESYNC_GAP_SEC_ (10 s) + debounce (5 s) which
        // limit how often resyncs abort fetches.
        if (mediaState.operation) {
          mediaState.operation.abort();
  ...
  update_(mediaState) {
    ...
    // During seek-based trick play, skip fetching for non-video types.
    // Only I-frame video segments are needed; audio and text downloads
    // would waste bandwidth that should go to I-frames.  We return a
    // poll interval so the update loop stays alive and resumes fetching
    // automatically when trick play ends.  This is safe because it
    // never touches timers, operations, or MSE state — the current
    // in-flight fetch (if any) completes naturally, and no new fetches
    // are started.
    //
    // Exception: when downloadAudioDuringTrickPlay is enabled, audio
    // fetching continues so that the audio SourceBuffer stays in sync
    // with video — required by platforms where HTMLMediaElement.buffered
    // is the intersection of all active SourceBuffer ranges.
    if (mediaState.type !== ContentType.VIDEO &&
        this.playerInterface_.isSeekBasedTrickPlayActive()) {
      if (mediaState.type === ContentType.AUDIO &&
          this.playerInterface_.shouldDownloadAudioDuringTrickPlay()) {
        // Audio should be fetched — fall through to normal update logic.
      } else {
    ...
    // During seek-based trick play the controller renders from whatever is
    // in the buffer, so the SE must keep fetching I-frame segments
    // continuously.  Without this bypass, backward trick play stalls because
    // bufferedAheadOf() measures FORWARD from video.currentTime — which is
    // moving backward — so it sees a large value and stops fetching.
    //
    // Direction-aware tuning (VTP-inspired): for reverse and scrub modes
    // each SE resync flushes the download, so there is no value in
    // buffering far ahead.  Use a reduced goal of 2 seconds.
    const Direction = shaka.media.SeekBasedTrickPlayController.Direction;
    const trickDir = this.playerInterface_.getTrickPlayDirection();
    const isReverseTrickPlay = trickDir === Direction.REVERSE ||
        trickDir === Direction.SCRUB;
    const trickPlayGoal = isReverseTrickPlay ? 2 :
        (this.playerInterface_.isSeekBasedTrickPlayActive() ? 2 :
        scaledBufferingGoal);
    if (bufferedAhead >= trickPlayGoal &&
        !this.playerInterface_.isSeekBasedTrickPlayActive()) {
    ...
    if (timeNeeded >= minTimeNeeded + maxRunAhead &&
      ...
      // During seek-based trick play this check is skipped: only I-frame
      // video is needed and audio should not throttle video fetching.
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    if (mediaState.segmentIterator) {
      ...
      // During seek-based trick play, when lastSegmentReference is null
      // (cleared by seeked()), we must NOT fall back to bufferEnd — that
      // value comes from the old buffer and points to the wrong position
      // (e.g. a forward position when we're rewinding).  Fall through to
      // branch 3 which uses presentationTime instead.
  ...
  async initSourceBuffer_(mediaState, reference, adaptation) {
    ...
    let timestampOffset = reference.timestampOffset;

    // During seek-based trick play, the timestamp offset is auto-corrected
    // in append_() because I-frame segments may carry internal decode
    // timestamps far from the manifest-declared presentation time.  If we
    // have already applied a correction (lastTimestampOffset differs from the
    // manifest value), preserve it instead of resetting back to the
    // manifest's timestampOffset — otherwise every subsequent segment would
    // undo the correction and data would land at the wrong buffer position.
    if (this.playerInterface_.isSeekBasedTrickPlayActive() &&
  ...
  async append_(mediaState, presentationTime, stream, reference, segment,
    ...
    // Auto-correct timestamp offset for I-frame trick play segments.
    // I-frame tracks (especially in cloud DVR / restart / time-shift
    // scenarios) may carry internal timestamps (baseMediaDecodeTime) far
    // from the segment reference's declared presentation time.  Without
    // correction the data lands at the wrong buffer position, so the
    // trick-play controller cannot seek through it at the expected times.
    //
    // IMPORTANT: The baseMediaDecodeTime MUST be extracted synchronously
    // from Mp4Parser.parse(), and the setProperties_ call MUST be awaited
    // OUTSIDE the parse callback.  Using an async callback inside parse()
    // creates a race condition: parse() is synchronous and doesn't await
    // the callback's returned Promise, so setProperties_() would complete
    // AFTER appendBuffer() — the offset would never take effect.
    if (this.playerInterface_.isSeekBasedTrickPlayActive()) {
      const isTrickPlayMp4 = stream.mimeType == 'video/mp4' ||
              stream.mimeType == 'audio/mp4';
      let trickTimescale = null;
      if (reference.initSegmentReference) {
        trickTimescale = reference.initSegmentReference.timescale;
      }
      if (isTrickPlayMp4 && trickTimescale) {
        // Step 1: Extract baseMediaDecodeTime synchronously.
        let trickBaseDecodeTime = null;
        new shaka.util.Mp4Parser()
            .box('moof', shaka.util.Mp4Parser.children)
            .box('traf', shaka.util.Mp4Parser.children)
            .fullBox('tfdt', (box) => {
              ...
              const parsedTFDT =
                  shaka.util.Mp4BoxParsers.parseTFDTInaccurate(
                      box.reader, box.version);

              trickBaseDecodeTime = parsedTFDT.baseMediaDecodeTime;
            })
            .parse(segment, /* partialOkay= */ false, isChunkedData);

        // Step 2: Compute and apply the correct offset (properly awaited).
        if (trickBaseDecodeTime) {
          goog.asserts.assert(typeof(trickTimescale) == 'number',
              'Should be a number!');

          const decodeTimeSec =
          /** @type {number} */ (trickBaseDecodeTime) / trickTimescale;
          const correctOffset = reference.startTime - decodeTimeSec;
          const currentOffset =
              Number(mediaState.lastTimestampOffset) || 0;

          // Only update when the mismatch is significant (> 0.5 s).
          if (Math.abs(correctOffset - currentOffset) > 0.5) {
            const lastAppendWindowStart = mediaState.lastAppendWindowStart;
            const lastAppendWindowEnd = mediaState.lastAppendWindowEnd;
            goog.asserts.assert(typeof(lastAppendWindowStart) == 'number',
                'Should be a number!');
            goog.asserts.assert(typeof(lastAppendWindowEnd) == 'number',
                'Should be a number!');
            await this.setProperties_(mediaState, correctOffset,
                lastAppendWindowStart, lastAppendWindowEnd, reference);
          }
        }
      }
    ...
    // During seek-based trick play, skip eviction.  The controller sets
    // video.currentTime far ahead/behind the buffer (it advances at the
    // requested rate), which makes the eviction heuristic think there is
    // too much "behind" content and it removes freshly-downloaded I-frame
    // data that we need for rendering.
    if (!this.playerInterface_.isSeekBasedTrickPlayActive()) {
      await this.evict_(mediaState, presentationTime);
      this.destroyer_.ensureNotDestroyed();
    }
 ...
 *   isSeekBasedTrickPlayActive: function():boolean,
 *   getTrickPlayDirection: function():string,
 *   shouldDownloadAudioDuringTrickPlay: function():boolean,
 * }}
 *
 ...
 * @property {function():boolean} isSeekBasedTrickPlayActive
 *   Returns true when seek-based trick play is active.  Used by the update
 *   loop to continue fetching segments even though the media element is paused.
 * @property {function():string} getTrickPlayDirection
 *   Returns the current trick-play direction as a string ('forward',
 *   'reverse', 'scrub', or 'none').  Used for direction-aware buffer tuning.
 * @property {function():boolean} shouldDownloadAudioDuringTrickPlay
 *   Returns true when audio segments should continue to be downloaded during
 *   seek-based trick play.  When true, the audio SourceBuffer stays attached
 *   and the update loop fetches audio alongside video I-frames.  This is
 *   needed on platforms (e.g. WebOS) where the browser requires audio data
 *   at the seek target to complete seeks and render I-frames.
 */
