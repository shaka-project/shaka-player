...
shaka.media.StreamingEngine = class {
  ...
  parsePRFT_(reference, box) {
    ...
    box.reader.readUint32(); // Ignore ntpTimestampFrac
    let mediaTime;
    if (version === 0) {
    ...
    const wca = this.convertNtp(ntpTimestampSec);
    ...
    prtData.wca = wca;
    prtData.pta = mediaTime / prtData.timescale * 1000;
  ...
  convertNtp(ntpTimeStamp) {
    ...
    return new Date(start.getTime() + ntpTimeStamp * 1000).getTime();
  }
