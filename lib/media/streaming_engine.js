...
shaka.media.StreamingEngine = class {
  ...
  update_(mediaState) {
    ...
    if (mediaState.lastInitSegmentReference &&
  ...
  resetMediaSourceForNextStream() {
    ...
    goog.asserts.assert(typeof(mediaState.lastAppendWindowEnd) == 'number',
        'Should be an number!');
    const windowEndOffset = mediaState.lastAppendWindowEnd -
      this.playerInterface_.getPresentationTime();
    if (windowEndOffset > shaka.media.StreamingEngine.MAX_END_STREAM_OFFSET) {
      return;
    }
    this.playerInterface_.seekTo(mediaState.lastAppendWindowEnd);
    this.resetMediaSource(/* force= */ true);
  }
...
shaka.media.StreamingEngine.MAX_END_STREAM_OFFSET = 0.1;
