...
shaka.media.StreamingEngine = class {
  ...
  resolveVideoAlignment_(videoIndex, region) {
    const startSeg = videoIndex.getIteratorForTime(region.start);
    const endSeg = videoIndex.getIteratorForTime(region.end);
    this.skipRangeController_.resolveAlignment(
        region,
        startSeg && startSeg.next().value,
        endSeg && endSeg.next().value);
  }
