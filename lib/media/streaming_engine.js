...
shaka.media.StreamingEngine = class {
   ...
   *   reference grid for skip-range alignment), or null if unavailable.
   * @private
   */
  getVideoSegmentIndex_() {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    const videoState = this.mediaStates_.get(ContentType.VIDEO);
    return (videoState && videoState.stream.segmentIndex) || null;
  }

  /**
   * Hands the video segments covering |region|'s endpoints to the controller,
   * which decides and records whether the range aligns to segment boundaries.
   * Runs once per range.
   *
   * @param {!shaka.media.SegmentIndex} videoIndex
   * @param {!shaka.media.SkipRangeController.SkipRange} region
   * @private
   */
  resolveVideoAlignment_(videoIndex, region) {
    this.skipRangeController_.resolveAlignment(
        region,
        this.segmentsNearTime_(videoIndex, region.start),
        this.segmentsNearTime_(videoIndex, region.end));
  }

  /**
   * The video segments that can carry a boundary matching |time| within the
   * skip tolerance: the one containing |time - tolerance| and the one after it.
   * Segment lookup is strict containment, so a single probe only exposes one
   * segment's boundaries; taking both sides of the window lets an endpoint that
   * misses a boundary by float jitter still find it, from either side.
   *
   * @param {!shaka.media.SegmentIndex} videoIndex
   * @param {number} time
   ...
   * @private
   */
  segmentsNearTime_(videoIndex, time) {
    const tolerance = shaka.media.SkipRangeController.TOLERANCE_SECONDS;
    const it = videoIndex.getIteratorForTime(Math.max(0, time - tolerance));
    if (!it) {
      return [];
    }
    const refs = [];
  ...
  /**
   * If |ref| starts inside a carveable skip range, re-seeds the media state's
   ...
   *
   * A range is carveable only after the video grid confirms it aligns to segment
   * boundaries on both ends; a misaligned range is left untouched on every
   * stream (with a one-time warning from the controller).  The video grid
   * resolves that verdict the first time any stream's frontier reaches the
  ...
  advanceRefPastSkipRanges_(mediaState, ref) {
    ...
    // Resolve alignment before any stream decides to skip, so the verdict exists
    // up front (no misaligned partial carve while unresolved).  Prefer the video
    // grid; fall back to this stream's index for audio-only content.
    if (this.playerInterface_.getPlaybackRate() >= 0) {
      ...
      if (rawRegion && !this.skipRangeController_.isResolved(rawRegion)) {
        const refIndex =
            this.getVideoSegmentIndex_() || mediaState.stream.segmentIndex;
        if (refIndex) {
          this.resolveVideoAlignment_(refIndex, rawRegion);
        }
      }
    }
    ...
    if (!region) {
      // No carveable range here (none, unresolved, or unalignable); stream as-is.
      return ref;
    }
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference, adaptation) {
    ...
    try {
      ...
      if (lowLatencyMode && isReadableStreamSupported && isMP4 &&
        ...
        // A carveable skip range may cover this segment (declared before the
