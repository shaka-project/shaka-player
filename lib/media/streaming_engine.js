...
shaka.media.StreamingEngine = class {
  ...
  forwardTimeForCrossBoundary(immediate = false) {
    ...
    const nearEndOffset = mediaState.lastAppendWindowEnd - presentationTime;
    const NEAR_END_THRESHOLD = 1;

    if (nearEndOffset > 0 && nearEndOffset < NEAR_END_THRESHOLD) {
      if (immediate) {
        this.crossBoundaryTimer_.tickNow();
      } else {
        this.crossBoundaryTimer_.tickAfter(nearEndOffset);
      }
    }
  }
