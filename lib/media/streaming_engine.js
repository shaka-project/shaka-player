  ...
  */
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    if (mediaState.stream == mediaState.lastStream) {
      ...
      // appended.
      return mediaState.stream.segmentIndex.current();
      ...
      const ref = mediaState.stream.segmentIndex.seek(
      ...
      shaka.log.v1(logPrefix, 'looking up segment',
          'bufferEnd:', bufferEnd);
      const ref = mediaState.stream.segmentIndex.seek(bufferEnd);
      ...
      shaka.log.v1(logPrefix, 'looking up segment',
          ...
          'presentationTime:', presentationTime);
      // Fall back to exact presentation time.
      // TODO(#1339): Remove fall back to exact time after period flattening.
      // This is only needed now because across periods, we have different
      // segment indexes.
      const ref = mediaState.stream.segmentIndex.seek(lookupTime) ||
          mediaState.stream.segmentIndex.seek(presentationTime);
  ...
  async append_(mediaState, presentationTime, stream, reference,
    ...
    const ref = stream.segmentIndex.next();
