...
shaka.media.StreamingEngine.prototype.onUpdate_ = function(mediaState) {
  ...
  var mediaStates = Object.keys(this.mediaStates_)
...
shaka.media.StreamingEngine.prototype.getSegmentReference_ = function(
  ...
  if (mediaState.buffer.length == 0) {
    ...
    if (currentPeriodIndex == lastReceipt.periodIndex) {
      ...
      shaka.log.v1(logPrefix, 'using next position:', 'position=' + position);
    } else {
  ...
  // Ensure the next segment always follows the last one buffered. This
  ...
  if (mediaState.buffer.length > 0) {
    ...
    var gap = nextStartTime - lastReceipt.endTime;

    // Small overlaps are okay.  We will tolerate an overlap of 1ms.
    if (gap < -0.001 ||
    ...
    // TODO: Remove byte limit and receipts, as these are the source of the
    // majority of our overly-strict floating point comparisons, as well as
