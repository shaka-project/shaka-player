...
shaka.media.StreamingEngine.prototype.getStream_ = function(type) {
  goog.asserts.assert(this.mediaStates_, 'Must be initialized');
  var state = this.mediaStates_[type];
...
/**
 * Unload text stream when the user chooses to hide the caption.
 */
...
shaka.media.StreamingEngine.prototype.append_ = function(
  ...
  return this.evict_(mediaState, playheadTime).then(function() {
    ...
    var presentationTimeOffset = null;
    if (mediaState != null && mediaState.stream != null &&
        mediaState.stream.presentationTimeOffset != null) {
      presentationTimeOffset = mediaState.stream.presentationTimeOffset;
    }

    return this.playerInterface_.mediaSourceEngine.appendBuffer(
        mediaState.type, segment, reference.startTime + period.startTime,
        reference.endTime + period.startTime, presentationTimeOffset);
  }.bind(this)).then(function() {
    if (this.destroyed_) return;
