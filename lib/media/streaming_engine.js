...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface, skipRangeController) {
    ...
    this.updateLiveSeekableRangeTimer_ = new shaka.util.Timer(() => {
      ...
      // After HLS ends, the timeline duration can be finite while MSE's is
      // still infinite. Keep the final range to allow seeks beyond the buffer.
      const retainFinalHlsRange = this.manifest_.type == 'HLS' &&
          timeline.getDuration() < Infinity &&
          mediaSourceEngine.getDuration() == Infinity;
      if (!timeline.isDynamic() && !retainFinalHlsRange) {
    ...
    /** @private {?number} */
    this.crossBoundarySeekTarget_ = null;

    /** @private {boolean} */
    this.crossBoundaryResetPending_ = false;
    ...
    this.crossBoundaryTimer_ = new shaka.util.Timer(() => {
      ...
      if (this.boundaryTime_) {
        ...
        this.crossBoundarySeekTarget_ = this.boundaryTime_;
        this.crossBoundaryResetPending_ = true;
  ...
  seeked() {
    ...
    const crossBoundarySeek =
        ...
        shaka.util.NumberUtils.isFloatEqual(
            presentationTime,
            this.crossBoundarySeekTarget_,
            shaka.media.StreamingEngine.CROSS_BOUNDARY_SEEK_TOLERANCE_);
    this.crossBoundarySeekTarget_ = null;

    if (!crossBoundarySeek) {
      // A user seek superseded or followed the internal boundary seek.
      // mediaState.seeked will carry any required reset through the normal
      // path.
      this.crossBoundaryResetPending_ = false;
    }
    ...
    try {
      ...
      if ((error.code == shaka.util.Error.Code.OPERATION_ABORTED ||
        ...
        if (reference.initSegmentReference) {
          mediaState.segmentPrefetch.removeReference(
  ...
  async clearBuffer_(mediaState, flush, safeMargin) {
    ...
    if (mediaState.segmentPrefetch &&
      ...
      // again unnecessarily.
      mediaState.segmentPrefetch.clearAll(/* clearInitSegments= */ flush);
    }
    ...
    if (safeMargin) {
      ...
      if (flush) {
        ...
        if (device.requiresPlayheadNudgeAfterBufferClear()) {
  ...
  discardReferenceByBoundary_(mediaState, reference) {
    ...
    const crossBoundaryReset =
        leadingMediaState && crossesBoundary &&
        this.crossBoundaryResetPending_;

    if (crossBoundaryReset) {
      // The next boundary reference completes the internal seek operation,
      // whether KEEP accepts the reference or a reset is required.
      this.crossBoundaryResetPending_ = false;
    }
