...
goog.require('shaka.config.PeriodSwitchingStrategy');
...
shaka.media.StreamingEngine = class {
  ...
  update_(mediaState) {
    ...
    const checkMediaSourceReset = this.config_.periodSwitchingStrategy !==
      PeriodSwitchingStrategy.NEVER;
    if (checkMediaSourceReset &&
        mediaState.lastInitSegmentReference &&
        // If they're not equal, we're going to check if an MSE reset is needed.
      ...
      if (reset && this.config_.periodSwitchingStrategy ===
        PeriodSwitchingStrategy.PLAIN_TO_ENCRYPTED
      ) {
        const {matchedStreams} = mediaState.stream;

        const matchedStream = matchedStreams.find((stream) => {
          const {references} = stream.segmentIndex;
        ...
        if (matchedStream && matchedStream.encrypted) {
          shaka.log.debug(logPrefix, 'next stream is encrypted, ' +
            'discard periodSwitchingStrategy', matchedStream);
          this.config_.periodSwitchingStrategy =
            shaka.config.PeriodSwitchingStrategy.NEVER;
        }
      }
      return null;
    }
  ...
  /**
   * Checks if we need to advance time into the next period, which will
   * trigger an MSE reset.
   */
  checkTimeAdvanceInNextStream() {
    const {video} = this.playerInterface_;
    ...
    if (!mediaState || mediaState.clearingBuffer || mediaState.seeked ||
      !mediaState.lastSegmentReference || !mediaState.lastAppendWindowEnd) {
      return;
    }
    const presentationTime = this.playerInterface_.getPresentationTime();
    // This is an arbitrary number but we can assume that when one waits due to
    // temporary lack of video data roughly 2 seconds from the stream end, we're
    // somewhat at the end.
    const NEAR_WINDOW_END_THRESHOLD_ = 2;
    if (mediaState.lastAppendWindowEnd - presentationTime <
      NEAR_WINDOW_END_THRESHOLD_) {
      video.currentTime = mediaState.lastAppendWindowEnd;
    }
  }
