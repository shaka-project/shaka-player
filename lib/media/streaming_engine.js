...
shaka.media.StreamingEngine = class {
   ...
   * @param {!shaka.media.SkipRangeController} skipRangeController
   */
  constructor(manifest, playerInterface, skipRangeController) {
    /** @private {?shaka.media.StreamingEngine.PlayerInterface} */
    this.playerInterface_ = playerInterface;

    /** @private {!shaka.media.SkipRangeController} */
    this.skipRangeController_ = skipRangeController;
    ...
    /** @private {boolean} */
    this.boundaryResetting_ = false;

    /** @private {?number} */
    this.mediaSourceResetTime_ = null;

    /** @private {boolean} */
    this.mediaSourceRestorePending_ = false;
  ...
  /**
   * Schedules an immediate update for every idle stream, so streaming
   * re-evaluates fetching after a skip range changes.
   */
  requestSkipRangeUpdate() {
  ...
  /**
   * Whether any part of [start, end) is buffered for any streaming type.  Backs
   * SkipRangeController's PlayerInterface (segment indices live here).  Each
   * in-range segment is tested at its midpoint, so a lead-in straddling the
   * range start (midpoint before it) is excluded without a boundary tolerance.
   *
   * @param {number} start
   * @param {number} end
   * @return {boolean}
   */
  isSkipRegionBuffered(start, end) {
    ...
    for (const mediaState of this.mediaStates_.values()) {
      ...
      const it = segmentIndex.getIteratorForTime(start);
      if (!it) {
        continue;
      }
      let ref = it.next().value;
      while (ref && ref.startTime < end) {
        const mid = (ref.startTime + ref.endTime) / 2;
        if (mid >= start && mid < end &&
            mediaSourceEngine.isBuffered(mediaState.type, mid)) {
          return true;
        }
        ref = it.next().value;
      }
  ...
  /**
   * @return {?shaka.media.SegmentIndex} The video stream's segment index (the
   *   reference grid for skip-range alignment), falling back to audio for
   *   audio-only content but never text, or null if unavailable.
   * @private
   */
  getAlignmentSegmentIndex_() {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    const state = this.mediaStates_.get(ContentType.VIDEO) ||
        this.mediaStates_.get(ContentType.AUDIO);
    return state?.stream.segmentIndex ?? null;
  }

  /**
   * Hands the reference-grid segments covering |region|'s endpoints to the
   * controller, which decides and records whether the range aligns to segment
   * boundaries.  Runs once per range.
   *
   * @param {!shaka.media.SegmentIndex} segmentIndex
   * @param {!shaka.media.SkipRangeController.SkipRange} region
   * @private
   */
  resolveAlignment_(segmentIndex, region) {
    this.skipRangeController_.resolveAlignment(
        region,
        this.segmentsNearTime_(segmentIndex, region.start),
        this.segmentsNearTime_(segmentIndex, region.end));
  }

  /**
   * The reference-grid segments that can carry a boundary matching |time|
   * within the skip tolerance: the one containing |time - tolerance| and the
   * one after it.  Segment lookup is strict containment, so a single probe only
   * exposes one segment's boundaries; taking both sides of the window lets an
   * endpoint that misses a boundary by float jitter still find it, from either
   * side.
   *
   * @param {!shaka.media.SegmentIndex} segmentIndex
   * @param {number} time
   ...
   * @private
   */
  segmentsNearTime_(segmentIndex, time) {
    const tolerance = shaka.media.SkipRangeController.TOLERANCE_SECONDS;
    const it = segmentIndex.getIteratorForTime(Math.max(0, time - tolerance));
    if (!it) {
      return [];
    }
    const refs = [];
    for (let i = 0; i < 2; i++) {
      const ref = it.next().value;
      if (!ref) {
        break;
      }
      refs.push(ref);
  ...
  /**
   * If |ref| starts inside a skippable skip range, re-seeds the media state's
   * iterator past the range and returns the first segment at or after its end,
   * forming the buffer hole; otherwise returns |ref| unchanged.
   *
   * A range is skippable only after the reference grid confirms it aligns to
   * segment boundaries on both ends; a misaligned range is left untouched on
   * every stream (with a one-time warning from the controller).  The reference
   * grid resolves that verdict the first time any stream's frontier reaches the
   * range; all streams then follow the stored verdict.
   *
   ...
   * @private
   */
  advanceRefPastSkipRanges_(mediaState, ref) {
    // Zero cost on the common path (no skip ranges).
    if (!ref || this.skipRangeController_.isEmpty()) {
      return ref;
    }
    // Resolve alignment before any stream decides to skip, so the verdict
    // exists up front (no misaligned partial carve while unresolved).  Always
    // judged against the same reference grid (video, or audio for audio-only
    // content), whichever stream's frontier gets here first.
    if (this.playerInterface_.getPlaybackRate() >= 0) {
      const rawRegion = this.skipRangeController_.getRangeAt(ref.startTime);
      if (rawRegion && !this.skipRangeController_.isResolved(rawRegion)) {
        const refIndex = this.getAlignmentSegmentIndex_();
        if (refIndex) {
          this.resolveAlignment_(refIndex, rawRegion);
        }
      }
    }

    const region = this.skipRangeController_.containingActiveRange(
        ref.startTime, ref.endTime);
    if (!region) {
      // No skippable range here (none, unresolved, or unalignable); stream it
      // as-is.
      return ref;
    }
    // region.end is a real segment start; re-seed past it and return the first
    // kept segment.
    const it = mediaState.stream.segmentIndex.getIteratorForTime(region.end);
    if (!it) {
      // Stream ends inside the range: exhaust the iterator so update_() treats
      // it as finished instead of holding the other streams back.
      while (mediaState.segmentIterator &&
          mediaState.segmentIterator.next().value) {
        // Exhaust.
      }
      return null;
    }
    mediaState.segmentIterator = it;
    ref = it.next().value;
    // Skip a segment straddling the range end (its front is inside the range);
    // a segment starting at the range end is the first kept segment.
    const NumberUtils = shaka.util.NumberUtils;
    const tolerance = shaka.media.SkipRangeController.TOLERANCE_SECONDS;
    while (ref && ref.startTime < region.end &&
        !NumberUtils.isFloatEqual(ref.startTime, region.end, tolerance)) {
      ref = it.next().value;
    }
    return ref;
  }
  ...
  seeked() {
    ...
    if (this.mediaSourceResetTime_ != null &&
        (!video.readyState ||
        shaka.util.NumberUtils.isFloatEqual(video.currentTime,
            this.mediaSourceResetTime_,
            shaka.media.StreamingEngine.CROSS_BOUNDARY_SEEK_TOLERANCE_))) {
      // Neither a transient reset event nor restoration is a user seek.
      // Completion is observed separately, after MSE reports assignment.
      return;
    }

    const presentationTime = this.playerInterface_.getPresentationTime();

    const crossBoundarySeek =
        this.boundaryTime_ != null &&
        shaka.util.NumberUtils.isFloatEqual(
            this.playerInterface_.video.currentTime,
            this.boundaryTime_,
            shaka.media.StreamingEngine.CROSS_BOUNDARY_SEEK_TOLERANCE_);

    if (!crossBoundarySeek) {
      // A user seek supersedes the boundary operation. Matching the target
      // alone is never sufficient to complete a reset.
      this.boundaryTime_ = null;
      this.boundaryResetting_ = false;
      this.crossBoundaryTimer_.stop();
    }
    ...
    for (const type of this.mediaStates_.keys()) {
      ...
      if (!crossBoundarySeek && !newTimeIsBuffered(type)) {
      ...
      if (crossBoundarySeek && !mediaState.clearingBuffer &&
  ...
  /**
   * Called before MSE replaces the media source.
   * @param {number} time
   */
  ...
  /** Called after MSE assigns the saved currentTime on the new source. */
  notifyOfMediaSourceResetEnd() {
    this.mediaSourceRestorePending_ = true;
  }
  ...
  createSegmentPrefetch_(stream) {
    ...
    if (this.config_.segmentPrefetchLimit > 0) {
      ...
      return new shaka.media.SegmentPrefetch(
          ...
          // Don't prefetch a reference a skip range may drop.  Conservative
          // while a range's alignment is unresolved (it may still be carved),
          // so we don't prefetch in-range segments the frontier will discard.
          (reference) =>
            !this.skipRangeController_.mayBeSkipped(
                reference.startTime, reference.endTime));
    }
    return null;
  }
  ...
  update_(mediaState) {
    ...
    // When the playback rate is negative we are moving backwards, so the buffer
    // is filled behind the playhead instead of ahead of it.  Several of the
    // checks below need to account for this reversed direction.
    const reverse = this.playerInterface_.getPlaybackRate() < 0;
    ...
    if (!reverse && timeUntilEnd < oneMicrosecond && !!bufferEnd) {
    ...
    if (reverse) {
      // When playing in reverse the buffer is filled behind the playhead, so
      // the boundary we can reach is the start of the presentation, not the
      // end.  Once we've buffered back to it there is nothing more to fetch.
      const seekRangeStart =
          this.manifest_.presentationTimeline.getSeekRangeStart();
      if (bufferEnd != null && timeNeeded <= seekRangeStart + oneMicrosecond) {
        shaka.log.debug(logPrefix, 'buffered to start of presentation');
        return null;
      }
    }
    ...
    const runAhead = reverse ?
        maxTimeNeeded - timeNeeded :
        timeNeeded - minTimeNeeded;
    if (runAhead >= maxRunAhead) {
  ...
  getTimeNeeded_(mediaState, presentationTime) {
    ...
    if (!mediaState.lastSegmentReference) {
      return this.skipRangeController_.timeNeededPast(presentationTime);
    }

    // When playing in reverse we fill the buffer behind the playhead, so the
    // next timestamp we need is the start of the last appended segment (the
    // point from which we continue fetching earlier segments).  Skip ranges
    // are not applied during reverse playback.
    if (this.playerInterface_.getPlaybackRate() < 0) {
      return mediaState.lastSegmentReference.startTime;
    }

    return this.skipRangeController_.timeNeededPast(
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    if (mediaState.segmentIterator) {
      ...
      return this.advanceRefPastSkipRanges_(mediaState, ref);
    } else if (mediaState.lastSegmentReference || bufferEnd) {
      // Something is buffered from another Stream.
      const reverse = this.playerInterface_.getPlaybackRate() < 0;
      let time;
      if (reverse) {
        // In reverse we extend the buffer backwards, so we continue from the
        // earliest buffered point.  Subtract a small epsilon so the lookup
        // lands on the segment *before* the current buffer start, rather than
        // the first already-buffered segment.
        const bufferStart = this.playerInterface_.mediaSourceEngine
            .bufferStart(mediaState.type);
        const refStart = mediaState.lastSegmentReference ?
            mediaState.lastSegmentReference.startTime :
            null;
        let frontier = bufferStart;
        if (refStart != null && (frontier == null || refStart < frontier)) {
          frontier = refStart;
        }
        time = frontier != null ? frontier - 1e-3 : bufferEnd;
      } else {
        time = mediaState.lastSegmentReference ?
            mediaState.lastSegmentReference.endTime :
            bufferEnd;
      }
      ...
      // Skip segments that fall within an active skip range.
      ref = this.advanceRefPastSkipRanges_(mediaState, ref);
      if (ref == null) {
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference, adaptation) {
    ...
    try {
      ...
      if (lowLatencyMode && isReadableStreamSupported && isMP4 &&
        ...
        // A skippable skip range may cover this segment (declared before the
        // fetch, or reached while it was in flight); discard it so the next
        // update skips past the range.  (containingActiveRange is buffer-
        // independent by design -- see its doc.)
        if (this.skipRangeController_.containingActiveRange(
            reference.startTime, reference.endTime)) {
          shaka.log.info(
              logPrefix, 'segment now within a skip range, skip append');
          mediaState.performingUpdate = false;
          if (!mediaState.updateTimer) {
            this.scheduleUpdate_(mediaState, 0);
          }
          return;
        }
            ...
            if (mediaState.stream) {
              ...
              if (videoLayout) {
                ...
                if (layouts.includes('PROJ-RECT')) {
                  ...
                  spatialVideoInfo.projection = 'aiv';
                }
              }
            }
            ...
            if (mediaState.type === ContentType.VIDEO &&
              ...
              parser.boxes([
                ...
                'stbl',
                  ...
                  .boxes([...Mp4Parser.AVC_HEVC_VVC_DV, 'encv'],
  ...
  async append_(mediaState, presentationTime, stream, reference, segment,
    ...
    if (this.config_.shouldFixTimestampOffset) {
      ...
      if (shouldFixTimestampOffset) {
        new shaka.util.Mp4Parser()
            .boxes(shaka.util.Mp4Parser.FRAGMENT_PATH,
  ...
  updateSegmentIteratorReverse_() {
    ...
    for (const mediaState of this.mediaStates_.values()) {
      ...
      if (reverse) {
        // When switching to reverse playback the update loop may have stopped
        // because we had buffered to the end of the presentation (endOfStream).
        // In reverse the relevant boundary is the start, not the end, so we
        // need to resume fetching content *behind* the playhead.  Drop the
        // segment iterator so it is recreated pointing in the new direction,
        // clear the end-of-stream flag, and wake the update loop if it is idle.
        mediaState.segmentIterator = null;
        mediaState.endOfStream = false;
        if (!mediaState.performingUpdate && !mediaState.updateTimer &&
            !mediaState.clearingBuffer && !mediaState.waitingToClearBuffer) {
          this.scheduleUpdate_(mediaState, 0);
        }
      }
    }
  ...
  setupCrossBoundaryListeners_() {
    ...
    this.crossBoundaryEventManager_.listenMulti(this.playerInterface_.video,
        ['seeked', 'canplay', 'timeupdate'], () => {
          const video = this.playerInterface_.video;
          // The reset promise only means buffers are ready. Finish after
          // explicit restoration assignment and the media element settling.
          // canplay/timeupdate also cover restoration without a new seek.
          if (this.mediaSourceRestorePending_ && !video.seeking &&
  ...
  forwardTimeForCrossBoundary_() {
    if (this.boundaryResetting_ || this.mediaSourceResetTime_ != null) {
      return;
    }
    if (!this.shouldUseCrossBoundaryLogic_()) {
    ...
    const currentTime = this.playerInterface_.video.currentTime;
    const fromEnd = lastInitRef.boundaryEnd - currentTime;
  ...
  discardReferenceByBoundary_(mediaState, reference) {
    ...
    const crossesBoundary =
        lastInitRef.boundaryEnd !== initRef.boundaryEnd;
    let discard = crossesBoundary;
    ...
    // Scheduling a boundary seek does not mean playback has crossed it yet.
    const crossBoundaryReset = leadingMediaState && crossesBoundary &&
        this.boundaryTime_ != null && !this.boundaryResetting_ &&
        lastInitRef.boundaryEnd != null &&
        this.playerInterface_.video.currentTime >= lastInitRef.boundaryEnd;

    if (crossBoundaryReset && !discard) {
      // KEEP completes the crossing when the boundary reference is accepted.
      this.boundaryTime_ = null;
      this.boundaryResetting_ = false;
      this.crossBoundaryTimer_.stop();
    }

    // crossBoundaryReset is used when a seek occurs due to crossing the
    ...
    if (leadingMediaState && discard && !this.boundaryResetting_ &&
        (crossBoundaryReset || mediaState.seeked)) {
      if (crossBoundaryReset) {
        this.boundaryResetting_ = true;
        this.crossBoundaryTimer_.stop();
      }
...
/**
 * Tolerance for matching the playhead to an internally requested boundary
 * seek, in seconds. This accounts for browsers rounding or clamping the media
 * element's currentTime value.
 *
 * @const {number}
 * @private
 */
shaka.media.StreamingEngine.CROSS_BOUNDARY_SEEK_TOLERANCE_ = 0.1;
