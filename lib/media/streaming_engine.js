  ...
  update_(mediaState) {
    ...
    if (this.config_.periodSwitchingStrategy !==
      ...
      if (mediaState.seeked && !mediaState.clearingBuffer) {
        ...
        if (this.config_.periodSwitchingStrategy ===
          shaka.config.PeriodSwitchingStrategy.ENCRYPTED) {
          ...
          const matchedStream = matchedStreams.find((stream) => {
            ...
            return stream.segmentIndex instanceof TimelineSegmentIndex ?
              stream.segmentIndex.isWithinPeriod(presentationTime) :
              false;
          });

          if (matchedStream.encrypted) {
