...
shaka.media.StreamingEngine = class {
  ...
  update_(mediaState) {
    ...
    if (this.config_.periodSwitchingStrategy !== PeriodSwitchingStrategy.NONE &&
      ...
      if (mediaSourceReset && this.config_.periodSwitchingStrategy ===
        ...
        if (matchedStream && matchedStream.encrypted) {
          ...
          this.config_.periodSwitchingStrategy =
            shaka.config.PeriodSwitchingStrategy.NONE;
        }
      }
      return null;
    }
  ...
  checkTimeAdvanceInNextStream() {
    ...
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    const mediaState = this.mediaStates_.get(ContentType.VIDEO);
    if (!mediaState || !mediaState.lastSegmentReference ||
      mediaState.seeked || mediaState.clearingBuffer ||
