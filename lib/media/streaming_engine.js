 ...
 * @param {function(!Event)} onEvent Called when an event occurs that should be
 *   sent to the app.
...
shaka.media.StreamingEngine.prototype.initSourceBuffer_ = function(
  ...
  var setStreamProperties = this.mediaSourceEngine_.setStreamProperties(
...
shaka.media.StreamingEngine.prototype.append_ = function(
  ...
  if (stream.containsEmsgBoxes) {
    var reader = new shaka.util.DataViewReader(
        new DataView(segment), shaka.util.DataViewReader.Endianness.BIG_ENDIAN);
    var boxSize = shaka.util.Mp4Parser.findBox(
        shaka.util.Mp4Parser.BOX_TYPE_EMSG, reader);
    if (boxSize != shaka.util.Mp4Parser.BOX_NOT_FOUND) {
      var endByte = reader.getPosition() - 8 + boxSize;
      // skip version and flags
      reader.skip(4);

      var schemeId = reader.readTerminatedString();
      ...
      var messageData = reader.readBytes(endByte - reader.getPosition());
