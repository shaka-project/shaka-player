...
shaka.media.StreamingEngine = class {
  ...
  switchInternal_(stream, clearBuffer, safeMargin, force, adaptation) {
    ...
    console.log(`[JULIAN]: mediaState.adaptation=${mediaState.adaptation}` +
  ...
  async initStreams_() {
    ...
    console.log(`[JULIAN]: forceTransmuxTS: ${forceTransmuxTS}`);
  ...
  async append_(mediaState, presentationTime, stream, reference, segment) {
    ...
    // Ensure any 'seeked' or 'adaptation' triggered logic applies only to this
    // appendBuffer() call.
    const seeked = mediaState.seeked;
    mediaState.seeked = false;
    const adaptation = mediaState.adaptation;
    mediaState.adaptation = false;
    console.log(`media/streaming_engine.js: append(): ` +
        `adaptation=${adaptation}, seeked=${seeked}`);
  ...
  async evict_(mediaState, presentationTime) {
    ...
    console.log(`[JULIAN]: ${mediaState.type}: Evicting ` +
