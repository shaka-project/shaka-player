...
shaka.media.StreamingEngine = class {
      ...
      if (reference.isIndependent() && reference.initSegmentReference) {
        ...
        const append = async () => {
          try {
            ...
            await this.changeTypeIfNeeded_(
                mediaState, reference.initSegmentReference);
  ...
  async append_(mediaState, presentationTime, stream, reference, segment,
    ...
    await this.changeTypeIfNeeded_(mediaState, reference);
   ...
   * @private
   */
  async changeTypeIfNeeded_(mediaState, reference) {
    // TODO: Do I also need to handle new init segments for this content, or
    // do we handle that already? The test content I have is all
    // self-initializing segments.
    if (reference.codecs) {
      // There's codecs information for this SegmentReference. If it's different
      // from the current codecs of the buffer, changeType.
      if (reference.codecs != mediaState.codecs) {
        const fullMimeType = shaka.util.MimeUtils.getFullType(
            mediaState.stream.mimeType, reference.codecs);
        await this.playerInterface_.mediaSourceEngine.changeType(
            mediaState.type, fullMimeType, /* transmuxer= */ null);
      }
    }
  }
 ...
 *   The current codecs of the media state.
