...
shaka.media.StreamingEngine = class {
  ...
  update_(mediaState) {
    ...
    if (this.config_.periodSwitchingStrategy !==
        ...
        mediaState.lastInitSegmentReference.startTime !==
          reference.initSegmentReference.startTime) {
      // If we seeked to another period, we'll reset MSE.
      ...
      // When we end up in an encrypted period, we can stop resetting MSE.
      // There's devices out there that can decode plain when instantiated with
      // an encrypted init segment.
      if (this.config_.periodSwitchingStrategy ===
