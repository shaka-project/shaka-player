...
shaka.media.StreamingEngine.prototype.loadNewTextStream = async function(
  ...
  if (this.mediaStates_.get(ContentType.TEXT) &&
      stream == this.mediaStates_.get(ContentType.TEXT).stream) {
    // When we're turning on a text stream that is already being streamed,
    // there's no need to re-load the stream. This happens when cea closed
    // captions are already extracted from video streams but hidden, and we
    // show the captions.
    return;
  }
...
shaka.media.StreamingEngine.prototype.update_ = function(mediaState) {
  ...
  const isClosedCaptions =
      mediaState.type == ContentType.TEXT && mediaState.stream.mimeType ==
      shaka.util.MimeUtils.CLOSED_CAPTION_MIMETYPE;
  if (isClosedCaptions) {
    return null;
  }
...
shaka.media.StreamingEngine.prototype.clearBuffer_ =
  ...
  // We should set the CEA caption ID after clearing the buffer, so that
  // we can load the new captions as soon as possible.
  // Setting the ID too early results in a gap in the display of captions
  // when we switch languages.  Setting the ID causes the stored captions
  // to be sent to the displayer.
