...
shaka.media.StreamingEngine = class {
  ...
  async switchVariant(variant, clearBuffer = false,
      safeMargin = 0, force = false) {
    const previousVariant = Object.create(this.currentVariant_);
    ...
    if (!this.startupComplete_) {
      ...
      // Audio codec change type
      const ContentType = shaka.util.ManifestParserUtils.ContentType;
      const Strategy = shaka.config.CodecSwitchingStrategy;
      const changeRequired = (previousVariant && previousVariant.audio &&
        previousVariant.audio.codecs !== variant.audio.codecs);
      const changeStrategy = this.config_.codecSwitchingStrategy;
      const fullMimeType = shaka.util.MimeUtils.getFullType(
          variant.audio.mimeType, variant.audio.codecs);

      if (changeRequired) {
        shaka.log.debug('Audio Codec Switch Required, using', changeStrategy);
        switch (changeStrategy) {
          case Strategy.DISABLED:
            shaka.log.debug('Strategy DISABLED');
            break;
          case Strategy.SMOOTH:
            shaka.log.debug('Strategy SMOOTH,' +
             'attempting SourceBuffer.changeType');
            await this.playerInterface_.mediaSourceEngine.changeType(
                ContentType.AUDIO, fullMimeType);
            break;
          case Strategy.RELOAD:
            shaka.log.debug('Strategy RELOAD');
            this.unloadAudioStream();
            this.unloadVideoStream();
            await
            this.playerInterface_.mediaSourceEngine
                .reset(ContentType.AUDIO);
            await this.initStreams_();
            this.destroyer_.ensureNotDestroyed();
            break;
        }
        ...
        shaka.log.debug('No audio codec switch required.');
      }
    }
  ...
  loadAudioStream(value) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    this.mediaStates_.set(ContentType.AUDIO, value);
  }

  // eslint-disable-next-line require-jsdoc
  loadVideoStream(value) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    this.mediaStates_.set(ContentType.VIDEO, value);
  }
  ...
  unloadAudioStream() {
    ...
    if (state) {
      this.cancelUpdate_(state);
      this.abortOperations_(state).catch(() => {});
      this.mediaStates_.delete(ContentType.AUDIO);
    }
  }
  ...
  unloadVideoStream() {
    ...
    if (state) {
      this.cancelUpdate_(state);
      this.abortOperations_(state).catch(() => {});
      this.mediaStates_.delete(ContentType.VIDEO);
    }
  }
