...
shaka.media.StreamingEngine = class {
  ...
  async fetch_(mediaState, reference, streamDataCallback) {
    ...
    const stream = mediaState.stream;
    const isInit = reference instanceof shaka.media.InitSegmentReference;
    const mediaReference =
    /** @type {!shaka.media.SegmentReference} */(reference);
    const duration = isInit ? 0 :
        mediaReference.endTime - mediaReference.startTime;
    ...
    if (reference.hlsAes128Key) {
      ...
      const time = isInit ? null : mediaReference.startTime;
      if (!iter && time != null) {
