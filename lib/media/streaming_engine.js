...
shaka.media.StreamingEngine = class {
  ...
  seeked() {
    ...
    if (!crossBoundarySeek || this.boundaryResetting_) {
      // A different seek supersedes the crossing. A matching seek after the
      // reset starts consumes restoration, while retaining crossBoundarySeek
      // for this event so it does not enter normal user-seek handling.
      this.boundaryTime_ = null;
      this.boundaryResetting_ = false;
      this.crossBoundaryTimer_.stop();
    }
