...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface) {
    ...
    /**
     * Disjoint ranges to skip; see addSkipRange().
     * @private {!Array<shaka.media.StreamingEngine.SkipRange_>}
     */
    this.skipRanges_ = [];
  ...
  /**
   * Declares a range [start, end) whose segments are treated as if they do not
   * exist: not fetched or buffered, with buffering continuing from |end| as if
   * it followed |start|.  The gap-jumping logic carries the playhead across.
   *
   * Must be declared before streaming buffers into it, and must not overlap or
   * touch an existing range; either case is ignored.  Applies to all content
   * types.  Recoverable via removeSkipRange() / clearSkipRanges().  Supported
   * for DASH, non-live, segments-mode streams only.
   *
   * @param {number} start
   * @param {number} end
   */
  addSkipRange(start, end) {
    ...
    // Keep ranges disjoint so a segment falls in at most one (single-step skip,
    // no loop across abutting ranges).
    ...
    // removes buffered content, so the range must be a hole (unfetched, or
    // evicted via evictRange).
    for (const mediaState of this.mediaStates_.values()) {
      if (this.playerInterface_.mediaSourceEngine.isBuffered(
          mediaState.type, start)) {
  ...
  /**
   * Removes a skip range (matched by the same start/end), so its content is
   * fetched normally again.  Ignored once streaming has buffered past it (the
   * hole is committed; no eviction or rebuffer).
   *
   * @param {number} start
   * @param {number} end
   */
  removeSkipRange(start, end) {
    for (const mediaState of this.mediaStates_.values()) {
      ...
      if (bufferEnd != null && bufferEnd > start) {
        shaka.log.warning('removeSkipRange(): range already buffered; ignoring',
            start, end);
        return;
      }
    }
  ...
  /**
   * @return {!Array<{start: number, end: number}>} The active skip ranges.
   */
  getSkipRanges() {
  ...
  /**
   * Removes buffered media in [start, end) from every stream.  Await this
   * before addSkipRange() to exclude an already-buffered region (e.g. a watched
   * ad) from future streaming.  The caller must keep the playhead out of the
   * range, or playback stalls on the hole.
   *
   * @param {number} start
   * @param {number} end
   * @return {!Promise}
  ...
  evictRange(start, end) {
    ...
    for (const type of this.mediaStates_.keys()) {
      removals.push(
          this.playerInterface_.mediaSourceEngine.remove(type, start, end));
    }
    return Promise.all(removals).catch((error) => {
   ...
   *
   * @param {number} time
   * @return {?shaka.media.StreamingEngine.SkipRange_}
   * @private
   */
  getSkipRangeAt_(time) {
   ...
   * @private
   */
  getTimePastSkipRanges_(time) {
    const region = this.getSkipRangeAt_(time);
    return region ? region.end : time;
  }


  /**
   * If |ref| starts inside a skip range, re-seeds the media state's iterator
   * past the range -- in this stream's own index -- and returns the first
   * segment at or after the range end, creating the buffer hole.  Otherwise
   * returns |ref| unchanged (zero cost when no range applies).
   *
  ...
  advanceRefPastSkipRanges_(mediaState, ref) {
    if (!ref || !this.skipRanges_.length ||
        this.playerInterface_.getPlaybackRate() < 0) {
      // TODO: Support reverse trick play, which walks the index backwards.
      return ref;
    }
    const region = this.getSkipRangeAt_(ref.startTime);
    if (!region) {
      return ref;
    }
    ...
    ref = it.next().value;
    // Also skip a segment straddling the range end (its front is skipped).
  ...
  createSegmentPrefetch_(stream) {
    ...
    if (this.config_.segmentPrefetchLimit > 0) {
      ...
      return new shaka.media.SegmentPrefetch(
          ...
          (reference) => !this.getSkipRangeAt_(reference.startTime));
    }
    return null;
  }
  ...
  getTimeNeeded_(mediaState, presentationTime) {
    ...
    if (!mediaState.lastSegmentReference) {
      return this.getTimePastSkipRanges_(presentationTime);
    }
    ...
    return this.getTimePastSkipRanges_(mediaState.lastSegmentReference.endTime);
  }
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference, adaptation) {
    ...
    try {
      ...
      if (lowLatencyMode && isReadableStreamSupported && isMP4 &&
        ...
        // A skip range covering this segment may have been added while the
        // fetch was in flight.  Discard the segment instead of appending it;
        // the next update looks the reference up again and skips past the
        // range.
        if (this.getSkipRangeAt_(reference.startTime)) {
          shaka.log.info(
 ...
 * }}
 */
shaka.media.StreamingEngine.SkipRange_;
