...
shaka.media.StreamingEngine = class {
  ...
  /**
   * If |ref| starts inside an active skip range, re-seeds the media state's
   * iterator past the range -- in this stream's own index -- and returns the
   * first segment at or after the range end, creating the buffer hole.
   * Otherwise returns |ref| unchanged (zero cost when no range applies).
   *
  ...
  advanceRefPastSkipRanges_(mediaState, ref) {
    ...
    const region = this.skipRangeController_.activeRangeAt(ref.startTime);
    if (!region) {
      return ref;
    }
  ...
  createSegmentPrefetch_(stream) {
    ...
    if (this.config_.segmentPrefetchLimit > 0) {
      ...
      return new shaka.media.SegmentPrefetch(
          ...
          // Don't prefetch a reference an acting skip range will drop.
          (reference) =>
            !this.skipRangeController_.activeRangeAt(reference.startTime));
    }
    return null;
  }
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference, adaptation) {
    ...
    try {
      ...
      if (lowLatencyMode && isReadableStreamSupported && isMP4 &&
        ...
        // An acting skip range may have covered this segment while its fetch
        // was in flight; discard it so the next update skips past the range.
        if (this.skipRangeController_.activeRangeAt(reference.startTime)) {
          shaka.log.info(
