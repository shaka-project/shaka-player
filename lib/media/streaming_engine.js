  ...
  */
  ...
  switchInternal_(stream, clearBuffer, safeMargin, force) {
    ...
    if (stream.type == ContentType.TEXT) {
      ...
      // Set needTextParserReinitialisation flag in order to
      // postpone reinitialization of the text parser until the moment
      // we are about to fetch an init segment again. It prevents:
      // https://github.com/google/shaka-player/issues/2237
      mediaState.needTextParserReinitialisation = true;
    }
  ...
  createMediaState_(stream, needPeriodIndex, resumeAt) {
    return /** @type {shaka.media.StreamingEngine.MediaState_} */ ({
      ...
      needTextParserReinitialisation: false,
  ...
  async initSourceBuffer_(mediaState, reference) {
    ...
    const needTextParserReinitialisation =
        mediaState.needTextParserReinitialisation;
    ...
    if (needTextParserReinitialisation) {
      const fullMimeType = shaka.util.MimeUtils.getFullType(
          mediaState.stream.mimeType, mediaState.stream.codecs);
      shaka.log.v1(logPrefix, 'reinitializing a text parser');
      this.playerInterface_.mediaSourceEngine.reinitText(fullMimeType);
      mediaState.needTextParserReinitialisation = false;
    }
    ...
    if (!reference.initSegmentReference) {
      ...
      shaka.log.v1(logPrefix, 'the Stream is self initializing');
      await setStreamProperties;
      return;
    }
 ...
 *   needTextParserReinitialisation,
