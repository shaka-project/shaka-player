...
shaka.media.StreamingEngine = function(
  ...
  /** @private {?shakaExtern.TextConfiguration} */
 ...
 *
 * @param {shakaExtern.StreamingConfiguration} config
 * @param {?shakaExtern.TextConfiguration} textconfig
 */
shaka.media.StreamingEngine.prototype.configure = function(config, textconfig) {
  this.config_ = config;
  this.textconfig_ = textconfig;
...
shaka.media.StreamingEngine.prototype.setTrickPlay = function(on) {
  var mediaState = this.mediaStates_['video'];
  ...
  if (on) {
    ...
    this.switch('video', trickModeVideo, false);
    ...
    this.switch('video', normalVideo, true);
  }
};
...
shaka.media.StreamingEngine.prototype.initStreams_ = function(streamsByType) {
  ...
  // Override default VTTCue Region
  var vttRegion = null;
  if (this.textconfig_ && this.textconfig_.override) {
    vttRegion = new window.VTTRegion();
    if (this.textconfig_.viewportAnchorX) {
      vttRegion.viewportAnchorX = this.textconfig_.viewportAnchorX;
    }
    if (this.textconfig_.viewportAnchorY) {
      vttRegion.viewportAnchorY = this.textconfig_.viewportAnchorY;
    }
  }
...
shaka.media.StreamingEngine.prototype.update_ = function(mediaState) {
  ...
  var bufferedAhead = this.getBufferedAhead_(mediaState,
                                             playheadTime,
                                             currentPeriodIndex,
                                             needPeriodIndex);
...
/**
 * Gets time buffered ahead of current playhead time (in seconds).
 *
 * @param {shaka.media.StreamingEngine.MediaState_} mediaState
 ...
 * @private
 */
shaka.media.StreamingEngine.prototype.getBufferedAhead_ = function(
    mediaState, playheadTime, currentPeriodIndex, needPeriodIndex) {
  ...
  if (reference) {
    return needPeriod.startTime + reference.startTime - playheadTime;
  } else {
    return this.mediaSourceEngine_.bufferedAheadOf(
