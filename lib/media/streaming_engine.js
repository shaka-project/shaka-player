...
shaka.media.StreamingEngine.prototype.init = function(streamsByType) {
  ...
  this.setupStreams_(streams).then(function() {
    ...
    // Subtlety: onInitialStreamsSetup_() may call us, so we must
    // schedule an update beforehand so |updateTimer| is set.
    if (this.onInitialStreamsSetup_) {
      shaka.log.v1('(all) calling onInitialStreamsSetup_()...');
...
shaka.media.StreamingEngine.prototype.onUpdate_ = function(mediaState) {
  ...
  if (mediaState.needPeriod != currentPeriod) {
    ...
    if (needSamePeriod) {
      ...
      shaka.log.v1('(all) calling onBufferNewPeriod_()...');
  ...
  if (mediaStates.every(function(ms) { return ms.endOfStream; })) {
    shaka.log.v1('(all) calling endOfStream()...');
...
shaka.media.StreamingEngine.prototype.update_ = function(mediaState) {
  ...
  shaka.asserts.assert(
      ...
      'should be contained within the Manifest');
    ...
    if (bufferEnd == null) {
      ...
      if (mediaState.buffer.length > 0) {
        ...
        throw new shaka.util.Error(
            ...
            shaka.util.Error.Code.STREAMING_INCONSISTENT_BUFFER_STATE,
            mediaState.type);
      }
      return playheadTime;
...
shaka.media.StreamingEngine.prototype.getSegmentReference_ = function(
  ...
  if (!reference) {
    ...
    throw new shaka.util.Error(
        shaka.util.Error.Category.MEDIA,
        shaka.util.Error.Code.STREAMING_INVALID_SEGMENT_INDEX,
        mediaState.type,
        currentPeriodIndex,
        position);
  }
...
shaka.media.StreamingEngine.prototype.fetchAndAppend_ = function(
  ...
  Promise.all([initSourceBuffer, fetchSegment]).then(function(results) {
    ...
    // Subtlety: handleStartup_() calls onStartupComplete_() which may call us,
    // so we must schedule an update beforehand so |updateTimer| is set.
    this.handleStartup_();
...
shaka.media.StreamingEngine.prototype.handleStartup_ = function() {
  ...
  this.startupComplete_ = mediaStates.every(function(ms) {
    ...
    //   2. if |drift| is non-null but we're clearing the buffer then there was
    //      positive drift and now we're waiting to recover; and
    //   3. if |drift| is non-null and we're not waiting to clear the buffer
    //      but the buffer is empty then we never recovered.
