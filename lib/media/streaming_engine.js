...
shaka.media.StreamingEngine.prototype.evict_ = function(
  ...
  var bufferBehind = Math.max(this.config_.bufferBehind,
  ...
  var overflow = bufferedBehind - bufferBehind;
  if (overflow <= 0) {
    shaka.log.v2(logPrefix,
                 'buffer behind okay:',
                 'playheadTime=' + playheadTime,
...
shaka.media.StreamingEngine.prototype.fetch_ = function(reference) {
  ...
  var op = this.playerInterface_.netEngine.request(requestType, request);
