...
shaka.media.StreamingEngine = class {
  ...
  discardReferenceByBoundary_(mediaState, reference) {
    ...
    const leadingMediaState = this.mediaStates_.get(ContentType.VIDEO) ||
        this.mediaStates_.get(ContentType.AUDIO);
    if (mediaState !== leadingMediaState) {
      // This function discards both audio and video references. When a reset is
      // triggered later, it does not need to be invoked per content type. Instead,
      // we only check whether the leading content type can reset the MSE. This
      // prevents an unnecessary call to resetMediaSource. If this is not the
      // leading content type, the only required action here is discarding.
      return discard;
    }
