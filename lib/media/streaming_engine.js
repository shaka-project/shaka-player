  ...
  seeked() {
    ...
    for (const type of this.mediaStates_.keys()) {
      ...
      if (!segment || segment.startTime > presentationTime ||
        ...
        // Always clear the iterator (if the seek is not produced by a gap)
        // since we need to start streaming from the new time.  This also
        // happens in clearBuffer_, but if we don't clear, we still want to
        // reset the iterator.
        mediaState.segmentIterator = null;
      }
