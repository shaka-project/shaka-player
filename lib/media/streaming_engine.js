...
shaka.media.StreamingEngine = class {
   ...
   *
   * A range is carveable only after the video grid confirms it aligns to
   * segment boundaries on both ends; a misaligned range is left untouched on
   * every stream (with a one-time warning from the controller).  The video grid
  ...
  advanceRefPastSkipRanges_(mediaState, ref) {
    ...
    // exists up front (no misaligned partial carve while unresolved).  Prefer
    // the video grid; fall back to this stream's index for audio-only content.
    ...
    if (!region) {
      // No carveable range here (none, unresolved, or unalignable); stream it
      // as-is.
      return ref;
    }
