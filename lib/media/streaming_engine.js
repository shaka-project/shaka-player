...
shaka.media.StreamingEngine = class {
  ...
  async initSourceBuffer_(mediaState, reference) {
    ...
    if (this.manifest_.sequenceMode) {
      ...
      const shouldResync = reference.discontinuitySequence !==
          lastDiscontinuitySequence || mediaState.needsResync;
      if (shouldResync) {
        mediaState.needsResync = false;
        operations.push(this.playerInterface_.mediaSourceEngine
            .resync(mediaState.type, reference.startTime));
      }
      // Reset the sequence mode text offset after discontinuity.
      // Prefer video timestamp for sync when available.
      const isBestMediaStateForTimestamps = (mediaState.type ===
        ContentType.VIDEO || !this.mediaStates_.has(ContentType.VIDEO));
      if (shouldResync && lastDiscontinuitySequence !== null &&
        isBestMediaStateForTimestamps) {
        const segment = await this.fetch_(mediaState, reference);
        const timestamp = this.playerInterface_.mediaSourceEngine
            .getTimestampAndDispatchMetadata(mediaState.type, segment,
                reference, mediaState.stream.mimeType, reference.startTime);
        if (timestamp !== null) {
          const offset = reference.startTime - timestamp;
          operations.push(this.playerInterface_.mediaSourceEngine
