...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface) {
    ...
    /**
     * @private {shaka.media.TimestampOffsetCorrector}
     */
    this.timestampOffsetCorrector_ =
        new shaka.media.TimestampOffsetCorrector(playerInterface.onEvent);
  }
  ...
  configure(config) {
    ...
    this.timestampOffsetCorrector_.configure(config);
  }
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference) {
    ...
    // Timestamp correction is not needed when using sequence mode
    if (!this.manifest_.sequenceMode && this.config_.correctTimestampOffset) {
      // When the timstamp offset correction has already been determined
      // by checkAndCorrectTimestampOffset on some previous segment, we
      // apply it to the current segment reference here, to avoid
      // an extra call to initSourceBuffer_ after fetching the
      // media segment.
      this.timestampOffsetCorrector_.correctTimestampOffset(
          mediaState.type, reference);
    }
    ...
    try {
      ...
      if (this.config_.lowLatencyMode && isReadableStreamSupported && isMP4 &&
        ...
        if (!this.manifest_.sequenceMode &&
            this.config_.correctTimestampOffset) {
          // If timestampOffset was incorrect we need to call initSourceBuffer_
          // to apply the corrected timestampOffset to the source buffer.
          if (this.timestampOffsetCorrector_.checkAndCorrectTimestampOffset(
              mediaState.type, reference, result)) {
            await this.initSourceBuffer_(mediaState, reference);
          }
        }
        ...
        const append = async () => {
          try {
            ...
            if (!this.manifest_.sequenceMode &&
                this.config_.correctTimestampOffset) {
              this.timestampOffsetCorrector_
                  .parseTimescalesFromInitSegment(mediaState.type, initSegment);
            }
