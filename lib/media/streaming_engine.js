...
goog.require('shaka.event.BoundaryCrossed');
goog.require('shaka.event.NoSpatialVideoInfo');
goog.require('shaka.event.SpatialVideoInfo');
  ...
  updateSpatialVideoInfo_(info) {
    if (this.spatialVideoInfo_.projection != info.projection ||
      ...
      if (info.projection != null || info.hfov != null) {
        event = new shaka.event.SpatialVideoInfo(info);
      } else {
        event = new shaka.event.NoSpatialVideoInfo();
      }
  ...
  discardReferenceByBoundary_(mediaState, reference) {
    ...
    if (leadingMediaState && discard &&
      ...
      this.resetMediaSource(/* force= */ true).then(() => {
        this.playerInterface_.onEvent(new shaka.event.BoundaryCrossed(
            lastInitRef.encrypted, initRef.encrypted));
      });
    }
    return discard;
  }
