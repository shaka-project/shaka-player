    ...
    for (const type of streamsByType.keys()) {
      ...
      if (!this.mediaStates_.has(type)) {
        ...
        if (segmentPrefetchById.has(stream.id)) {
          ...
          // TODO: Replace the fetchDispatcher on this with one that uses
          // the config and net engine on this object, so it is no longer
          // attached to the PreloadManager that made it.
