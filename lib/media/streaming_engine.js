...
/**
 * Gets an array of all the active streams.
...
shaka.media.StreamingEngine.prototype.getActiveStreams = function() {
  ...
  return Object.keys(this.mediaStates_).map(function(type) {
    return this.mediaStates_[type].stream;
  }.bind(this));
};
...
shaka.media.StreamingEngine.prototype.setDuration_ = function() {
  ...
  if (duration < Number.POSITIVE_INFINITY) {
    ...
    this.mediaSourceEngine_.setDuration(60 * 60 * 24 * 30);
  }
};
...
shaka.media.StreamingEngine.prototype.evict_ = function(
  ...
  for (var type in this.mediaStates_) {
    ...
    startTime = Math.min(this.mediaSourceEngine_.bufferStart(ms.type),
                         startTime);
  }
  ...
  for (var type in this.mediaStates_) {
    var ms = this.mediaStates_[type];
    async.push(this.mediaSourceEngine_.remove(
        ms.type, startTime, startTime + overflow));
  }
