...
shaka.media.StreamingEngine = class {
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference) {
    ...
    try {
      if (this.config_.lowLatencyMode) {
        const streamDataCallback = async (chunkedData) => {
          ...
          await this.playerInterface_.mediaSourceEngine.appendBuffer(
              ...
              reference.startTime,
              reference.endTime,
              // TODO: add support for closed captions with chunked
              // ReadableStream data.
              false);
        };

        await this.fetch_(mediaState, reference,
            streamDataCallback);
      } else {
      ...
      this.advanceToNextSegment_(mediaState, reference, stream);
  ...
  async append_(mediaState, presentationTime, stream, reference,
    ...
    // TODO: add support for parsing EMSG box with chunked ReadableStream data.
  ...
  /**
   * Move to the next segment after finished appending the current segment.
   *
   * @param {!shaka.media.StreamingEngine.MediaState_} mediaState
   * @param {!shaka.media.SegmentReference} reference
   * @param {shaka.extern.Stream} stream
