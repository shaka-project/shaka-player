...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface) {
    ...
    /** @private {string} */
    this.lastProcessedVideoSegment = '';
  ...
  async initStreams_() {
    ...
    mediaSourceEngine.setLowLatencyMode(this.config_.lowLatencyMode);
    this.destroyer_.ensureNotDestroyed();
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    if (mediaState.segmentIterator) {
      ...
      if (ref == null) {
        ...
        mediaState.segmentIterator = null;
      } else if (this.config_.lowLatencyMode) {
        // If the segment reference we get is behind presentation time for
        // more than 4 segments, invalidate the query and retry in next update
        const segmentStartTime = ref.getStartTime() +
          this.manifest_.presentationTimeline.getMaxSegmentDuration() * 4;
        if (segmentStartTime < presentationTime) {
          mediaState.segmentIterator = null;
          ref = null;
        }
      }
  ...
  /**
   * Based on active video stream
   * @return {Array.<Object>}
   */
  getPresentationLatency() {
    const wc = Date.now();
    const pt = this.playerInterface_.getPresentationTime() * 1000;
    const data = this.playerInterface_.getPresentationLatencyData();
    if (data == null) {
      return null;
    }
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    const repId = this.mediaStates_.get(ContentType.VIDEO).stream.originalId;
    if (repId == null) {
      shaka.log.warning('getPresentationLatency cannot get video repId');
      return null;
    }
    if (data.periodIdMap == null) {
      shaka.log.warning('getPresentationLatency cannot get periodIdMap');
      return null;
    }
    const periodId = this.getPeriodIdFromPresentationTime(pt, data.periodIdMap);
    const rv = [];
    const keyPrefix = `${periodId}_${repId}`;
    for (const [key, producerReferenceTime] of data.prtMap.entries()) {
      if (!key.startsWith(keyPrefix)) {
        continue;
      }
      const wca = producerReferenceTime.wca;
      const pta = producerReferenceTime.pta;
      const type = producerReferenceTime.type;
      const latency = Math.round((wc - wca) - (pt - pta));
      shaka.log.debug(`computing presentation latency for 
          ...
          type=${type}
          latency=${latency}ms`);
      rv.push({type, latency});
    }
    return rv;
  }

  /**
   * Get string from presentation time
   * @param {number} pt
   * @param {Map<number,string>} periodIdMap
   * @return {string}
   */
  getPeriodIdFromPresentationTime(pt, periodIdMap) {
    const keys = Array.from(periodIdMap.keys()).sort((a, b) => a-b);
    let targetPt = keys[0];
    ...
    return periodIdMap.get(targetPt);
  }

  /**
   * Parse PRFT box and override the default WCA and PTA in Manifest.
   * @param {!shaka.media.SegmentReference} reference
   * @param {!shaka.extern.ParsedBox} box
   * @private
   */
  parsePRFT_(reference, box) {
    if (this.lastProcessedVideoSegment === reference.getUris()[0]) {
      shaka.log.debug(`skipping parsePRFT for parsed segment`);
      return;
    }
    this.lastProcessedVideoSegment = reference.getUris()[0];

    const version = box.version;
    const flag = box.flags;
    let type = 'encoder';
    if (flag != null && (flag & 24) === 24) {
      type = 'captured';
    }
    ...
    if (version === 0) {
      ...
      mediaTime = box.reader.readUint64BigInt();
    }

    const wca = this.convertNtp(ntpTimestamp);

    const data = this.playerInterface_.getPresentationLatencyData();
    if (data == null) {
      return;
    }
    const periodId = this.getPeriodIdFromPresentationTime(
        reference.startTime, data.periodIdMap);
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    const repId = this.mediaStates_.get(ContentType.VIDEO).stream.originalId;
    const key = `${periodId}_${repId}_${type}`;
    if (!data.prtMap.has(key)) {
      return;
    }
    const prtData = data.prtMap.get(key);
    if (!prtData.isInBand) {
      return;
    }
    prtData.wca = wca;
    // eslint-disable-next-line new-cap
    prtData.pta = Number(BigInt(mediaTime) / BigInt(prtData.timescale))*1000;

    shaka.log.debug(`parsed prft box 
         segment:${reference.getUris()}
         type ${type}
         ntpTimestampSec ${ntpTimestampSec}          
         converted wca ${prtData.wca}
         pta ${prtData.pta} 
         segmentTime ${reference.startTime} 
         `);
  }
   ...
   * @return {Object}
   */
  getMediaState(contentType) {
    return this.mediaStates_.get(
        /** @type {shaka.util.ManifestParserUtils.ContentType}*/(contentType));
  }
};
 ...
 *   (function():shaka.media.StreamingEngine.PresentationLatencyData|undefined)
 * }}
 *
 ...
 * }}
 */
shaka.media.StreamingEngine.ProducerReferenceTime;

/**
 * @typedef {{
 *   periodIdMap: Map<number,string>,
 *   prtMap: Map<string,shaka.media.StreamingEngine.ProducerReferenceTime>
 * }}
 */
shaka.media.StreamingEngine.PresentationLatencyData;
