...
shaka.media.QualityObserver = class {
  ...
  /**
   * Get the ContenMediaQuality for a contentType, creating a new
   * one if necessary.
   *
  ...
  /**
   * Adds a QualityChange for the contentType identified by
  ...
  addMediaQualityChange(mediaQuality, position) {
    ...
    this.purgeQualityChangesForContentType_(mediaQuality.contentType);
    ...
    const qualityChanges =
      this.getContentMediaQuality_(mediaQuality.contentType).qualityChanges;
    const insertBeforeIndex = qualityChanges.findIndex(
        (qualityChange) => (qualityChange.position >= position));

    if (insertBeforeIndex >= 0) {
      const duplicatePositions =
        (qualityChanges[insertBeforeIndex].position == position) ? 1 : 0;
      qualityChanges.splice(
          insertBeforeIndex, duplicatePositions, newQualityChange);
    } else {
      qualityChanges.push(newQualityChange);
    }
  }
  ...
  static getMediaQualityAtPosition_(position, contentMediaQuality) {
    // The qualityChanges must be ordered by position ascending
    // Find the last QualityChange prior to the position
    const qualityChanges = contentMediaQuality.qualityChanges;
    for (let i = qualityChanges.length - 1; i >= 0; i--) {
      const qualityChange = qualityChanges[i];
    ...
    for (const contentMediaQuality of this.contentMediaQualities_.values()) {
      ...
      if (mediaQualityAtPosition &&
        ...
        if (this.positionIsBuffered_(
            positionInSeconds, mediaQualityAtPosition.contentType)) {
          contentMediaQuality.currentQuality = mediaQualityAtPosition;
   ...
   * invoked just before adding the quality change event associated with
   * the next media segment to be appended.
   *
   * @param {!string} contentType
   * @private
   */
  purgeQualityChangesForContentType_(contentType) {
    const bufferedInfo = this.getBufferedInfo_();
