...
goog.require('shaka.event.AudioTrackChanged');
goog.require('shaka.event.MediaQualityChanged');
    ...
    for (const contentTypeState of this.contentTypeStates_.values()) {
      ...
      if (differentLabel || differentLanguage || differentRoles) {
        if (this.positionIsBuffered_(
          ...
          this.dispatchEvent(new shaka.event.AudioTrackChanged(
      ...
      if (differentQualities) {
        if (this.positionIsBuffered_(
          ...
          this.dispatchEvent(new shaka.event.MediaQualityChanged(
              qualityAtPosition, positionInSeconds));
        }
      }
    }
