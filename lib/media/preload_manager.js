...
shaka.media.PreloadManager = class extends shaka.util.FakeEventTarget {
  ...
  setPrefetchVariant(variant) {
    ...
    if (this.prefetchedVariant_) {
      console.log(`[APPLE, PreloadManager]: Set prefetched variant: ${shaka.util.StreamUtils.getVariantSummaryString(this.prefetchedVariant_)}`);
    }
  }
  ...
  async chooseInitialVariant_() {
    ...
    if (!this.currentAdaptationSetCriteria_) {
      ...
      this.currentAdaptationSetCriteria_.configure({
        ...
        preferredVideoCodecs: this.config_.preferredVideoCodecs,
      });
    }
    ...
    if (this.shouldCreateSegmentIndexBeforeDrmEngineInitialization_()) {
      ...
      console.log(`[APPLE, PreloadManager]: Initial prefetched variant: ${shaka.util.StreamUtils.getVariantSummaryString(this.prefetchedVariant_)}`);
      if (this.prefetchedVariant_) {
  ...
  async validatePrefetchedVariant_() {
    if (this.prefetchedVariant_ &&
      ...
      this.prefetchedVariant_ = this.configureAbrManagerAndChooseVariant_();
      console.log(`[APPLE, PreloadManager]: Corrected prefetched variant: ${shaka.util.StreamUtils.getVariantSummaryString(this.prefetchedVariant_)}`);
      if (this.prefetchedVariant_) {
        await this.createSegmentIndexFromVariant_(this.prefetchedVariant_);
      }
    }
  ...
  configureAbrManagerAndChooseVariant_() {
    ...
    for (const v of playableVariants) {
      console.log(`[APPLE, PreloadManager]: After playbale variant filtering: ${shaka.util.StreamUtils.getVariantSummaryString(v)}`);
    }
