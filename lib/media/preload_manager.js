...
shaka.media.PreloadManager = class {
   ...
   * @param {!shaka.media.PreloadManager.PlayerInterface} playerInterface
   */
  constructor(assetUri, mimeType, startTimeOfLoad, playerInterface) {
    ...
    this.createDrmEngine_ = playerInterface.createDrmEngine;

    /** @private {function(!shaka.extern.Manifest):!Promise} */
    this.filterManifest_ = playerInterface.filterManifest;
  ...
  chooseInitialVariantInner_() {
    ...
    // up to a load object?
    // TODO: Pass this.currentAdaptationSetCriteria_ into the filterManifest_
    // method?
    ...
    const prefetch = new shaka.media.SegmentPrefetch(
        prefetchLimit, stream, (reference, stream, streamDataCallback) => {
          return shaka.media.StreamingEngine.dispatchFetch(
              ...
              this.config_.retryParameters, this.networkingEngine_);
        });
 ...
 *   filterManifest: function(!shaka.extern.Manifest):!Promise,
 ...
 * @property {function(!shaka.extern.Manifest):!Promise} filterManifest
