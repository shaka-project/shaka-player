...
shaka.media.PreloadManager = class extends shaka.util.FakeEventTarget {
  ...
  start() {
    (async () => {
      ...
      try {
        shaka.log.alwaysWarn('parseManifestInner_');
        await this.parseManifestInner_();
        this.throwIfDestroyed_();

        shaka.log.alwaysWarn('setupStreaming_');
        this.setupStreaming_();

        shaka.log.alwaysWarn('chooseInitialVariant_');
        await this.chooseInitialVariant_();
        this.throwIfDestroyed_();

        shaka.log.alwaysWarn('initializeDrm');
        if (!shaka.drm.DrmUtils.isMediaKeysPolyfilled('webkit')) {
          await this.initializeDrm();
          this.throwIfDestroyed_();
        }

        shaka.log.alwaysWarn('prefetchInner_');
  ...
  async parseManifestInner_() {
    ...
    shaka.log.alwaysWarn(this.manifest_);
    ...
    shaka.log.alwaysWarn('ManifestParsed');
    ...
    shaka.log.alwaysWarn('tracksChangedInitial');
  ...
  /**
   * Create the currentAdaptationSetCriteria and abrManager.
   *
   * @private
   */
  setupStreaming_() {
