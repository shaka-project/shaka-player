...
shaka.media.PreloadManager = class extends shaka.util.FakeEventTarget {
  ...
  async parseManifestInner_() {
    ...
    if (!this.manifest_) {
      ...
      // Before locking in an initial variant, remove variants with unsupported
      // media types.
      this.manifestFilterer_.filterManifestByTypeSupported(this.manifest_);
  ...
  /** Filter manifest by mcap */
  async filterManifest() {
    if (this.manifest_) {
      let langs = new Set();
      for (const v of this.manifest_.variants) {
        langs.add('videoId=' + v.video.id + ';' + v.audio.label + ';' + v.audio.language);
      }
      console.log(`[APPLE, PreloadManager]: initDrm before filtering variants based on manifestFilterer: ` +
        `${[...langs].join(',')}`);
      ...
      console.log(`[APPLE, PreloadManager]: initDrm after filtering variants based on manifestFilterer: ` +
        `${[...langs].join(',')}`);
  ...
  async chooseInitialVariant_() {
    ...
    if (this.shouldCreateSegmentIndexBeforeDrmEngineInitialization_()) {
      console.log(`[APPLE, PreloadManager]: configureAbrManagerAndChooseVariant_() from chooseInitialVariant_()`);
      this.prefetchedVariant_ = this.configureAbrManagerAndChooseVariant_();
      console.log(`[APPLE, PreloadManager]: chooseInitialVariant_(): ${shaka.util.StreamUtils.getVariantSummaryString(this.prefetchedVariant_)}`);
      if (this.prefetchedVariant_) {
  ...
  async validatePrefetchedVariant_() {
    if (this.prefetchedVariant_ &&
      ...
      console.log(`[APPLE, PreloadManager]: configureAbrManagerAndChooseVariant_() from validatePrefetchedVariant_()`);
      this.prefetchedVariant_ = this.configureAbrManagerAndChooseVariant_();
      if (this.prefetchedVariant_) {
        await this.createSegmentIndexFromVariant_(this.prefetchedVariant_);
      }
  ...
  async prefetchInner_() {
    if (!this.prefetchedVariant_) {
      console.log(`[APPLE, PreloadManager]: configureAbrManagerAndChooseVariant_() from prefetchInner_()`);
  ...
  configureAbrManagerAndChooseVariant_() {
    ...
    const playableVariants = /** @type {!Array<!shaka.extern.Variant>} */ (shaka.util.StreamUtils.getPlayableVariants(
        this.manifest_.variants));
    let langs = new Set();
    for (const v of playableVariants) {
      langs.add('videoId=' + v.video.id + ';' + v.audio.label + ';' + v.audio.language);
    }
    console.log(`[APPLE, PreloadManager]: before filtering variants based on adaptationSetCriteria: ` +
        `${[...langs].join(',')}`);
    langs = new Set();
    const adaptationSet = this.currentAdaptationSetCriteria_.create(
        playableVariants);
    for (const v of adaptationSet.values()) {
      langs.add('videoId=' + v.video.id + ';' + v.audio.label + ';' + v.audio.language);
    }
    console.log(`[APPLE, PreloadManager]: after filtering variants based on adaptationSetCriteria: ` +
        `${[...langs].join(',')}`);
  ...
  static filterForAVVariants_(manifest) {
    const isAVVariant = (variant) => {
      ...
      const res = (variant.video && variant.audio) ||
             (variant.video && variant.video.codecs.includes(','));
      if (!res) {
        console.log(`[APPLE, PreloadManager]: ${variant.audio.language};${variant.audio.label} is not an AV variant! Removing...`);
      }
      return res;
    };
