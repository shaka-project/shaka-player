...
shaka.media.PreloadManager = class extends shaka.util.FakeEventTarget {
  ...
  constructor(assetUri, mimeType, startTime, playerInterface) {
     ...
     */
    this.scte35Timeline_ = typedPlayerInterface.scte35Timeline ||
        new shaka.media.RegionTimeline(() => ({start: 0, end: Infinity}));

    /**
     * Whether playback has taken ownership of the timeline above.  While this
     * is false, the preload is its only owner and releases it on destroy().
     * @private {boolean}
     */
    this.scte35TimelineEntrusted_ = false;
    ...
    /** @private {function(!shaka.extern.Manifest)} */
    this.onManifestParsed_ = typedPlayerInterface.onManifestParsed;
  ...
  /**
   * Hands the SCTE-35 timeline over to playback, which owns it from then on.
   *
   * A preload discovers messages before playback exists, so it has to create
   * and own the timeline itself.  Once playback adopts it the timeline
   * outlives this preload, and destroy() must leave it alone; an abandoned
   * preload is still the only owner and releases it.
   *
   * @return {!shaka.media.RegionTimeline<shaka.extern.Scte35Event>}
   */
  receiveScte35Timeline() {
    this.scte35TimelineEntrusted_ = true;
    return this.scte35Timeline_;
  }
  ...
  async parseManifestInner_() {
    ...
    if (!this.manifest_) {
      ...
      // Let the player apply manifest-level configuration (for example
      // MPD-signaled CMCD parameters) before the initial variant's segment
      // index is fetched. A manager destroyed while the parser was running
      // has a manifest that will never play, so it stays out of the
      // player's live configuration.
      if (!this.isDestroyed()) {
        this.onManifestParsed_(this.manifest_);
      }
  ...
  async destroy() {
    ...
    if (!this.scte35TimelineEntrusted_) {
 ...
 *       shaka.extern.TimelineRegionInfo>,
 *   scte35Timeline: (!shaka.media.RegionTimeline<
 *       shaka.extern.Scte35Event>|undefined),
 ...
 *   onManifestParsed: function(!shaka.extern.Manifest),
 * }}
 *
 ...
 *           } regionTimeline
 * @property {(!shaka.media.RegionTimeline<shaka.extern.Scte35Event>
 *           |undefined)} scte35Timeline
 ...
 * @property {function(!shaka.extern.Manifest)} onManifestParsed
 *   Called at most once, after the manifest has been parsed and before the
 *   initial variant is chosen (and therefore before any segment index is
 *   fetched), so manifest-level configuration such as CMCD parameters can
 *   take effect for the very first media requests. Not called when the
 *   manifest was supplied through attachManifest(), nor when this manager
 *   was destroyed while the parser was running.
 */
