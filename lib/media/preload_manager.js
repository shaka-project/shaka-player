  ...
  async makePrefetchForStream_(stream, isLive) {
    ...
    const prefetch = new shaka.media.SegmentPrefetch(
        prefetchLimit, stream, (reference, stream, streamDataCallback) => {
          shaka.log.debug('PRELOAD OWNED FETCHING',
              stream.type, reference.getUris());
    ...
    if (prefetchSegment) {
      if (isLive) {
        ...
        if (prefetchSegment.initSegmentReference) {
          shaka.log.debug('PRELOAD INIT SEGMENT', stream.type);
        ...
        shaka.log.debug('PRELOAD MEDIA SEGMENTS BY TIME',
            prefetchSegment.startTime, stream.type,
            {
              method: prefetch.prefetchSegmentsByTime,
              obj: prefetch,
            });
