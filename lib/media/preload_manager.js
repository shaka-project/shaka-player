...
shaka.media.PreloadManager = class extends shaka.util.FakeEventTarget {
  ...
  /**
   * Hands this PreloadManager an already-initialized DrmEngine to use
   * instead of negotiating its own (skips initializeDrm()'s guard above).
   * Marks it entrusted, matching receiveDrmEngine(), so destroy() won't
   * tear it down.
   * @param {!shaka.drm.DrmEngine} drmEngine
   */
  entrustDrmEngine(drmEngine) {
    this.drmEngine_ = drmEngine;
    this.manifestFilterer_.setDrmEngine(drmEngine);
    this.drmEngineEntrusted_ = true;
  }
