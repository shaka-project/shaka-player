...
shaka.media.PreloadManager = class extends shaka.util.FakeEventTarget {
  ...
  async reconfigurePrefetched_() {
    ...
    const clearCurrentTextStream = () => {
      if (this.prefetchedTextStream_) {
        if (this.segmentPrefetchById_.has(this.prefetchedTextStream_.id)) {
          this.segmentPrefetchById_.get(
              this.prefetchedTextStream_.id).clearAll();
          this.segmentPrefetchById_.delete(this.prefetchedTextStream_.id);
        }
        this.prefetchedTextStream_ = null;
      }
    };
