...
shaka.media.PreloadManager = class extends shaka.util.FakeEventTarget {
  ...
  addQueuedOperation(callback) {
    if (this.queuedOperations_) {
      this.queuedOperations_.push(callback);
    } else {
      callback();
    }
  }
  ...
  chooseInitialVariantInner_() {
    ...
    for (const variant of adaptationSet.values()) {
      ...
      for (const stream of [variant.video, variant.audio]) {
        if (stream) {
          this.makePrefetchForStream_(stream, isLive);
        }
      }
    }
  }
  ...
  async makePrefetchForStream_(stream, isLive) {
    const prefetchLimit = isLive ? 1 : 2;
    ...
    if (prefetchSegment) {
      ...
      // Preload a segment, too... either the first segment, or the segment that
      // corresponds with this.startTime_, as appropriate.
      prefetch.prefetchSegments(prefetchSegment);
    }
  }
