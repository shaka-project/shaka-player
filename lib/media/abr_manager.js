...
shaka.media.AbrManager = function(estimator, videoSource) {
  ...
  /**
   * The timestamp after which we are allowed to adapt, in seconds.
   * @private {number}
   */
 ...
 * seconds.  This gives the bandwidth estimator time to get some real data
 ...
 * seconds.  This keeps us from checking for adaptation opportunities too often.
 *
 * @private
 * @const {number}
 */
...
shaka.media.AbrManager.prototype.onBandwidth_ = function(event) {
  ...
  if (chosen) {
    if (chosen.active) {
      ...
      this.nextAdaptationTime_ = now + AbrManager.MIN_EVAL_INTERVAL_;
      return;
    }
