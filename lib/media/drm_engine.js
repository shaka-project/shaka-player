...
shaka.media.DrmEngine.prototype.destroy = function() {
  ...
  this.activeSessions_.forEach(function(activeSession) {
    ...
    let closeTimeout = shaka.media.DrmEngine.timeout_(1000);
...
shaka.media.DrmEngine.prototype.prepareMediaKeyConfigs_ =
  ...
  if (clearKeyDrmInfo) {
    shaka.media.DrmEngine.overrideDrmInfo_(manifest, [clearKeyDrmInfo]);
  }
  ...
  if (!isEncryptedContent) {
    ...
    if (overrideDrmInfos.length) {
      shaka.media.DrmEngine.overrideDrmInfo_(manifest, overrideDrmInfos);
    }
  }
  ...
  manifest.periods.forEach((period) => {
    period.variants.forEach((variant) => {
      this.prepareMediaKeyConfigsForVariant_(
          variant, offline, configsByKeySystem, keySystemsInOrder);
    });
  });
};
...
/**
 * @param {shaka.extern.Manifest} manifest
 * @param {!Array.<shaka.extern.DrmInfo>} info
 * @private
 */
shaka.media.DrmEngine.overrideDrmInfo_ = function(manifest, info) {
  manifest.periods.forEach((period) => {
    period.variants.forEach((variant) => {
      variant.drmInfos = info;
    });
  });
};
...
shaka.media.DrmEngine.prototype.prepareMediaKeyConfigsForVariant_ =
    function(variant, offline, configsByKeySystem, keySystemsInOrder) {
      ...
      .then(function(response) {
        ...
        return session.update(response.data).then(function() {
          ...
          if (activeSession) {
            ...
            shaka.media.DrmEngine.timeout_(5000).then(() => {
              activeSession.loaded = true;
              if (this.activeSessions_.every((s) => s.loaded)) {
                this.allSessionsLoaded_.resolve();
              }
...
/**
 * Create a promise that will be resolve after the given amount of time as
 * elapsed.
 *
 * @param {number} ms
 * @return {!Promise}
 */
shaka.media.DrmEngine.timeout_ = function(ms) {
