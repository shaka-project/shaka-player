...
shaka.media.DrmEngine = class {
  ...
  initForStorage(variants, usePersistentLicenses, useMediaCapabilities) {
    // this.initializedForStorage_ = true; //Modified by sridhar
   ...
   * @return {!Promise}
   */
  initForPlayback(variants, offlineSessionIds, usePersistentLicenses,
      useMediaCapabilities) {
    this.offlineSessionIds_ = offlineSessionIds;

    this.usePersistentLicenses_ = usePersistentLicenses ||
     (offlineSessionIds.length > 0);
  ...
  async removeSession(sessionId) {
    ...
    // sridhar modified
    const session = await this.loadOfflineSession_(sessionId, null);
  ...
  createOrLoad() {
    ...
    if (this.offlineSessionIds_.length <= 0) {
    ...
    for (const sessionId of this.offlineSessionIds_) {
      this.loadOfflineSession_(sessionId,
        initDatas.length > 0 ? initDatas[0].initData : null);
    }
    // Modified by sridhar - end
  ...
  /**
   * @param {string} sessionId
   * @param {null|Uint8Array} initData
   * @return {!Promise.<MediaKeySession>}
   * @private
   */
  async loadOfflineSession_(sessionId, initData) {
    // Modified by sridhar - end
    let session;
    ...
    const metadata = {
      initData: initData ? initData : null, // Modified by sridhar
      loaded: false,
  ...
  async sendLicenseRequest_(event) {
    ...
    // Modified by sridhar - start
    const msgLen = event.message.byteLength;
    if ((msgLen > 4) && window && window['curseldrm'] &&
      (window['curseldrm'][0] === 'l') && (window['curseldrm'][1] === 'g') &&
      (window['curseldrm'][2] === 'd') && (window['curseldrm'][3] === 'r') &&
      (window['curseldrm'][4] === 'm')) {
      const uint8Msg = shaka.util.BufferUtils.toUint8(event.message);
      const newArrBufMsg = new ArrayBuffer(msgLen + 10);
      const newUint8Msg = shaka.util.BufferUtils.toUint8(newArrBufMsg);

      for (let idx = 0; idx < msgLen; idx++) {
        newUint8Msg[idx] = uint8Msg[idx];
      }

      newUint8Msg[msgLen + 0] = 0xac;
      newUint8Msg[msgLen + 1] = 0xa9;
      newUint8Msg[msgLen + 2] = 0x6c;
      newUint8Msg[msgLen + 3] = 0x45;
      newUint8Msg[msgLen + 4] = (window && window['iswtrmktprd']) ? 1 : 0;
      newUint8Msg[msgLen + 5] = (window && window['iscdetprd']) ? 1 : 0;
      newUint8Msg[msgLen + 6] = (window && window['isinbrwr']) ? 1 : 0;
      newUint8Msg[msgLen + 7] = 0x00;
      newUint8Msg[msgLen + 8] = 0x00;
      newUint8Msg[msgLen + 9] = 0x00;

      request.body = newArrBufMsg;
    } else {
