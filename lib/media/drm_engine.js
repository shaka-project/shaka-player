...
shaka.media.DrmEngine = class {
  ...
  async queryMediaKeys_(configsByKeySystem, variants, useMediaCapabilities) {
    ...
    try {
      ...
      shaka.log.error('createMediaKeys - 940');
  ...
  async getKeySystemAccessByConfigs_(configsByKeySystem) {
    ...
    for (const shouldHaveLicenseServer of [true, false]) {
      for (const keySystem of configsByKeySystem.keys()) {
        ...
        try {
          shaka.log.error('requestMediaKeySystemAccess - 1070');
  ...
  async loadOfflineSession_(sessionId, initData) {
    ...
    try {
      ...
      shaka.log.error('createSession - 1158');
  ...
  createTemporarySession_(initDataType, initData) {
    ...
    try {
      ...
      shaka.log.error('createSession - 1238');
      session = this.mediaKeys_.createSession(sessionType);
    } catch (exception) {
      this.onError_(new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
    ...
    shaka.log.error('generateRequest - 1285');
  ...
  async sendLicenseRequest_(event) {
    ...
    try {
      ...
      shaka.log.error('session.update - 1434');
  ...
  onKeyStatusesChange_(event) {
    ...
    if (msUntilExpiration < 0 || (hasExpiredKeys && msUntilExpiration < 1000)) {
      ...
      if (found && !found.updatePromise) {
        ...
        this.activeSessions_.delete(session);
        shaka.log.error('session.close - 1612');
  ...
  static async probeSupport() {
    ...
    const testSystem = async (keySystem) => {
      try {
        shaka.log.error('requestMediaKeySystemAccess - 1714');
        ...
        shaka.log.error('createMediaKeys - 1736');
  ...
  async closeSession_(session) {
    ...
    shaka.log.error('DDDD: Closesesion');

    try {
      shaka.log.error('session.close 1787');
      await Promise.race([
        Promise.all([session.close(), session.closed]),
        timeout,
      ]);
      /* sridhar changed */
      window['shakaSessionCloseStatus'] = 'success';
      shaka.log.error('DDDD: Closesesion success');
    } catch (e) {
      /* sridhar changed */
      shaka.log.error('Timeout waiting for session close!!!!');
      window['shakaSessionCloseStatus'] = 'failed';
      shaka.log.error('DDDD: Closesesion failed');
    }
  }
