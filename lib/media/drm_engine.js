...
shaka.media.DrmEngine.prototype.createTemporarySession_ =
  ...
  this.activeSessions_.set(session, metadata);
    ...
    try {
      if (this.config_.initDataTransform) {
        initData = this.config_.initDataTransform(initData);
      } else if (this.keySystem().startsWith('com.apple.fps')) {
        const cert = this.currentDrmInfo_.serverCertificate;
        const contentId =
