...
shaka.media.DrmEngine.prototype.destroy = function() {
  ...
  if (this.video_) {
    shaka.asserts.assert(!this.video_.src, 'video src must be removed first!');
...
shaka.media.DrmEngine.prototype.unpackPlayReadyRequest_ = function(request) {
  ...
  for (var i = 0; i < headers.length; ++i) {
    ...
    shaka.asserts.assert(name && value, 'Malformed PlayReady headers!');
  ...
  shaka.asserts.assert(challenge, 'Malformed PlayReady challenge!');
  shaka.asserts.assert(challenge.getAttribute('encoding') == 'base64encoded',
