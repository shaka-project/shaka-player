...
shaka.media.DrmEngine = class {
  ...
  onSessionMessage_(event) {
    shaka.log.debug('*** onSessionMessage_ ***',event);
  ...
  async sendLicenseRequest_(event) {
    ...
    shaka.log.debug('***session***',session);
    shaka.log.debug(
    ...
    try {
      ...
      // inject license failures
      this.licenseRequestCount_ ++;
      shaka.log.error('Injected error count ***',  this.licenseRequestCount_);
      if ((this.licenseRequestCount_ % this.failEveryNRequests_) == 0){
        // shaka.log.error('Injected failure of the license request ***', request);
        request.uris[0] = request.uris[0] + '_BAD';
        shaka.log.error('Injected failure of the license request ***');
      }
      ...
      // we could assume that a 403 us a clear rejection of the license request
      // this.onError_(shakaErr);
      shaka.log.error('License request failed or rejected ***', this.currentDrmInfo_);
      
      session.close().catch((e) => {
        shaka.log.debug('closing session errored ***', e);
      }); 
      
      session.remove().catch((e) => {
        shaka.log.debug('removing session errored ***', e);
      }); 
      ...
      // set the metadata loaded so areAllSessionsLoaded can succeed
      if (metadata){
        metadata.loaded = true;
      }
      ...
      // are all sessions completed?
      if (this.areAllSessionsLoaded_()) {
        this.allSessionsLoaded_.resolve();
        return;
      }

      // unsure of the value of this, a catch all
      const timer = new shaka.util.Timer(() => {
        metadata.loaded = true;
        
        shaka.log.debug('Session load timeout ***', this.areAllSessionsLoaded_());
        if (this.areAllSessionsLoaded_()) {
          this.allSessionsLoaded_.resolve();
        }
      });
