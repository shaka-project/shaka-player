...
shaka.media.DrmEngine.prototype.onWaitingForKey_ = function(event) {
  ...
  var statuses = shaka.util.MapUtils.values(this.keyStatusByKeyId_);
  var hasExpiredKeys = statuses.some(function(status) {
    return status == 'expired';
  });

  // If we have expired keys, odds are good that's why we got 'waitingforkeys'.
  // If not, the license server may be serving the wrong keys.
  // In either case, playback cannot continue, so dispatch an error.
  var code = hasExpiredKeys ?
      shaka.util.Error.Code.EXPIRED : shaka.util.Error.Code.WRONG_KEYS;
  this.onError_(new shaka.util.Error(shaka.util.Error.Category.DRM, code));
};
...
shaka.media.DrmEngine.prototype.onKeyStatusesChange_ = function(event) {
  for (var i = 0; i < this.activeSessions_.length; ++i) {
    var session = this.activeSessions_[i].session;
