...
shaka.media.DrmEngine.prototype.sendLicenseRequest_ = function(event) {
  ...
  // FairPlay key system IDs all start with com.apple.fps, followed by an
  // optional suffix indicating a FairPlay version.
  if (this.currentDrmInfo_.keySystem.indexOf('com.apple.fps') == 0) {
    this.formatFairPlayRequest_(request, event['contentId']);
  }
...
/**
 * Format FairPlay license requests.  Modifies the request object.
 * @param {shakaExtern.Request} request
 * @param {string} contentId
 * @private
 */
shaka.media.DrmEngine.prototype.formatFairPlayRequest_ =
    function(request, contentId) {
  // The FairPlay license message as it comes from the CDM & the polyfill can't
  // be directly delivered to a license server.  It must be formatted into a
  // URL-encoded string, with parameters spc=BASE64_ENCODED_REQUEST and
  // assetId=URL_ENCODED_CONTENT_ID.
  var spc = shaka.util.Uint8ArrayUtils.toBase64(new Uint8Array(request.body));
  var assetId = encodeURIComponent(contentId);
  var params = 'spc=' + spc + '&assetId=' + assetId;
  shaka.log.debug('FairPlay message', new Uint8Array(request.body));
  shaka.log.debug('FairPlay request params', params);
