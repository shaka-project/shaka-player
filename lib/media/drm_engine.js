...
shaka.media.DrmEngine = class {
  ...
  constructor(playerInterface) {
     ...
     * ourselves, this is used to ensure that subsequent calls to |destroy|
     * don't resolve before the first call to |destroy|.
     *
     * @private {!shaka.util.PublicPromise}
     */
  ...
  /** @override */
  async destroy() {
    // If we have started destroying ourselves, wait for the common "I am
    // finished being destroyed" promise to be resolved.
  ...
  sendLicenseRequest_(event) {
    ...
    this.playerInterface_.netEngine.request(requestType, request).promise
        .then((response) => {
          ...
          return session.update(response.data).then(() => {
   ...
   * @private
   */
  formatFairPlayRequest_(request) {
   ...
   * @private
   */
  parseFairPlayResponse_(response) {
