...
shaka.media.DrmEngine = class {
  ...
  /**
   * Returns the license request time in seconds, or NaN.
   *
   * @return {number}
   */
  getLicenseTime() {
    if (this.licenseRequestTime_) {
      return this.licenseRequestTime_ / 1000;
    }
    return NaN;
  }
  ...
  async sendLicenseRequest_(event) {
    ...
    this.licenseRequestTime_ += Date.now() - startTimeRequest;
