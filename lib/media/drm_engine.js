...
shaka.media.DrmEngine = class {
  ...
  /**
   * Returns the initData of the sessions currently active.
   *
   * @return {!Array.<string>}
   */
  getInitDatas() {
    const sessions = this.activeSessions_.values();
    const initDatas = shaka.util.Iterables.map(sessions, (s) => s.initData);

    // TODO: Make |getInitDatas| return |Iterable| instead of |Array|.
    return Array.from(initDatas);
  }
