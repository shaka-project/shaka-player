...
shaka.media.DrmEngine = class {
  ...
  static async probeSupport() {
    ...
    const processMediaKeySystemAccess = async (keySystem, access) => {
      ...
      const returnedRobustness = videoCapabilities[0].robustness;
      if (returnedRobustness &&
          !supportValue.robustnessLevels.includes(returnedRobustness)) {
        supportValue.robustnessLevels.push(returnedRobustness);
      }
    };

    const testSystemEme = async (keySystem, encryptionScheme, robustness) => {
      try {
        const basicConfig =
    ...
    const testSystemMcap = async (keySystem, encryptionScheme, robustness) => {
      try {
