...
shaka.media.DrmEngine = class {
  ...
  async attachMediaKeys_() {
    ...
    try {
      await this.video_.setMediaKeys(this.mediaKeys_);
    } catch (exception) {
  ...
  async attach(video) {
     ...
     */
    if (nonEmptyInitData || shaka.util.Platform.willMediaKeysBePolyfilled()) {
      await this.attachMediaKeys_();
    }
  ...
  createSession(initDataType, initData, sessionType) {
     ...
     */
    if (shaka.util.Platform.willMediaKeysBePolyfilled()) {
      try {
