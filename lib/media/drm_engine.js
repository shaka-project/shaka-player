...
shaka.media.DrmEngine = class {
  ...
  unpackPlayReadyRequest_(request) {
    ...
    for (const header of headers) {
      ...
      goog.asserts.assert(name && value &&
          name.innerText && value.innerText, 'Malformed PlayReady headers!');
      request.headers[name.innerText] = value.innerText;
    }

    // Unpack the base64-encoded challenge.
    const challenge = TXml.getElementsByTagName(dom, 'Challenge')[0];
    goog.asserts.assert(challenge && challenge.innerText,
    ...
    request.body = shaka.util.Uint8ArrayUtils.fromBase64(challenge.innerText);
  }
