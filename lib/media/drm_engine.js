...
shaka.media.DrmEngine = function(networkingEngine, onError, onKeyStatus) {
  ...
  /** @private {shaka.util.Timer} */
  this.processWaitingForKeyTimer_ = new shaka.util.Timer(
      this.processWaitingForKey_.bind(this));
...
shaka.media.DrmEngine.prototype.destroy = function() {
  ...
  if (this.processWaitingForKeyTimer_) {
    this.processWaitingForKeyTimer_.cancel();
  }
  this.processWaitingForKeyTimer_ = null;
...
shaka.media.DrmEngine.prototype.onWaitingForKey_ = function(event) {
  if (this.activeSessions_.some(function(s) { return !s.loaded; })) {
    ...
    shaka.log.debug('Suppressed waitingforkey, still loading');
    return;
  }

  // Wait to process this event so that key status changes have time to
  // propagate before we read them.
  this.processWaitingForKeyTimer_.schedule(0.5 /* seconds */);
};


/** @private */
shaka.media.DrmEngine.prototype.processWaitingForKey_ = function() {
  var allExpired = shaka.util.MapUtils.every(
      this.keyStatusByKeyId_, function(keyId, status) {
        return status == 'expired';
      });
  if (allExpired) {
    shaka.log.debug('Suppressed waitingforkey, expired');
    // If all keys are expired, odds are good that's why we got 'waitingforkey'.
    // Since we've already fired an EXPIRED error, don't also fire WRONG_KEYS.
    return;
  }
  var anyRestricted = shaka.util.MapUtils.some(
      this.keyStatusByKeyId_, function(keyId, status) {
        return status == 'output-restricted';
      });
  if (anyRestricted) {
    shaka.log.debug('Suppressed waitingforkey, output-restricted');
    // If any of the keys are output-restricted, it's possible that we are stuck
    // due to unmet restrictions.  The Player and ABR manager will decide if
    // playback can continue and fire an appropriate error if not.  Don't also
    // fire WRONG_KEYS, as this would be misleading.
    // Note that in the case of output restrictions, we only sometimes get an
    // additional 'waitingforkeys' event after the license is loaded.
    return;
  }
  this.onError_(new shaka.util.Error(
      shaka.util.Error.Category.DRM,
...
shaka.media.DrmEngine.prototype.onKeyStatusesChange_ = function(event) {
  // This must run right away to resolve the 'loading' state of sessions before
  // a subsequent 'waitingforkeys'.  Otherwise, we might suppress that event and
  // be unable to report WRONG_KEYS.

  // Make 'waitingforkeys' wait a little longer so we can process all key status
  // changes before we look at the statuses in processWaitingForKey_().
  this.processWaitingForKeyTimer_.rescheduleIfRunning();
