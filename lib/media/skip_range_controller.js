...
shaka.media.SkipRangeController = class {
   ...
   * range must not push the frontier past it, or it stalls at the buffered edge.
   * The per-segment skip decision instead uses containingActiveRange, which is
   * buffer-independent -- see there.
   *
   * @param {number} time
   * @return {?shaka.media.SkipRangeController.SkipRange}
   */
  ...
  /**
   * The skip range a concrete segment [startTime, endTime) may be carved out of,
   * or null.  Backs the per-segment skip/discard decisions on every stream and
   * entry point (fetch frontier, append guard, prefetch).
   *
   ...
   * gap-jump can't clear (the seek-into-range stall).  A range is carveable only
   * once the video grid confirms it aligns to segment boundaries on both ends
   * (markAlignable); until resolved, or if unalignable, this returns null.
   *
   ...
   * defer.  True while its containing range is alignable or unresolved (may yet
   * be carved); false once the range is confirmed unalignable (it will be kept).
   *
   * @param {number} startTime
   * @param {number} endTime
   * @return {boolean}
   ...
   * @return {boolean} Whether the video grid has resolved this range's boundary
   *   alignment yet (in either alignable_ or unalignable_).
   */
  isResolved(region) {
   ...
   * carveable; misaligned ranges are never skipped and warn once.  Runs once
   * per range.
   *
   * @param {shaka.media.SkipRangeController.SkipRange} region
   * @param {?shaka.media.SegmentReference} startSeg Segment covering region.start.
   * @param {?shaka.media.SegmentReference} endSeg Segment covering region.end.
   */
  resolveAlignment(region, startSeg, endSeg) {
    const NumberUtils = shaka.util.NumberUtils;
    const tolerance = shaka.media.SkipRangeController.TOLERANCE_SECONDS;

    if (!startSeg ||
        !NumberUtils.isFloatEqual(startSeg.startTime, region.start, tolerance)) {
      this.markUnalignable_(region);
      return;
    }
    let alignedEnd = null;
    if (endSeg &&
        NumberUtils.isFloatEqual(endSeg.startTime, region.end, tolerance)) {
      alignedEnd = endSeg.startTime;
    } else if (endSeg &&
        NumberUtils.isFloatEqual(endSeg.endTime, region.end, tolerance)) {
      alignedEnd = endSeg.endTime;
    }
    if (alignedEnd == null) {
      this.markUnalignable_(region);
      return;
    }
    this.markAlignable_(region, startSeg.startTime, alignedEnd);
  }

  /**
   * Snaps |region|'s endpoints to the aligned boundaries (so merge/split stay
   * aligned) and marks it carveable.
   *
   * @param {shaka.media.SkipRangeController.SkipRange} region
   * @param {number} start
   * @param {number} end
  ...
  /**
   * Marks |region| non-carveable and warns once (visible by default) that the
