...
shaka.media.TextTracksManager.prototype.bufferedAheadOf =
  ...
  if ((this.textTracksSource_ || []).length) {
    try {
      ...
      var objLastInserted = this.inserted_
        .slice(0)
        .filter(function (segmentReference) {
          return segmentReference.url.toString().indexOf(objTrack.label) !== -1;
        })
      ...
      if (!objLastInserted) {
        shaka.log.debug('Cannot figure out last added text track, can lead to buffer overrun');
      }

      numDifference = objLastInserted && objLastInserted.endTime - timestamp || 0;

      shaka.log.debug('Text Tracks Manager: bufferedAheadOf', numDifference, objLastInserted);
    } catch (e) {
      numDifference = Number.POSITIVE_INFINITY;
