...
shaka.media.VideoWrapper = function(video, onSeek, startTime) {
  ...
  this.seekTimer_ = new shaka.util.Timer(onSeek);
  ...
  /**
   * The move playhead timer is used to force the playhead to move. Sometimes,
   * IE and Edge ignore re-seeks.  Check every 100ms and try again if need be,
   * up to |remainingMoveAttempts_|.
   *
   ...
   * @private {shaka.util.Timer}
   * */
  this.movePlayheadTimer_ = new shaka.util.Timer(() => {
    if (this.remainingMoveAttempts_ <= 0) {
      this.movePlayheadTimer_.stop();
      return;
    }

    if (this.video_.currentTime != this.oldTime_) {
      this.movePlayheadTimer_.stop();
      return;
    }

    // Sigh... Try again...
    this.video_.currentTime = this.newTime_;
    this.remainingMoveAttempts_--;
  });
...
shaka.media.VideoWrapper.prototype.release = function() {
  ...
  if (this.movePlayheadTimer_ != null) {
    this.movePlayheadTimer_.stop();
    this.movePlayheadTimer_ = null;
  }

  if (this.seekTimer_ != null) {
...
shaka.media.VideoWrapper.prototype.setTime = function(time) {
  if (this.video_.readyState > 0) {
    ...
    // The video element is not ready, try again by seeking next interpreter
    // cycle.
    this.startTime_ = time;
    this.seekTimer_.tickAfter(/* seconds= */ 0);
  }
};
...
shaka.media.VideoWrapper.prototype.onSeekingToStartTime_ = function() {
  ...
  this.eventManager_.listen(
      this.video_, 'seeking', () => this.seekTimer_.tickNow());
};
...
shaka.media.VideoWrapper.prototype.movePlayhead_ = function(
  ...
  // Reset the values that the timer will use to force the playhead to
  // |targetTime|.
  this.oldTime_ = currentTime;
  this.newTime_ = targetTime;
  this.remainingMoveAttempts_ = 10;

  this.movePlayheadTimer_.tickEvery(/* seconds= */ 0.1);
};
