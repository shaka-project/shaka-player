...
shaka.media.Playhead = function(
  ...
  var pollGap = this.onPollGapJump_.bind(this, /* segmentAppended */ false);
...
shaka.media.Playhead.prototype.onSegmentAppended = function() {
  this.onPollGapJump_(/* segmentAppended */ true);
};
 ...
 *
 * @param {boolean} segmentAppended
 * @private
 */
shaka.media.Playhead.prototype.onPollGapJump_ = function(segmentAppended) {
  if (this.video_.readyState == 0)
    return;
  ...
  if (gapIndex == null) {
    for (var i = 0; i < buffered.length; i++) {
      if (currentTime >= buffered.start(i) &&
          currentTime < buffered.end(i) - 0.1 && this.video_.readyState < 3) {
        // Some platforms/browsers can get stuck in the middle of a buffered
        // range (e.g. when seeking in a background tab).  Flush the media
        // pipeline to help.
        shaka.log.debug(
