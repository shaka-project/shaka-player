...
shaka.media.MediaSourcePlayhead = class {
   ...
   * @param {(function():number)=} getPlaybackRate
   *     Returns the current playback rate.  A negative value means the content
   *     is being played in reverse.
   * @param {function():!Array<{start: number, end: number}>=} getSkipRanges
   *     Returns the active skip ranges; a seek landing inside one is redirected
   *     to its end.  See shaka.Player#addSkipRange.
   * @param {function()=} onMediaSourceResetEnd
   *     Called after Playhead handles the seek that restores currentTime.
   */
  constructor(mediaElement, manifest, config, startTime, onSeek, onEvent,
      getPlaybackRate, getSkipRanges, onMediaSourceResetEnd) {
    /**
    ...
    /** @private {?number} */
    this.mediaSourceResetTime_ = null;

    /** @private {boolean} */
    this.mediaSourceResetEndPending_ = false;

    /** @private {function()} */
    this.onMediaSourceResetEnd_ = onMediaSourceResetEnd || (() => {});

    /** @private {function():!Array<{start: number, end: number}>} */
    this.getSkipRanges_ = getSkipRanges || (() => []);

    /** @private {function():number} */
    this.getPlaybackRate_ =
  ...
  /**
   * Notify the playhead that MediaSource is being reset. While the reset is in
   * progress, the media element's currentTime may temporarily be invalid.
   *
   * @param {number} time
   */
  notifyOfMediaSourceReset(time) {
    this.mediaSourceResetTime_ = time;
    this.mediaSourceResetEndPending_ = false;
  }

  /**
   * Notify the playhead that MediaSource has restored its currentTime.
   */
  notifyOfMediaSourceResetEnd() {
    if (this.mediaElement_.seeking) {
      this.mediaSourceResetEndPending_ = true;
    } else {
      this.finishMediaSourceReset_();
    }
  }
  ...
  /** @override */
  getTime() {
    if (this.mediaSourceResetTime_ != null) {
      return this.mediaSourceResetTime_;
    }
  ...
  getStartTime_(startTime) {
    if (startTime == null) {
      if (!this.timeline_.isDynamic()) {
        // If the presentation is VOD, then start from the beginning.
   ...
   */
  onPollWindow_() {
    // Replacing the MediaSource temporarily resets currentTime. Let
    // MediaSourceEngine restore it before enforcing the live window.
    if (this.mediaSourceResetTime_ != null) {
      return;
    }
  ...
  onSeeking_() {
    ...
    if (this.mediaSourceResetTime_ != null) {
      // Ignore transient seeks caused while replacing MediaSource. Once
      // currentTime has been restored, forward that seek while the saved time
      // is still visible to Playhead and then complete the reset lifecycle.
      if (this.mediaSourceResetEndPending_) {
        this.onSeek_();
        this.finishMediaSourceReset_();
      }
      return;
  ...
  /**
   * Completes the MediaSource reset lifecycle.
   *
   * @private
   */
  finishMediaSourceReset_() {
    this.mediaSourceResetTime_ = null;
    this.mediaSourceResetEndPending_ = false;
    this.onMediaSourceResetEnd_();
  }
  ...
  reposition_(currentTime) {
    ...
    // A forward seek into a skip range lands in an unbuffered hole; redirect
    // to the range end.  Forward only -- in reverse the end is behind us.
    if (this.getPlaybackRate_() > 0) {
      for (const region of this.getSkipRanges_()) {
        if (currentTime >= region.start && currentTime < region.end) {
          shaka.log.v1('Playhead in a skip range; seeking to its end.');
          return region.end;
        }
      }
    }
