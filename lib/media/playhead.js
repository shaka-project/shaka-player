...
shaka.media.Playhead.prototype.getStartTime_ = function(startTime) {
  ...
  if (startTime == null) {
    ...
    // For live stream, if the startTime is negative, start from a certain
    // offset time from the live edge. If the offset from live is not available,
    // start from the current available segment start point instead, handled by
    // clampTime_().
...
shaka.media.Playhead.prototype.onPollWindow_ = function() {
  ...
  // To avoid repeatedly seeking with extremely small seek ranges, allow us to
  // be within a small amount of the seek range.
  const MIN_SEEK_RANGE = 0.2;
  if (seekEnd - seekStart < MIN_SEEK_RANGE) {
    seekEnd += MIN_SEEK_RANGE / 2;
    seekStart -= MIN_SEEK_RANGE / 2;
  }
