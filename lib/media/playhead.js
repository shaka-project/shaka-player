...
shaka.media.Playhead.prototype.reposition_ = function(currentTime) {
  ...
  // Browsers behave differently when you seek to the end of the video. Chrome
  // will not accept you have reached the end of the video if you set current
  // time to duration. Firefox will throw an error if you set currentTime to
  // duration. So if someone is seeking to the end of the video, adjust the time
  // to be just before the duration.
  if (this.video_.duration) {
    end = Math.min(end, this.video_.duration - 0.01);
  }
