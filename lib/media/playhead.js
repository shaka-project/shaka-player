...
shaka.media.Playhead.prototype.onPollWindow_ = function() {
  ...
  // paused.
  if (this.video_.readyState == 0 || this.video_.seeking || this.video_.paused)
    return;
...
shaka.media.Playhead.prototype.reposition_ = function(currentTime) {
  ...
  let timeline = this.manifest_.presentationTimeline;
  let start = timeline.getSafeAvailabilityStart(0);
  ...
  let safe = timeline.getSafeAvailabilityStart(rebufferingGoal);
  ...
  let seekStart = timeline.getSafeAvailabilityStart(5);
  let seekSafe = timeline.getSafeAvailabilityStart(rebufferingGoal + 5);
...
shaka.media.Playhead.prototype.clampTime_ = function(time) {
  let start = this.manifest_.presentationTimeline.getSegmentAvailabilityStart();
  if (time < start) return start;

  let end = this.manifest_.presentationTimeline.getSegmentAvailabilityEnd();
