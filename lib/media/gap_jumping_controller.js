...
shaka.media.GapJumpingController = class {
   ...
   *     is being played in reverse (during reverse trick play the media
   *     element's playbackRate is 0, so this is the only source of direction).
   * @param {function():!Array<{start: number, end: number}>=} getSkipRanges
   *     Returns the active skip ranges (see shaka.Player#addSkipRange).  A gap
   *     created by one is jumped only once the playhead reaches its edge, so no
   *     content leading into the range is skipped.
   */
  constructor(video, timeline, config, onEvent, getPlaybackRate,
      getSkipRanges = () => []) {
    /** @private {?function(!Event)} */
    this.onEvent_ = onEvent;

    /** @private {function():number} */
    this.getPlaybackRate_ = getPlaybackRate || (() => video.playbackRate);
  ...
  onPollGapJump_() {
    ...
    // When playing in reverse (reverse trick play) the playhead moves
    // backwards.  This forward gap-jumping logic would fight that motion by
    // pulling the playhead forward over any gap behind it, leaving it stuck at
    // the edge of a buffered range.  Reverse gaps are instead handled by the
    // playhead-advancing logic (see MediaSourcePlayhead's movePlayhead), so
    ...
    // A skip-range gap is jumped at its edge, not up to |gapDetectionThreshold|
    // early, so content leading into the range isn't skipped.  0.1s is a floor;
    // smaller and the poll-stepped playhead never satisfies it.
    if (gapIndex > 0) {
      const tolerance = 0.1;
      const gapEdge = buffered.end(gapIndex - 1);
      const inSkipRange = this.getSkipRanges_().some((region) => {
        return gapEdge >= region.start - tolerance && gapEdge < region.end;
      });
      if (inSkipRange && gapEdge - currentTime > tolerance) {
        return;
      }
    }
