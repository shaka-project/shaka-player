...
goog.require('shaka.event.GapJumped');
goog.require('shaka.event.StallDetected');
...
shaka.media.GapJumpingController = class {
  ...
  onPollGapJump_() {
    ...
    this.gapsJumped_++;
    this.onEvent_(new shaka.event.GapJumped());
  }
  ...
  createStallDetector_() {
    ...
    const onStall = async (at, duration) => {
      ...
      this.onEvent_(new shaka.event.StallDetected());
    };
