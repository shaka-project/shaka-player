...
shaka.media.ClosedCaptionParser = class {
  /**
   * @param {string} streamMimeType
   * @param {string} ceaMimeType
   */
  constructor(streamMimeType, ceaMimeType) {
    /** @private {!shaka.extern.ICeaParser} */
    this.ceaParser_ = new shaka.cea.DummyCeaParser();

    const parserFactory =
        shaka.media.ClosedCaptionParser.findParser(streamMimeType);
    if (parserFactory) {
    ...
    const decoderFactory =
        shaka.media.ClosedCaptionParser.findDecoder(ceaMimeType);
    if (decoderFactory) {
   ...
   * @export
   */
  static registerDecoder(mimeType, plugin) {
    shaka.media.ClosedCaptionParser.decoderMap_[mimeType] = plugin;
  }
   ...
   * @export
   */
  static unregisterDecoder(mimeType) {
    delete shaka.media.ClosedCaptionParser.decoderMap_[mimeType];
  }
   ...
   * @export
   */
  static findDecoder(mimeType) {
    return shaka.media.ClosedCaptionParser.decoderMap_[mimeType];
  }
};
...
/** @private {!Object<string, shaka.extern.CaptionDecoderPlugin>} */
shaka.media.ClosedCaptionParser.decoderMap_ = {};
