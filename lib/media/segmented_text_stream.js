...
shaka.media.SegmentedTextStream.prototype.destroy = function() {
  shaka.log.debug('Stream webvtt/mp4: Destroying SegmentedTextStream');
...
shaka.media.SegmentedTextStream.prototype.hasEnded = function() {
  shaka.log.debug('Stream webvtt/mp4: Cannot determine ending SegmentedTextStream',
...
shaka.media.SegmentedTextStream.prototype.onUpdate_ = function() {
  ...
  if (bufferedAhead >= bufferingGoal) {
    shaka.log.v1('Stream webvtt/mp4: Buffering goal reached.');
  ...
  var reference = reference = this.getNext_(currentTime, segmentIndex);
  if (!reference) {
...
shaka.media.SegmentedTextStream.prototype.isBuffered = function(time) {
  return this.ttm_.isBuffered(time);
}
