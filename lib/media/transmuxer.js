...
shaka.media.Transmuxer = class {
  ...
  static isSupported(mimeType, contentType) {
    ...
    if (isAac) {
      return MediaSource.isTypeSupported(Transmuxer.convertAacCodecs());
    }
  ...
  static convertCodecs(contentType, mimeType) {
    ...
    if (Transmuxer.isAacContainer(mimeType)) {
      return Transmuxer.convertAacCodecs();
    } else if (Transmuxer.isTsContainer(mimeType)) {
      return Transmuxer.convertTsCodecs(contentType, mimeType);
    }
    return mimeType;
  }
