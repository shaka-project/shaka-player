...
shaka.media.SegmentPrefetch = class {
   ...
   * @param {?function(!shaka.media.SegmentReference):boolean=} allowReference
   *     If provided and it returns false for a reference, that reference is
   *     not prefetched but iteration continues with the following one.
   */
  constructor(prefetchLimit, stream, fetchDispatcher, reverse,
      shouldPrefetchNextSegment, getPlaybackRate, allowReference) {
    /** @private {number} */
     ...
     */
    this.getPlaybackRate_ = getPlaybackRate || defaultGetPlaybackRate;

    const defaultAllowReference = () => true;

    /**
     * @private {function(!shaka.media.SegmentReference):boolean}
     */
    this.allowReference_ = allowReference || defaultAllowReference;
  }
  ...
  prefetchSegmentsByTime(currTime, skipFirst = false) {
    ...
    const promises = [];
    const rate = Math.abs(this.getPlaybackRate_());
    const prefetchLimit = Math.ceil(this.prefetchLimit_ * rate);
    while (this.segmentPrefetchMap_.size < prefetchLimit) {
      ...
      // Owner-excluded references (e.g. skip ranges) are stepped over.
      if (!this.allowReference_(reference)) {
        prefetchAllowed = false;
      }
