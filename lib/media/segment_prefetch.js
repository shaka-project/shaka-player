...
shaka.media.SegmentPrefetch = class {
  ...
  prefetchSegmentsByTime(currTime, skipFirst = false) {
    shaka.log.debug('PREFETCH SEGMENTS BY TIME **REAL**', this.stream_.type);
    ...
    if (!iterator) {
      shaka.log.debug(logPrefix, 'TIME NOT FOUND IN INDEX');
      return;
    }
    ...
    if (!reference) {
      shaka.log.debug(logPrefix, 'REFERENCE NOT FOUND UNDER ITERATOR',
          {skipFirst});
      return;
    }
    shaka.log.debug(logPrefix, {
      size: this.segmentPrefetchMap_.size,
      limit: this.prefetchLimit_,
      reference,
    });
    while (this.segmentPrefetchMap_.size < this.prefetchLimit_ &&
      ...
      shaka.log.debug(logPrefix, {prefetchAllowed});
      if (prefetchAllowed && reference.initSegmentReference) {
        shaka.log.debug(logPrefix, 'prefetching init segment');
        this.prefetchInitSegment(reference.initSegmentReference);
      }
      shaka.log.debug(logPrefix, {
        has: this.segmentPrefetchMap_.has(reference),
      });
      if (prefetchAllowed && !this.segmentPrefetchMap_.has(reference)) {
        shaka.log.debug(logPrefix, 'fetching via', {
          fetchDispatch: this.fetchDispatcher_,
        });
  ...
  getPrefetchedSegment(reference, streamDataCallback) {
    ...
    const logPrefix = shaka.media.SegmentPrefetch.logPrefix_(this.stream_);
    shaka.log.debug(logPrefix, 'getting prefetched', reference.getUris());
    ...
    if (prefetchMap.has(reference)) {
      shaka.log.debug(logPrefix, 'has ref');
      const segmentPrefetchOperation = prefetchMap.get(reference);
      if (streamDataCallback) {
        shaka.log.debug(logPrefix, 'setting callback', {streamDataCallback});
  ...
  evict(time) {
    const logPrefix = shaka.media.SegmentPrefetch.logPrefix_(this.stream_);
    shaka.log.debug(logPrefix, 'evict', time);
  ...
  clearInitSegments_() {
    const logPrefix = shaka.media.SegmentPrefetch.logPrefix_(this.stream_);
    shaka.log.debug(logPrefix, 'clearInitSegments_');
...
shaka.media.SegmentPrefetchOperation = class {
  ...
  dispatchFetch(reference, stream) {
    ...
    shaka.log.debug('PREFETCH FETCHING', stream.type, reference.getUris());
    this.operation_ = this.fetchDispatcher_(
        reference, stream, async (data) => {
          if (buffered.byteLength > 0) {
            shaka.log.debug('PREFETCH CONCAT',
                stream.type,
                reference.getUris(),
                shaka.utils.Uint8ArrayUtils.toBase64(data),
                'onto',
                shaka.utils.Uint8ArrayUtils.toBase64(buffered));
            buffered = shaka.util.Uint8ArrayUtils.concat(buffered, data);
          } else {
            shaka.log.debug('PREFETCH REPLACE BUFFER',
                stream.type,
                reference.getUris(),
                shaka.utils.Uint8ArrayUtils.toBase64(data));
            buffered = data;
          ...
          if (this.streamDataCallback_) {
            ...
            shaka.log.debug('PREFETCH STREAM DATA CALLBACK POST',
                stream.type,
                reference.getUris(),
                shaka.utils.Uint8ArrayUtils.toBase64(buffered));
