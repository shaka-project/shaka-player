...
shaka.media.StreamInfo = function() {
  ...
  /**
   * The current segment's start time, in seconds, relative to the start of
   * this stream's period (i.e., in presentation time). A value of null
   * indicates that this stream may be started from any segment.
   * @type {?number}
   */
...
shaka.media.StreamInfo.prototype.getSegmentIndex = function() {
  ...
  if (!this.segmentIndexInfo) {
    var error = new Error(
        'Cannot create segment index without segment index metadata.');
    error.type = 'stream';
