...
shaka.media.SegmentIndex.prototype.merge = function(segmentIndex) {
  if (this.timestampCorrection_ != segmentIndex.timestampCorrection_) {
    ...
    segmentIndex = new shaka.media.SegmentIndex(
        shaka.media.SegmentIndex.shift_(segmentIndex.references_, delta));
  }
...
shaka.media.SegmentIndex.prototype.correct = function(timestampCorrection) {
  ...
  this.references_ =
      shaka.media.SegmentIndex.shift_(this.references_, timestampCorrection);
  this.assertCorrectReferences_();
};
...
shaka.media.SegmentIndex.prototype.assertCorrectReferences_ = function() {
  var previousItem;
  shaka.asserts.assert(
      this.references_.every(function(item, index) {
        var result = true;
        if (index != 0) {
          result = (item.startTime >= previousItem.startTime) &&
                   ...
                   ((item.endTime && item.endTime >= previousItem.endTime) ||
                    (index == this.references_.length - 1));
        }
        previousItem = item;
        return result;
      }),
  ...
  shaka.asserts.assert(
      shaka.util.ArrayUtils.removeDuplicates(this.references_, function(ref) {
        return ref.id;
      }).length == this.references_.length,
