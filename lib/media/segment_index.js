...
shaka.media.SegmentIterator = class {
  ...
  next() {
    ...
    if (ref && ref.hasPartialSegments()) {
      ...
      // If the current regular SegmentReference has been published completely
      // (has a valid Uri), and we reached the end of its Partial
      // SegmentReferences list, move to the next regular segment.
