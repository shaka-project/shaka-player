...
shaka.media.SegmentIndex = class {
   ...
   * @param {!Object=} options Optional. May contain endTimeTolerance (number):
   *   tolerance in seconds past the last segment's end time; if time is within
   *   this range, the last segment is returned (e.g. for HLS EXTINF drift when
   *   seeking to the end). Default 0 (no tolerance); use 1 for HLS.
   */
  constructor(references, options = {}) {
    ...
    /**
     * Tolerance in seconds past the last segment's end time (default 0).
     * @private {number}
     */
    const tolerance = options['endTimeTolerance'];
    this.endTimeTolerance_ = Number.isFinite(tolerance) && tolerance > 0 ?
        tolerance : 0;
  }
  ...
  find(time) {
    ...
    // When endTimeTolerance > 0 (e.g. HLS): if time is past the last segment
    // but within tolerance, return the last segment so seeking to the end
    // works with EXTINF drift. When 0 (default, DASH), no tolerance so the
    // next period can match.
    if (this.endTimeTolerance_ > 0 && this.references.length) {
      const lastRef = this.references[lastReferenceIndex];
      if (time >= lastRef.startTime &&
          time <= lastRef.endTime + this.endTimeTolerance_) {
        return lastReferenceIndex + this.numEvicted_;
      }
    }
...
shaka.media.SegmentIterator = class {
  ...
  resetToLastIndependent() {
    ...
    if (current && current.isPartial() && !current.isIndependent()) {
      const ref = this.segmentIndex_.get(this.currentPosition_);
      if (ref && ref.hasPartialSegments()) {
