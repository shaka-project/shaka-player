 ...
 * to the start of the presentation. Returns the position of the segment with
 * the largest end time if more than one segment is known for the given time.
 *
 * @param {number} time
...
shaka.media.SegmentIndex.prototype.merge = function(references) {
  ...
  while ((i < this.references_.length) && (j < references.length)) {
    ...
    if (r1.position < r2.position) {
      ...
      // Drop the old reference if there's a new reference with the same
      // position.
      newReferences.push(r2);
      i++;
      j++;
