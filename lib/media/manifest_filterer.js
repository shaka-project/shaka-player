      ...
      for (const stream of streams) {
        if (stream.keyIds.size) {
          for (const keyId of stream.keyIds) {
            let keyStatus = keyStatusMap[isGlobalStatus ? '00' : keyId];
            if (!isGlobalStatus && !keyStatus && keyId.length == 22) {
              keyStatus = keyStatusMap[
                  Uint8ArrayUtils.toHex(Uint8ArrayUtils.fromBase64(keyId))];
            }
            if (!keyStatus) {
