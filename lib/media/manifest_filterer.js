...
shaka.media.ManifestFilterer = class {
  ...
  async filterManifest(manifest) {
    ...
    for (const v of manifest.variants) {
      langs.add('videoId=' + v.video.id + ';' + v.audio.label + ';' + v.audio.language);
    }
    console.log(`[APPLE, ManifestFilterer]: before filtering variants based on streamutils.filterManifest: ` +
        `${[...langs].join(',')}`);
    ...
    console.log(`[APPLE, ManifestFilterer]: after filtering variants based on streamutils.filterManifest: ` +
        `${[...langs].join(',')}`);
    ...
    console.log(`[APPLE, ManifestFilterer]: after filtering variants based on chooseCodecsAndFilterManifest: ` +
        `${[...langs].join(',')}`);
  ...
  /**
   * Filters the manifest variants by whether their audio and/or video
   * streams are supported by MediaSource.isTypeSupported().
   * Currently only applicable to Chromecast devices, as other platforms cannot
   * accurately rely on this API for capability checks.
   * @param {!shaka.extern.Manifest} manifest
   */
  filterManifestByTypeSupported(manifest) {
    const device = shaka.device.DeviceFactory.getDevice();
    if (device.getDeviceType() == shaka.device.IDevice.DeviceType.CAST) {
      const filteredVariants = [];
      for (const variant of manifest.variants) {
        if (this.isVariantTypeSupported_(variant)) {
          filteredVariants.push(variant);
        }
      }
      manifest.variants = filteredVariants;
    }
  }
  ...
  processDrmInfos(keySystem, stream) {
    for (const drmInfo of stream.drmInfos) {
      ...
      if (drmInfo.keySystem == keySystem) {
        ...
        this.drmEngine_.updateCurrentDrmInfo(drmInfo);
      }
    }
  }
  ...
  checkRestrictedVariants(manifest) {
    ...
    const streamsChecked = new WeakSet();

    const checkStream = (stream) => {
      if (!stream || streamsChecked.has(stream)) {
        return;
      }
      streamsChecked.add(stream);
    ...
    for (const variant of manifest.variants) {
      if (!variant.allowedByApplication) {
        ...
        // At least one variant is playable, so no need for further processing.
        return;
      }

      checkStream(variant.audio);
      checkStream(variant.video);
    }

    // We throw an error only if there are no playable variants.
  ...
  /**
   * Determines if the provided Variant has a user agent supported MIME type
   * for its enclosed audio and/or video Stream, if available.
   * @param {!shaka.extern.Variant} variant
   * @return {boolean}
   * @private
   */
  isVariantTypeSupported_(variant) {
    let videoSupported = true;
    let audioSupported = true;
    if (variant.video) {
      const mimeType = variant.video.mimeType;
      const codecs =
          shaka.util.StreamUtils.getCorrectVideoCodecs(variant.video.codecs);
      const videoType = shaka.util.MimeUtils.getFullType(mimeType, codecs);
      videoSupported =
          videoSupported && shaka.media.Capabilities.isTypeSupported(videoType);
    }
    if (variant.audio) {
      const mimeType = variant.audio.mimeType;
      const codecs = shaka.util.StreamUtils.getCorrectAudioCodecs(
          variant.audio.codecs, mimeType);
      const audioType = shaka.util.MimeUtils.getFullType(mimeType, codecs);
      audioSupported =
          audioSupported && shaka.media.Capabilities.isTypeSupported(audioType);
    }
