...
shaka.media.TimeRangesUtils = class {
   ...
   * @return {?shaka.extern.BufferedRange}
   */
  static computeAddedRanges(oldRanges, newRanges) {
    const TimeRangesUtils = shaka.media.TimeRangesUtils;
    ...
    const added = [];
    for (const newRange of TimeRangesUtils.getBufferedInfo(newRanges)) {
      let foundOverlap = false;

      for (const oldRange of TimeRangesUtils.getBufferedInfo(oldRanges)) {
        if (oldRange.end >= newRange.start && oldRange.end <= newRange.end) {
          ...
          if (newRange.end > oldRange.end) {
            added.push({start: oldRange.end, end: newRange.end});
          }
        }
      }
