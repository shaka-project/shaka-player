...
shaka.media.TimeRangesUtils = class {
  ...
  /**
   * Computes how far behind the given timestamp is buffered.  As with
   * bufferedAheadOf, we don't include the gaps when calculating this, so this
   * only includes the amount of content that is buffered.
   *
   * @param {TimeRanges} b
   * @param {number} time
   * @return {number} The number of seconds buffered, in seconds, behind the
   *   given time.
   */
  static bufferedBehindOf(b, time) {
    if (!b || !b.length) {
      return 0;
    }
    if (shaka.media.TimeRangesUtils.isBufferNegligible_(b)) {
    ...
    // We calculate the buffered amount by ONLY accounting for the content
    // buffered (i.e. we ignore the times of the gaps).  We add up all buffers
    // up until |time|.
    let result = 0;
    for (const {start, end} of shaka.media.TimeRangesUtils.getBufferedInfo(b)) {
      if (start < time) {
        result += Math.min(end, time) - start;
      }
    }
