...
shaka.media.PresentationTimeline = class {
  ...
  constructor(presentationStartTime, presentationDelay,
    ...
    /**
     * A sorted (by start) list of program-date-time regions.  Each region maps
     * a presentation-time boundary, created at a discontinuity that introduces
     * a new PROGRAM-DATE-TIME base, to the wall-clock time at that boundary.
     * This lets us report an accurate playhead date across discontinuities
     * (e.g. SSAI or live restarts) even though the presentation timeline itself
     * is kept continuous.  Only populated when there is at least one such
     * discontinuity; otherwise we fall back to |initialProgramDateTime_|.
     *
     * @private {!Array<{start: number, pdt: number}>}
     */
    this.programDateTimeRegions_ = [];
  }
  ...
  setProgramDateTimeRegions(regions) {
    this.programDateTimeRegions_ = regions;
  }
  ...
  getProgramDateTimeRegionForTime(time) {
    if (!this.programDateTimeRegions_.length) {
      return null;
    }
    let result = this.programDateTimeRegions_[0];
    for (const region of this.programDateTimeRegions_) {
  ...
  getProgramDateTimeForTime(time) {
    const region = this.getProgramDateTimeRegionForTime(time);
    if (region) {
      return region.pdt + (time - region.start);
    }
    if (this.initialProgramDateTime_ == null) {
      return null;
    }
    return this.initialProgramDateTime_ + time;
  }
