...
shaka.media.TextEngine.makeCue = function(startTime, endTime, payload) {
  ...
  var cc = shaka.media.TextEngine.CueConstructor;
  try {
    return new cc(startTime, endTime, payload);
  } catch (error) {}

  try {
    return new cc(payload, startTime, endTime, '', '', '', true);
  } catch (error) {}
...
/**
 * @type {function(new:TextTrackCue, (string|number), number, (number|string),
