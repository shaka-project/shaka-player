...
shaka.media.EmeManager.prototype.playReadyLicensePreProcessor_ = function(info){
  ...
  try {
    ...
    var authentication = {viewingToken: null};
    var insertionPoint = decodedChallenge.indexOf('</Challenge>');
    var isInsertable = (insertionPoint !== -1);
    var customData = '' +
      '<CustomData>' +
        window.btoa(JSON.stringify(authentication)) +
      '</CustomData>';
    if (isInsertable) {
      decodedChallenge = decodedChallenge.slice(0, insertionPoint) +
        customData + decodedChallenge.slice(insertionPoint);
    }

    console.debug('Adjusted license request on </Challenge>', decodedChallenge);
