...
shaka.media.PreferenceBasedCriteria = class {
  ...
  create(variants) {
    ...
    current = Class.filterByAudioGroupId_(current);
  ...
  /**
   * Filter variants based on audio group ID of
   * the highest bandwidth variant, if present
   * @param {!Array.<shaka.extern.Variant>} variants
   * @return {!Array.<shaka.extern.Variant>}
   * @private
   */
  static filterByAudioGroupId_(variants) {
    const Class = shaka.media.PreferenceBasedCriteria;
    const bestVariant = Class.getBestVariantByBandwidth_(variants);

    if (bestVariant && bestVariant.audio &&
        bestVariant.audio.groupId !== undefined) {
      return variants.filter((variant) => {
        return variant.audio.groupId === bestVariant.audio.groupId;
      });
    }
   ...
   * @private
   */
  static getBestVariantByBandwidth_(variants) {
    return variants.reduce((prev, current) => {
      const prevBandwidth = /** @type {number} */
          (prev.bandwidth || 0);
      const currBandwidth = /** @type {number} */
          (current.bandwidth || 0);
      return (prevBandwidth > currBandwidth) ? prev : current;
    });
  }
};
