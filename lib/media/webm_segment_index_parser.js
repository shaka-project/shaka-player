...
shaka.media.WebmSegmentIndexParser.prototype.parse = function(
  ...
  try {
    references = this.parseInternal_(
        new DataView(cuesData), new DataView(initData), url);
  } catch (exception) {
    // TODO(modmaker): Update to new Error object.
 ...
 *     scale in seconds and duration in seconds, or null if an error occurred.
 * @private
 */
...
shaka.media.WebmSegmentIndexParser.prototype.parseCues_ = function(
  ...
  while (parser.hasMoreData()) {
    ...
    if (lastTime >= 0) {
      shaka.asserts.assert(lastOffset >= 0, 'last offset cannot be 0');

      references.push(
          new shaka.media.SegmentReference(
              lastTime, currentTime, url, lastOffset, currentOffset - 1));
    }
  ...
  if (lastTime >= 0) {
    ...
    references.push(
        new shaka.media.SegmentReference(
            lastTime, duration, url, lastOffset, null));
  }
