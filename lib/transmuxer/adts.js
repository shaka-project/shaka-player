...
shaka.transmuxer.ADTS = class {
   ...
   * @return {boolean}
   */
  static canGetFrameLength(data, offset) {
  ...
  static canParse(data, offset) {
    const ADTS = shaka.transmuxer.ADTS;
    return ADTS.canGetFrameLength(data, offset) &&
        ADTS.isHeaderPattern(data, offset) &&
        ADTS.getFullFrameLength(data, offset) <= data.length - offset;
  }
