...
shaka.transmuxer.TsTransmuxer = class extends shaka.transmuxer.BaseTransmuxer {
  ...
  constructor(mimeType) {
    ...
    /**
     * The AV1 sequence header only rides in key frames, so cache what it says
     * and let the inter frames that follow reuse it.
     *
     * @private {?shaka.codec.AV1.Info}
     */
    this.av1Info_ = null;
  }
  ...
  transmux(data, stream, reference, duration, contentType) {
    ...
    if (contentType == ContentType.AUDIO &&
      ...
      for (; offset < uint8ArrayData.length; offset++) {
        if (shaka.codec.MpegAudio.probe(uint8ArrayData, offset)) {
      ...
      for (; offset < uint8ArrayData.length; offset++) {
        if (shaka.codec.ADTS.probe(uint8ArrayData, offset)) {
          if (!this.aacTransmuxer_) {
            this.aacTransmuxer_ =
    ...
    try {
      ...
      if (contentType == ContentType.VIDEO) {
        switch (codecs.video) {
          ...
          case 'av1':
            streamInfo =
              this.getAv1StreamInfo_(tsParser, stream, duration, reference);
            break;
        }
        if (streamInfo) {
    ...
    for (const audioData of tsParser.getAudioData()) {
      ...
      while (offset < data.length) {
        ...
        samples.push({
          ...
          duration: Opus.getPacketSampleCount(sample),
          cts: 0,
          flags: shaka.transmuxer.TransmuxerUtils.AUDIO_SAMPLE_FLAGS,
        });
        offset = index + size;
  ...
  getHvcStreamInfo_(tsParser, stream, duration, reference) {
    ...
    /** @type {!shaka.codec.H265.H265Frame} */
    const frame = {
  ...
  /**
   * Builds the StreamInfo for an AV1 stream.
   *
   * Unlike AVC/HEVC there are no frames to reassemble: a PES carries one, and
   * only one, AV1 access unit, which is what an `av01` sample holds.  All that
   * the payload needs is to be converted back from the start code based format
   * MPEG-2 TS wraps it in, and the OBUs are only walked afterwards to find the
   * sequence header and the frame type.
   *
   * @param {shaka.util.TsParser} tsParser
   * @param {shaka.extern.Stream} stream
   * @param {number} duration
   ...
   * @private
   */
  getAv1StreamInfo_(tsParser, stream, duration, reference) {
    ...
    for (let i = 0; i < videoData.length; i++) {
      const pes = videoData[i];
      const accessUnit = Obu.parseAv1Ts(pes.data);
      if (!accessUnit) {
        continue;
      }
      ...
      if (!this.av1Info_) {
        // Nothing has described the stream yet, so this access unit cannot be
        // decoded on its own either.
        continue;
      }
      if (baseMediaDecodeTime == null && pes.dts != null) {
        baseMediaDecodeTime = pes.dts;
    ...
    if (!this.av1Info_ || baseMediaDecodeTime == null) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.MEDIA,
          shaka.util.Error.Code.TRANSMUXING_FAILED,
    ...
    stream.height = stream.height || this.av1Info_.height;
    stream.width = stream.width || this.av1Info_.width;
