...
shaka.transmuxer.TsTransmuxer = class {
  ...
  transmux(data, stream, reference, duration, contentType) {
    ...
    if (shaka.util.TsParser.probe(uint8ArrayData)) {
      transmuxData = this.tsTransmuxing_(
          uint8ArrayData, stream, reference, duration, contentType);
    } else if (contentType == ContentType.AUDIO) {
      transmuxData = this.aacTransmuxing_(
   ...
   * @private
   */
  tsTransmuxing_(uint8ArrayData, stream, reference, duration, contentType) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    if (!this.tsParser_) {
    ...
    this.frameIndex_++;
    return shaka.util.Uint8ArrayUtils.concat(initSegment, segmentData);
  }
  ...
  /** @private */
  aacTransmuxing_(uint8ArrayData, stream, reference, duration) {
    const ADTS = shaka.transmuxer.ADTS;
