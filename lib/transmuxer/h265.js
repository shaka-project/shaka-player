...
shaka.transmuxer.H265 = class {
  ...
  static parseInfo(nalus) {
    ...
    const vpsConfiguration = this.parseVPS_(vpsNalu.fullData);
    const spsConfiguration = this.parseSPS_(spsNalu.fullData);
    const ppsConfiguration = this.parsePPS_(ppsNalu.fullData);
    ...
    const videoConfig = this.getVideoConfiguration_(
  ...
  static parseVPS_(data) {
    const gb = new shaka.util.ExpGolomb(this.ebsp2rbsp_(data));
  ...
  static parseSPS_(data) {
    ...
    for (let i = 0; i < maxSubLayersMinus1; i++) {
      if (subLayerProfilePresentFlag[i]) {
        ...
        gb.readUnsignedByte();
        gb.readUnsignedByte();
        gb.readUnsignedByte(); // sub_layer_profile_compatibility_flag
        gb.readUnsignedByte();
        gb.readUnsignedByte();
        gb.readUnsignedByte();
        gb.readUnsignedByte();
    ...
    for (
      let i = subLayerOrderingInfoPresentFlag ? 0 : maxSubLayersMinus1;
