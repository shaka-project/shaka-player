...
shaka.abr.EwmaBandwidthEstimator = class {
  ...
  getBandwidthEstimate(defaultEstimate) {
    ...
    const awmaEstimation = Math.min(this.fast_.getEstimate(), this.slow_.getEstimate());

    // Return the minimum of defaultEstimate and the network estimate
    //This is relevant ONLY if the data come from the network API (else 
    //it's just a const that effectively disables the ABR)
    if (this.config_.defaultBandwidthEstimate !== defaultEstimate)
    {
      return Math.min(calcEstimation, defaultEstimate);
    }
