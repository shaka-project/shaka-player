...
shaka.abr.SimpleAbrManager = class {
   ...
   */
  configure(config) {
    // Codecs restrictions apply only at the player level, not the ABR level.
    if (config.restrictions.restrictedVideoCodecs.length > 0) {
      config.restrictions.restrictedVideoCodecs.length = 0;
    }
    if (config.restrictions.restrictedAudioCodecs.length > 0) {
      config.restrictions.restrictedAudioCodecs.length = 0;
    }
    this.config_ = config;
