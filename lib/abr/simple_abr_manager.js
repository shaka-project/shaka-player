...
shaka.abr.SimpleAbrManager.prototype.chooseStreams = function(
  ...
  if (mediaTypesToUpdate.indexOf('audio') > -1 ||
    ...
    if (variant && variant.video) chosen['video'] = variant.video;
    if (variant && variant.audio) chosen['audio'] = variant.audio;
  }

  if (mediaTypesToUpdate.indexOf('text') > -1) {
    // We don't adapt text, so just choose stream 0.
    chosen['text'] = this.textStreams_[0];
  }
...
shaka.abr.SimpleAbrManager.prototype.suggestStreams_ = function() {
  ...
  var chosen = this.chooseStreams(['audio', 'video']);
