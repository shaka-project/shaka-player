...
shaka.abr.SimpleAbrManager = class {
  constructor() {
    ...
    /** @private {?shaka.extern.AbrManager.RestrictVideoCallback} */
    this.restrictVideoCallback_ = null;
  ...
  init(switchCallback, restrictVideoCallback) {
    ...
    this.restrictVideoCallback_ = restrictVideoCallback;
  }
  ...
  startDroppedFramePoller_() {
    ...
    const config = this.config_.droppedFrameProtection;
    const element = /** @type {!HTMLVideoElement} */ (this.mediaElement_);
    if (!config.enabled || !element.getVideoPlaybackQuality) {
      return;
    }
    ...
    }).tickEvery(config.checkInterval);
  }
   ...
   */
  checkDroppedFrames_() {
    // Skip checks paused state o ABR disabled.
    ...
    const currentDropped = Number(info.droppedVideoFrames);
    const currentTotal = Number(info.totalVideoFrames);
    ...
    if (dropRatio >= config.dropThreshold && this.restrictVideoCallback_) {
      shaka.log.warning(
          'Dropped frame ratio exceeded threshold: ' + dropRatio.toFixed(2) +
          ' >= ' + config.dropThreshold + '. Disabling current video stream.');

      this.restrictVideoCallback_(config.banDuration);
    }
  }
  ...
  updateFramesInfo_() {
    ...
    if (config.enabled && element?.getVideoPlaybackQuality) {
      const info = element.getVideoPlaybackQuality();
      this.lastDroppedFrames_ = Number(info.droppedVideoFrames);
      this.lastTotalFrames_ = Number(info.totalVideoFrames);
    }
  }
};
