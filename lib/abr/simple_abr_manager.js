...
shaka.abr.SimpleAbrManager = class {
  ...
  filterAndSortVariants_(restrictions, variants, maxHeight, maxWidth) {
    let langs = new Set();
    variants.forEach(v => {
      langs.add(`${v.audio.language};${v.audio.label}`);
    });
    console.log(`[APPLE, SimpleAbr]: filterAndSortVariants(), before filter: ` +
        `${[...langs].join(',')}`);
    langs.clear();
    if (this.cmsdManager_) {
    ...
    console.log(`[APPLE, SimpleAbr]: filterAndSortVariants(), after filter: ` +
        `${[...langs].join(',')}`);
