...
shaka.abr.SimpleAbrManager = class {
  ...
  getBandwidthEstimate() {
    ...
    // Return the minimum bandwidthEstimate and the network estimate
    // This is relevant ONLY if the data come from the network API (else
    // It's just a const that effectively disables the ABR) and if the
    // value is not capped (maxNetApiValue_)
    if (this.config_.defaultBandwidthEstimate !== defaultBandwidthEstimate &&
       this.maxNetApiValue_ !== defaultBandwidthEstimate) {
      bandwidthEstimate = Math.min(defaultBandwidthEstimate,
          bandwidthEstimate);
    }
