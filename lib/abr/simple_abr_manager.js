...
shaka.abr.SimpleAbrManager = class {
  ...
  chooseVariant() {
    ...
    for (const {item, next} of enumerate(sortedVariants)) {
      const itemBandwidth = this.getPlaybackRate() * item.bandwidth;
      const minBandwidth =
          itemBandwidth / this.config_.bandwidthDowngradeTarget;
      const nextBandwidth =
          this.getPlaybackRate() * (next || {bandwidth: Infinity}).bandwidth;
  ...
  setPlaybackRate(rate) {
    this.playbackRate_ = Number(rate);
  }
   ...
   */
  getPlaybackRate() {
    return !isNaN(this.playbackRate_) ? Math.abs(this.playbackRate_) : 1;
  }
