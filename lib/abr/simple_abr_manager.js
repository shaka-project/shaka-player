...
shaka.abr.SimpleAbrManager.prototype.stop = function() {
  shaka.asserts.assert(this.switch_, 'expected to be initialized');
...
shaka.abr.SimpleAbrManager.prototype.init = function(switchCallback) {
  shaka.asserts.assert(!this.switch_, 'expected not to be initialized');
...
shaka.abr.SimpleAbrManager.prototype.chooseStreams = function(
    streamSetsByType) {
  this.streamSetsByType_ = streamSetsByType;
  this.streamsByType_ = {};

  return this.chooseStreams_();
};
...
shaka.abr.SimpleAbrManager.prototype.segmentDownloaded = function(
  ...
  shaka.log.v2('Segment downloaded:',
               ...
               'endTimeMs=' + endTimeMs,
               'numBytes=' + numBytes);
  shaka.asserts.assert(endTimeMs >= startTimeMs,
...
shaka.abr.SimpleAbrManager.prototype.suggestStreams_ = function() {
  ...
  shaka.asserts.assert(this.lastTimeChosenMs_ != null,
  ...
  this.switch_(this.chooseStreams_());
};
