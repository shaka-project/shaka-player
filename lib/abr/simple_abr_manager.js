...
goog.require('shaka.abr.DroppedFramesDetector');
...
shaka.abr.SimpleAbrManager = function() {
  ...
  /** @private {shaka.abr.DroppedFramesDetector} */
  this.droppedFramesDetector_ = null;
  ...
  this.detectedDeviceCapacityLevel_ = shaka.abr.DroppedFramesDetector.DeviceCapacityLevels.NORMAL;
};
...
shaka.abr.SimpleAbrManager.prototype.stop = function() {
  ...
  this.droppedFramesDetector_.stop();
 ...
 * @export
 */
shaka.abr.SimpleAbrManager.prototype.init = function(switchCallback, statsProvider) {
  this.switch_ = switchCallback;
  this.droppedFramesDetector_ = new shaka.abr.DroppedFramesDetector(
      statsProvider, 
      this.deviceCapacityLevelUpdate_.bind(this)
  );

  this.droppedFramesDetector_.start();
};
...
shaka.abr.SimpleAbrManager.prototype.deviceCapacityLevelUpdate_ = function(deviceCapacityLevel) {
    const DeviceCapacityLevels = shaka.abr.DroppedFramesDetector.DeviceCapacityLevels;
    const restrictions = this.config_.restrictions;
    var isCapacityDrop;

    console.log('new detected device capacity level:', deviceCapacityLevel);

    isCapacityDrop = deviceCapacityLevel < this.detectedDeviceCapacityLevel_;
    this.detectedDeviceCapacityLevel_ = deviceCapacityLevel;

    restrictions['maxPixels'] = DeviceCapacityLevels.mapToMaxPixels(deviceCapacityLevel);

    console.log('restrictions update:', restrictions);

    if (isCapacityDrop) {
        console.warn('re-evaluating stream choice with flush option as device capacity drop has been detected');
