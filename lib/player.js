...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async loadInner_(startTimeOfLoad, prefetchedVariant, segmentPrefetchById) {
    ...
    do {
      ...
      if (!activeVariant) {
        // Run chooseVariant_ unconditionally so the reused AbrManager registers an
        // initial selection; only adopt its choice if no variant was prefetched.
        // See #10045.
        const chosenVariant = this.chooseVariant_(/* initialSelection= */ true);
