...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async load(assetUriOrPreloader, startTime = null, mimeType) {
    ...
    if (this.speechToText_ && this.speechToText_.isSupported()) {
      this.speechToTextTrack_ = {
  ...
  getTextTracks() {
    if (this.manifest_) {
      if (this.isRemotePlayback()) {
        ...
        if (this.speechToTextTrack_ && this.speechToText_) {
          this.speechToTextTrack_.active = this.speechToText_.isEnabled();
          tracks.push(this.speechToTextTrack_);
        }
        return tracks;
      }
  ...
  selectTextTrack(track) {
    ...
    const isSpeechToText = track && track.language == 'speech-to-text';
    if (track == null || isSpeechToText) {
      this.onTextChanged_();
      this.setTextTrackVisibility(false);
      if (isSpeechToText && this.speechToText_) {
