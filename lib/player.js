...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, videoContainer = null, dependencyInjector) {
    ...
    shaka.log.alwaysWarn('Shaka Player version: s16');
    ...
    /**
     * Controls seek-based trick play.  When active the media element is paused
     * and the controller repeatedly seeks to simulate fast-forward / rewind.
     *
     * @private {shaka.media.SeekBasedTrickPlayController}
     */
    this.seekBasedTrickPlayController_ = null;

    /**
     * Whether audio was muted before seek-based trick play started.
     * Used to restore the mute state when trick play is cancelled.
     *
     * @private {boolean}
     */
    this.wasMutedBeforeSeekBasedTrickPlay_ = false;

    /**
     * Whether audio was muted before scrub mode started.
     * Used to restore the mute state when scrub ends.
     *
     * @private {boolean}
     */
    this.wasMutedBeforeScrub_ = false;

    /**
     * Whether the video was playing before scrub mode started.
     * Used to resume playback when scrub ends.
     *
     * @private {boolean}
     */
    this.wasPlayingBeforeScrub_ = false;
  ...
  async destroy() {
    ...
    if (this.seekBasedTrickPlayController_) {
      this.seekBasedTrickPlayController_.release();
      this.seekBasedTrickPlayController_ = null;
    }
  ...
  async unload(initializeMediaSource = true, keepAdManager = false,
    ...
    try {
      ...
      // Clean up seek-based trick play so stale closures don't survive
      // across stream loads.
      if (this.seekBasedTrickPlayController_) {
        this.seekBasedTrickPlayController_.release();
        this.seekBasedTrickPlayController_ = null;
  ...
  startBufferManagement_(mediaElement, srcEquals) {
    ...
    this.loadEventManager_.listen(mediaElement, Event.SEEKED, () => {
      if (this.seekBasedTrickPlayController_) {
        this.seekBasedTrickPlayController_.onSeekComplete();
      }
    });
  ...
  createStreamingEngine() {
    ...
    const playerInterface = {
      ...
      onSegmentAppended: (reference, stream, isMuxed, isDependency) => {
        ...
        // During seek-based trick play, notify the controller immediately
        // when a new video segment is appended.  This provides zero-delay
        // frame detection instead of waiting for the next cadence-timer
        // poll, which can lag by up to 150 ms (or much longer under the
        // old throttle model).
        const ContentType =
            shaka.util.ManifestParserUtils.ContentType;
        if (stream.type === ContentType.VIDEO &&
            this.seekBasedTrickPlayController_ &&
            this.seekBasedTrickPlayController_.isActive()) {
          this.seekBasedTrickPlayController_.notifyNewSegment();
        }
      },
      ...
      getKeySystem: () => this.keySystem(),
      isSeekBasedTrickPlayActive: () =>
        this.isSeekBasedTrickPlayActive(),
      getTrickPlayDirection: () => {
        if (this.seekBasedTrickPlayController_) {
          return this.seekBasedTrickPlayController_.getDirection();
        }
        return shaka.media.SeekBasedTrickPlayController.Direction.NONE;
      },
      shouldDownloadAudioDuringTrickPlay: () =>
        !!this.config_.streaming.seekBasedTrickPlay
            .downloadAudioDuringTrickPlay,
    };
  ...
  getPlaybackRate() {
    ...
    // When seek-based trick play is active the real playbackRate is
    // meaningless (the element is paused).  Return the trick-play rate
    // instead so that callers — including the UI fast-forward / rewind
    // buttons — can determine the current effective rate for progression.
    if (this.isSeekBasedTrickPlayActive()) {
      return this.seekBasedTrickPlayController_.getRate();
    }
  ...
  trickPlay(rate, useTrickPlayTrack = true) {
    ...
    // Delegate to seek-based trick play when enabled.
    if (this.config_.streaming.seekBasedTrickPlay.enabled) {
      // Rate of 1 means normal playback — cancel trick play instead.
      if (rate === 1) {
        this.cancelSeekBasedTrickPlay();
      } else {
        this.seekBasedTrickPlay(rate);
      }
      return;
    }
   ...
   */
  cancelTrickPlay() {
    // If seek-based trick play is active, cancel that instead.
    if (this.isSeekBasedTrickPlayActive()) {
      this.cancelSeekBasedTrickPlay();
      return;
    }
  ...
  /**
   * Start seek-based trick play at the given rate.  The media element will be
   * paused and the controller will repeatedly seek to simulate fast-forward or
   * rewind.  If an I-frame (trick-play) track is available it will be
   * activated automatically so that only keyframes are fetched and rendered.
   *
   * <p>
   * This is the recommended trick-play approach for platforms that do not
   * support non-standard playback rates (e.g. WebOS, Tizen).
   *
   * @param {number} rate  Positive for fast-forward, negative for rewind.
   *     Must not be 0.
   * @export
   */
  seekBasedTrickPlay(rate) {
    if (rate === 0) {
      shaka.log.alwaysWarn('seekBasedTrickPlay: rate 0 is not supported!');
      return;
    }

    // Rate of 1 means normal playback — cancel trick play.
    if (rate === 1) {
      this.cancelSeekBasedTrickPlay();
      return;
    }

    if (this.loadMode_ != shaka.Player.LoadMode.MEDIA_SOURCE || !this.video_) {
      shaka.log.alwaysWarn(
          'seekBasedTrickPlay: no content loaded or no video element.');
      return;
    }
    ...
    if (seekRange.start === seekRange.end) {
      shaka.log.alwaysWarn(
          'seekBasedTrickPlay: no seekable range available.');
      return;
    }
    ...
    if (isNewSession) {
      // Stash mute state and mute audio.
      this.wasMutedBeforeSeekBasedTrickPlay_ = this.video_.muted;
      this.video_.muted = true;
    }

    // Always ensure the element is paused — the UI fast-forward / rewind
    // buttons call video.play() before trickPlay() when the video is paused,
    // which would un-pause us on rate changes.  Guard against it here.
    if (!this.video_.paused) {
      this.video_.pause();
    }

    // Activate I-frame trick-play track (no-op if unavailable).
    // Clear the old video buffer so that isBuffered() reflects I-frame data
    // only — without this, the buffer contains normal-video segments whose
    // keyframes are 2-5 s apart, making trick play appear frozen.
    if (isNewSession && this.streamingEngine_) {
      this.streamingEngine_.setTrickPlay(true, /* clearBuffer= */ true);
    } else {
      this.useTrickPlayTrackIfAvailable(true);
    }

    if (!this.seekBasedTrickPlayController_) {
      this.ensureSeekBasedTrickPlayController_();
    }

    // Remove the audio SourceBuffer from the MediaSource so it drops
    // out of activeSourceBuffers.  This makes HTMLMediaElement.buffered
    // reflect only video, allowing the browser to complete seeks to
    // I-frame-only positions.  We cannot use audioTracks.enabled because
    // that API is not available in Chrome/Edge/Firefox by default.
    //
    // When downloadAudioDuringTrickPlay is enabled, the audio SB stays
    // attached and the SE keeps fetching audio alongside video I-frames.
    // This is required on platforms (e.g. WebOS) where the browser needs
    // audio data at the seek target to complete seeks and render I-frames.
    if (isNewSession && this.mediaSourceEngine_ &&
        !this.config_.streaming.seekBasedTrickPlay
            .downloadAudioDuringTrickPlay) {
      this.mediaSourceEngine_.detachSourceBufferForTrickPlay(
          shaka.util.ManifestParserUtils.ContentType.AUDIO);
    }

    // start() handles the already-active case internally (delegates to
    // changeRate).
    this.seekBasedTrickPlayController_.start(rate);

    // Dispatch trick-play specific event.
    if (isNewSession) {
      const eventName = shaka.util.FakeEvent.EventName.TrickPlayStarted;
      const data = (new Map()).set('rate', rate);
      this.dispatchEvent(shaka.Player.makeEvent_(eventName, data));
    } else {
      const eventName = shaka.util.FakeEvent.EventName.TrickPlayRateChanged;
      const data = (new Map()).set('rate', rate);
      this.dispatchEvent(shaka.Player.makeEvent_(eventName, data));
    }

    // Also dispatch a RateChange event so that existing UI controls (e.g.
    // fast-forward / rewind buttons) that listen for 'ratechange' update
    // their displayed rate.  They read getPlaybackRate() which now returns
    // the seek-based rate when active.
    this.dispatchEvent(shaka.Player.makeEvent_(
        shaka.util.FakeEvent.EventName.RateChange));
  }

  /**
   * Cancel seek-based trick play and resume normal playback.
   *
   * @export
   */
  cancelSeekBasedTrickPlay() {
    ...
    // Overshoot correction (VTP-inspired): if enabled, seek back to the
    // position of a recently-rendered frame to compensate for the user's
    // reaction time when releasing the trick-play button.
    let correctedPosition = null;
    if (this.config_.streaming.seekBasedTrickPlay
        .enableOvershootCorrection) {
      correctedPosition =
          this.seekBasedTrickPlayController_
              .getOvershootCorrectionPosition();
    }

    this.seekBasedTrickPlayController_.stop();

    // Restore normal track.
    this.useTrickPlayTrackIfAvailable(false);

    // Reattach the audio SourceBuffer that was removed during trick play,
    // and reset its init-segment state so the SE re-sends the init data
    // to the fresh SourceBuffer.  This must happen BEFORE SE.seeked() so
    // the SE can process all types (audio included) when resetting state.
    //
    // When downloadAudioDuringTrickPlay was enabled, the audio SB was
    // never detached, so reattach is unnecessary (and would be a no-op in
    // reattachSourceBufferForTrickPlay since sourceBuffers_ already has
    // the audio entry).
    if (this.mediaSourceEngine_ &&
        !this.config_.streaming.seekBasedTrickPlay
            .downloadAudioDuringTrickPlay) {
      const ContentType = shaka.util.ManifestParserUtils.ContentType;
      this.mediaSourceEngine_.reattachSourceBufferForTrickPlay(
          ContentType.AUDIO);
      if (this.streamingEngine_) {
        this.streamingEngine_.resetInitSegmentState(ContentType.AUDIO);
      }
    }

    // Notify the streaming engine that the playhead has moved so it
    // resets its iterator and fetches normal-video segments from the
    // current position.  Without this, the SE's stale trick-play state
    // (reverse iterator, cleared lastSegmentReference) persists and
    // normal playback cannot resume.
    if (this.streamingEngine_) {
      this.streamingEngine_.seeked();
    }

    if (this.video_) {
      // Apply overshoot correction if a corrected position was computed.
      if (correctedPosition != null) {
        this.video_.currentTime = correctedPosition;
      }

      // Restore mute state.
      this.video_.muted = this.wasMutedBeforeSeekBasedTrickPlay_;

      // Resume playback.
      this.video_.play();
    }

    const eventName = shaka.util.FakeEvent.EventName.TrickPlayStopped;
    this.dispatchEvent(shaka.Player.makeEvent_(eventName));

    // Dispatch RateChange so UI controls update their displayed rate back to
    // the normal playback rate (getPlaybackRate() now returns the real rate
    // since seek-based trick play is no longer active).
    this.dispatchEvent(shaka.Player.makeEvent_(
        shaka.util.FakeEvent.EventName.RateChange));
  }
   ...
   */
  isSeekBasedTrickPlayActive() {
    return !!this.seekBasedTrickPlayController_ &&
        this.seekBasedTrickPlayController_.isActive();
  }

  /**
   * @return {number} The current seek-based trick play rate, or 0 if inactive.
   * @export
   */
  getSeekBasedTrickPlayRate() {
    if (this.seekBasedTrickPlayController_) {
      return this.seekBasedTrickPlayController_.getRate();
    }
    return 0;
  ...
  /**
   * Start scrub mode.  The media element is paused, audio is muted, and
   * the I-frame trick-play track is activated (if available).  While in
   * scrub mode, call {@link scrubSeek} to render individual I-frames at
   * given positions.  Call {@link endScrub} to resume normal playback.
   *
   * This is intended for seek-bar drag interactions: the UI calls
   * {@code startScrub()} on drag-start, {@code scrubSeek(pos)} on each
   * drag-move, and {@code endScrub()} on drag-end.
   *
   * @export
   */
  startScrub() {
    if (this.loadMode_ != shaka.Player.LoadMode.MEDIA_SOURCE ||
        !this.video_) {
      shaka.log.alwaysWarn('startScrub: no content loaded.');
      return;
    }

    // Cancel any active trick play first.
    if (this.isSeekBasedTrickPlayActive()) {
      this.cancelSeekBasedTrickPlay();
    }

    // Stash playing and mute state.
    this.wasPlayingBeforeScrub_ = !this.video_.paused;
    this.wasMutedBeforeScrub_ = this.video_.muted;
    this.video_.muted = true;
    ...
    // Activate I-frame track.
    if (this.streamingEngine_) {
      this.streamingEngine_.setTrickPlay(true, /* clearBuffer= */ true);
    }

    // Create the controller if it doesn't exist, then enter scrub mode.
    this.ensureSeekBasedTrickPlayController_();
    this.seekBasedTrickPlayController_.startScrub();

    this.dispatchEvent(shaka.Player.makeEvent_(
        shaka.util.FakeEvent.EventName.ScrubStarted));
  }

  /**
   * Seek to the given position while in scrub mode.  The controller gates
   * out positions that are too close to the previous scrub seek and waits
   * for the previous seek to render before issuing a new one.
   *
  ...
  scrubSeek(positionSec, force = false) {
    if (!this.seekBasedTrickPlayController_ ||
        !this.seekBasedTrickPlayController_.isScrubActive()) {
      shaka.log.alwaysWarn('scrubSeek: not in scrub mode.');
      return false;
    }
    return this.seekBasedTrickPlayController_.scrubSeek(positionSec, force);
  }

  /**
   * End scrub mode and resume normal playback.  If overshoot correction is
   * enabled, the position is adjusted to the last rendered frame.
   *
   * @export
   */
  endScrub() {
    ...
    this.seekBasedTrickPlayController_.stopScrub();
    ...
    if (this.video_) {
      this.video_.muted = this.wasMutedBeforeScrub_;
      if (this.wasPlayingBeforeScrub_) {
        this.video_.play();
      }
    }

    this.dispatchEvent(shaka.Player.makeEvent_(
        shaka.util.FakeEvent.EventName.ScrubStopped));
  }

  /**
   * Lazily create the seek-based trick play controller if it doesn't exist.
   * @private
   */
  ensureSeekBasedTrickPlayController_() {
    if (this.seekBasedTrickPlayController_) {
      return;
    }
    this.seekBasedTrickPlayController_ =
        new shaka.media.SeekBasedTrickPlayController({
          getPresentationTime: () => this.video_.currentTime,
          getSeekRange: () => this.seekRange(),
          getSeekCadence: () => this.config_.streaming
              .seekBasedTrickPlay.seekCadence,
          seekTo: (time) => {
          ...
          isBuffered: (time) => {
            // Use a gap-aware check instead of the simplified
            // TimeRangesUtils.isBuffered() which only tests the outer
            // bounds (start(0)..end(last)) and gives false positives
            // for positions that fall in gaps between buffer ranges.
            // During trick play, SE resyncs create disjoint buffer
            // ranges; the simplified check would treat gap positions
            // as buffered, causing lastRenderedTime_ to advance past
            // real content and starving findNewFrame_().
            if (!this.mediaSourceEngine_) {
              return false;
            }
            const info = this.mediaSourceEngine_.getBufferedInfo();
            for (const range of info.video) {
          ...
          getBufferEnd: () => {
            ...
            return this.mediaSourceEngine_.bufferEnd(ContentType.VIDEO);
          },
          getBufferStart: () => {
            const ContentType =
                shaka.util.ManifestParserUtils.ContentType;
            ...
            return this.mediaSourceEngine_.bufferStart(ContentType.VIDEO);
          },
          onBoundaryReached: () => {
            this.cancelSeekBasedTrickPlay();
          },
          onTrickFrameRendered: (positionSec) => {
            const eventName =
                shaka.util.FakeEvent.EventName.TrickFrameRendered;
            const data = (new Map()).set('position', positionSec);
            this.dispatchEvent(shaka.Player.makeEvent_(eventName, data));
          },
          watchFrames: (onFrame) => {
            if (!this.video_ ||
            ...
            const started = handler.start((now, metadata) => {
              const pos = metadata && metadata.mediaTime != undefined ?
                  metadata.mediaTime : this.video_.currentTime;
              onFrame(pos);
            });
            if (!started) {
              handler.release();
          ...
          getConfig: () => this.config_.streaming.seekBasedTrickPlay,
        });
  }
   ...
   */
  onRateChange_() {
    // During seek-based trick play or scrub mode the element is paused and
    // any native ratechange events are spurious (e.g. caused by pause/play
    // toggling).  Ignore them so we don't interfere with the play rate
    // controller.
    if (this.isSeekBasedTrickPlayActive() ||
        (this.seekBasedTrickPlayController_ &&
         this.seekBasedTrickPlayController_.isScrubActive())) {
  ...
  onSeek_() {
    ...
    if (this.streamingEngine_) {
      // During seek-based trick play or scrub mode the controller manages SE
      // notifications explicitly (seekTo vs seekVideoOnly).  Calling seeked()
      // here on every browser 'seeking' event would abort in-flight segment
      // downloads and reset the iterator, preventing I-frames from ever
      // finishing their download and rendering.
      if (!this.isSeekBasedTrickPlayActive() &&
          !(this.seekBasedTrickPlayController_ &&
            this.seekBasedTrickPlayController_.isScrubActive())) {
        this.streamingEngine_.seeked();
