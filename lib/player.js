...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  selectVariantTrack(track, clearBuffer = false, safeMargin = 0) {
    const selectMediaSourceMode = () => {
      ...
      const variantLanguage = variant.language;
      const variantRole =
        (variant.audio && variant.audio.roles &&
        variant.audio.roles[0]) || '';
      const configCodecSwitchingStrategy =
        this.config_.mediaSource.codecSwitchingStrategy;

      this.config_.adaptationSetCriteriaFactory = () => {
        const criteria = new shaka.media.PreferenceBasedCriteria();
        criteria.configure({
          language: variantLanguage,
          role: variantRole,
          channelCount: (variant.audio && variant.audio.channelsCount != null) ?
          ...
          videoLabel: '',
          codecSwitchingStrategy: configCodecSwitchingStrategy,
      ...
      // Workaround for
      // https://github.com/shaka-project/shaka-player/issues/1299
      // When track is selected, back-propagate the language and role
      // to the current AdaptationSetCriteria to prevent unwanted switches.
      this.currentAdaptationSetCriteria_ =
        this.config_.adaptationSetCriteriaFactory();
