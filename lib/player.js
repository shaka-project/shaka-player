...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  startDroppedFramePoller_() {
    ...
    if (!config.enabled || this.isAudioOnly() ||
  ...
  checkDroppedFrames_() {
    ...
    // because frame drops are expected.
  ...
  switchVariant_(variant, fromAdaptation, clearBuffer, safeMargin,
    ...
    // Reset dropped frame counters to the current values on variant change so
    // that only frames from the new stream are measured.
    const config = this.config_.streaming.droppedFrameProtection;
    const element = /** @type {!HTMLVideoElement} */ (this.video_);
    if (config.enabled && !this.isAudioOnly() &&
        element.getVideoPlaybackQuality) {
