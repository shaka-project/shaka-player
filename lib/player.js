...
shaka.Player = class extends shaka.util.FakeEventTarget {
   ...
   *    video will be drawn.
   */
  constructor(mediaElement, dependencyInjector, canvas) {
    super();
   ...
   * @param {LcevcDil.LcevcDilConfig} config
   * @private
   */
  createDIL_(config) {
    ...
    if (tracks && tracks[0] &&
      ...
      if (edge) {
        if (!config.streaming.forceTransmuxTS) {
          // Warning for MPEG-2 TS LCEVC data is not passed to lib unless
          // forceTransmux is enabled.
  ...
  attachCanvas(canvas) {
    ...
    // List to the events that can occur with our request.
    events.onStart = () => shaka.log.info('Starting Canvas attach...');
    return this.wrapWalkerListenersWithPromise_(events);
  }
    ...
    this.drmEngine_ = this.createDrmEngine({
      ...
      onEvent: (e) => {
        ...
        if (e.type == shaka.util.FakeEvent.EventName.DrmSessionUpdate &&
          ...
          // Currently LCEVC is not supported with DRM Streams,
          // hence hiding the canvas.
          if (this.lcevcDil_) {
