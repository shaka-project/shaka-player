...
shaka.Player.prototype.applyConfig_ = function(
  ...
  if (this.streamingEngine_) {
    ...
    if (!shaka.util.MapUtils.empty(streamsToSwitch)) {
      // Send an adaptation event so that the UI can show the new language.
      this.onAdaptation_();
    }
...
shaka.Player.prototype.onKeyStatus_ = function(keyStatusMap) {
  ...
  var allowedStatuses =
      ['usable', 'status-pending', 'output-downscaled', 'output-restricted'];
  var period = this.streamingEngine_.getCurrentPeriod();
  ...
  period.streamSets.forEach(function(streamSet) {
    streamSet.streams.forEach(function(stream) {
      ...
      var originalHasOutputRestrictions = stream.hasOutputRestrictions;
      ...
      if (stream.keyId && stream.keyId in keyStatusMap) {
        ...
        stream.hasOutputRestrictions = keyStatus == 'output-restricted';
      }

      if (originalAllowed != stream.allowedByKeySystem ||
          originalHasOutputRestrictions != stream.hasOutputRestrictions) {
        tracksChanged = true;
      }
    });
  });
