...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  onSrcEquals_(has, wants) {
    ...
    if (this.video_.textTracks) {
      this.eventManager_.listen(this.video_.textTracks, 'addtrack', (e) => {
        this.onTracksChanged_();
        this.processTimedMetadata_(/** @type {!TrackEvent} */(e));
      });
      this.eventManager_.listen(
  ...
  setupPreferredAudioOnSrc_() {
    ...
    const audioLanguagesAndRoles = this.getAudioLanguagesAndRoles();
    const matchingByLanguage = [];
    for (const languageAndRole of audioLanguagesAndRoles) {
      const language = languageAndRole.language;
      if (LanguageUtils.normalize(language) == preferredLanguage) {
        matchingByLanguage.push(languageAndRole);
      }
    }

    // If there is no language match, the browser preference is left.
    if (!matchingByLanguage.length) {
      return;
    }

    const preferredVariantRole = this.config_.preferredVariantRole;

    // If the user has not selected a role preference, the first match is
    // selected.
    if (preferredVariantRole == '') {
      const languageAndRole = matchingByLanguage[0];
      this.selectAudioLanguage(languageAndRole.language, languageAndRole.role);
      return;
    }

    const matchingByLanguageAndRole = matchingByLanguage.find(
        (languageAndRole) => languageAndRole.role == preferredVariantRole);
    if (matchingByLanguageAndRole) {
      this.selectAudioLanguage(matchingByLanguageAndRole.language,
          matchingByLanguageAndRole.role);
      return;
    }

    // If there is no role match, the first match is selected.
    const languageAndRole = matchingByLanguage[0];
    this.selectAudioLanguage(languageAndRole.language, languageAndRole.role);
  }
  ...
  setupPreferredTextOnSrc_() {
    ...
    const textLanguagesAndRoles = this.getTextLanguagesAndRoles();
    const matchingByLanguage = [];
    for (const languageAndRole of textLanguagesAndRoles) {
      const language = languageAndRole.language;
      if (LanguageUtils.normalize(language) == preferredLanguage) {
        matchingByLanguage.push(languageAndRole);
      }
    ...
    // If there are not matching, the browser preference is left.
    if (!matchingByLanguage.length) {
      return;
    }
    ...
    if (preferredTextRole == '') {
      const languageAndRole = matchingByLanguage[0];
      this.selectTextLanguage(languageAndRole.language, languageAndRole.role);
      return;
    }

    const matchingByLanguageAndRole = matchingByLanguage.find(
        (languageAndRole) => languageAndRole.role == preferredTextRole);
    if (matchingByLanguageAndRole) {
      this.selectTextLanguage(matchingByLanguageAndRole.language,
          matchingByLanguageAndRole.role);
      return;
    }
