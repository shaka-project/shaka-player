...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  makePreloadManager_(assetUri, mimeType, startTimeOfLoad,
    ...
    const drmPlayerInterface = {
      ...
      onKeyStatus: (map) => {
        // this.onKeyStatus_ expects the streaming engine to exist, which
        // only happens once the preloadManager has already been linked, so
        // this should be safe to call on the player.
