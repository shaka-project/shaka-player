...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  addResetMediaSourceOnStreamSwitch_(mediaElement) {
    ...
    const timer = new shaka.util.Timer(() => {
      ...
      if (endTime !== null) {
        const NEXT_STREAM_TIME_NUDGE_ = 0.1;
        mediaElement.currentTime = endTime + NEXT_STREAM_TIME_NUDGE_;
      }
    });
    ...
    this.loadEventManager_.listen(mediaElement, 'timeupdate', () => {
      const endTime = getLastEndTime();
      const time = this.playhead_.getTime();
      // When we're 1 second away from the end of the current stream,
      // we'll spawn a timer and assume the current stream ended by then.
      const NEXT_STREAM_MIN_TIME_ = 1;
      if (endTime !== null && endTime - time < NEXT_STREAM_MIN_TIME_) {
