...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, videoContainer = null, dependencyInjector) {
    ...
    this.currentAdaptationSetCriteria_.configure({
      languages: this.config_.preferredAudioLanguages,
    ...
    /** @private {string} */
    this.currentTextLanguage_ = this.config_.preferredTextLanguages[0] || '';
  ...
  async loadInner_(startTimeOfLoad, prefetchedVariant, segmentPrefetchById) {
    ...
    if (!this.config_.preferredAudioLanguages.length && !hasPrimary) {
  ...
  setupPreferredAudioOnSrc_() {
    const preferredAudioLanguages = this.config_.preferredAudioLanguages;
  ...
  setupPreferredTextOnSrc_() {
    ...
    const preferredTextLanguages = this.config_.preferredTextLanguages;

    // Try each preferred language in order
    for (const preferredLanguage of preferredTextLanguages) {
  ...
  selectAudioTrack(audioTrack, safeMargin = 0) {
    ...
    const selectMediaSourceMode = () => {
      ...
      config.languages = [audioTrack.language];
