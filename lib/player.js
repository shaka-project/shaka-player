...
shaka.Player = class extends shaka.util.FakeEventTarget {
   ...
   * cues as timeline regions. Returns <code>null</code> when the player has not
   * loaded content with MediaSource or when using src= without MSE.
   *
   * @return {?Iterable<shaka.extern.MetadataTimelineRegionInfo>}
   * @export
   */
  getAllMetadataRegions() {
    return this.metadataRegionTimeline_.regions();
  }

  /**
   * Returns the EMSG region timeline, which contains inband event message
   * boxes as timeline regions. Returns <code>null</code> when the player has
   * not loaded content with MediaSource or when using src= without MSE.
   *
   * @return {?Iterable<shaka.extern.EmsgTimelineRegionInfo>}
   * @export
   */
  getAllEmsgRegions() {
    return this.emsgRegionTimeline_.regions();
  }

  /**
   * Returns the timeline region timeline, which contains all timeline regions.
   * MediaSource or when using src= without MSE.
   *
   * @return {?Iterable<shaka.extern.TimelineRegionInfo>}
   * @export
   */
  getAllTimelineRegions() {
    return this.regionTimeline_.regions();
  }
