...
shaka.Player = function(mediaElement, dependencyInjector) {
  ...
  actions.set(this.manifestNode_, (has, wants) => {
    const p = this.onParseManifest_(has, wants);

    return new AbortableOperation(p, async () => {
      if (this.parser_) {
...
/**
 * Create a parser and parse the manifest at |wants.uri|. This should only be
 * called as part of the load graph.
 *
 * This method assumes that it is safe for it to execute, the load-graph is
 * responsible for ensuring all assumptions are true.
 *
 * TODO(vaage): Look at splitting this node into multiple nodes. Previously
 *              we could interrupt between picking a parser and parsing. We
 *              also need a way to interrupt the manifest parser between
 *              retries.
 *
 * @param {shaka.routing.Payload} has
 * @param {shaka.routing.Payload} wants
 ...
 * @private
 */
shaka.Player.prototype.onParseManifest_ = async function(has, wants) {
  goog.asserts.assert(
...
shaka.Player.prototype.onLoad_ = async function(has, wants) {
  ...
  goog.asserts.assert(
      this.manifest_,
      'The manifest should have already been initialized');
  this.drmEngine_ = await this.createDrmEngine(this.manifest_);
 ...
 * @private
 */
shaka.Player.prototype.getNextAfterManifest_ = function(
    goingTo, has, wants) {
  // We can only go to load or unload.
  //
  // In order to go to load we need to have everything we used in or before
  // parsing the manifest to match between |has| and |wants.
