...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async load(assetUriOrPreloader, startTime = null, mimeType) {
    ...
    try {
      ...
      if (shouldUseSrcEquals) {
        ...
        // Manifest-signaled CMCD parameters must be in effect before the
        // first segment requests go out.
   ...
   * to the CMCD manager. Called at load, on manifest updates, and (through
   * CmcdManager.reset()) cleared on unload.
   *
   * @private
   */
  updateCmcdManifestParameters_() {
    if (!this.cmcdManager_) {
      return;
    }
    const serviceDescription =
        this.manifest_ ? this.manifest_.serviceDescription : null;
