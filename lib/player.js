...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async load(assetUriOrPreloader, startTime = null, mimeType) {
    ...
    try {
      ...
      if (!preloadManager) {
        ...
        // paths.
        preloadManager = await this.preloadInner_(assetUri, mimeType);
      }
      ...
      const startTimeOfLoad = preloadManager.getStartTimeOfLoad();
      ...
      if (shouldUseSrcEquals) {
        ...
        // TODO: Re-filter for audio-only assets here.
        ...
        await mutexWrapOperation(async () => {
          await this.loadInner_(startTimeOfLoad, segmentPrefetchById);
        }, 'loadInner_');
      }
      this.dispatchEvent(this.makeEvent_(
  ...
  async unloadAndSavePreload(
    ...
    let preloadManager = null;
    if (this.manifest_ && this.parser_ && this.assetUri_) {
      ...
      preloadManager = this.makePreloadManager_(
          this.assetUri_, this.mimeType_, startTimeOfLoad);
      preloadManager.attachManifest(this.manifest_, this.parser_);
    ...
    // TODO: Test this!
    // TODO: Also save the parser factory?
  }
   ...
   * @export
   */
  async preload(assetUri, mimeType) {
    const preloadManager = await this.preloadInner_(assetUri, mimeType);
    if (!preloadManager) {
   ...
   * @private
   */
  async preloadInner_(assetUri, mimeType) {
    ...
    const preloadManager = this.makePreloadManager_(
        assetUri, mimeType || null, startTimeOfLoad);
    preloadManager.start();
    return preloadManager;
  }
   ...
   * @private
   */
  makePreloadManager_(assetUri, mimeType, startTimeOfLoad) {
    ...
    if (this.video_) {
      if (this.video_.nodeName === 'AUDIO') {
        config.manifest.disableVideo = true;
      }
    } else {
      // TODO: Mark a flag such that, once this preload manager has attached to
      // a player, we will set disableVideo and filter for audio-only.
    }
    ...
    const manifestPlayerInterface = {
      ...
      filter: async (manifest) => {
        ...
        if (tracksChanged) {
          ...
          // TODO: Check that this hasn't become destroyed?
          preloadManager.dispatchEvent(event);
        }
      },
      makeTextStreamsForClosedCaptions: (manifest) => {
        // TODO: Call on preload manager?
      ...
      onTimelineRegionAdded: (region) => {
        // TODO: Get regionTimeline_ of preload manager?
      ...
      // TODO: Get from config of preload manager?
      isLowLatencyMode: () => this.config_.streaming.lowLatencyMode,
      // TODO: Get from config of preload manager?
      isAutoLowLatencyMode: () => this.config_.streaming.autoLowLatencyMode,
      enableLowLatencyMode: () => {
        // TODO: Apply to config of PreloadManager for this?
        this.configure('streaming.lowLatencyMode', true);
      },
      updateDuration: () => {
        // TODO: Get streaming engine of PreloadManager for this?
        if (this.streamingEngine_) {
      ...
      newDrmInfo: (stream) => {
        ...
        // TODO: Get drm engine of PreloadManager for this?
      ...
      onManifestUpdated: () => {
        ...
        preloadManager.dispatchEvent(this.makeEvent_(eventName, data));

        // TODO: Handle when not yet attached; I guess the preloadManager will
        // need to save these when detached and apply when attached?
        if (this.adManager_) {
    ...
    const drmPlayerInterface = {
      ...
      onKeyStatus: (map) => {
        // TODO: Make a generic version of this that works for preload, without
        // breaking src=
    ...
    const preloadManager = new shaka.media.PreloadManager(
        assetUri, mimeType, startTimeOfLoad, playerInterface);
    return preloadManager;
  }
   ...
   * @private
   */
  async loadInner_(startTimeOfLoad, segmentPrefetchById) {
    goog.asserts.assert(
  ...
  onRegionEvent_(eventName, region, eventTarget = this) {
    ...
    eventTarget.dispatchEvent(this.makeEvent_(eventName, data));
  }
