...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  processTimedMetadataSrcEqls_(track) {
    ...
    this.loadEventManager_.listen(track, 'cuechange', () => {
      ...
      let interstitialStartTime = null;
      let interstitialEndTime = null;
      const interstitialValues = [];
      ...
      for (const cue of track.activeCues) {
        ...
        if (cue.type == 'com.apple.quicktime.HLS') {
          interstitialStartTime = cue.startTime;
          interstitialEndTime = cue.endTime;
          interstitialValues.push(cue.value);
        }
      }
      const isValidInterstitial = interstitialValues.some((value) => {
        return value.key == 'X-ASSET-URI' || value.key == 'X-ASSET-LIST';
      });
      if (interstitialStartTime != null && isValidInterstitial) {
        /** @type {shaka.extern.Interstitial} */
        const interstitial = {
          startTime: interstitialStartTime,
          endTime: interstitialEndTime,
          values: interstitialValues,
        };
        if (this.adManager_) {
          goog.asserts.assert(this.video_, 'Must have video');
          this.adManager_.onInterstitialMetadata(
