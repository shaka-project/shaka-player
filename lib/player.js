...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onInitializeDrm_(has, wants) {
    ...
    let offlineSessionIds = this.config_.learnystCustomCfg.offlineSessionIds;
    if (offlineSessionIds.length <= 0) {
      offlineSessionIds = this.manifest_.offlineSessionIds;
    }

    await this.drmEngine_.initForPlayback(
        this.manifest_.variants,
        offlineSessionIds,
        this.config_.learnystCustomCfg.usePersistentLicenses,
 ...
 * @export
 */
shaka.Player.version = 'v3.1.1060-uncompiled';
