...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  defaultConfig_() {
    ...
    config.streaming.failureCallback = (error) => {
      ...
      if (this.config_.autoRetryOnLiveHttpError == null) {
        shaka.Deprecate.deprecateFeature(5,
          'PlayerConfiguration.autoRetryOnLiveHttpError = null',
          'Auto retry in StreamingConfiguration.failureCallback is deprecated' +
          ' and will move to earlier stages. Please use "PlayerConfiguration.' +
          'autoRetryOnLiveHttpError = true|false" to migrate.');
  ...
  async onError_(error) {
    ...
    // Call the default streaming failure callback before dispatching the error.
    if (this.config_.autoRetryOnLiveHttpError) {
      this.defaultStreamingFailureCallback_(error);
    }
