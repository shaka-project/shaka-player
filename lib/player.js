...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async addSrcTrackElement_(uri, language, kind, mimeType, label,
    ...
    eventManager.listenOnce(trackElement, 'load', () => {
      p.resolve();
    });
    eventManager.listenOnce(trackElement, 'error', (e) => {
      p.reject(e);
    });
    ...
    try {
      await shaka.util.Functional.promiseWithTimeout(
          this.config_.streaming.loadTimeout, p.promise);
    } catch (e) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.RECOVERABLE,
          shaka.util.Error.Category.TEXT,
          shaka.util.Error.Code.ERROR_LOADING_EXTERNAL_TRACK_TO_SRC_EQUALS);
    } finally {
