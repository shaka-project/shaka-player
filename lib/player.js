...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async destroy() {
    ...
    shaka.log.error('destroy detach');
  ...
  async attach(mediaElement, initializeMediaSource = true) {
    ...
    try {
      ...
      shaka.log.error('detach 3', error);
      await this.detach();
      throw error;
    } finally {
      this.mutex_.release();
  ...
  async unload(initializeMediaSource = true, keepAdManager = false) {
    ...
    try {
      ...
      if (this.mediaSourceEngine_) {
        shaka.log.error('Destroy mediaSourceEngine_');
  ...
  async load(assetUriOrPreloader, startTime = null, mimeType) {
    ...
    if (this.assetUri_) {
      ...
      shaka.log.error('New load', assetUriOrPreloader);
