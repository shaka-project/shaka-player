 ...
 * @export
 */
shaka.Player.version = 'v2.4.0-uncompiled';
...
shaka.Player.prototype.loadManifest_ = async function(
    manifestUri, manifestParserFactory) {
 ...
 *
 * @param {string} manifestUri
 * @param {number=} setStartTime Optional start time, in seconds, to begin
 *   playback.
 ...
 * @export
 */
shaka.Player.prototype.load = async function(manifestUri, setStartTime,
                                             manifestParserFactory) {
  if (!this.video_) {
    throw new shaka.util.Error(
        shaka.util.Error.Severity.CRITICAL,
  ...
  try {
    ...
    this.playhead_ = this.createPlayhead(setStartTime);
...
shaka.Player.prototype.createPlayhead = function(startTime) {
  ...
  return new shaka.media.Playhead(
      ...
      startTime || null, this.onSeek_.bind(this), this.onEvent_.bind(this));
};
