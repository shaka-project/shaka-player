...
shaka.Player = class extends shaka.util.FakeEventTarget {
   ...
   * PlayerInterface closures are null-safe and report "nothing streaming" until
   * the engines exist.
   *
  ...
  createSkipRangeController_() {
    ...
    const playerInterface = {
      getContentTypes: () =>
        this.streamingEngine_ ? this.streamingEngine_.getContentTypes() : [],
      isBuffered: (type, time) => this.mediaSourceEngine_ ?
          this.mediaSourceEngine_.isBuffered(type, time) : false,
      bufferEnd: (type) => this.mediaSourceEngine_ ?
          this.mediaSourceEngine_.bufferEnd(type) : null,
      requestUpdate: () => {
        if (this.streamingEngine_) {
   ...
   * via removeSkipRange / clearSkipRanges.  DASH segments-mode, non-live,
  ...
  /**
   * @return {boolean} Whether the current manifest supports skip ranges (DASH,
   *   segments mode, non-live).  Logs a warning and returns false otherwise.
   * @private
   */
  skipRangesSupported_() {
    // TODO: Support HLS, sequence mode, and live.
    if (this.manifest_.type != shaka.media.ManifestParser.DASH ||
        this.manifest_.sequenceMode ||
        this.manifest_.presentationTimeline.isDynamic()) {
      shaka.log.warning('addSkipRange() supports DASH segments mode, VOD ' +
          'only; ignoring');
      return false;
    }
    return true;
   ...
   * (matched by the same start/end) and refetches it.  A removed range that
   * is already behind the playhead is not refetched until the user seeks
   * back across it.
   *
   * @param {number} start
   * @param {number} end
  ...
  /**
   * Removes all skip ranges and refetches them.
   *
   * @export
   */
  clearSkipRanges() {
  ...
  /**
   * Removes buffered media in [start, end) from every stream.  Intended to be
   * paired with <code>addSkipRange</code> to exclude a region that has already
   * been buffered (e.g. a watched ad): await this, then add the skip range.
   * The playhead must be outside the range or playback will stall on the hole.
   *
   * @param {number} start
   * @param {number} end
   * @return {!Promise}
   * @export
   */
  evictRange(start, end) {
    if (this.streamingEngine_) {
      return this.streamingEngine_.evictRange(start, end);
    }
