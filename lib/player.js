...
/**
 * @event shaka.Player.SwitchingToNewStreamEvent
 * @description Fired when an automatic stream change is done.
 * @property {string} type
 *   'switchingtonewstream'
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, videoContainer = null, dependencyInjector) {
    ...
    /** @private {!Array<string|shaka.media.PreloadManager>} */
    this.nextUrls_ = [];
  ...
  /**
   * Loads a new sequence of streams.
   * If another stream was already playing, first unloads that stream.
   *
   * @param {!Array<string|shaka.media.PreloadManager>} uris
   * @return {!Promise}
   * @export
   */
  async loadSequence(uris) {
    // Do not allow the player to be used after |destroy| is called.
    if (this.loadMode_ == shaka.Player.LoadMode.DESTROYED) {
      throw this.createAbortLoadError_();
    }
    this.nextUrls_ = uris || [];
    const nextUrl = this.nextUrls_.shift();
    if (!nextUrl) {
    ...
    try {
      ...
      if (shouldUseSrcEquals) {
        ...
        await mutexWrapOperation(async () => {
          ...
          const nextUrl = this.manifest_.nextUrl;
          if (nextUrl) {
            this.nextUrls_.unshift(nextUrl);
          }
        }, 'waitForFinish');
      ...
      if (!this.isLive()) {
        ...
        if (nextUrl) {
          if (this.config_.streaming.preloadNextUrlWindow > 0 &&
              !(nextUrl instanceof shaka.media.PreloadManager)) {
            /** @type {string} */
            const preloadUrl = nextUrl;
              ...
              if (!isNaN(timeToEnd)) {
                if (timeToEnd <= this.config_.streaming.preloadNextUrlWindow) {
                  this.loadEventManager_.unlisten(
                      this.video_, 'timeupdate', onTimeUpdate);
                  try {
                    this.preloadNextUrl_ = await this.preload(preloadUrl);
                  } catch (e) {
                    // Ignore errors.
                  }
                }
          ...
          this.loadEventManager_.listen(this, 'complete', () => {
            const data = new Map()
                .set('from', this.assetUri_)
                .set('to', nextUrl);
            const event = shaka.Player.makeEvent_(
                shaka.util.FakeEvent.EventName.SwitchingToNewStream, data);
            this.dispatchEvent(event);
            this.load(this.preloadNextUrl_ || nextUrl);
          });
        }
      }
      this.dispatchEvent(shaka.Player.makeEvent_(
