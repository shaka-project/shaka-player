...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onLoad_(has, wants) {
    ...
    console.info('onLoad_() currentTime=' + has.mediaElement.currentTime +
        ' startTime=' + has.startTime);
    const hasNoStartTime = has.startTime == null || has.startTime == undefined;
    if (has.mediaElement.currentTime && hasNoStartTime) {
      // If the application has set currentTime while the stream is loading
      // (e.g. in the 'manifestparsed' event handler) use that as the startTime
      has.startTime = has.mediaElement.currentTime;
    }
