...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, dependencyInjector) {
    ...
    if (mediaElement) {
      ...
      // TODO: Add deprecation warning, saying that in the future this will
      // be removed and users will be required to attach manually, since attach
      // is now asynchronous?
    }
  }
  ...
  async load(assetUri, startTime, mimeType) {
    try {
      ...
      if (shouldUseSrcEquals) {
        ...
        await this.initializeParserInner_(mimeType);
        await this.parseManifestInner_();
        await this.initializeDrmInner_(mimeType);
        await this.loadInner_(startTime, startTimeOfLoad);
      }
    } catch (error) {
      await this.unload(/* initializeMediaSource= */ false);
      throw error;
  ...
  async shouldUseSrcEquals_(assetUri, mimeType) {
    ...
    if (!mimeType) {
      ...
      const extension = shaka.media.ManifestParser.getExtension(assetUri);
    ...
    if (mimeType) {
      ...
      const canPlayMediaSource =
          shaka.media.ManifestParser.isSupported(assetUri, mimeType);
   ...
   * @private
   */
  async initializeParserInner_(mimeType) {
    goog.asserts.assert(
        this.assetUri_,
  ...
  async parseManifestInner_() {
    goog.asserts.assert(
        this.assetUri_,
        'should have a uri when parsing.');
    ...
    this.manifest_ = await this.parser_.start(this.assetUri_, playerInterface);
   ...
   * @private
   */
  async initializeDrmInner_(mimeType) {
    goog.asserts.assert(
        this.networkingEngine_,
   ...
   * @private
   */
  async loadInner_(startTime, startTimeOfLoad) {
    goog.asserts.assert(
        this.video_,
        'We should have a media element by now.');
    goog.asserts.assert(
        this.assetUri_,
        'We should have an asset uri by now.');
    goog.asserts.assert(
        !isNaN(startTime),
    ...
    if (this.updatedStartTime_ != null) {
      startTime = this.updatedStartTime_;
      this.updatedStartTime_ = null;
    }
  ...
  async initializeSrcEqualsDrmInner_(mimeType) {
    ...
    const variant = {
      ...
      video: {
        ...
        segmentIndex: null,
        mimeType: mimeType ?
            shaka.util.MimeUtils.getBasicType(mimeType) : mimeTypeGuess,
        codecs: mimeType ?
            shaka.util.MimeUtils.getCodecs(mimeType) : '',
        encrypted: true,
   ...
   * @private
   */
  async srcEqualsInner_(startTime, startTimeOfLoad) {
    goog.asserts.assert(
        this.video_,
        'We should have a media element when loading.');
    goog.asserts.assert(
        this.assetUri_,
        'We should have a valid uri when loading.');
    goog.asserts.assert(
        !isNaN(startTimeOfLoad),
        '|wants| should tell us when the load was originally requested');
    ...
    const extension = shaka.media.ManifestParser.getExtension(this.assetUri_);
    const mimeType =
      shaka.Player.SRC_EQUAL_EXTENSIONS_TO_MIME_TYPES_[extension];
