 ...
 * @description Fired when the manifest parsed find info about session data.
 * @property {string} type
 *   'sessiondata'
 * @property {string} id
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onInitializeMediaSourceEngine_(has, wants) {
    ...
    const onMetadata = (metadata, offset) => {
      if (this.adManager_) {
        // TODO: launch a shaka.Player.EventName.Metadata event here.
        // Convert metadata's timestamps into the start/end format.
        this.adManager_.onHlsTimedMetadata(metadata, offset);
      }
    };
    const mediaSourceEngine = this.createMediaSourceEngine(
        has.mediaElement, closedCaptionsParser, textDisplayer, onMetadata);
  ...
  async onInitializeParser_(has, wants) {
    ...
    shaka.log.warning(this.parser_);

    const onEvent = (event) => this.dispatchEvent(event);
    this.parser_.setOnEvent(onEvent);
  }
 ...
 * @export
 */
shaka.Player.version = 'v3.0.0-uncompiled';
