...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  makePreloadManager_(assetUri, mimeType, startTimeOfLoad) {
    ...
    // TODO: Refactor how the player interfaces for manifest and DRM parser work
    // so that they can hook into either the player or preload manager.
    ...
    // TODO: Filter this based on the saved config, not the current config.
    const filterManifest = async (manifest) => {
      await this.filterManifest_(manifest);
    };
    ...
    return new shaka.media.PreloadManager(
