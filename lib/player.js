...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  /**
   * Gets information about the currently playing video, audio, and text.
   * In the case of a multi-codec or multi-mimeType manifest, this can let you
   * determine the exact codecs and mimeTypes being played at the moment.
  ...
  getCurrentPlaybackInfo() {
    ...
    for (const stream of [variant.video, variant.audio, textStream]) {
      ...
      const position = stream.segmentIndex.find(this.video_.currentTime);
      const reference = stream.segmentIndex.get(position);
      const info = /** @type {!shaka.extern.PlaybackStreamInfo} */ ({
        'codecs': reference.codecs.split(','),
        'mimeType': reference.mimeType,
      });
      if (stream == variant.video) {
        output['video'] = info;
      } else if (stream == variant.audio) {
        output['audio'] = info;
      } else {
        output['text'] = info;
      }
