 ...
 * @export
 */
shaka.Player.version = 'v2.5.0-beta-master-uncompiled';
...
shaka.Player.prototype.load = async function(
  ...
  try {
    ...
    let variants = shaka.util.StreamUtils.filterVariantsByPreference(
...
shaka.Player.prototype.getAudioLanguagesAndRoles = function() {
  ...
  const audioStreams = new Set(variants.map((variant) => variant.audio));
...
shaka.Player.prototype.getAudioLanguages = function() {
  ...
  const period = this.streamingEngine_.getCurrentPeriod();
  const variants = shaka.util.StreamUtils.getPlayableVariants(period.variants);
...
shaka.Player.prototype.getTextLanguages = function() {
  ...
  const languages = new Set(textStreams.map((stream) => stream.language));
 ...
 * available for them.
 *
 * @param {!Iterable.<?shaka.extern.Stream>|
 *         !Iterable.<shaka.extern.Stream>} streams
 * @return {!Array.<shaka.extern.LanguageRole>}
 * @private
 */
shaka.Player.prototype.getLanguagesAndRoles_ = function(streams) {
  ...
  for (const stream of streams) {
    // If there are no roles, add a "no role" role so that we will add a
    // language-only option.
    const language = stream ? stream.language : noLanguage;
    const roles = stream && stream.roles.length ? stream.roles : [noRole];

    for (const role of roles) {
      // Look for a combination that looks like the combination we are about
      // to make.
      const found = combinations.some((existing) => {
        return existing.language == language && existing.role == role;
      });

      if (!found) {
        /** @type {shaka.extern.LanguageRole} */
        const combination = {language: language, role: role};
...
shaka.Player.prototype.onChooseStreams_ = function(period) {
  ...
  let variants = StreamUtils.filterVariantsByPreference(
      period.variants,
