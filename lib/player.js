...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  configure(config, value) {
    ...
    // Deprecate 'manifest.dash.sequenceMode' configuration.
    if (config['manifest'] && config['manifest']['dash'] &&
        'sequenceMode' in config['manifest']['dash']) {
      shaka.Deprecate.deprecateFeature(6,
          'manifest.dash.sequenceMode configuration',
          'Sequence mode has been removed. Segments mode is always used.');
      delete config['manifest']['dash']['sequenceMode'];
    }

    // Deprecate 'manifest.hls.sequenceMode' configuration.
    if (config['manifest'] && config['manifest']['hls'] &&
        'sequenceMode' in config['manifest']['hls']) {
      shaka.Deprecate.deprecateFeature(6,
          'manifest.hls.sequenceMode configuration',
          'Sequence mode has been removed. Segments mode is always used.');
      delete config['manifest']['hls']['sequenceMode'];
    }
