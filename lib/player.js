 ...
 * That may be caused by and adapatation change or DASH period transition.
 * Separate events are emmited for audio and video contentTypes
 * @property {string} type
 *   'mediaqualitychanged'
 * @property {shaka.extern.MediQualityInfo} detail
 *   An object containing information about the media quality.
 * @exportDoc
 */
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  onParseManifest_(has, wants) {
    ...
    this.qualityObserver_.setListeners((mediaQualityInfo) => {
      this.onMediaQualityChange_(mediaQualityInfo);
    });
  ...
  createPlayheadObserversForMSE_() {
    ...
    goog.asserts.assert(this.qualityObserver_, 'Must have quality timeline');
  ...
  createStreamingEngine() {
    ...
    const playerInterface = {
      ...
      onInitSegmentAppended: (position, initSegment) => {
        if (initSegment.mediaQuality) {
          this.qualityObserver_.addMediaQualityChange(
              initSegment.mediaQuality, position);
        }
      },
    };
   ...
   * @private
   */
  onMediaQualityChange_(mediaQuality) {
    // Always make a copy to avoid exposing our internal data to the app.
    const clone = {
      ...
      numChannels: mediaQuality.numChannels,
      pixelAspectRatio: mediaQuality.pixelAspectRatio,
      width: mediaQuality.width,
    };

    this.dispatchEvent(this.makeEvent_(
        shaka.Player.EventName.MediaQualityChanged,
        {detail: clone}));
  }
