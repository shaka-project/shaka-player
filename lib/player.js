...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, videoContainer = null, dependencyInjector) {
    ...
    /**
     * DrmEngine kept alive by config.drm.keepDrmEngineOnDetach, separate
     * from this.drmEngine_ so an intervening load() for unrelated content
     * doesn't clobber it. Reused by a matching load(), else destroyed.
     * @private {shaka.drm.DrmEngine}
     */
    this.keptDrmEngine_ = null;

    /**
     * The assetUri_ this.keptDrmEngine_ belongs to.
     * @private {?string}
     */
    this.drmEngineAssetUri_ = null;
  ...
  async destroy() {
    ...
    // unload() never destroys a kept DrmEngine; destroy it here instead.
    if (this.keptDrmEngine_) {
      await this.keptDrmEngine_.destroy();
      this.keptDrmEngine_ = null;
    }
    this.drmEngineAssetUri_ = null;
  ...
  async unload(initializeMediaSource = true, keepAdManager = false,
    ...
    // Only keep an engine that actually negotiated DRM — a clear-content
    // engine has nothing worth preserving and would otherwise evict a real
    // kept engine from the single slot.
    const keepDrmEngine =
        this.config_.drm.keepDrmEngineOnDetach && !!this.drmEngine_ &&
        !!this.drmEngine_.getDrmInfo();
    if (keepDrmEngine) {
      if (this.keptDrmEngine_ && this.keptDrmEngine_ != this.drmEngine_) {
        // Earlier kept engine not reused — destroy before it's replaced.
        await this.keptDrmEngine_.destroy();
      }
      this.keptDrmEngine_ = this.drmEngine_;
      this.drmEngineAssetUri_ = this.assetUri_;
      this.drmEngine_ = null;
    }
  ...
  async load(assetUriOrPreloader, startTime = null, mimeType) {
    ...
    // Reuse this.keptDrmEngine_ only if this load targets the exact asset
    // it was kept for. Otherwise leave it alone (not destroy it) — this
    // load() may be for an unrelated transient asset (e.g. an ad) with the
    // app intending to return to the kept asset afterwards.
    const reuseDrmEngine = this.config_.drm.keepDrmEngineOnDetach &&
        !!this.keptDrmEngine_ && this.drmEngineAssetUri_ === assetUri;
    ...
    try {
      ...
      if (!preloadManager) {
        ...
        if (preloadManager) {
          ...
          if (reuseDrmEngine && this.keptDrmEngine_) {
            // Before start(), so its own initializeDrm() skips negotiating
            // an unused throwaway DrmEngine of its own.
            preloadManager.entrustDrmEngine(this.keptDrmEngine_);
          }
      ...
      if (shouldUseSrcEquals) {
        await mutexWrapOperation(async () => {
          ...
          await this.initializeSrcEqualsDrmInner_(mimeType, reuseDrmEngine);
        ...
        if (reuseDrmEngine) {
          // Reuse the kept engine instead of negotiating a new session.
          this.drmEngine_ = this.keptDrmEngine_;
          this.keptDrmEngine_ = null;
          this.drmEngineAssetUri_ = null;
          await mutexWrapOperation(async () => {
   ...
   *
   * @param {string} mimeType
   * @param {boolean=} reuseDrmEngine If true, re-attach this.keptDrmEngine_
   *   instead of creating a new one.
   * @return {!Promise}
   * @private
   */
  async initializeSrcEqualsDrmInner_(mimeType, reuseDrmEngine = false) {
    goog.asserts.assert(
