...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onLoad_(has, wants) {
    ...
    shaka.util.StreamUtils.chooseCodecsAndFilterManifest(
        ...
        this.config_.streaming.codecSwitchingStrategy);
    ...
    // Note: Skip if an audio codec switching strategy is being enforced.
    if (this.config_.streaming.codecSwitchingStrategy ===
      ...
      'due to codec switching strategy.');
    }
  ...
  createStreamingEngine() {
    ...
    const playerInterface = {
      ...
      beforeInitAppendSegment: (contentType, stream,
          codecSwitchingStrategy) => {
        return this.mediaSourceEngine_.determineCodecSwitch(contentType,
            stream, codecSwitchingStrategy);
      },
