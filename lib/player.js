...
shaka.Player.prototype.getTextTracks = function() {
  ...
  if (!this.activeStreams_.get(currentPeriodIndex, ContentType.TEXT)) {
    ...
    if (textStreams.length) {
      this.activeStreams_.update(
          currentPeriodIndex, ContentType.TEXT, textStreams[0].id);
    }
  }

  const tracks = [];

  const activeTextId = this.activeStreams_.get(
      currentPeriodIndex,
      ContentType.TEXT);
