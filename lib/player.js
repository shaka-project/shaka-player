...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async onLoad_(has, wants) {
    ...
    // Filter out things with the incorrect HDR level, if desired.
    if (this.config_.preferredVideoHdrLevel) {
      shaka.util.StreamUtils.filterManifestByHDRLevel(
          this.manifest_, this.config_.preferredVideoHdrLevel);
    }
