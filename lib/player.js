...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  getStats() {
    ...
    const audioNormalization = Math.round(
        this.video_.volume / this.getMaxAudioVolume() * 100) + '% / ' +
        Math.round(this.video_.volume * 100) + '%';
    this.stats_.setAudioNormalization(audioNormalization);
  ...
  /**
   * Get max volume of the audio.
   *
   * @return {number}
   * @export
   */
  getMaxAudioVolume() {
    return this.config_.maxAudioVolume || 1;
  }
};
