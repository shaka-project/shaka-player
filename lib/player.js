...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  /**
   * Returns the value of video buffered by video buffering goals
   * to check if the downloading can keep pace with the video playback.
   * @return {number}
   * @export
   */
  getBufferFullness() {
    ...
    const lengthToBeBuffered = this.video_ ? this.video_.currentTime +
        Math.min(this.getConfiguration().streaming.bufferingGoal,
            (this.seekRange().end - this.video_.currentTime)) : 0;
    ...
    if (this.video_) {
      if (bufferedEnd_ >= lengthToBeBuffered) {
        ...
        return (bufferedEnd_/lengthToBeBuffered);
      }
    }
    return 0;
  }
