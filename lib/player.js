...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  onTimeUpdate_() {
    ...
    if (this.config_.streaming.vodDynamicPlaybackRate) {
      const minPlaybackRate = this.config_.streaming.vodDynamicPlaybackRateMin;
      const bufferFullness = this.getBufferFullness();
      const bufferThreshold =
        this.config_.streaming.vodDynamicPlaybackRateBufferPercentage;

      if (bufferFullness <= bufferThreshold) {
        if (playbackRate != minPlaybackRate) {
          shaka.log.debug('Buffer fullness ratio (' + bufferFullness + ') ' +
            'is less than the vodDynamicPlaybackRateBufferPercentage (' +
