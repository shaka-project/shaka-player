...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, videoContainer = null, dependencyInjector) {
    ...
    /** @private {?function(): ?shaka.media.PreloadManager} */
    this.getPreloadManager_ = null;
  ...
  async makePreloadManager_(assetUri, startTime, mimeType, preloadConfig,
    ...
    const getPreloadManager = () => {
      ...
      if (!preloadManager ||
          (preloadManager.hasBeenAttached() && preloadManager.isDestroyed())) {
        return null;
      }
      return preloadManager;
    };
    this.getPreloadManager_ = getPreloadManager;
  ...
  applyConfig_(prevConfig) {
    ...
    if (this.getPreloadManager_) {
      const preloadManager = this.getPreloadManager_();
      if (preloadManager) {
        preloadManager.getManifestFilterer().setConfig(this.config_);
      }
    }
    if (this.drmEngine_) {
      this.drmEngine_.configure(this.config_.drm);
    ...
    if (this.abrManager_) {
      ...
      if (this.config_.abr.restrictions.restrictedVideoCodecs.length > 0) {
        this.config_.abr.restrictions.restrictedVideoCodecs.length = 0;
      }
      if (this.config_.abr.restrictions.restrictedAudioCodecs.length > 0) {
        this.config_.abr.restrictions.restrictedAudioCodecs.length = 0;
      }
      this.abrManager_.configure(this.config_.abr);
