...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async loadInner_(startTimeOfLoad, prefetchedVariant, segmentPrefetchById) {
    ...
    if (this.config_.streaming.resetMediaSourceOnStreamSwitch) {
      this.setupResetMediaSourceOnStreamSwitch_(mediaElement);
    }
   ...
   * @param {HTMLMediaElement} mediaElement
   */
  setupResetMediaSourceOnStreamSwitch_(mediaElement) {
    const getLastEndTime = () => {
      const activeVariant = this.streamingEngine_.getCurrentVariant();
      if (!activeVariant) {
        return null;
      }
      const time = this.playhead_.getTime();
      for (const stream of activeVariant.video.matchedStreams) {
        const references = stream.segmentIndex.references;
        if (!references.length) {
          continue;
        }
        const lastEndTime = references[references.length - 1].endTime;
        if (time >= references[0].startTime && time < lastEndTime) {
          return lastEndTime;
        }
      }
      return null;
    ...
    const fireTimerIfNecessary = () => {
      const endTime = getLastEndTime();
      const time = this.playhead_.getTime();
      // When we're within 1s from the end, we spawn a timer and assume the
      // period will end within that timeframe.
      if (endTime !== null && endTime - time < /* MIN_THRESHOLD_SECONDS= */ 1) {
        timer.tickAfter(endTime - time);
      }
    };
    ...
    const timer = new shaka.util.Timer(() => {
      ...
      const endTime = getLastEndTime();
      if (endTime !== null) {
        mediaElement.currentTime = endTime + /* END_TIME_NUDGE= */ 0.01;
      }
    });

    this.loadEventManager_.listen(mediaElement, 'timeupdate',
        fireTimerIfNecessary);
    // When we're in paused state and we want to play, possibly reset MSE.
    this.loadEventManager_.listen(mediaElement, 'play', fireTimerIfNecessary);
    // When we seeked within the threshold, possibly reset MSE.
    this.loadEventManager_.listen(mediaElement, 'seeked', fireTimerIfNecessary);
  }
};
