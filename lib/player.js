...
shaka.Player = class extends shaka.util.FakeEventTarget {
      ...
      if (dimensions) {
        ...
        for (let i = 0; i < numThumbnails; i++) {
          ...
          const thumbnail = this.getThumbnailByReference_(reference,
              ...
              dimensions);
          // If the thumbnail is missing, all thumbnails in this reference are
          // also missing.
          if (!thumbnail) {
            break;
          }
   ...
   * @param {number} thumbnailTime
   * @param {{columns: number, rows: number}} dimensions
   * @return {?shaka.extern.Thumbnail}
   * @private
   */
  getThumbnailByReference_(reference, imageStream, thumbnailTime, dimensions) {
    ...
    if (totalImages > 1) {
      const thumbnailPosition =
          Math.floor((thumbnailTime - reference.startTime) / thumbnailDuration);
