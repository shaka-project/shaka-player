...
shaka.Player.prototype.getTextTracks = function() {
  ...
  const activeTextId = this.activeStreams_.get(currentPeriod, ContentType.TEXT);
  ...
  for (const stream of currentPeriod.textStreams) {
    ...
    track.active = stream.id == activeTextId;
...
shaka.Player.prototype.addTextTrack = function(
  ...
  return this.streamingEngine_.loadNewTextStream(stream)
    ...
    if (activeText) {
      ...
      this.activeStreams_.update(period, ContentType.TEXT, activeText.id);
    }
...
shaka.Player.prototype.updateActiveStreams_ = function(stream) {
  ...
  this.activeStreams_.update(period, stream.type, stream.id);
};
...
shaka.Player.prototype.assertCorrectActiveStreams_ = function() {
  ...
  activeStreams.filter((stream) => stream).forEach((stream) => {
    ...
    goog.asserts.assert(
        this.activeStreams_.get(currentPeriod, type) == id,
...
shaka.Player.prototype.getPresentationVariant_ = function() {
  ...
  const activeAudioId = this.activeStreams_.get(
      currentPeriod, shaka.util.ManifestParserUtils.ContentType.AUDIO);
  const activeVideoId = this.activeStreams_.get(
      currentPeriod, shaka.util.ManifestParserUtils.ContentType.VIDEO);

  for (const variant of currentPeriod.variants) {
    // Make sure that we treat a null stream as having a null id and then check
    // for a match.
    const audioId = variant.audio ? variant.audio.id : null;
    const videoId = variant.video ? variant.video.id : null;

    if (activeAudioId == audioId && activeVideoId == videoId) {
      return variant;
    }
  }
