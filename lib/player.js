  ...
  async loadInner_(startTimeOfLoad, prefetchedVariant, segmentPrefetchById) {
    ...
    if ((isLive && (this.config_.streaming.liveSync ||
      ...
      if (this.manifest_.nextUrl) {
        if (this.config_.streaming.minCompletionPreloadNextUrl > 0 &&
            this.config_.streaming.minCompletionPreloadNextUrl < 100) {
          const onTimeUpdate = async () => {
            const completionRatio =
                this.video_.currentTime / this.video_.duration;
            if (!isNaN(completionRatio)) {
              const percent = Math.round(100 * completionRatio);
              const minCompletionPreloadNextUrl =
                  this.config_.streaming.minCompletionPreloadNextUrl;
              if (percent >= minCompletionPreloadNextUrl) {
