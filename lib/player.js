 ...
 */
goog.define('GIT_VERSION', 'v2.1.7-debug');
...
shaka.Player.prototype.chooseStreams_ =
  ...
  if (activeVariant) {
    ...
    // Make a list of all possible pairs of video & audio.
    // This may include duplicates or nulls.
    var allPossibleStreamPairs = variants.map(function(variant) {
      return [variant.video, variant.audio];
    });
    // Collapse the pairs into a flat list, then add text streams.
    var possibleStreams = allPossibleStreamPairs
        .reduce(shaka.util.Functional.collapseArrays, []);
    possibleStreams.push.apply(possibleStreams, textStreams);
    // Finally, remove nulls and duplicates.
    possibleStreams = possibleStreams
        .filter(shaka.util.Functional.isNotNull)
        .filter(shaka.util.Functional.isNotDuplicate);

    // Now ask AbrManager to update any type where the active stream could no
    // longer be used (based on the variants we could choose from).
    for (var type in activeStreams) {
      var stream = activeStreams[type];
      if (possibleStreams.indexOf(stream) == -1) {
  ...
  if (needsUpdate.length > 0) {
    ...
    var chosen = {};
    try {
      // AbrManager must choose new streams for all the types in |needsUpdate|.
      // For example, 'video', 'audio', and/or 'text'.
