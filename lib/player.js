...
goog.require('shaka.event.AbrStatusChanged');
goog.require('shaka.event.Adaptation');
goog.require('shaka.event.AudioTrackChanged');
goog.require('shaka.event.AudioTracksChanged');
goog.require('shaka.event.Buffering');
goog.require('shaka.event.CanUpdateStartTime');
goog.require('shaka.event.Complete');
goog.require('shaka.event.ConfigurationChanged');
goog.require('shaka.event.DownloadCompleted');
goog.require('shaka.event.DownloadFailed');
goog.require('shaka.event.DownloadHeadersReceived');
goog.require('shaka.event.Emsg');
goog.require('shaka.event.Error');
goog.require('shaka.event.ExpirationUpdated');
goog.require('shaka.event.FirstQuartile');
goog.require('shaka.event.KeyStatusChanged');
goog.require('shaka.event.Loaded');
goog.require('shaka.event.Loading');
goog.require('shaka.event.ManifestUpdated');
goog.require('shaka.event.MediaQualityChanged');
goog.require('shaka.event.MediaSourceRecovered');
goog.require('shaka.event.Metadata');
goog.require('shaka.event.MetadataAdded');
goog.require('shaka.event.Midpoint');
goog.require('shaka.event.RateChange');
goog.require('shaka.event.SegmentAppended');
goog.require('shaka.event.Started');
goog.require('shaka.event.StateChange');
goog.require('shaka.event.StateChanged');
goog.require('shaka.event.Streaming');
goog.require('shaka.event.TextChanged');
goog.require('shaka.event.TimelineRegionAdded');
goog.require('shaka.event.TimelineRegionEnter');
goog.require('shaka.event.TimelineRegionExit');
goog.require('shaka.event.ThirdQuartile');
goog.require('shaka.event.TracksChanged');
goog.require('shaka.event.Unloading');
goog.require('shaka.event.VariantChanged');
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  makeStateChangeEvent_(nodeName) {
    this.dispatchEvent(new shaka.event.StateChange(nodeName));
  }
  ...
  async unload(initializeMediaSource = true, keepAdManager = false,
    ...
    try {
      ...
      this.dispatchEvent(new shaka.event.Unloading(isSwitchingContent));
  ...
  async load(assetUriOrPreloader, startTime = null, mimeType) {
    ...
    try {
      ...
      this.dispatchEvent(new shaka.event.Loading());
      ...
      this.metadataRegionTimeline_.addEventListener('regionadd', (event) => {
        ...
        this.dispatchMetadataAddedEvent_(region);
      });
      ...
      this.dispatchEvent(new shaka.event.Loaded());
    } catch (error) {
  ...
  async makePreloadManager_(assetUri, startTime, mimeType, preloadConfig,
    ...
    const manifestPlayerInterface = {
      ...
      filter: async (manifest) => {
        ...
        if (tracksChanged) {
          ...
          preloadManager.dispatchEvent(new shaka.event.TracksChanged());
        }
      },
      ...
      onManifestUpdated: () => {
        preloadManager.dispatchEvent(new shaka.event.ManifestUpdated(
            this.isLive(), this.isInProgress()));
    ...
    regionTimeline.addEventListener('regionadd', (event) => {
      ...
      const region = event['region'];
      this.onRegionAdded_(region, preloadManager);
    ...
    const drmPlayerInterface = {
      ...
      onExpirationUpdated: (id, expiration) => {
        preloadManager.dispatchEvent(new shaka.event.ExpirationUpdated());
  ...
  async loadInner_(startTimeOfLoad, prefetchedVariant, segmentPrefetchById) {
    ...
    this.dispatchEvent(new shaka.event.Streaming());
    ...
    this.dispatchEvent(new shaka.event.CanUpdateStartTime());
  ...
  async initializeSrcEqualsDrmInner_(mimeType) {
    ...
    this.drmEngine_ = this.createDrmEngine({
      ...
      onExpirationUpdated: (id, expiration) => {
        this.dispatchEvent(new shaka.event.ExpirationUpdated());
      },
      onEvent: (e) => {
  ...
  /**
   * Construct and fire metadata event
   *
   * @param {shaka.extern.MetadataTimelineRegionInfo} region
   * @private
   */
  dispatchMetadataEvent_(region) {
    this.dispatchEvent(new shaka.event.Metadata(
        region.startTime, region.endTime, region.schemeIdUri, region.payload));
  }

  /**
   * Construct and fire metadata added event
   *
   * @param {shaka.extern.MetadataTimelineRegionInfo} region
   * @private
   */
  dispatchMetadataAddedEvent_(region) {
    this.dispatchEvent(new shaka.event.MetadataAdded(
        region.startTime, region.endTime, region.schemeIdUri, region.payload));
  }
  ...
  dispatchEmsgEvent_(region) {
    this.dispatchEvent(new shaka.event.Emsg(region.emsg));
  }
  ...
  createNetworkingEngine(getPreloadManager) {
    ...
    const onHeadersReceived_ = (headers, request, requestType) => {
      dispatchEvent(new shaka.event.DownloadHeadersReceived(
    ...
    const onDownloadCompleted_ = (requestType, request, context, response) => {
      dispatchEvent(new shaka.event.DownloadCompleted(
          requestType, request, context, response));
    };
    ...
    const onDownloadFailed_ =
        (requestType, request, context, error, httpResponseCode, aborted) => {
          dispatchEvent(new shaka.event.DownloadFailed(
              requestType, request, context, error, httpResponseCode, aborted));
        };
  ...
  createPlayheadObserversForMSE_(startTime) {
    ...
    regionObserver.addEventListener('enter', (event) => {
      /** @type {shaka.extern.TimelineRegionInfo} */
      const region = event['region'];
      this.onRegionEnter_(region);
    });

    regionObserver.addEventListener('exit', (event) => {
      /** @type {shaka.extern.TimelineRegionInfo} */
      const region = event['region'];
      this.onRegionExit_(region);
    });
  ...
  applyConfig_(prevConfig) {
    ...
    this.dispatchEvent(new shaka.event.ConfigurationChanged());
  }
  ...
  updateBufferState_() {
    ...
    // buffering.
    this.dispatchEvent(new shaka.event.Buffering(isBuffering));
  }
  ...
  onRateChange_() {
    ...
    if (newRate == 0) {
      ...
      if (currentPlaybackRate !== 0 && currentPlaybackRate !== 1) {
        this.dispatchEvent(new shaka.event.RateChange());
      }
      return;
    }
  ...
  updateStateHistory_() {
    ...
    if (stateChanged) {
      this.dispatchEvent(new shaka.event.StateChanged(updateState));
    }
  }
  ...
  onVideoProgress_() {
    ...
    if (this.isQuartile_(0, percent)) {
      event = new shaka.event.Started();
    } else if (this.isQuartile_(25, percent)) {
      event = new shaka.event.FirstQuartile();
    } else if (this.isQuartile_(50, percent)) {
      event = new shaka.event.Midpoint();
    } else if (this.isQuartile_(75, percent)) {
      event = new shaka.event.ThirdQuartile();
    } else if (this.isQuartile_(100, percent) || percent > 100) {
      event = new shaka.event.Complete();
      this.checkEnded_();
    } else {
      this.checkEnded_();
    }
  ...
  onSegmentAppended_(start, end, contentType, isMuxed, isDependency) {
    ...
    this.dispatchEvent(new shaka.event.SegmentAppended(
        start, end, contentType, isMuxed, isDependency));
  }
  ...
  onAdaptation_(from, to) {
    ...
    this.delayDispatchEvent_(new shaka.event.Adaptation(from, to));
  }
  ...
  onTracksChanged_() {
    ...
    this.delayDispatchEvent_(new shaka.event.TracksChanged());
  ...
  onVariantChanged_(from, to) {
    ...
    // the changes before the user tries to query it.
    this.delayDispatchEvent_(new shaka.event.VariantChanged(from, to));
  }
  ...
  onAudioTracksChanged_() {
    ...
    // absorb the changes before the user tries to query it.
    this.delayDispatchEvent_(new shaka.event.AudioTracksChanged());
  }
  ...
  onTextChanged_() {
    ...
    // changes before the user tries to query it.
    this.delayDispatchEvent_(new shaka.event.TextChanged());
  }
  ...
  onAbrStatusChanged_() {
    ...
    this.delayDispatchEvent_(
        new shaka.event.AbrStatusChanged(this.config_.abr.enabled));
  }
  ...
  async onError_(error) {
    ...
    if (this.manifest_ && this.streamingEngine_ &&
      ...
      if (playableVariants.length > 1 || this.fullyLoaded_) {
        ...
        try {
          ...
          if (ret) {
            this.dispatchEvent(new shaka.event.MediaSourceRecovered());
            return;
          }
        } catch (e) {
          // Ignore errors.
    ...
    const event = new shaka.event.Error(error);
    this.dispatchEvent(event);
   ...
   * @private
   */
  onRegionAdded_(region, eventTarget = this) {
    eventTarget.dispatchEvent(
        new shaka.event.TimelineRegionAdded(this.cloneRegion_(region)));
  }
   ...
   * @private
   */
  onRegionEnter_(region, eventTarget = this) {
    eventTarget.dispatchEvent(
        new shaka.event.TimelineRegionEnter(this.cloneRegion_(region)));
  }
   ...
   * @private
   */
  onRegionExit_(region, eventTarget = this) {
    eventTarget.dispatchEvent(
        new shaka.event.TimelineRegionExit(this.cloneRegion_(region)));
  }
  ...
  onMediaQualityChange_(mediaQuality, position, audioTrackChanged = false) {
    ...
    if (audioTrackChanged) {
      this.dispatchEvent(new shaka.event.AudioTrackChanged(clone, position));
    } else {
      this.dispatchEvent(new shaka.event.MediaQualityChanged(clone, position));
    }
  }
  ...
  onKeyStatus_(keyStatusMap) {
    ...
    this.dispatchEvent(new shaka.event.KeyStatusChanged());
