...
shaka.Player.prototype.getVariantTracks = function() {
  ...
  const activeAudio = this.activeStreams_.get(currentPeriod, ContentType.AUDIO);
  const activeVideo = this.activeStreams_.get(currentPeriod, ContentType.VIDEO);

  return shaka.util.StreamUtils.getVariantTracks(
      currentPeriod,
      activeAudio ? activeAudio.id : null,
...
shaka.Player.prototype.getTextTracks = function() {
  ...
  if (!this.activeStreams_.get(currentPeriod, ContentType.TEXT)) {
    ...
    if (textStreams.length) {
      this.activeStreams_.use(currentPeriod, textStreams[0]);
    }
  }

  const tracks = [];

  const activeText = this.activeStreams_.get(currentPeriod, ContentType.TEXT);

  for (const stream of currentPeriod.textStreams) {
    // Don't show any tracks that are being loaded still.
    if (this.loadingTextStreams_.has(stream)) {
      continue;
    }

    const track = shaka.util.StreamUtils.textStreamToTrack(stream);
    track.active = stream == activeText;
...
shaka.Player.prototype.addTextTrack = function(
  ...
  return this.streamingEngine_.loadNewTextStream(stream)
    ...
    const activeText = this.streamingEngine_.getActiveText();
    if (activeText) {
      ...
      this.activeStreams_.use(period, activeText);
    }
...
shaka.Player.prototype.updateActiveStreams_ = function(stream) {
  ...
  const periodIndex = shaka.util.StreamUtils.findPeriodContainingStream(
      this.manifest_, stream);
  const period = this.manifest_.periods[periodIndex];

  this.activeStreams_.use(period, stream);
};
...
shaka.Player.prototype.assertCorrectActiveStreams_ = function() {
  ...
  const currentPeriod = this.streamingEngine_.getCurrentPeriod();
  const currentPeriodIndex = this.manifest_.periods.indexOf(currentPeriod);
  ...
  activeStreams.filter((stream) => stream).forEach((stream) => {
    ...
    if (type == ContentType.TEXT && this.deferredTextStream_) {
      expectedStream = this.deferredTextStream_;
    }

    if (type == ContentType.AUDIO && this.deferredVariant_) {
      expectedStream = this.deferredVariant_.audio;
    }

    if (type == ContentType.VIDEO && this.deferredVariant_) {
      expectedStream = this.deferredVariant_.video;
    }

    goog.asserts.assert(
        this.activeStreams_.get(currentPeriod, type) == expectedStream,
...
shaka.Player.prototype.onTextChanged_ = async function() {
  ...
  let event = new shaka.util.FakeEvent('textchanged');
  this.dispatchEvent(event);
};
...
shaka.Player.prototype.getSelectableText_ = function() {
  ...
  // Don't return streams that are still loading.
...
shaka.Player.prototype.getPresentationVariant_ = function() {
  ...
  // Can't have a variant when there is no period.
  if (currentPeriod == null) { return null; }

  const activeAudio = this.activeStreams_.get(
      currentPeriod, shaka.util.ManifestParserUtils.ContentType.AUDIO);
  const activeVideo = this.activeStreams_.get(
      currentPeriod, shaka.util.ManifestParserUtils.ContentType.VIDEO);

  for (const variant of currentPeriod.variants) {
    if (activeAudio == variant.audio && activeVideo == variant.video) {
      return variant;
    }
  }
