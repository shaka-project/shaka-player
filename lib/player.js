...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  /**
   * Return a list of uris with the image tracks that can be switched to.
   *
   * <p>
   * If the player has not loaded content, this will return an empty list.
   *
  ...
  async getThumbnails(trackId, time) {
    if (this.manifest_) {
      ...
      if (totalImages > 1) {
        const position = Math.floor((time - reference.startTime) *
            totalImages / (reference.endTime - reference.startTime));
        left = (position % columns) / columns * 100;
        top = (position % rows) / rows * 100;
      }
      return Promise.resolve({
        height: height,
        left: left,
