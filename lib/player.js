...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  /**
   * Select an video track compatible with the current audio track.
   * If the player has not loaded any content, this will be a no-op.
   *
  ...
  getVideoTracks() {
    ...
    /** @type {!Set<shaka.extern.VideoTrack>} */
    const videoTracksSet = new Set();
    for (const track of filteredTracks) {
      ...
      if (!videoTracksSet.has(videoTrack)) {
        videoTracksSet.add(videoTrack);
      }
    }
    if (!videoTracksSet.size) {
      return [];
    }
    return Array.from(videoTracksSet);
  }
