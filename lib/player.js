  ...
  async load(assetUriOrPreloader, startTime = null, mimeType) {
    ...
    try {
      ...
      if (shouldUseSrcEquals) {
        ...
        // await preloadManager.filterManifest();
        await mutexWrapOperation(async () => {
          await preloadManager.filterManifest();
        }, 'filterManifest');
  ...
  getVariantTracks() {
    if (this.manifest_ && !this.isRemotePlayback()) {
      ...
      if (currentVariant !== null) {
        console.log(`[APPLE, Player]: getVariantTracks(); currentVariant=${shaka.util.StreamUtils.getVariantSummaryString(currentVariant)}; full info: ${JSON.stringify(currentVariant)}`);
      } else {
        console.log(`[APPLE, Player]: getVariantTracks(); currentVariant=null`);
      }
      ...
      for (const variant of this.manifest_.variants) {
        console.log(`[APPLE]: getVariantTracks(): Converting track=${shaka.util.StreamUtils.getVariantSummaryString(variant)}`);
        if (!shaka.util.StreamUtils.isPlayable(variant)) {
          console.log(`[APPLE]: Not playable!`);
          continue;
        }
        ...
        if (!track.active && activeTracks != 1 && currentVariant != null &&
          ...
          console.log(`[APPLE, Player]: getVariantTracks():  Making ${shaka.util.StreamUtils.getVariantSummaryString(variant)} active; full info: ${JSON.stringify(variant)}`);
          track.active = true;
        }
...
shaka.Player.version = 'v4.16.9' + '-uncompiled';  // x-release-please-version
