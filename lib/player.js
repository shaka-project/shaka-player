...
shaka.Player = class extends shaka.util.FakeEventTarget {
   ...
   * @export
   */
  async addTextTrack(uri, language, kind, mimeType, codec, label) {
    if (this.loadMode_ != shaka.Player.LoadMode.MEDIA_SOURCE &&
        this.loadMode_ != shaka.Player.LoadMode.SRC_EQUALS) {
      shaka.log.error(
      ...
      return Promise.reject(() => {
        return new shaka.util.Error(
            ...
            shaka.util.Error.Category.PLAYER,
            shaka.util.Error.Code.PLAYER_UNINITIALIZED);
      });
    }
    ...
    if (this.loadMode_ == shaka.Player.LoadMode.SRC_EQUALS) {
      if (mimeType == 'text/srt') {
        try {
          ...
          const vvtText = SrtTextParser.srt2webvtt(stringData);
      ...
      if (srcTrack) {
        this.onTracksChanged_();
        return Promise.resolve(srcTrack);
      }
