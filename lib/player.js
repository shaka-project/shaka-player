...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, dependencyInjector) {
    ...
    /** @private {shaka.media.LiveCatchUpController} */
    this.liveCatchUpController_ = null;
  ...
  unload(initializeMediaSource = true) {
    ...
    const events = this.walker_.startNewRoute((currentPayload) => {
      ...
      if (this.liveCatchUpController_) {
        this.liveCatchUpController_.disable();
      }
  ...
  async onLoad_(has, wants) {
    ...
    if (!this.abrManager_ || this.abrManagerFactory_ != abrFactory) {
      ...
      if (this.abrManager_ instanceof shaka.abr.SimpleLLAbrManager) {
        const playerInterface = {
          getBufferLevel: () => {
            const video = mediaElement;
            const bufferInfo = this.getBufferedInfo();
            if (video.paused || bufferInfo.video.length < 1) {
              return 0;
            }
            const videoBufferEnd =
                bufferInfo.video[bufferInfo.video.length - 1].end;
            const bufferLevel =
                Math.max(0, videoBufferEnd - video.currentTime);
            return bufferLevel;
          },
          getPresentationLatency: () => this.getPresentationLatency(),
          getServiceDescription: () => this.getServiceDescription(),
        };

        /** @type {shaka.abr.SimpleLLAbrManager} */
        const llabr = this.abrManager_;
        llabr.setPlayerInterface(playerInterface);
      }
    ...
    if (this.config_.streaming.lowLatencyMode &&
        this.config_.streaming.liveCatchUp.enabled) {
      this.liveCatchUpController_ = this.createLiveCatchUpController();
      this.liveCatchUpController_.configure(this.config_.streaming.liveCatchUp);
      this.liveCatchUpController_.setVideoElement(mediaElement);
      this.liveCatchUpController_.enable();
    }
  ...
  createStreamingEngine() {
    ...
    const getPresentationLatencyData = () => {
      if (this.parser_ instanceof shaka.dash.DashParser) {
        return this.parser_.getPresentationLatencyData();
      } else {
        return null;
      }
    };
    ...
    const playerInterface = {
      ...
      getPresentationLatencyData: getPresentationLatencyData,
    };
  ...
  /**
   *
   * @return {!shaka.media.LiveCatchUpController}
   */
  createLiveCatchUpController() {
    goog.asserts.assert(
        this.playhead_ && this.manifest_,
        'Must not be destroyed');

    const LiveCatchUpController = shaka.media.LiveCatchUpController;
    const TimeRangesUtils = shaka.media.TimeRangesUtils;

    const getServiceDescription = () => {
      if (this.parser_ instanceof shaka.dash.DashParser) {
        return this.parser_.getServiceDescription();
      } else {
        return null;
      }
    };

    /** @type {shaka.media.LiveCatchUpController.PlayerInterface} */
    const playerInterface = {
      getBufferEnd: () => TimeRangesUtils.bufferEnd(this.video_.buffered),
      getPlayRate: () => this.playRateController_ ?
        this.playRateController_.getRealRate() : 0,
      getPresentationTime: () => this.playhead_.getTime(),
      getPresentationLatency: () => this.getPresentationLatency(),
      trickPlay: (rate) => this.trickPlay(rate),
      getServiceDescription: getServiceDescription,
    };

    return new LiveCatchUpController(playerInterface);
  }
  ...
  applyConfig_() {
    ...
    if (this.liveCatchUpController_) {
      this.liveCatchUpController_.configure(
          this.config_.streaming.liveCatchUp);
    }
  }
  ...
  /**
   * Get presentation Latency
   *
   * @return {Array.<Object>} PresentationLatency as ms
   * @export
   */
  getPresentationLatency() {
    ...
    return this.streamingEngine_.getPresentationLatency();
  }
  ...
  getMediaState(contentType) {
    ...
    return this.streamingEngine_.getMediaState(contentType);
  }
  ...
  updateBufferState_() {
    ...
    if (isBuffering) {
      this.abrManager_.onBuffering();
    } else {
      this.abrManager_.onBufferingEnd();
    }
  }
