...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  /**
   * Returns latest heigth of variant.
   *
   * @return {number}
   * @export
   */
  getCurrentVariantHeight() {
    const currentVariant = this.streamingEngine_ ?
          this.streamingEngine_.getCurrentVariant() : null;
    if (currentVariant === null) {
      return 0;
    } else {
      return Number(currentVariant.video.height);
    }
  }
   ...
   * @export
   */
  selectAudioLanguage(language, role, _trackheight = 0) {
    ...
    if (this.manifest_ && this.playhead_) {
      ...
      this.chooseVariantAndSwitch_(_trackheight);
    } else if (this.video_ && this.video_.audioTracks) {
      const audioTracks = Array.from(this.video_.audioTracks);
   ...
   * @private
   */
  chooseVariant_(_trackheight = 0) {
    if (this.updateAbrManagerVariants_()) {
      return this.abrManager_.chooseVariant(_trackheight);
    } else {
      return null;
    }
  }
   ...
   * @private
   */
  chooseVariantAndSwitch_(_trackheight = 0) {
    ...
    // here.
    const chosenVariant = this.chooseVariant_(_trackheight);
    if (chosenVariant) {
