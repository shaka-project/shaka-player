...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, dependencyInjector) {
    ...
    if (shaka.Player.adManagerFactory_) {
      const AdFactory = shaka.Player.adManagerFactory_;
      this.adManager_ = new AdFactory();
    }
  ...
  load(assetUri, startTime, mimeType) {
    ...
    if (mimeType && typeof mimeType != 'string') {
      shaka.Deprecate.deprecateFeature(
          ...
          'Please register a manifest parser for the mime-type.');
      const Factory =
  ...
  trickPlay(rate) {
    ...
    if (this.loadMode_ == shaka.Player.LoadMode.MEDIA_SOURCE) {
      this.abrManager_.setPlaybackRate(rate);
  ...
  cancelTrickPlay() {
    ...
    if (this.loadMode_ == shaka.Player.LoadMode.MEDIA_SOURCE) {
      this.playRateController_.set(1);
      this.abrManager_.setPlaybackRate(1);
  ...
  getStats() {
    ...
    if (this.loadMode_ == shaka.Player.LoadMode.MEDIA_SOURCE) {
      ...
      if (variant) {
        const rate = this.abrManager_.getPlaybackRate();
        this.stats_.setVariantBandwidth(rate * variant.bandwidth);
      }
   ...
   * @private
   */
  chooseTextAndSwitch_(period, fireAdaptationEvent = true) {
    goog.asserts.assert(this.config_, 'Must not be destroyed');
