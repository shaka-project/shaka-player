...
/**
 * @event shaka.Player.MetadataTimelineRegionAddedEvent
 * @description Fired when a media metadata timeline region is added.
 * @property {string} type
 *   'metadatatimelineregionadded'
 * @property {shaka.extern.MetadataTimelineRegionInfo} detail
 *   An object which contains a description of the region.
 * @exportDoc
 */
...
/**
 * @event shaka.Player.MetadataTimelineRegionEnterEvent
 * @description Fired when the playhead enters a metadata timeline region.
 * @property {string} type
 *   'metadatatimelineregionenter'
 * @property {shaka.extern.MetadataTimelineRegionInfo} detail
 *   An object which contains a description of the region.
 * @exportDoc
 */
...
/**
 * @event shaka.Player.MetadataTimelineRegionExitEvent
 * @description Fired when the playhead exits a metadata timeline region.
 * @property {string} type
 *   'metadatatimelineregionexit'
 * @property {shaka.extern.MetadataTimelineRegionInfo} detail
 *   An object which contains a description of the region.
 * @exportDoc
 */
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  addMetadataToRegionTimeline_(startTime, endTime, metadataType, payload) {
    ...
    region.id = shaka.util.StringUtils.hash(JSON.stringify(region));
  ...
  createPlayheadObserversForMSE_(startTime) {
    ...
    metadataRegionObserver.addEventListener('enter', (event) => {
      /** @type {shaka.extern.MetadataTimelineRegionInfo} */
      const region = event['region'];
      this.onMetadataRegionEvent_(
          shaka.util.FakeEvent.EventName.MetadataTimelineRegionEnter, region);
    });

    metadataRegionObserver.addEventListener('exit', (event) => {
      /** @type {shaka.extern.MetadataTimelineRegionInfo} */
      const region = event['region'];
      this.onMetadataRegionEvent_(
          shaka.util.FakeEvent.EventName.MetadataTimelineRegionExit, region);
    });

    metadataRegionObserver.addEventListener('skip', (event) => {
      /** @type {shaka.extern.MetadataTimelineRegionInfo} */
      const region = event['region'];
      /** @type {boolean} */
      const seeking = event['seeking'];
   ...
   * @private
   */
  onMetadataRegionEvent_(eventName, region, eventTarget = this) {
    // Always make a copy to avoid exposing our internal data to the app.
    /** @type {shaka.extern.MetadataTimelineRegionInfo} */
    const clone = {
      schemeIdUri: region.schemeIdUri,
