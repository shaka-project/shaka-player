...
shaka.Player = class extends shaka.util.FakeEventTarget {
   ...
   * @private
   */
  addBasicMediaListeners_(mediaElement) {
  ...
  async loadInner_(startTimeOfLoad, prefetchedVariant, segmentPrefetchById) {
    ...
    this.addBasicMediaListeners_(mediaElement);
   ...
   */
  getVariantTracks() {
    if (this.manifest_ && !this.isRemotePlayback_()) {
  ...
  selectTextTrack(track) {
    if (this.manifest_ && this.streamingEngine_&& !this.isRemotePlayback_()) {
  ...
  selectAudioLanguage(language, role, channelsCount = 0, safeMargin = 0,
      codec = '') {
    if (this.manifest_ && this.playhead_&& !this.isRemotePlayback_()) {
      this.currentAdaptationSetCriteria_ =
          new shaka.media.PreferenceBasedCriteria(
              language,
              role || '',
