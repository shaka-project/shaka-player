...
shaka.Player.prototype.selectVariantTrack = function(
  ...
  if (this.manifest_ && this.streamingEngine_) {
    ...
    // TODO: Safari's native HLS has audioTracks/vidoeTracks on the element.
  }
};
...
shaka.Player.prototype.getAudioLanguagesAndRoles = function() {
  ...
  // TODO: Make generic through variant tracks instead
  if (this.manifest_ && this.playhead_) {
...
shaka.Player.prototype.getTextLanguagesAndRoles = function() {
  // TODO: Make generic through text tracks instead
  if (this.manifest_ && this.playhead_) {
...
shaka.Player.prototype.getTextLanguages = function() {
  ...
  if (this.manifest_ && this.playhead_) {
    return Array.from(
        shaka.Player.getLanguagesFrom_(this.getSelectableText_()));
  } else {
    // TODO: Safari's native HLS has textTracks on HTMLMediaElement.
    return [];
  }
...
shaka.Player.prototype.setTextTrackVisibility = async function(isVisible) {
  ...
  // TODO: Move this inside updateTextVisibility_?  It's called after all
  // but one call to updateTextVisibility_.
};
...
shaka.Player.prototype.updateTextVisibility_ = async function(isVisible) {
  ...
  } else { // if (this.loadMode_ == shaka.Player.LoadMode.SRC_EQUALS) {
    // TODO: Safari's native HLS has textTracks on HTMLMediaElement.
  }
};
