...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async parseManifestInner_(mimeType) {
    ...
    const playerInterface = {
      ...
      configureLowLatency: () => this.configureLowLatency_(),
    };
  ...
  /**
   * Configure the default lowLatency values.
   *
   * @private
   */
  configureLowLatency_() {
    this.configure('streaming.inaccurateManifestTolerance', 0);
    this.configure('streaming.rebufferingGoal', 0.01);
    this.configure('streaming.segmentPrefetchLimit', 2);
    this.configure('streaming.retryParameters.baseDelay', 100);
    this.configure('manifest.retryParameters.baseDelay', 100);
    this.configure('drm.retryParameters.baseDelay', 100);
  }
