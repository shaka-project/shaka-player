...
shaka.Player.prototype.load = function(manifestUri, opt_startTime,
  ...
  return loadChain.then(function() {
    ...
    return this.streamingEngine_.init(this.config_.text);
  }.bind(this)).then(function() {
    ...
    // period.
    var variants = shaka.util.StreamUtils.filterVariantsByConfig(
        this.manifest_.periods[0], this.config_);
...
shaka.Player.prototype.selectTrack = function(track, opt_clearBuffer) {
  ...
  var period = this.streamingEngine_.getCurrentPeriod();
  if (track.type == 'text') {
    var text = StreamUtils.findTextStreamForTrack(period, track);
  ...
  streams.forEach(function(stream) {
    streamsToSwitch[stream.type] = stream;
  });
...
shaka.Player.prototype.defaultConfig_ = function() {
  return {
    ...
    text: shaka.media.TextEngine.defaultTextConfig(),
    restrictions: {
      minWidth: 0,
      maxWidth: Infinity,
      minHeight: 0,
...
shaka.Player.prototype.chooseStreamsAndSwitch_ = function(period) {
  ...
  var textStreams = shaka.util.StreamUtils.filterTextStreamsByConfig(
      period, this.config_, languageMatches);
