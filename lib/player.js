...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  /**
   * Starts the dropped frame poller if the feature is enabled and
   * MediaCapabilities is not supported.
   *
   * @private
   */
  startDroppedFramePoller_() {
    const config = this.config_.streaming.droppedFrameProtection;
    if (!config.enabled) {
      return;
    }

    // If MediaCapabilities is supported, it already handles smooth playback
    // so if MediaCapabilities is supported, this feature is not needed.
  ...
  checkDroppedFrames_() {
    ...
    // Skip checks during buffering or paused state.
    if (this.isBuffering() || this.video_.paused) {
      return;
    }
    ...
    if (dropRatio >= config.dropThreshold) {
      ...
      if (currentVariant && currentVariant.video) {
        this.disableStream(currentVariant.video, config.banDuration);
      }
    }
  }
  ...
  switchVariant_(variant, fromAdaptation, clearBuffer, safeMargin,
    ...
    // Reset dropped frame counters on variant change so we don't penalize the
    // new stream for frames dropped during the previous one.
    this.lastDroppedFrames_ = 0;
    this.lastTotalFrames_ = 0;
