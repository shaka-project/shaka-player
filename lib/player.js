...
goog.require('shaka.util.Metadata');
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async srcEqualsInner_(startTimeOfLoad, mimeType) {
    ...
    if (this.mimeType_ &&
        ...
        shaka.util.Metadata.supports(this.mimeType_)) {
      ...
      // VOD detection (only materialize as Blob when the response is a finite,
      // non-ICY resource).
      // ICY headers (icy-br, icy-name, etc.) indicate a live Shoutcast/Icecast
      // stream, which has no Content-Length and cannot be buffered into a Blob.
      // In that case we leave |data| undefined and fall back to the original
      // URL, skipping metadata extraction entirely.
      if (response.headers['content-length'] &&
          !response.headers['icy-br'] &&
          !response.headers['icy-description'] &&
          !response.headers['icy-genre'] &&
    ...
    if (this.mimeType_ && data) {
      ...
      const frames =
          shaka.util.Metadata.getMetadataFrames(data, this.mimeType_);
      if (frames.length) {
