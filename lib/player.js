...
goog.require('shaka.media.Scte35Observer');
...
/**
 * @event shaka.Player.Scte35UpdatedEvent
 * @description Fired when a retained SCTE-35 message gains new information.
 * @property {string} type 'scte35updated'
 * @property {shaka.extern.Scte35Event} detail The updated normalized message.
 * @exportDoc
 */

/**
 * @event shaka.Player.Scte35Event
 * @description Fired when playback reaches or crosses a SCTE-35 message.
 * Seeking across a message does not fire this event. Replaying it may fire
 * again. Messages discovered after their presentation time are not replayed.
 * @property {string} type 'scte35'
 * @property {shaka.extern.Scte35Event} detail The normalized message.
 * @exportDoc
 */
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async makePreloadManager_(assetUri, startTime, mimeType, preloadConfig,
    ...
    const manifestPlayerInterface = {
      ...
      onScte35Event: (event) => {
        preloadManager.getScte35Timeline().addEvent(event);
      },
    ...
    const scte35Timeline = new shaka.media.Scte35Timeline(() => {
      // A preload may belong to a different asset from the current player.
      const timeline = preloadManager?.getManifest()?.presentationTimeline;
      return {
        start: timeline ? timeline.getSeekRangeStart() : 0,
        end: timeline ? timeline.getSeekRangeEnd() : Infinity,
      };
    });
    for (const type of ['scte35added', 'scte35updated']) {
      scte35Timeline.addEventListener(type, (event) => {
        preloadManager.dispatchEvent(event);
      });
    }
    ...
    const mediaSourceEngine = this.createMediaSourceEngine(
        ...
        this.textDisplayer_,
        {
          ...
          onEmsg: (emsg) => {
            ...
            const event = shaka.util.Scte35.fromEmsg(emsg);
            if (event && this.scte35Timeline_) {
              this.scte35Timeline_.addEvent(event);
            }
          },
  ...
  createPlayheadObserversForMSE_(startTime) {
    ...
    if (this.scte35Timeline_) {
      const scte35Observer = new shaka.media.Scte35Observer(
          this.scte35Timeline_, startsPastZero);
      scte35Observer.addEventListener('scte35', (event) => {
        this.dispatchEvent(event);
      });
  ...
  getAllScte35Events() {
    ...
    return Array.from(this.scte35Timeline_.events()).map(
        (event) => shaka.media.Scte35Timeline.clone(event));
  }
