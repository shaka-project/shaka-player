...
shaka.Player.prototype.applyPlayRange_ = function() {
  ...
  if (playRangeStart > 0) {
    if (this.isLive()) {
      ...
      this.manifest_.presentationTimeline.setAvailabilityStart(playRangeStart);
    }
  }
 ...
 *   Set a negative number to start with a certain offset from the end. This
 *   is intended for use with live streams, to start at a fixed offset from
 *   the live edge.
 * @param {shakaExtern.ManifestParser.Factory=} opt_manifestParserFactory
 *   Optional manifest parser factory to override auto-detection or use an
 *   unregistered parser.
