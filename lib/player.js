...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  getLiveLatency() {
    ...
    // MSF/MoQ live has no PDT or wall-clock PST; the live edge is defined
    // purely by segment timestamps. Latency is computed from the latest
    // segment end minus current playback time.
    if (this.manifest_?.type == shaka.media.ManifestParser.MSF) {
      const timeline = this.manifest_?.presentationTimeline;
      if (timeline?.isDynamic()) {
        const maxSegmentEndTime = timeline.getMaxSegmentEndTime();
        if (maxSegmentEndTime != null) {
          const latencySeconds = maxSegmentEndTime - this.video_.currentTime;
          return Math.max(0, Math.floor(latencySeconds * 1000));
        }
      }
    }
