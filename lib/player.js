...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async attach(mediaElement, initializeMediaSource = true) {
    try {
      ...
      if (this.video_ && this.video != mediaElement) {
        await this.detach();
      }
      this.operationId_ += 1;
  ...
  async load(assetUri, startTime = null, mimeType) {
    ...
    try {
      // Add a mechanism to detect if the load process has been interrupted by
  ...
  async guessMimeType_() {
    ...
    let mimeType = '';
    try {
      await this.mutex_.acquire('guessMimeType_');
  ...
  async initializeMediaSourceEngineInner_() {
    try {
      await this.mutex_.acquire('initializeMediaSourceEngineInner_');
  ...
  async parseManifestInner_(mimeType) {
    try {
      await this.mutex_.acquire('parseManifestInner_');
  ...
  async initializeDrmInner_() {
    try {
      await this.mutex_.acquire('initializeDrmInner_');
  ...
  async loadInner_(startTimeOfLoad) {
    try {
      await this.mutex_.acquire('loadInner_');
  ...
  async initializeSrcEqualsDrmInner_(mimeType) {
    ...
    try {
      await this.mutex_.acquire('initializeSrcEqualsDrmInner_');
  ...
  async srcEqualsInner_(startTimeOfLoad, mimeType) {
    try {
      await this.mutex_.acquire('srcEqualsInner_');
