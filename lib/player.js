...
goog.require('shaka.media.ProgramDateTimeObserver');
...
goog.require('shaka.media.SkipRangeController');
...
/**
 * @event shaka.Player.Scte35AddedEvent
 * @description Fired as soon as a SCTE-35 message is discovered.
 * @property {string} type 'scte35added'
 * @property {shaka.extern.Scte35Event} detail The normalized message.
 * @exportDoc
 */

/**
 * @event shaka.Player.Scte35EnterEvent
 * @description Fired when playback reaches a SCTE-35 message.  Seeking across
 * a message does not fire this event.
 * @property {string} type 'scte35enter'
 * @property {shaka.extern.Scte35Event} detail The normalized message.
 * @exportDoc
 */

/**
 * @event shaka.Player.Scte35ExitEvent
 * @description Fired when playback leaves a SCTE-35 message.  A message the
 * transport gave no duration applies at a single point in time, so it is
 * entered and left in the same poll.
 * @property {string} type 'scte35exit'
 * @property {shaka.extern.Scte35Event} detail The normalized message.
 * @exportDoc
 */
...
/**
 * @event shaka.Player.TimelineChangedEvent
 * @description Fired when the playhead moves into a part of the presentation
 *   that has a different PROGRAM-DATE-TIME base than the part it was in before.
 *   A new base is introduced by an HLS discontinuity whose PROGRAM-DATE-TIME
 *   jumps relative to the media timeline (e.g. server-side ad insertion or a
 *   live restart); the presentation timeline itself remains continuous.  Only
 *   fired for MSE playback of HLS content with EXT-X-PROGRAM-DATE-TIME; never
 *   fired for DASH or src= playback.
 * @property {string} type
 *   'timelinechange'
 * @property {Date} programDateTime
 *   The new wall-clock program date time base for the region the playhead has
 *   entered.
 * @property {number} timestamp
 *   The presentation time, in seconds, where the new region begins.
 * @exportDoc
 */
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, videoContainer = null, dependencyInjector) {
    ...
    /**
     * @private {?shaka.media.RegionTimeline<shaka.extern.Scte35Event>}
     */
    this.scte35Timeline_ = null;
    ...
    /**
     * The set of skip ranges.  Owned by the Player so ranges added before load
     * survive until streaming starts (e.g. a preroll skipped before buffering).
     * @private {!shaka.media.SkipRangeController}
     */
    this.skipRangeController_ = this.createSkipRangeController_();
  ...
  async destroy() {
    ...
    // destroy() is terminal: the Player cannot be used afterward, so it must
    // not queue behind an in-flight operation.  shaka.util.Mutex.acquire() has
    // no timeout and no abort, so an operation that wedges below the JS layer
    // (for example a MediaSource append whose updateend never fires, seen on
    // some smart TV and console platforms) holds the mutex forever.  Without
    // this, detach() would wait on that mutex and destroy() would stay pending
    // indefinitely -- never resolving and never rejecting, so callers cannot
    // even detect the failure.  Releasing here lets unload() run, which tears
    // down the wedged MediaSourceEngine.  Operations displaced this way are
    // already guarded by the operationId_ check in
    // atomicOperationAcquireMutex_ and bail out when they next wake.
  ...
  async unload(initializeMediaSource = true, keepAdManager = false,
    ...
    try {
      ...
      if (this.scte35Timeline_) {
        this.scte35Timeline_.release();
        this.scte35Timeline_ = null;
      }
      ...
      this.skipRangeController_.clear();
  ...
  async load(assetUriOrPreloader, startTime = null, mimeType) {
    ...
    try {
      ...
      if (shouldUseSrcEquals) {
        ...
        // For a standard load() the preload hook has already applied
        // manifest-signaled CMCD parameters; this covers a preload manager
        // that was created by preload() and attached afterwards.
        this.updateCmcdManifestParameters_();
        ...
        this.scte35Timeline_ = preloadManager.receiveScte35Timeline();
  ...
  async makePreloadManager_(assetUri, startTime, mimeType, preloadConfig,
    ...
    const manifestPlayerInterface = {
      ...
      onScte35Event: (event) => {
        preloadManager.getScte35Timeline().addRegion(event);
      },
      ...
      onManifestUpdated: () => {
        ...
        // A background preload must not reconfigure the active player's
        // CMCD reporter; only forward once this manifest is the one playing.
        if (preloadManager.hasBeenAttached()) {
          this.updateCmcdManifestParameters_();
        }
      },
    ...
    /** @type {!shaka.media.RegionTimeline<shaka.extern.Scte35Event>} */
    const scte35Timeline =
        new shaka.media.RegionTimeline(() => this.seekRange());
    scte35Timeline.addEventListener('regionadd', (event) => {
      preloadManager.dispatchEvent(shaka.Player.makeEvent_(
          shaka.util.FakeEvent.EventName.Scte35Added,
          new Map().set('detail', event['region'])));
    });
    ...
    const playerInterface = {
      ...
      onManifestParsed: (manifest) => {
        // A background preload must not reconfigure the active player's CMCD
        // reporter; a standard load() attaches before it starts parsing.
        if (preloadManager.hasBeenAttached()) {
          this.updateCmcdManifestParameters_(manifest);
        }
      },
    };
    ...
    const mediaSourceEngine = this.createMediaSourceEngine(
        ...
        this.textDisplayer_,
        {
          ...
          onEmsg: (emsg) => {
            ...
            if (this.scte35Timeline_) {
              const scte35 = shaka.util.Scte35.fromEmsg(emsg);
              if (scte35) {
                this.scte35Timeline_.addRegion(scte35);
              }
            }
          },
    ...
    this.playRateController_ = new shaka.media.PlayRateController({
      ...
      movePlayhead: (delta) => {
        ...
        if (newTime < seekRange.start) {
          ...
          // When rewinding (negative rate) the step may be large enough to land
          // inside a gap between two buffered ranges, where the media element
          // would stall (readyState drops and the guard above then freezes the
          // playhead).  Snap back into the buffered range just below the target
          // so playback can keep moving.  We land a bit inside the range (not
          // right at its end) because seeking to the very edge of a range can
          // fall after its last decodable frame and stall.
          if (delta < 0) {
            const gapMargin = 0.5;
            const info = shaka.media.TimeRangesUtils.getBufferedInfo(
                mediaElement.buffered);
            for (let i = info.length - 1; i >= 0; i--) {
              if (newTime >= info[i].start) {
                if (newTime > info[i].end) {
                  newTime = Math.max(info[i].start, info[i].end - gapMargin);
                }
                break;
              }
            }
  ...
  async loadInner_(startTimeOfLoad, prefetchedVariant, segmentPrefetchById) {
    ...
    // Re-apply skip ranges added before load, now that streaming exists to run
    // the guard and buffered checks.  Drain first so they are not double-added.
    const preloadSkipRanges = this.skipRangeController_.getAll().slice();
    this.skipRangeController_.clear();
    for (const range of preloadSkipRanges) {
      this.addSkipRange(range.start, range.end);
    }
    ...
    const serviceDescription = this.manifest_.serviceDescription;
    const hasLatencyTargets = !!serviceDescription && (
      serviceDescription.targetLatency != null ||
      serviceDescription.maxLatency != null ||
      serviceDescription.minLatency != null ||
      serviceDescription.maxPlaybackRate != null ||
      serviceDescription.minPlaybackRate != null);
  ...
  createPlayhead(startTime) {
    ...
    return new shaka.media.MediaSourcePlayhead(
        ...
        () => this.skipRangeController_.getAll());
  }
  ...
  createPlayheadObserversForMSE_(startTime) {
    ...
    goog.asserts.assert(this.scte35Timeline_, 'Must have SCTE-35 timeline');
    ...
    /** @type {!shaka.media.RegionObserver<shaka.extern.Scte35Event>} */
    const scte35Observer = new shaka.media.RegionObserver(
        this.scte35Timeline_, startsPastZero);

    scte35Observer.addEventListener('enter', (event) => {
      this.dispatchEvent(shaka.Player.makeEvent_(
          shaka.util.FakeEvent.EventName.Scte35Enter,
          new Map().set('detail', event['region'])));
    });

    scte35Observer.addEventListener('exit', (event) => {
      this.dispatchEvent(shaka.Player.makeEvent_(
          shaka.util.FakeEvent.EventName.Scte35Exit,
          new Map().set('detail', event['region'])));
    });

    // Watch for the playhead crossing a discontinuity that introduces a new
    // PROGRAM-DATE-TIME base, so we can notify the app via 'timelinechange'.
    const programDateTimeObserver = new shaka.media.ProgramDateTimeObserver(
        this.manifest_.presentationTimeline);

    programDateTimeObserver.addEventListener('change', (event) => {
      this.dispatchEvent(shaka.Player.makeEvent_(
          shaka.util.FakeEvent.EventName.TimelineChanged, new Map([
            ['programDateTime', event['programDateTime']],
    ...
    manager.manage(scte35Observer);
    manager.manage(programDateTimeObserver);
  ...
  pollBufferState_(event) {
    ...
    // When playing in reverse the buffer is filled behind the playhead, so the
    // amount of lead we have is what is buffered behind the current time.
    const bufferLead = this.getPlaybackRate() < 0 ?
        shaka.media.TimeRangesUtils.bufferedBehindOf(
            this.video_.buffered, this.video_.currentTime) :
        shaka.media.TimeRangesUtils.bufferedAheadOf(
            this.video_.buffered, this.video_.currentTime);
  ...
  /**
   * Forward the manifest's client data reporting configuration (DASH
   * ServiceDescription/ClientDataReporting, ISO/IEC 23009-1:2026 Annex K)
   * to the CMCD manager. Called when a foreground preload has parsed its
   * manifest, at load, on manifest updates, and (through
   * CmcdManager.reset()) cleared on unload.
   *
   * @param {?shaka.extern.Manifest=} manifest Defaults to the manifest
   *   being played.
   * @private
   */
  updateCmcdManifestParameters_(manifest = this.manifest_) {
    if (!this.cmcdManager_) {
      return;
    }
    const serviceDescription = manifest ? manifest.serviceDescription : null;
    this.cmcdManager_.setManifestParameters(
        serviceDescription ? serviceDescription.clientDataReporting : null);
  }
  ...
  /**
   * Creates the SkipRangeController.  Called from the constructor, so the
   * PlayerInterface closure is null-safe and no-ops until the streaming engine
   * exists.
   *
   * @return {!shaka.media.SkipRangeController}
   * @private
   */
  createSkipRangeController_() {
    /** @type {shaka.media.SkipRangeController.PlayerInterface} */
    const playerInterface = {
      requestUpdate: () => {
        this.streamingEngine_?.requestSkipRangeUpdate();
      },
      getPlaybackRate: () => this.getPlaybackRate(),
      isRegionBuffered: (start, end) =>
        this.streamingEngine_?.isSkipRegionBuffered(start, end) ?? false,
    };
  ...
  createStreamingEngine() {
    ...
    return new shaka.media.StreamingEngine(
        this.manifest_, playerInterface, this.skipRangeController_);
  }
  ...
  /**
   * Get SCTE-35 messages retained on the presentation timeline.
   * @return {!Array<shaka.extern.Scte35Event>}
   * @export
   */
  getAllScte35Events() {
    return Array.from(this.scte35Timeline_?.regions() || []);
  }
  ...
  /**
   * Declares a range [start, end) to be skipped: its segments are not fetched
   * or buffered, and buffering continues from <code>end</code> as if it
   * followed <code>start</code>, with the gap-jumping logic carrying the
   * playhead across.  Must be declared before the range buffers, and must not
   * overlap an existing range.
   *
   * <p>
   * May be called before load(); such ranges are applied when streaming
   * starts, which lets a preroll be skipped before it buffers.  Recoverable
   * via removeSkipRange / clearSkipRanges.  Segments-mode, non-live,
   * MediaSource-only (not src=).
   *
   * @param {number} start
   * @param {number} end
   * @return {boolean} True if the range was accepted; false if it was rejected
   *   (unsupported manifest, empty/reversed interval, overlaps an existing
   *   range, or already buffered).  Before load() only the interval is checked;
   *   the range is re-validated when streaming starts.
   * @export
   */
  addSkipRange(start, end) {
    // The stream-type guard is here, not in the controller, so the controller
    // has no manifest dependency.  Before load there is no manifest; the range
    // is queued and re-validated when streaming starts.
    if (this.manifest_ && !this.skipRangesSupported_()) {
      return false;
    }
    return this.skipRangeController_.add(start, end);
  }

  /**
   * @return {boolean} Whether the current manifest supports skip ranges
   *   (segments mode, non-live).  Logs a warning and returns false otherwise.
   * @private
   */
  skipRangesSupported_() {
    if (this.manifest_.sequenceMode ||
        this.manifest_.presentationTimeline.isDynamic()) {
      shaka.log.warning('addSkipRange() supports segments mode, VOD only; ' +
          'ignoring');
      return false;
    }
    return true;
  ...
  /**
   * Removes a skip range previously added with <code>addSkipRange</code>
   * (matched by the same start/end) and re-fetches it.  A removed range that
   * is already behind the playhead is not re-fetched until the user seeks
   * back across it.
   *
   * @param {number} start
   * @param {number} end
   * @export
   */
  removeSkipRange(start, end) {
    this.skipRangeController_.remove(start, end);
  }

  /**
   * Removes all skip ranges and re-fetches them.
   *
   * @export
   */
  clearSkipRanges() {
  ...
  /**
   * Gets the current skip ranges as a sorted, disjoint list of
   * <code>{start, end}</code> intervals.  Reflects any merging done at
   * <code>addSkipRange</code> time and any splitting/trimming done at
   * <code>removeSkipRange</code> time.  The returned array is a copy; mutating
   * it does not affect the player.
   *
   * @return {!Array<shaka.extern.BufferedRange>}
   * @export
   */
  getSkipRanges() {
    return this.skipRangeController_.getAll().slice();
  }
  ...
  selectVariantTrack(track, clearBuffer = false, safeMargin = 0) {
    const selectMediaSourceMode = () => {
      ...
      const videoPref = {
        ...
        language: (variant.video && variant.video.language) || '',
  ...
  selectVideoTrack(videoTrack, clearBuffer = false, safeMargin = 0) {
    ...
    const variantsWithVideo = variants.filter((t) => {
      return (t.videoBandwidth || t.bandwidth) == videoTrack.bandwidth &&
          ...
          (t.videoLanguage || 'und') == (videoTrack.language || 'und') &&
  ...
  getVideoTracks() {
    ...
    for (const track of filteredTracks) {
      ...
      const videoTrack = {
        active: track.active,
        language: track.videoLanguage || 'und',
  ...
  getPlayheadTimeAsDate() {
    ...
    if (this.manifest_ && !this.isRemotePlayback()) {
      const timeline = this.manifest_.presentationTimeline;
      // Accounts for PROGRAM-DATE-TIME jumps at discontinuities when region
      // information is available; otherwise extrapolates from the initial
      // program date time.
      const programDateTime = timeline.getProgramDateTimeForTime(
          presentationTime);
      if (programDateTime != null) {
        return new Date(/* ms= */ programDateTime * 1000);
      }
      ...
      if (video && video.closedCaptions) {
        for (const id of video.closedCaptions.keys()) {
          if (!closedCaptionsSet.has(id)) {
            ...
            const textStream = shaka.util.StreamUtils.createStream({
 ...
 * @export
 */
shaka.Player.version = goog.define('shaka.Player.version',
    // eslint-disable-next-line no-useless-concat
    'v5.2.0' + '-uncompiled');  // x-release-please-version
