...
shaka.Player = function(video, opt_dependencyInjector) {
  // TODO(modmaker): Remove once Player uses the Observer.  We need the
  // goog.require so it is loaded in uncompiled mode.  This line is needed so
  // we don't get extra require errors.
  goog.asserts.assert(shaka.media.PlayheadObserver, '');
...
shaka.Player.prototype.load = function(manifestUri, opt_startTime,
  ...
  return loadChain.then(function() {
    ...
    this.pendingTimelineRegions_.forEach(
        this.playhead_.addTimelineRegion.bind(this.playhead_));
    this.pendingTimelineRegions_ = [];
...
/**
 * Callback from Playhead.
 * @param {number} prevPeriod
 * @param {number} curPeriod
 * @private
 */
shaka.Player.prototype.onChangePeriod_ = function(prevPeriod, curPeriod) {
  // TODO(modmaker): Track streams for current Period.
  this.onTracksChanged_();
};
...
shaka.Player.prototype.onTimelineRegionAdded_ = function(region) {
  if (this.playhead_) {
    this.playhead_.addTimelineRegion(region);
  } else {
