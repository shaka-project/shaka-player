...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, videoContainer = null, dependencyInjector) {
    ...
    /**
     * Skip ranges added before streaming exists; applied when it starts.
     * @private {!Array<{start: number, end: number}>}
     */
    this.pendingSkipRanges_ = [];
  ...
  async loadInner_(startTimeOfLoad, prefetchedVariant, segmentPrefetchById) {
    ...
    // Apply any skip ranges added before load, before streaming buffers into
    // them (e.g. a preroll).
    for (const range of this.pendingSkipRanges_) {
      this.streamingEngine_.addSkipRange(range.start, range.end);
    }
    this.pendingSkipRanges_ = [];
  ...
  createPlayhead(startTime) {
    ...
    return new shaka.media.MediaSourcePlayhead(
        ...
        () => this.streamingEngine_ ?
            this.streamingEngine_.getSkipRanges() : []);
  }
  ...
  addSkipRange(start, end) {
    if (this.streamingEngine_) {
      this.streamingEngine_.addSkipRange(start, end);
    } else {
      // Not loaded yet; queue the range and apply it when streaming starts.
      this.pendingSkipRanges_.push({start, end});
    }
  }
  ...
  removeSkipRange(start, end) {
    if (this.streamingEngine_) {
      this.streamingEngine_.removeSkipRange(start, end);
    } else {
      this.pendingSkipRanges_ = this.pendingSkipRanges_.filter((r) => {
  ...
  clearSkipRanges() {
    this.pendingSkipRanges_ = [];
    if (this.streamingEngine_) {
      this.streamingEngine_.clearSkipRanges();
    }
  }
