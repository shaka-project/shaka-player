...
goog.require('shaka.media.QueueManager');
...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, videoContainer = null, dependencyInjector) {
    ...
    /** @private {?shaka.media.QueueManager} */
  ...
  /**
   * Returns a shaka.media.QueueManager instance, responsible for queue
   * management.
   *
   * @return {?shaka.media.QueueManager}
   * @export
   */
  getQueueManager() {
    if (!this.queueManager_) {
      this.queueManager_ = new shaka.media.QueueManager(this);
      if (this.config_) {
        this.queueManager_.configure(this.config_.queue);
      }
    }
