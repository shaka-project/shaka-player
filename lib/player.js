...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  async loadInner_(startTimeOfLoad, prefetchedVariant, segmentPrefetchById) {
    ...
    if (this.cmcdManager_) {
      ...
      this.loadEventManager_.listen(mediaElement, 'play', onPlaybackPlay);
      this.loadEventManager_.listen(mediaElement, 'playing', onPlaybackPlaying);
    }
  ...
  getStats() {
    ...
    if (this.loadMode_ == shaka.Player.LoadMode.MEDIA_SOURCE) {
      ...
      if (this.isLive()) {
        this.stats_.setLiveLatency(this.getLiveLatency() / 1000);
      }
