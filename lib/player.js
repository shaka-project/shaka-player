...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, dependencyInjector) {
    ...
    const walkerImplementation = {
      ...
      handleError: async (has, error) => {
        ...
        // TODO
        // Depending on the load type we move to the attach state at a different
        // stage of the loading process. In case of error we favor returning in
        // the media source state for media source load mode, otherwise we
        // return to detach (Maybe return to attach state for src=?).
        return has.mediaElement || !has.useSrcEquals ?
            this.mediaSourceNode_ : this.detachNode_;
      },
      onIdle: (node) => {
        this.dispatchEvent(this.makeEvent_(
  ...
  async unload(initializeMediaSource = true, keepAdManager = false) {
    ...
    // If we have an ongoing attach operation, make sure it terminates first
    await this.attachPromise_;
   ...
   */
  createMediaSourceEngine(textDisplayer, onMetadata, lcevcDec) {
