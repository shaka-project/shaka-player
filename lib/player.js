...
/**
 * @event shaka.Player.periodReadyForStreamingEvent
 * @description Fired after a period transition when the app can safely switch
 * streams without the request being deferred.
 * @property {string} type
...
shaka.Player.prototype.onUnload_ = async function(has, wants) {
  ...
  // for us, but for src=, we need to do it manually.
  //
  // Do it last so that anything relying on the ready state won't have the rug
...
shaka.Player.prototype.getPlaybackRate = function() {
  ...
  return loaded ? this.playRateController_.getActiveRate() : 0;
};
...
shaka.Player.prototype.usingEmbeddedTextTrack = function() {
  ...
  if (activeText) {
    return activeText.mimeType == shaka.util.MimeUtils.CLOSED_CAPTION_MIMETYPE;
  }
...
shaka.Player.prototype.getStats = function() {
  // If we are not loaded, then return an stats blob so that this call will
  // never fail.
  //
 ...
 * content or is still loading content, this will return |null|.
 *
...
shaka.Player.prototype.updateBufferState_ = function() {
  ...
  if (loaded) {
    const modifier = shaka.media.PlayRateController.Modifier.BUFFERING;

    if (isBuffering) {
      this.playRateController_.addModifier(modifier);
    } else {
      this.playRateController_.removeModifier(modifier);
    }
...
shaka.Player.prototype.onRateChange_ = function() {
  ...
  if (newRate == 0) {
    // Use a modifier so that we don't override our previous playback rate.
    this.playRateController_.addModifier(Modifier.ZERO_RATE);
    return;
  }

  // The playback rate has changed. This could be us or someone else. Make sure
  // that there is no zero-rate modifier on the controller and set the new rate.
  // If this was us, setting the rate again will be a no-op.
  this.playRateController_.removeModifier(Modifier.ZERO_RATE);
...
shaka.Player.prototype.canSwitch_ = function() {
  ...
  // Dispatch a 'periodreadyforstreaming' event.
  this.delayDispatchEvent_(new shaka.util.FakeEvent('periodreadyforstreaming'));
};
...
shaka.Player.prototype.getPresentationVariant_ = function() {
  goog.asserts.assert(
      this.loadMode_ == shaka.Player.LoadMode.MEDIA_SOURCE,
      'Only ask for the presentation variant when loaded with media source.');
...
shaka.Player.prototype.getNextAfterAttach_ = function(goingTo, has, wants) {
  ...
  // If we are going to src=, then we can go straight there since we already
  // checked that we had the correct media element.
