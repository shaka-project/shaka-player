...
shaka.Player = class extends shaka.util.FakeEventTarget {
    ...
    if (mediaElement.textTracks) {
      const trackChange = () => {
        if (
          ...
          this.textDisplayer_ instanceof shaka.text.SimpleTextDisplayer
        ) {
          this.onTextChanged_();
        }
        this.onTracksChanged_();
    ...
    const selectSrcEqualsMode = () => {
      if (this.video_ && this.video_.textTracks) {
        ...
        if (this.textDisplayer_ instanceof shaka.text.SimpleTextDisplayer) {
          ...
          newTrack.mode = this.isTextVisible_ ? 'showing' : 'hidden';
        } else {
  ...
  /**
   * Ignore the TextTracks with the 'metadata' or 'chapters' kind.
   *
  ...
  getFilteredTextTracks_() {
    ...
    return Array.from(this.video_.textTracks).filter(
        (textTrack) =>
          textTrack.kind != 'metadata' && textTrack.kind != 'chapters',
    );
  }
    ...
    config.textDisplayFactory = () => {
      ...
      if (this.videoContainer_ &&
        ...
        if (typeof HTMLTrackElement !== 'undefined') {
          return new shaka.text.SimpleTextDisplayer(this);
        } else {
