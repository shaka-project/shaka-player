...
shaka.Player = class extends shaka.util.FakeEventTarget {
   ...
   * @export
   */
  addTextTrack(uri, language, kind, mimeType, codec, label) {
    ...
    if (this.loadMode_ == shaka.Player.LoadMode.SRC_EQUALS) {
      ...
      // not support the Track element, so in that case an error is thrown.
  ...
  chooseVariantAndSwitch_() {
    ...
    if (chosenVariant) {
      ...
      shaka.log.debug('*** chosen variant ***', chosenVariant);
    }
  ...
  onKeyStatus_(keyStatusMap) {
    shaka.log.debug('*** onKeyStatus_ ***', keyStatusMap);
    if (!this.streamingEngine_) {
    ...
    shaka.log.debug('*** onKeyStatus_ tracks changed ***', this.manifest_.variants);
    ...
    shaka.log.debug('*** onKeyStatus_ tracks currentVariant ***', currentVariant);
