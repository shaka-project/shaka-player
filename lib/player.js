...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  constructor(mediaElement, dependencyInjector) {
    ...
    /** @private {shaka.cmcd.CmcdManager} */
  ...
  async onInitializeParser_(has, wants) {
    ...
    // Create the CMCD manager so client data can be attached to the manifest
    // request.
    this.cmcdManager_ = this.createCmcd_(has.mimeType);
  ...
  onParseManifest_(has, wants) {
    ...
    const playerInterface = {
      ...
      modifyRequest: (request, manifest) => {
        this.cmcdManager_.applyManifestData(request, manifest);
      },
   ...
   * @private
   */
  createCmcd_(mimeType) {
    const abr = this.abrManager_;

    /** @type {shaka.cmcd.PlayerInterface} */
    const playerInterface = {
      getPlaybackRate: () => this.getPlaybackRate(),
      getBandwidthEstimate: () => abr ? abr.getBandwidthEstimate() : undefined,
      getVariantTracks: () => this.getVariantTracks(),
      getBufferedInfo: () => this.getBufferedInfo(),
      isLive: () => this.isLive(),
    };

    return new shaka.cmcd.CmcdManager(playerInterface);
  }
  ...
  createStreamingEngine() {
    ...
    const playerInterface = {
      ...
      modifyRequest: (request, segment) => {
        this.cmcdManager_.applySegmentData(request, segment);
      },
