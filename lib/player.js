...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  shouldUseSrcEquals_(payload) {
    ...
    if (!mimeType && uri.includes('.ism') && uri.includes('format=m3u8')) {
      mimeType = shaka.Player.SRC_EQUAL_EXTENSIONS_TO_MIME_TYPES_['m3u8'];
    }
    if (!mimeType && uri.includes('.ism') && uri.includes('format=mpd')) {
      mimeType = shaka.Player.SRC_EQUAL_EXTENSIONS_TO_MIME_TYPES_['mpd'];
    }
