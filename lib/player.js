...
shaka.Player = class extends shaka.util.FakeEventTarget {
  ...
  addResetMediaSourceOnStreamSwitch_(mediaElement) {
    ...
    const timer = new shaka.util.Timer(() => {
      mediaElement.currentTime = endTime + 0.1;
      endTime = undefined;
    });
    ...
    this.loadEventManager_.listen(mediaElement, 'timeupdate', () => {
      ...
      for (const stream of activeVariant.video.matchedStreams) {
        const timeline = stream.segmentIndex.getTimeline();
        if (presentationTime >= timeline[0].start &&
          presentationTime < timeline[timeline.length - 1].end) {
          endTime = timeline[timeline.length - 1].end;
          break;
        }
      }

      if (endTime === undefined || endTime - presentationTime > 1) {
        return;
      }

      timer.tickAfter(endTime - presentationTime);
    });
  }
};
