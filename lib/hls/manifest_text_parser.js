  ...
  lines.forEach(function(line) {
    if (/^(#EXT)/.test(line)) {
      ...
      if (tag.name == 'EXT-X-KEY') {
        // Presence of 'EXT-X-KEY' tag means that content is encrypted.
        // We don't have support for encrypted content implemented yet.
        // Until we do, we will reject it.
        throw new shaka.util.Error(
            shaka.util.Error.Category.MANIFEST,
            shaka.util.Error.Code.HLS_ENCRYPTED_CONTENT_NOT_SUPPORTED);
...
shaka.hls.ManifestTextParser.prototype.parseTag_ = function(word) {
  ...
  if (data && data.indexOf('=') >= 0) {
    ...
    return new shaka.hls.Tag(this.globalId_++, name, attributes, data);
  }

  return new shaka.hls.Tag(this.globalId_++, name, attributes);
};
