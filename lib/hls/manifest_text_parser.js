...
/**
 * @namespace
 * @summary Parses an HLS manifest into shaka's internal HLS objects.
 * @param {ArrayBuffer} data
 * @return {!shaka.hls.Playlist}
 * @throws {shaka.util.Error}
 */
shaka.hls.ManifestTextParser.parsePlaylist = function(data) {
  ...
  var ManifestTextParser = shaka.hls.ManifestTextParser;
  ...
  while (i < lines.length) {
    ...
    var tag = shaka.hls.ManifestTextParser.parseTag_(lines[i]);


    if (ManifestTextParser.MEDIA_PLAYLIST_TAGS.indexOf(tag.name) >= 0) {
      playlistType = shaka.hls.PlaylistType.MEDIA;
    } else if (ManifestTextParser.SEGMENT_TAGS.indexOf(tag.name) >= 0) {
 ...
 * @private
 */
shaka.hls.ManifestTextParser.parseTag_ = function(word) {
