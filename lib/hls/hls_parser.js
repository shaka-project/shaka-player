...
shaka.hls.HlsParser.prototype.updateStream_ = function(streamInfo, uri) {
  this.requestManifest_(uri).then(function(response) {
    ...
    var playlist = this.manifestTextParser_.parsePlaylist(response.data,
                                                          response.uri);
    ...
    this.createSegments_(playlist, startPosition, timeOffset)
        .then(function(segments) {
          segments = this.adjustSegments_(segments, streamInfo.lastSegmentSeen);
          streamInfo.segmentIndex.merge(segments);
          var firstAvailableSegment =
              streamInfo.segmentIndex.get(startPosition);
          var currentStartTime = firstAvailableSegment.startTime;
          if (this.presentationType_ == PresentationType.LIVE) {
            streamInfo.segmentIndex.evict(currentStartTime);
          }
          ...
          if (endListTag) {
            ...
            if (this.presentationType_ == PresentationType.EVENT)
...
shaka.hls.HlsParser.prototype.createStreamInfo_ = function(uri, allCodecs,
  ...
  return this.requestManifest_(uri).then(function(response) {
    ...
    return this.createSegments_(playlist, startPosition, timeOffset);
  }.bind(this)).then(function(segmentsArg) {
    // Save the parsed segments into a captured variable so that the value is
    // available to other steps in the Promise chain.
    segments = segmentsArg;

    segmentIndex = new shaka.media.SegmentIndex(segments);
  ...
  }.bind(this)).then(function(mimeType) {
    var stream = {
      id: this.globalId_++,
...
shaka.hls.HlsParser.prototype.determinePresentationType_ = function(playlist) {
  ...
  var isVod = presentationTypeTag && (presentationTypeTag.value == 'VOD' ||
      endListTag && presentationTypeTag.value == 'EVENT');
...
shaka.hls.HlsParser.prototype.createSegments_ =
    function(playlist, startPosition, timeOffset) {
  var Utils = shaka.hls.Utils;
  var hlsSegments = playlist.segments;
  var segments = [];

  goog.asserts.assert(hlsSegments.length, 'Playlist should have segments!');
  // In LIVE presentation segments can get removed from the playlist and
  // we have no way of knowing a segment's start time without looking
  // at the media itself.
  var firstSegmentUri = Utils.constructAbsoluteUri(playlist.uri,
                                                   hlsSegments[0].uri);
  // TODO: refactor and defer getting start time until createSegmentIndex().
  return this.getStartTime_(firstSegmentUri, timeOffset, startPosition)
      .then(function(firstStartTime) {
        hlsSegments.forEach(function(segment) {
          ...
          segments.push(
              new shaka.media.SegmentReference(startPosition + index,
                                               startTime,
                                               endTime,
                                               function() { return [uri]; },
                                               startByte,
...
/**
 * Gets start time of a segment by either downloading it and parsing
 * the start time from it (for ongoing or finished LIVE content)
 * or assuming it's 0 (for VOD and EVENT).
 *
 * @param {string} uri
 * @param {?number} timeOffset
...
shaka.hls.HlsParser.prototype.getStartTime_ =
    function(uri, timeOffset, segmentPosition) {
  // If the segment is the first segment of the presentation, assume
  // its start time to be 0. Otherwise, download the segment and
  // parse start time from it.
  // Also, if this is a manifest update, there's no need to look at
  // the media either, cause we'll be able to derive the start time
  // from the previous segments.
  // TODO: Remove the exception for segmentPosition == 0, as this is sometimes
  // wrong for live streams.  Also, we should use this for VOD streams as well.
  // The exception for updates should be handled elsewhere, by making calls to
  // getStartTime_ conditional on not knowing the start time of the first
  // segment.
  if (segmentPosition == 0 || this.manifest_ != null) {
    return Promise.resolve(0);
  }

  // If it is LIVE, get start time directly from the media
  ...
  return this.playerInterface_.networkingEngine.request(requestType, request)
      .then(function(response) {
        ...
        // TODO: use deduced mime type?  sadly, we deduce that later.
        if (contentType == 'video/mp4' || contentType == 'audio/mp4') {
          var startTime = this.getStartTimeFromMp4Segment_(response.data);
          if (timeOffset)
            startTime -= timeOffset;

          return startTime;
        } else {
          // TODO: Parse TS & WebM?  Or make an assumption and drop the error?
          throw new shaka.util.Error(
              shaka.util.Error.Severity.CRITICAL,
              shaka.util.Error.Category.MANIFEST,
...
/**
 * Parses an mp4 segment to get it's start time.
 *
 * @param {!ArrayBuffer} data
 * @return {number}
 * @throws {shaka.util.Error}
...
shaka.hls.HlsParser.prototype.getTimeOffset_ = function(playlist) {
  var Utils = shaka.hls.Utils;
  // TODO: We are misusing the EXT-X-START tag. Rather than setting an
  // offset for Media it specifies a point to start playing the presentation.
  // We should be parsing the offset from the media itself instead.
  ...
  // TODO: Deprecate this parameter once we handle EXT-X-START correctly and
  // always extract start time from the content itself.
