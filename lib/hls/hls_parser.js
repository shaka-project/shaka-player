...
shaka.hls.HlsParser = class {
  ...
  createStreamInfosFromMediaTags_(mediaTags) {
    ...
    for (const key in groupedTags) {
      ...
      this.createStreamInfoFromMediaTag_(groupedTags[key]);
    }
  }
  ...
  createVariantsForTags_(tags, sessionKeyTags, mediaTags) {
    ...
    for (const key in groupedTags) {
      ...
      const streamInfos = this.createStreamInfosForVariantTag_(tags,
   ...
   * @private
   */
  createStreamInfosForVariantTag_(tags, mediaTags, resolution, frameRate) {
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    ...
    if (!ignoreStream) {
      const streamInfo =
          this.createStreamInfoFromVariantTag_(tags, allCodecs, type);
      if (globalGroupId) {
   ...
   * @private
   */
  createStreamInfoFromMediaTag_(tags) {
  ...
  createStreamInfoFromIframeTag_(tag) {
    ...
    const streamInfo = this.createStreamInfo_(
        verbatimIFramePlaylistUri, codecs, type, language, /* primary= */ false,
   ...
   * @private
   */
  createStreamInfoFromVariantTag_(tags, allCodecs, type) {
    const verbatimMediaPlaylistUris = [];
    for (const tag of tags) {
