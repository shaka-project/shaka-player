    ...
    for (const audioInfo of audioInfos) {
      for (const videoInfo of videoInfos) {
        ...
        if (audioStream) {
          ...
          mappedAudioVariantSet.add(variant);
        ...
        if (videoStream) {
          ...
          mappedVideoVariantSet.add(variant);
  ...
  createStreamInfo_(streamId, verbatimMediaPlaylistUris, codecs, type,
    ...
    const downloadSegmentIndex = async (pendingRequest) => {
      ...
      try {
        ...
        console.log(`[APPLE, HlsParser]: Downloading stream for: ${JSON.stringify(stream, null, 2)}`);
        ...
        if (stream.drmInfos.length) {
          if (this.hasLazyLoaded_.has(stream.type)) {
            const variants = this.streamToVariants_.get(stream) || new Set();
            for (const variant of variants) {
              console.log(`[APPLE, Clearing variant.decodingInfos for variant: ${JSON.stringify(shaka.util.StreamUtils.getVariantSummaryString(variant))}`);
              variant.decodingInfos = [];
            }

            if (this.manifest_) {
              console.log(`[APPLE, Start re-filtering manifest after clearing decodingInfos...`);
              await this.playerInterface_.filter(this.manifest_);
              console.log(`[APPLE, End re-filtering manifest after clearing decodingInfos...`);
            }
          }
          this.hasLazyLoaded_.set(stream.type, true);        
          this.playerInterface_.newDrmInfo(stream);
        }
