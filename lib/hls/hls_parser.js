...
shaka.hls.HlsParser.prototype.createVariants_ =
  ...
  for (var i = 0; i < audioInfos.length; i++) {
    for (var j = 0; j < videoInfos.length; j++) {
      var audio = audioInfos[i] ? audioInfos[i].stream : null;
      var video = videoInfos[j] ? videoInfos[j].stream : null;
      var drmInfos = [];
      if (audio && audioInfos[i].drmInfo)
        drmInfos.push(audioInfos[i].drmInfo);
      if (video && videoInfos[j].drmInfo)
        drmInfos.push(videoInfos[j].drmInfo);
 ...
 */
// TODO: Extend support
shaka.hls.HlsParser.KEYFORMATS_TO_EME_KEYSYSTEM_IDS_ = {
  'com.apple.streamingkeydelivery': 'com.apple.fps.1_0'
};
