...
shaka.hls.HlsParser = class {
  ...
  async updateStream_(streamInfo) {
    ...
    const initSegmentRef = this.getInitSegmentReference_(playlist,
        firstSegment.tags, streamInfo.getUris, mediaVariables);

    const {segments, bandwidth} = this.createSegments_(
        playlist, mediaSequenceToStartTime, mediaVariables,
        streamInfo.getUris, streamInfo.type, initSegmentRef);
    if (bandwidth) {
  ...
  async convertParsedPlaylistIntoStreamInfo_(streamId, variables, playlist,
    ...
    // Get first segment once and early and pass it in
    const firstSegment = playlist.segments[0];
    const initSegmentRef = this.getInitSegmentReference_(playlist,
        firstSegment.tags, getUris, variables);
  ...
  createSegmentReference_(
      ...
      variables, type, getUris, mimeType, aesKey) {
  ...
  createSegments_(playlist, mediaSequenceToStartTime, variables,
    ...
    let currentInitSegmentRef = initSegmentRef;
    ...
    let mimeType = getMimeType(hlsSegments[0].verbatimSegmentUri);

    for (let i = 0; i < hlsSegments.length; i++) {
      const item = hlsSegments[i];

      // Check if this specific segment has its own MAP tag, which would
      // override the default one for this and subsequent segments.
      const mapTag = shaka.hls.Utils.getFirstTagWithName(item.tags, 'EXT-X-MAP');
      if (mapTag) {
        // A new MAP tag was found. Create a new init segment reference for it.
        // This will now become the "current" init ref for this and all
        // following segments, until another MAP tag is found.
        currentInitSegmentRef = this.getInitSegmentReference_(
          playlist, item.tags, getUris, variables);
      }
      ...
      if (discontinuityTag) {
        // Double check of mimetype should be only with EXT-X-DISCONTINUITY
        mimeType = getMimeType(hlsSegments[0].verbatimSegmentUri);
      ...
      const reference = this.createSegmentReference_(
          ...
          type,
          getUris,
          mimeType, // Pass the mimetype in instead of call in inside here
          aesKey,
      );
      if (reference) {
