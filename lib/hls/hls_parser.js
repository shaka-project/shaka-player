...
shaka.hls.HlsParser = class {
  ...
  createSegmentReference_(
    ...
    const getMimeType = (uri) => {
      ...
      const extension = parsedUri.getPath().split('.').pop();
      const map = HlsParser.EXTENSION_MAP_BY_CONTENT_TYPE_[type];
    ...
    if (this.lowLatencyMode_ && hlsSegment.partialSegments.length) {
      ...
      for (let i = 0; i < hlsSegment.partialSegments.length; i++) {
        ...
        if (pMimeType) {
          ...
          if (HlsParser.MIME_TYPES_WITHOUT_INIT_SEGMENT_.includes(pMimeType)) {
    ...
    if (mimeType) {
      ...
      if (HlsParser.MIME_TYPES_WITHOUT_INIT_SEGMENT_.includes(mimeType)) {
