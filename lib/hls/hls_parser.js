  ...
  getStartTimeFromMp4Segment_(playlistUri, segmentUri, mediaData, initData) {
    ...
    new Mp4Parser()
        ...
        .fullBox('tfdt', (box) => {
          ...
          startTime = (
            () => {
              if ((typeof baseTime !== 'number') && BigInt) {
                // Lint rejects 'bigint', so it's a bigint if it's not a number.
                /* eslint-disable new-cap */
                startTime = baseTime / BigInt(timescale);
                /* eslint-enable new-cap */
                return Number(startTime);
              } else {
                return baseTime / timescale;
              }
            })();

          console.log('startTime', startTime, Number.MAX_SAFE_INTEGER);
          parsedMedia = true;
          box.parser.stop();
