...
shaka.hls.HlsParser = class {
  ...
  createSegmentReference_(
    ...
    const shouldSetInitialProgramDateTime = !this.initialProgramDateTime_ &&
        (stream.type == ContentType.AUDIO || stream.type == ContentType.VIDEO);
    if (shouldSetInitialProgramDateTime ||
    ...
    const syncTime = firstProgramDateTime;
    if (shouldSetInitialProgramDateTime && firstProgramDateTime) {
  ...
  processDateRangeTags_(tags) {
    if (this.initialProgramDateTime_ == null ||
      ...
      // EXT-X-PROGRAM-DATE-TIME or when the user has decided that
