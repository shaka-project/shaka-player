...
shaka.hls.HlsParser = class {
  ...
  createSegments_(playlist, mediaSequenceToStartTime, variables,
    ...
    let firstStartTimeIsCached = false;
    ...
    if (someSyncTime) {
      ...
      // Seed the accumulated discontinuity gap from the first reference we are
      // about to build.  On a live update we only reparse the new tail, so once
      // the discontinuity that produced a gap has scrolled out of the window
      // the loop below would otherwise restart from 0 and place the segments at
      // their raw (un-corrected) syncTime, which syncAgainst() then throws far
      // off the continuous timeline.  The first reference's start time is its
      // continuous position, so the difference from its raw syncTime is exactly
      // the gap already in effect; seeding it keeps the tail contiguous.  For a
      // continuous stream this is 0, so behavior there is unchanged.  This
      // derivation is only valid when the window's first position was cached
      // from a previous parse: when recovering from a disconnect, the start
      // time is just an availability-based estimate, and the raw
      // PROGRAM-DATE-TIME (applied by syncAgainst()) is the only reliable way
      // to re-position the segments.
      const lowestSyncTime = this.lowestSyncTime_;
      ...
      for (const reference of references) {
        if (reference.syncTime != null) {
          ...
          // accumulated gap below.  Remember it keyed by the reference so it
          // survives both the gap correction and reuse of this reference across
          // live updates.  This is what lets us report an accurate playhead
          // date across discontinuities via the presentation timeline's PDT
          // regions.
          if (!this.realProgramDateTimeByReference_.has(reference)) {
