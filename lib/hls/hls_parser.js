...
shaka.hls.HlsParser = class {
   ...
   * source of truth).  A region is emitted at the start of the index and at
   * each discontinuity that introduces a new PROGRAM-DATE-TIME base, mapping a
   * presentation time to its real wall-clock time.  Reading the whole window -
   * rather than only the freshly parsed tail - keeps getPlayheadTimeAsDate()
   * accurate as discontinuities scroll through a live window.  Each segment's
   * real PDT is looked up from |realProgramDateTimeByReference_|, which is
   * unaffected by the discontinuity-gap correction applied to |syncTime|.
   *
   * @param {shaka.extern.Stream} stream
   * @param {string} type
   * @private
  ...
  setProgramDateTimeRegionsFromIndex_(stream, type) {
    ...
    const regions = [];
    let previousDiscontinuitySequence = null;
    for (const reference of stream.segmentIndex) {
      ...
      if (regions.length == 0 ||
          reference.discontinuitySequence != previousDiscontinuitySequence) {
        regions.push({
          start: reference.startTime,
          pdt: programDateTime,
        });
      }
      previousDiscontinuitySequence = reference.discontinuitySequence;
    }
