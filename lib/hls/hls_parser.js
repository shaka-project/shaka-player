...
shaka.hls.HlsParser = class {
  ...
  async updateStream_(streamInfo) {
    ...
    // Parse the manifest and extract needed data
    const { keyIds, drmInfos } = this.parseDrmInfo_(playlist);

    // Check if the old keys Set is the same
    let keysNotUpdated = (a, b) => a.size === b.size && [...a].every(value => b.has(value));

    // Keys were updated
    if (!keysNotUpdated(stream.keyIds, keyIds)) {
      stream.keyIds = keyIds;
      stream.drmInfos = drmInfos;

      // https://github.com/shaka-project/shaka-player/commit/6143a6d31465c7715be452fa6c60f770aeb474c5
      // 
      // "it sends DRM init data to DrmEngine for all streams in case new sessions need to be created.
