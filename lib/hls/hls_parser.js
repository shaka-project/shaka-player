...
shaka.hls.HlsParser = class {
  ...
  async convertParsedPlaylistIntoStreamInfo_(playlist, verbatimMediaPlaylistUri,
    ...
    for (const drmTag of drmTags) {
      ...
      if (method != 'NONE') {
        ...
        const keySystemsMapping = this.config_.hls.keySystemsMapping;

        const drmInfo = drmParser ?
            drmParser(drmTag, keySystemsMapping, mimeType) : null;
        if (drmInfo) {
          if (drmInfo.keyIds) {
   ...
   * @private
   */
  static fairplayDrmParser_(drmTag, keySystemsMapping, mimeType) {
    ...
    const keySystem = keySystemsMapping['com.apple.fps'];
    /*
   ...
   * @private
   */
  static widevineDrmParser_(drmTag, keySystemsMapping) {
    const method = drmTag.getRequiredAttrValue('METHOD');
    ...
    const keySystem = keySystemsMapping['com.widevine.alpha'];
    const drmInfo = shaka.util.ManifestParserUtils.createDrmInfo(
        keySystem, [
   ...
   * @private
   */
  static playreadyDrmParser_(drmTag, keySystemsMapping) {
    const method = drmTag.getRequiredAttrValue('METHOD');
    const VALID_METHODS = ['SAMPLE-AES', 'SAMPLE-AES-CTR'];
    if (!VALID_METHODS.includes(method)) {
    ...
    const keySystem = keySystemsMapping['com.microsoft.playready'];
    const drmInfo = shaka.util.ManifestParserUtils.createDrmInfo(
        keySystem, [
          {initDataType: 'cenc', initData: pssh},
        ]);
...
/**
 * @typedef {function(!shaka.hls.Tag, !Object.<string, string>,
 *  string):?shaka.extern.DrmInfo}
 * @private
 */
