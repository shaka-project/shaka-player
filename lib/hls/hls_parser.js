...
shaka.hls.HlsParser = class {
  ...
  createSegments_(playlist, mediaSequenceToStartTime, variables,
    ...
    // While removing discontinuity gaps to keep the presentation timeline
    // continuous, also remember the real PROGRAM-DATE-TIME at the start of each
    // region (the first synced segment and each discontinuity that introduces a
    // new PDT base) so the playhead date can be reported accurately later.
    /** @type {!Array<{ref: !shaka.media.SegmentReference, pdt: number}>} */
    const pdtRegions = [];
    if (someSyncTime) {
      ...
      for (const reference of references) {
        if (reference.syncTime != null) {
          ...
          // accumulated gap below.
          const realProgramDateTime = reference.syncTime;
          let startsRegion = pdtRegions.length == 0;
          ...
          if (startsRegion) {
            pdtRegions.push({ref: reference, pdt: realProgramDateTime});
          }
        }
    ...
    // Publish the program-date-time regions for the playhead-date calculation.
    // We use the video stream as the source of truth (and only when an actual
    // discontinuity gap exists), so the regions stay consistent across live
    // updates and we fall back to the default behavior otherwise.  The region
    // start times are read here, after syncAgainst, so they are in the same
    // coordinate system as the playhead.
    if (type == shaka.util.ManifestParserUtils.ContentType.VIDEO &&
        !this.ignoreManifestProgramDateTimeFor_(type)) {
      const regions = pdtRegions.length > 1 ? pdtRegions.map((region) => ({
        start: region.ref.startTime,
        pdt: region.pdt,
      })) : [];
