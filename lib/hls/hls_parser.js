...
shaka.hls.HlsParser.prototype.createPeriod_ = async function(playlist) {
  ...
  const textStreamTags = mediaTags.filter(function(tag) {
    const type = shaka.hls.HlsParser.getRequiredAttributeValue_(tag, 'TYPE');
    return type == 'SUBTITLES';
  }.bind(this));

  const textStreamPromises = textStreamTags.map(function(tag) {
  ...
  const variantsPromises = variantTags.map(function(tag) {
...
shaka.hls.HlsParser.prototype.createVariantsForTag_ =
  ...
  if (textCodecs) {
    ...
    if (subGroupId) {
      const textTags = Utils.findMediaTags(mediaTags, 'SUBTITLES', subGroupId);
  ...
  const promises = mediaTags.map(function(tag) {
  ...
  if (!audioStreamInfos.length && !videoStreamInfos.length) {
    ...
    const firstAudioStreamURI = audioStreamInfos[0].verbatimMediaPlaylistUri;
...
shaka.hls.HlsParser.prototype.createStreamInfoFromMediaTag_ =
  ...
  const channelsAttribute = tag.getAttributeValue('CHANNELS');
...
shaka.hls.HlsParser.prototype.determinePresentationType_ = function(playlist) {
  const Utils = shaka.hls.Utils;
  const PresentationType = shaka.hls.HlsParser.PresentationType_;
  const presentationTypeTag = Utils.getFirstTagWithName(playlist.tags,
                                                      'EXT-X-PLAYLIST-TYPE');
  const endListTag = Utils.getFirstTagWithName(playlist.tags, 'EXT-X-ENDLIST');

  const isVod = (presentationTypeTag && presentationTypeTag.value == 'VOD') ||
      endListTag;
  const isEvent = presentationTypeTag && presentationTypeTag.value == 'EVENT' &&
      !isVod;
...
shaka.hls.HlsParser.prototype.createInitSegmentReference_ = function(playlist) {
  const Utils = shaka.hls.Utils;
  const mapTags = Utils.filterTagsByName(playlist.tags, 'EXT-X-MAP');
...
shaka.hls.HlsParser.prototype.createSegmentReference_ =
  ...
  let startByte = 0;
  let endByte = null;
  const byterange = Utils.getFirstTagWithName(tags, 'EXT-X-BYTERANGE');
