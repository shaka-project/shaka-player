...
shaka.hls.HlsParser = class {
  ...
  determineLastTargetDuration_(playlist) {
    ...
    if (this.manifest_.lowLatency && this.partialTargetDuration_) {
    ...
    if (hlsSegment.partialSegments.length) {
      if (!this.lowLatencyMode_) {
        shaka.log.alwaysWarn('Low-latency HLS live stream detected, but ' +
                  'low-latency mode is not enabled in Shaka ' +
                  'Player. Set manifest.lowLatencyMode configuration to ' +
                  'true, and see https://bit.ly/3clctcj for details.');
      } else if (!this.manifest_.lowLatency) {
        this.manifest_.lowLatency = true;
