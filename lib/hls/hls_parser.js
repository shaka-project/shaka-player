...
shaka.hls.HlsParser = class {
  ...
  createSegments_(playlist, mediaSequenceToStartTime, variables,
    ...
    // Whether |firstStartTime| is a trusted position on the continuous
    // timeline, cached from a previous parse of this stream.  When recovering
    // from a disconnect (below), the start time is only an availability-based
    // estimate, so it must not be used to derive timeline corrections.
    const firstStartTimeIsCached =
        this.isLive_() && mediaSequenceToStartTime.has(position);

    // For live stream, use the cached value in the mediaSequenceToStartTime
    // map if available.
    if (firstStartTimeIsCached) {
    ...
    if (someSyncTime) {
      ...
      // Seed the accumulated discontinuity gap, so that a gap whose
      // discontinuity has already scrolled out of the window still applies to
      // the segments we are about to build (on live updates we only reparse
      // the new tail).  Otherwise the loop below would restart from 0 and
      // place them at their raw (un-corrected) syncTime, which syncAgainst()
      // then throws far off the continuous timeline.
      //  - When the window's first position was cached from a previous parse,
      //    its start time is its continuous position, so the difference from
      //    its raw syncTime is exactly the gap in effect.  For a continuous
      //    stream this is 0, so behavior there is unchanged.
      //  - When recovering from a disconnect there is no trusted position, but
      //    a brief outage is unlikely to have crossed another discontinuity,
      //    so the gap in effect at the end of the known window is the most
      //    plausible estimate.  It keeps the recovered segments
      //    media-continuous with the missed ones when that assumption holds.
      const lowestSyncTime = this.lowestSyncTime_;
      const firstRef = references[0];
      if (firstStartTimeIsCached && lowestSyncTime != Infinity && firstRef &&
          firstRef.syncTime != null) {
        accumulatedGap =
            firstRef.syncTime - firstRef.startTime - lowestSyncTime;
      } else if (existingSegmentIndex) {
        const lastKnownRef = existingSegmentIndex.get(
            existingSegmentIndex.getNumEvicted() +
            existingSegmentIndex.getNumReferences() - 1);
        const lastKnownRealPdt = lastKnownRef ?
            this.realProgramDateTimeByReference_.get(lastKnownRef) : null;
        if (lastKnownRef && lastKnownRef.syncTime != null &&
            lastKnownRealPdt != null) {
          accumulatedGap = lastKnownRealPdt - lastKnownRef.syncTime;
        }
      }
