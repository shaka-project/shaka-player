 ...
 * @private
 */
shaka.hls.HlsParser.fairplayDrmParser_ = function(drmTag) {
  var HlsParser = shaka.hls.HlsParser;
  var method = HlsParser.getRequiredAttributeValue_(drmTag, 'METHOD');
  if (method != 'SAMPLE-AES') {
    shaka.log.error(
        'FairPlay in HLS is only supported with SAMPLE-AES, not', method);
    return null;
  }

  // The URI is used as init data, and contains the content ID.
  var uri = HlsParser.getRequiredAttributeValue_(drmTag, 'URI');
  // We expect the format skd://HEX_CONTENT_ID
  goog.asserts.assert(uri.indexOf('skd://') == 0, 'Unexpected FairPlay URI!');

  // For compatibility with Safari's built-in HLS implementation, encode
  // the skd URI as little-endian UTF-16, prepended with "L\x00".
  var initData = shaka.util.StringUtils.toUTF16(
      'L\x00' + uri, /* littleEndian */ true);

  return shaka.util.ManifestParserUtils.createDrmInfo('com.apple.fps.1_0', [{
    initDataType: 'skd',
