...
shaka.hls.HlsParser.prototype.getStartTimeFromTsSegment_ = function(data) {
  ...
  var skipPacket = function() {
    if ((packetStart + 188) < reader.getLength()) {
  ...
  while (true) {
    ...
    foundPts.push(pts / shaka.hls.HlsParser.TS_TIMESCALE_);
    if (skipPacket())
      continue;
    else
      break;
  ...
  return Math.min.apply(null, foundPts);
};
 ...
 * @private
 */
shaka.hls.HlsParser.PARTIAL_SEGMENT_SIZE_ = 1024 * 20;
