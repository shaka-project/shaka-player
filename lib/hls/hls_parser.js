...
shaka.hls.HlsParser = class {
  ...
  constructor() {
    ...
    /**
     * For variant tags with no CODECS attribute, the raw format MIME type of
     * their media playlist's segments, keyed by the verbatim media playlist
     * URI.  Filled out by peekRawFormats_ before any Stream is built.  A URI
     * is absent if its playlist does not hold a raw format, or if we could not
     * find out.
     *
     * @private {!Map<string, string>}
     */
    this.rawFormatByPlaylistUri_ = new Map();
  ...
  stop() {
    ...
    this.rawFormatByPlaylistUri_.clear();
  ...
  async parseManifest_(data) {
    ...
    if (playlist.type == shaka.hls.PlaylistType.MEDIA) {
      ...
      // Settle what the CODECS attributes don't tell us before anything starts
      // guessing from them.
      await this.peekRawFormats_(variantTags);
  ...
  /**
   * The CODECS attribute is only a SHOULD in older revisions of the HLS spec,
   * so plenty of real content omits it.  Without it we would have to assume
   * defaults for both audio and video, and audio-only content would be
   * mistaken for multiplexed audio+video.
   *
   * Rather than guess, look: download the media playlists of the variant tags
   * that have no CODECS attribute and check what their segments are.  A raw
   * format (mp3, aac, ac3, ec3) is a containerless audio elementary stream, so
   * finding one settles both the content type and the codec before any Stream
   * or Variant is built.
   *
   * All of the playlists are fetched in parallel, so this costs one round trip
   * no matter how many variants need it.  Playlists that turn out not to be a
   * raw format, and playlists we fail to fetch, fall back to the old defaults.
   *
   * @param {!Array<!shaka.hls.Tag>} tags EXT-X-STREAM-INF tags
   * @return {!Promise}
   * @private
   */
  async peekRawFormats_(tags) {
    /** @type {!Set<string>} */
    const verbatimUris = new Set();
    for (const tag of tags) {
      if (!tag.getAttributeValue('CODECS')) {
        verbatimUris.add(tag.getRequiredAttrValue('URI'));
      }
    }
    if (!verbatimUris.size) {
      return;
    }

    await Promise.all(Array.from(verbatimUris).map(async (verbatimUri) => {
      const rawFormat = await this.getRawFormatOfPlaylist_(verbatimUri);
      if (rawFormat) {
        this.rawFormatByPlaylistUri_.set(verbatimUri, rawFormat);
      }
    }));
  }

  /**
   * Download a media playlist and report the raw format of its segments, if
   * they are in one.  Returns null for any other kind of content, and for any
   * failure to fetch or parse the playlist, so that the caller can fall back
   * to its usual assumptions.
   *
   * @param {string} verbatimUri the media playlist URI, as written in the
   *   master playlist
   * @return {!Promise<?string>} a raw format MIME type, or null
   * @private
   */
  async getRawFormatOfPlaylist_(verbatimUri) {
    const uris = shaka.hls.Utils.constructUris(
        [this.masterPlaylistUri_], [verbatimUri], this.globalVariables_);

    let response;
    try {
      response = await this.requestManifest_(
          uris, /* isPlaylist= */ true).promise;
    } catch (error) {
      shaka.log.debug('Could not peek at media playlist', verbatimUri, error);
      return null;
    }
    // Make sure the parser has not been destroyed while we waited.
    if (!this.playerInterface_) {
      return null;
    }
    ...
    let playlist;
    try {
      playlist = this.manifestTextParser_.parsePlaylist(response.data);
    } catch (error) {
      shaka.log.debug('Could not parse media playlist', verbatimUri, error);
      return null;
    }
    if (playlist.type != shaka.hls.PlaylistType.MEDIA ||
    ...
    const variablesTags = shaka.hls.Utils.filterTagsByName(
        playlist.tags, 'EXT-X-DEFINE');
    const variables = this.parseMediaVariables_(variablesTags, response.uri);

    // Every segment of a media playlist has the same container, so the first
    // one is representative.
    const segmentUris = shaka.hls.Utils.constructSegmentUris(
        [response.uri], playlist.segments[0].verbatimSegmentUri, variables);
    const extension = shaka.net.NetworkingUtils.getExtension(segmentUris[0]);
    return shaka.hls.HlsParser.RAW_FORMATS_TO_MIME_TYPES_.get(extension) ||
        null;
  }
  ...
  getCodecsForVariantTag_(tag) {
    ...
    if (!this.codecInfoInManifest_) {
      // We looked at the media playlist rather than guessing (see
      // peekRawFormats_).  A raw format is a containerless audio elementary
      // stream, so we know both that there is no video and which codec is in
      // there.
      const rawFormat = this.rawFormatByPlaylistUri_.get(
          tag.getRequiredAttrValue('URI'));
      if (rawFormat) {
        codecsString =
            shaka.hls.HlsParser.RAW_FORMATS_TO_CODECS_.get(rawFormat);
      } else if (!this.config_.hls.disableCodecGuessing) {
...
/**
 * The codec a CODECS attribute would have named for each raw format.  Used to
 * fill in for a missing CODECS attribute once we have seen that the media
 * playlist holds a raw format.
 *
 * @const {!Map<string, string>}
 * @private
 */
shaka.hls.HlsParser.RAW_FORMATS_TO_CODECS_ = new Map()
    .set('audio/aac', 'mp4a.40.2')
    .set('audio/ac3', 'ac-3')
    .set('audio/ec3', 'ec-3')
    // See: https://bugs.chromium.org/p/chromium/issues/detail?id=489520
    .set('audio/mpeg', 'mp4a.40.34');
