...
shaka.hls.HlsParser = class {
  ...
  async parseManifest_(data) {
    ...
    if (this.isLive_()) {
      ...
      if (this.presentationType_ == PresentationType.LIVE) {
        ...
        // HLS implementation.
        // For low latency streaming, widen the seek window by one partial
        // segment target duration.
        let segmentAvailabilityDuration =
            this.presentationTimeline_.getDelay() + this.partialTargetDuration_;
  ...
  determinePresentationType_(playlist) {
    ...
    if (isVod) {
      ...
      if (this.config_.lowLatencyMode && partialTargetDurationTag) {
  ...
  createSegmentReference_(
    ...
    if (this.config_.lowLatencyMode && hlsSegment.partialSegments.length) {
  ...
  async createSegments_(verbatimMediaPlaylistUri, playlist, type, mimeType,
    ...
    for (const {i, item} of enumerate(hlsSegments)) {
      ...
      if (this.config_.lowLatencyMode || extinfTag) {
