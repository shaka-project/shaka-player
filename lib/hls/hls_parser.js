...
shaka.hls.HlsParser.prototype.createVariantsForTag_ =
    async function(tag, playlist) {
      ...
      if (audioGroupId) {
        mediaTags = shaka.hls.Utils.findMediaTags(
            mediaTags, 'AUDIO', audioGroupId);
      } else if (videoGroupId) {
        mediaTags = shaka.hls.Utils.findMediaTags(
            mediaTags, 'VIDEO', videoGroupId);
      }

      // There may be a codec string for the text stream.  We should identify
      // it, add it to the appropriate stream, then strip it out of the variant
      // to avoid confusing our multiplex detection below.
...
shaka.hls.HlsParser.prototype.createStreamInfoFromMediaTag_ =
    async function(tag, allCodecs) {
      ...
      // attributes: FORCED, INSTREAM-ID, CHARACTERISTICS, CHANNELS? Attribute
...
shaka.hls.HlsParser.prototype.determinePresentationType_ = function(playlist) {
  ...
  const presentationTypeTag =
      shaka.hls.Utils.getFirstTagWithName(playlist.tags, 'EXT-X-PLAYLIST-TYPE');
  const endListTag =
      shaka.hls.Utils.getFirstTagWithName(playlist.tags, 'EXT-X-ENDLIST');
...
shaka.hls.HlsParser.prototype.createInitSegmentReference_ = function(playlist) {
  const mapTags = shaka.hls.Utils.filterTagsByName(playlist.tags, 'EXT-X-MAP');
