...
shaka.hls.HlsParser = class {
  ...
  createStreamInfosForVariantTags_(tags, mediaTags, resolution) {
    ...
    for (const tag of tags) {
      ...
      if (audioGroupId && !audioGroupIds.includes(audioGroupId)) {
        audioGroupIds.push(audioGroupId);
        if (pathwayId) {
          groupIdPathwayIdMapping.set(audioGroupId, pathwayId);
        }
      }
      if (videoGroupId && !videoGroupIds.includes(videoGroupId)) {
        videoGroupIds.push(videoGroupId);
        if (pathwayId) {
    ...
    const globalGroupIds = audioGroupIds.concat(videoGroupIds);
  ...
  getGroupStreamInfos_(groupIds) {
    ...
    const globalGroupId = groupIds.slice().sort().join(',');
