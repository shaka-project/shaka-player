...
shaka.hls.HlsParser = class {
  ...
  determinePresentationType_(playlist) {
    ...
    if (isVod) {
      ...
      // 1. Update the longest target duration if need be to use as a
      // presentation delay later.
      // 2. Update the shortest one to use as update period and segment
      ...
      if (this.config_.lowLatencyMode && partialTargetDurationTag) {
        ...
        const partialTargetDuration = Number(
            partialTargetDurationTag.getRequiredAttrValue('PART-TARGET'));
        this.minTargetDuration_ = Math.min(
            partialTargetDuration, this.minTargetDuration_);
        this.maxTargetDuration_ = Math.max(
            partialTargetDuration, this.maxTargetDuration_);
      } else {
