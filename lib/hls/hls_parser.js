  ...
  createSegmentReference_(
    ...
    if (this.lowLatencyMode_) {
      ...
      for (let i = 0; i < hlsSegment.partialSegments.length; i++) {
        ...
        // The spec doesn't say we can assume INDEPENDENT=YES for the first
        // partial segment. It does call the flag "optional", though, and that
        // cases where there are no such flags on any partial segments, it is
        // sensible to assume the first one is independent.
