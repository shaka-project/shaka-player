...
shaka.hls.HlsParser = class {
  ...
  async parseManifest_(data) {
    ...
    if (playlist.type == shaka.hls.PlaylistType.MEDIA) {
      ...
      const chapterUris = this.processSessionData_(sessionDataTags);
      ...
      [imageStreams, chapterStreams] = await Promise.all([
        this.parseImages_(imageTags, iFrameTags),
        this.parseChapters_(chapterUris),
      ]);
    }
  ...
  /**
   * @param {!Array<string>} chapterUris
   * @return {!Promise<!Array<!shaka.extern.Stream>>}
   * @private
   */
  async parseChapters_(chapterUris) {
    ...
    const promises = [];
    for (const uri of chapterUris) {
      promises.push(this.processChapters_(uri, chapterStreams));
    }
  ...
  /**
   * @param {string} uri
   * @param {!Array<!shaka.extern.Stream>} chapterStreams
   * @return {!Promise}
   * @private
   */
  async processChapters_(uri, chapterStreams) {
    ...
    if (!ignoreStream) {
      const streamInfo =
          this.createStreamInfoFromVariantTags_(tags, allCodecs, type,
              supplementalCodecs);
      if (globalGroupId && this.config_.hls.enableAudioGroups) {
  ...
  createStreamInfoFromMediaTags_(tags, groupIdPathwayIdMapping,
    ...
    if (streamInfo.stream && this.config_.hls.enableAudioGroups) {
      streamInfo.stream.groupId = globalGroupId;
    }
