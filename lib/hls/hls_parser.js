...
shaka.hls.HlsParser.prototype.update = function() {
  ...
  let promises = [];
  this.uriToStreamInfosMap_.forEach((streamInfo, uri) => {
...
shaka.hls.HlsParser.prototype.createVariants_ =
  ...
  for (let i = 0; i < audioInfos.length; i++) {
    for (let j = 0; j < videoInfos.length; j++) {
      ...
      const videoStreamUri = videoInfos[i] ? videoInfos[i].relativeUri : '';
      const audioStreamUri = audioInfos[i] ? audioInfos[i].relativeUri : '';
...
shaka.hls.HlsParser.prototype.createStreamInfoFromMediaTag_ =
  ...
  if (this.uriToStreamInfosMap_.has(uri)) {
    return Promise.resolve(this.uriToStreamInfosMap_.get(uri));
  }
  ...
  return this.createStreamInfo_(uri, allCodecs, type, language, primary,
    ...
    if (this.uriToStreamInfosMap_.has(uri)) {
      return this.uriToStreamInfosMap_.get(uri);
    }
...
shaka.hls.HlsParser.prototype.getStartTime_ =
  ...
  if (this.manifest_) {
    const streamInfo = this.uriToStreamInfosMap_.get(playlistUri);
