...
shaka.hls.HlsParser = class {
  ...
  async updateStream_(streamInfo) {
    ...
    const {segments, totalCount, bandwidth} = this.createSegments_(
        ...
        stream.segmentIndex);
    this.fixRawFormatSegmentMimeTypes_(segments, stream.mimeType);
    if (bandwidth) {
  ...
  async getBasicInfoFromSegments_(segments, probeClosedCaptions) {
    ...
    const isTs = !isMp4 && shaka.util.TsParser.probe(
    ...
    if (!isMp4 && !isTs) {
      // Some servers publish packed audio with a container extension and MIME
      // type, such as a .ts file served as video/mp2t.  Neither says anything
      // about what is really inside, so ask the content itself.
      const rawMimeType =
          shaka.media.SegmentUtils.getRawAudioMimeType(data);
      if (rawMimeType) {
        return shaka.media.SegmentUtils.getBasicInfoFromMimeType(rawMimeType);
      }
    }
  ...
  async convertParsedPlaylistIntoStreamInfo_(streamId, variables, playlist,
    ...
    this.fixRawFormatSegmentMimeTypes_(segments, mimeType);
    ...
    for (let i = 0; i < dateRangeTags.length; i++) {
      ...
      try {
        ...
        for (const name of ['SCTE35-OUT', 'SCTE35-IN', 'SCTE35-CMD']) {
          const data = shaka.util.Scte35.fromHex(tag.getAttributeValue(name));
          if (!data) {
            continue;
          }
          // An IN closes the range it is attached to, so it applies at the
          // end of that range rather than at its start.
          const time = name == 'SCTE35-IN' && endTime != null ?
              endTime : startTime;
          this.playerInterface_.onScte35Event({
            schemeIdUri: 'urn:scte:scte35:2013:bin',
            startTime: time,
            endTime: name == 'SCTE35-OUT' && endTime != null ? endTime : time,
            id: tag.getRequiredAttrValue('ID'),
            source: 'hls',
            kind: name.substring(7).toLowerCase(),
            data,
            node: null,
          });
        }
  ...
  /**
   * The MIME type of each segment reference is guessed from its file
   * extension, which some servers get wrong: packed audio is often published
   * with a container extension such as .ts.  Once the content itself has told
   * us that this is a raw format, drop the guesses that contradict it, or
   * MediaSourceEngine would try to switch to a container type that the stream
   ...
   * @private
   */
  fixRawFormatSegmentMimeTypes_(segments, mimeType) {
    if (!shaka.util.MimeUtils.RAW_FORMATS.includes(mimeType)) {
      return;
    }
    for (const segment of segments) {
      if (segment.mimeType && segment.mimeType != mimeType) {
