...
shaka.hls.HlsParser = class {
  ...
  async createPeriod_(playlist) {
    ...
    const textStreams = await this.parseTexts_(tags);
  ...
  async createVariantsForTag_(tag, playlist) {
    ...
    const mediaTags = this.getMediaTags_(playlist.tags, audioGroupId,
        videoGroupId);
    const streamInfos = await this.createStreamInfosForAudioAndVideo_(tag,
        mediaTags, audioGroupId, videoGroupId, resolution, frameRate);

    if (streamInfos) {
      goog.asserts.assert(streamInfos.audio.length || streamInfos.video.length,
   ...
   *
   * @param {!shaka.hls.Tag} tag
   * @param {!Array.<!shaka.hls.Tag>} mediaTags An array of EXT-X-MEDIA tags.
   ...
   * @private
   */
  async createStreamInfosForAudioAndVideo_(tag, mediaTags, audioGroupId,
      videoGroupId, resolution, frameRate) {
    ...
    const firstAudioStreamURI =
        res.audio.length ? res.audio[0].verbatimMediaPlaylistUri : '';
    const areSameUri = streamURI == firstAudioStreamURI;
    ...
    if (!streamInfos.length && codecs.length == 1 && !hasVideoRelatedInfo) {
      ...
      codecs = [codecs.join(',')];
    } else if (res.audio.length && areSameUri) {
    ...
    if (streamInfo) {
      streamInfo.stream.type == ContentType.AUDIO ?
        res.audio = [streamInfo] : res.video = [streamInfo];
    } else if (streamInfo === null) {
      // Triple-equals for undefined.
      shaka.log.debug('streamInfo is null');
      return null;
    }
    this.filterLegacyCodecs_(res.audio);
    this.filterLegacyCodecs_(res.video);
    return res;
  }
  ...
  async getStartTime_(
    ...
    if (mimeType == 'application/mp4' || mimeType.startsWith('text/')) {
      return this.getStartTimeFromTextSegmentRef_(mimeType, codecs, segmentRef);
    }
  ...
  /**
   * Requests then parses a text segment to get its start time.
   *
   * @param {string} mimeType
   * @param {string} codecs
   * @param {!shaka.media.SegmentReference} segmentRef
   ...
   * @private
   */
  async getStartTimeFromTextSegmentRef_(mimeType, codecs, segmentRef) {
    ...
    const fullOnly = !textEngine.getSupportsPartial();
    const response = await this.fetchPartialSegment_(segmentRef, fullOnly);
    goog.asserts.assert(response.data, 'Should have a response body!');
    return textEngine.getStartTime(response.data);
  }
