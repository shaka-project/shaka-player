...
shaka.hls.HlsParser.prototype.createPeriod_ = async function(playlist) {
  ...
  let textStreamPromises = textStreamTags.map(async function(tag) {
    if (this.config_.disableText) {
      return null;
    }

    try {
      return await this.createTextStream_(tag, playlist);
    } catch (e) {
  ...
  return {
    startTime: 0,
    variants: variants,
    textStreams: textStreams.filter((t) => t != null),
  };
};
...
shaka.hls.HlsParser.prototype.createVariants_ =
  ...
  // NOTE: we currently don't have support for audio-only content.
    const disableAudio = this.config_ ? this.config_.disableAudio : false;
    if (!audioInfos.length || disableAudio) {
    audioInfos = [null];
  }
    const disableVideo = this.config_ ? this.config_.disableVideo : false;
...
shaka.hls.HlsParser.prototype.createStreamInfo_ = async function(
    verbatimMediaPlaylistUri, allCodecs, type, language, primary, name,
    channelsCount, closedCaptions, characteristics) {
