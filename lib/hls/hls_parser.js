  ...
  createVariants_(
    ...
    for (const audioInfo of audioInfos) {
      for (const videoInfo of videoInfos) {
        ...
        const variant = {
          id: this.globalId_++,
          language: audioStream?.language ?? 'und',
          disabledUntilTime: 0,
          primary: audioStream?.primary || videoStream?.primary,
          audio: audioStream,
          video: videoStream,
          bandwidth,
          allowedByApplication: true,
  ...
  createSegments_(playlist, mediaSequenceToStartTime, variables,
    ...
    for (let i = 0; i < hlsSegments.length; i++) {
      ...
      if (discontinuityTag) {
        ...
        if (previousReference?.initSegmentReference) {
