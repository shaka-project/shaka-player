...
shaka.hls.HlsParser = class {
   ...
   * The MP4 registration authority (mp4ra.org) lists a value of ec+3 for
   * Enhanced AC-3 audio with JOC (Dolby Atmos). That value is not used by HLS.
   * Instead, it uses ec-3 and marks the presence of the additional JOC
   * content with JOC in the CHANNELS attribute of the audio rendition. The JOC
   * MUST be capitalized. For example, CHANNELS="16/JOC". The numeric value
   * should match the value of the complexity_index_type_a field in the
   * EC3SpecificBox of the Dolby Digital Plus audio track.
  ...
  async createStreamInfoFromMediaTag_(tag) {
    ...
    const spatialAudio = type == 'audio' ? this.getSpatialAudio_(tag) : false;
