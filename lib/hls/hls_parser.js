...
shaka.hls.HlsParser = class {
  ...
  async updateStream_(streamInfo) {
    ...
    console.log(`[APPLE, HlsParser]: parseDrmInfo_() from updateStream() for stream: ${stream.type}Id=${stream.id};drmInfos=${drmInfos.length};language=${stream.language};label=${stream.label}`);
  ...
  async parseManifest_(data) {
    ...
    if (playlist.type == shaka.hls.PlaylistType.MEDIA) {
      ...
      let langs = new Set();
      variants.forEach(v => {
        langs.add(`videoId=${v.video.id};${v.audio.language};${v.audio.label}`);
      });
      console.log(`[APPLE, HlsParser]: parseManifestInner(): ${[...langs].join(',')}`);
  ...
  createStreamInfosFromMediaTags_(mediaTags, groupIdPathwayIdMapping) {
    ...
    for (const key in groupedTags) {
      console.log(`[APPLE, HlsParser]: createStreamInfo for media group tag: ${groupedTags[key].map(tag => JSON.stringify(tag)).join(' ; ')};`);
  ...
  async createVariantsForTags_(tags, sessionKeyTags, mediaTags, getUris,
    ...
    for (const key in groupedTags) {
      ...
      console.log(`[APPLE, HlsParser]: Creating variants for: ${tags.map(tag => tag.toString()).join(';')}`);
      allVariants.push(...this.createVariants_(
          ...
          keyIds,
          iFrameStreams));
      console.log(`[APPLE, HlsParser]: Finished creating variants for: ${tags.map(tag => tag.toString()).join(';')}`);
    }
  ...
  createStreamInfosForVariantTags_(tags, mediaTags, resolution, frameRate) {
    console.log(`[APPLE, HlsParser]: createStreamInfosForVariantTags: mediaTags=${mediaTags.map(tag => tag.toString()).join(';')};resolution=${resolution};frameRate=${frameRate}`);
    const ContentType = shaka.util.ManifestParserUtils.ContentType;
    ...
    let x = new Set();
    if (isAudioGroup) {
      for (const info of res.audio) {
        x.add(`${info.stream.language};${info.stream.label}`);
      }
      console.log(`[APPLE, HlsParser]: createStreamInfosForVariantTags: ${[...x].join('  ')}`);
    }
  ...
  createVariants_(
    ...
    for (const audioInfo of audioInfos) {
      for (const videoInfo of videoInfos) {
        ...
        console.log(`[APPLE, HlsParser]: createVariants_: videoId=${videoStream.id}; audioId=${audioStream.id}; audioLanguage=${audioStream.language}; ` +
            `audioLabel=${audioStream.label}; codecs=${codecs.sort().toString()}; audioDrmInfos=${audioDrmInfos.length}; videoDrmInfos=${videoDrmInfos.length}`);
        ...
        if (this.variantUriSet_.has(variantUriKey)) {
          ...
          shaka.log.debug(
              '[APPLE]: Skipping variant which only differs in text streams.');
          continue;
        }
  ...
  createStreamInfoFromMediaTags_(tags, groupIdPathwayIdMapping,
    ...
    if (type == 'audio') {
      console.log(`[APPLE, HlsParser]: audioStreamId=${streamInfo.stream.id}; globalGroupId=${globalGroupId}; language=${streamInfo.stream.language}; label=${streamInfo.stream.label}`);
    }
  ...
  createStreamInfo_(streamId, verbatimMediaPlaylistUris, codecs, type,
    ...
    const downloadSegmentIndex = async (pendingRequest) => {
      ...
      try {
        ...
        this.setFullTypeForStream_(stream);
        console.log(`[APPLE, HlsParser]: segmentIndex realstream: ${stream.type}Id=${stream.id};drmInfos=${stream.drmInfos.length};language=${stream.language};label=${stream.label}`);
  ...
  async convertParsedPlaylistIntoStreamInfo_(streamId, variables, playlist,
    ...
    console.log(`[APPLE, HlsParser]: convertParsedMediaPlaylistIntoStreamInfo: ${stream.type}Id=${streamId}/${stream.id};drmInfos=${stream.drmInfos.length};language=${stream.language};label=${stream.label}`);
  ...
  createSegmentReference_(
    ...
    let syncTime = null;
    if (previousReference && previousReference.syncTime) {
      syncTime = previousReference.syncTime +
          previousReference.endTime - previousReference.startTime;
    } else if (!this.config_.hls.ignoreManifestProgramDateTime) {
      const dateTimeTag =
