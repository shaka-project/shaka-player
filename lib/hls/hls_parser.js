...
goog.require('shaka.event.SessionData');
...
shaka.hls.HlsParser = class {
  ...
  processSessionData_(tags) {
    ...
    for (const tag of tags) {
      ...
      let url;
      if (uri) {
        url = shaka.hls.Utils.constructSegmentUris(
            [this.masterPlaylistUri_], uri, this.globalVariables_)[0];
      }
      if (id == 'com.apple.hls.chapters' && url) {
        chapterUris.push(url);
      }
      if (this.playerInterface_) {
        this.playerInterface_.onEvent(new shaka.event.SessionData(
            id, url || null, language || null, value || null));
      }
    }
    return chapterUris;
  }
