...
shaka.device.AbstractDevice = class {
   ...
   * @override
   */
  requiresTimestampOffsetFudge() {
    // Only legacy Edge (EdgeHTML) needs this.  Chromium, Gecko and WebKit have
    // all been measured to place the first frame of a segment exactly on
    // appendWindowStart for a negative timestampOffset, so they keep the
    // keyframe without any nudge.  Note that BrowserEngine.EDGE means EdgeHTML
    // specifically: Chromium-based Edge reports "Edg/" and maps to CHROMIUM.
    return this.getBrowserEngine() === shaka.device.IDevice.BrowserEngine.EDGE;
  }

  /**
   * @override
   */
  requiresPipelineFlush() {
    return false;
  }
