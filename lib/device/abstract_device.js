...
shaka.device.AbstractDevice = class {
  ...
  needWaitForEncryptedEvent(keySystem) {
    // FairPlay with src= loading on Safari requires MediaKeys attached BEFORE
    // video.src is set. Returning true here would delay attachment until the
    // 'encrypted' event, but Safari errors before that event can fire.
    return false;
  }
