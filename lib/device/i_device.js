...
shaka.device.IDevice = class {
  ...
  /**
   * Returns true if the platform rounds the SourceBuffer's timestampOffset in
   * a way that can push the first frame of a segment just below
   * appendWindowStart, which makes MSE drop everything up to the next
   * keyframe.  For such platforms, MediaSourceEngine nudges negative offsets
   * forward by a millisecond.
   *
   * See https://github.com/shaka-project/shaka-player/issues/1281
   *
   * @return {boolean}
   */
  ...
  /**
   * Returns true if the platform needs the media pipeline to be flushed after
   * the buffer has been cleared, in order for new content to replace the old
   * content.  For such platforms, MediaSourceEngine nudges the playhead once
   * the buffer is empty.
   *
   * @return {boolean}
   */
  ...
  /**
   * Returns true if the platform requires the dvvC box content workaround.
   *
   * Dolby box removal is needed on LG devices with HDR10 support that do not
   * support dolby vision. LG has fixes starting with Webos23, which is Chrome
   * version 94.
  ...
  /**
   * Returns the RemotePlayback available on the device.
   *
