 ...
 * @template T
 */
shaka.util.ArrayUtils.hasSameElements = function(a, b, comp = undefined) {
  if (!comp) {
    comp = shaka.util.ArrayUtils.defaultEquals;
  }
  ...
  let copy = b.slice(0);
  for (const item of a) {
    const idx = shaka.util.ArrayUtils.indexOf(copy, item, comp);
    if (idx == -1) {
      return false;
    }
