...
shaka.util.CmcdManager = class {
   ...
   * @param {string} format The manifest format
   */
  applyManifestData(request, format) {
    try {
      if (!this.config_.enabled) {
        return;
      }

      this.sf_ = this.getStreamingFormat_(format);
   ...
   */
  applySegmentData(request, type, init, duration, mimeType, codecs, bandwidth) {
    try {
      if (!this.config_.enabled) {
        return;
      }
      ...
      data.ot = this.getObjectType_(type, init, mimeType, codecs);
      ...
      if (isMedia) {
        data.bl = this.getBufferLength_(type);
      }

      if (bandwidth) {
        data.br = bandwidth / 1000;
      }

      data.tb = this.getTopBandwidth_(type) / 1000;
   ...
   * @private
   */
  getStreamingFormat_(streamingFormat) {
