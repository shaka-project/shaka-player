...
shaka.util.CmcdManager = class {
  ...
  */
  ...
  /**
   * Set playbackPlayTime_
   * @param {number} time
   */
  setPlaybackPlay(time) {
    if (!this.playbackPlayTime_) {
      this.playbackPlayTime_ = time;
    }
  }
  ...
  */
  setupMSDEventListeners_() {
    const onPlaybackPlay = () => this.onPlaybackPlay();
    this.eventManager_.listenOnce(
        this.video_, 'play', onPlaybackPlay);

    const onPlaybackPlaying = () => this.onPlaybackPlaying();
    this.eventManager_.listenOnce(
