...
shaka.util.CmcdManager = class {
  ...
  constructor(playerInterface, config) {
    ...
    */
    ...
    /**
     * Attach CMCD data to all HEAD requests so they can be
     * associated with the playback session.
     *
     * @private {shaka.extern.RequestFilter}
     */
    this.requestFilter_ = (type, request) => {
      if (!this.config_.enabled || request.method !== 'HEAD') {
        return;
      }
    ...
    playerInterface.getNetworkingEngine()
        .registerRequestFilter(this.requestFilter_);
  }
   ...
   */
  release() {
    this.playerInterface_.getNetworkingEngine()
        .unregisterRequestFilter(this.requestFilter_);
  }
