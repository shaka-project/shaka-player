...
shaka.util.CmcdManager = class {
  ...
  applyResponse_(response, data = {}, useHeaders = this.config_.useHeaders) {
    ...
    if (useHeaders) {
      ...
      response.uri = shaka.util.CmcdManager.appendQueryToUri(response.uri, query);
      ...
      const request = shaka.net.NetworkingEngine.makeRequest(['http://localhost:3003/response-mode'], retryParams);
