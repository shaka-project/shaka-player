...
shaka.util.CmcdManager = class {
  ...
  applyRequest_(request, data = {}, useHeaders = this.config_.useHeaders) {
    ...
    const rawOutput = this.getGenericData_(data);
    const includeKeys = this.config_.includeKeys || [];
    const output = this.filterKeys_(rawOutput, includeKeys);
  ...
  applyResponse_(response, data = {}) {
    ...
    const allowedKeys = Array.from(new Set([
      ...shaka.util.CmcdConstants.V2CommonKeys,
      ...shaka.util.CmcdConstants.V2ResponseModeKeys
    ]));
    ...
    for (const target of responseTargets) {
      ...
      const allowedKeysResponseMode = this.checkValidKeys(includeKeys, allowedKeys, 'response');
 ...
 * @enum {Array<string>}
 */
shaka.util.CmcdConstants = {
  V2CommonKeys : [
    'br', 'ab', 'bl', 'tbl', 'bs', 'cdn', 'cid', 'ltc', 'mtp', 'pr', 'sf', 'sid', 'bg',
    'sta', 'st', 'ts', 'tpb', 'tb', 'lb', 'tab', 'lab', 'pt', 'ec', 'msd', 'v',
  ],
  V2ResponseModeKeys : [
    'd', 'dl', 'nor', 'ot', 'rtp', 'rc', 'su', 'ttfb', 'ttfbb', 'ttlb', 'url', 'cmsdd', 'cmsds',
  ]
};
