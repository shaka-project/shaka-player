...
shaka.util.CmcdManager = class {
   ...
   */
  constructor(playerInterface, config, player) {
  ...
  setupEventListeners_() {
    ...
    // Waiting
    this.eventManager_.listen(this.video_, 'waiting', () => {
  ...
  reportEvent_(eventType, extraData = {}) {
    ...
    for (const target of eventTargets) {
      ...
      if (!allowedKeysEventMode.includes(CMCD_V2_TS_KEY)) {
        allowedKeysEventMode.push(CMCD_V2_TS_KEY);
      }

      const output = this.filterKeys_(
          rawOutput,
          allowedKeysEventMode,
      );
