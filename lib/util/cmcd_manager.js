...
shaka.util.CmcdManager = class {
  ...
  applyRequestData(type, request, context = {}) {
    ...
    // Learn the streaming format from every manifest request, even before a
    // reporter exists (the manifest may be what enables CMCD) and even when
    // includeInRequests excludes manifests from decoration: `sf` / `st` are
    ...
    // Track request initiation time for TTFB/TTLB derivation when the
    // response comes back. Response reporting is not gated on
    // includeInRequests, so the timestamp is recorded before that gate.
  ...
  toReporterConfig_(cfg) {
    ...
    const reporterConfig = /** @type {!cml.cmcd.CmcdReporterConfig} */ ({
      ...
      version: cfg.version === cml.cmcd.CMCD_V2 ?
          cml.cmcd.CMCD_V2 : cml.cmcd.CMCD_V1,
    });
  ...
  passesManifestFilters_(type, request, context) {
    ...
    if (effective.adaptationSets && context.stream) {
      ...
      if (!groupId) {
        shaka.log.warnOnce('CMCD_NO_GROUP_ID',
            'CMCD: the manifest restricts reporting to adaptation sets ' +
            effective.adaptationSets.join(' ') + ' but some streams have ' +
            'no group id; they are not decorated. For DASH this happens ' +
            'when dash.enableAudioGroups is false.');
        return false;
      }
...
shaka.util.CmcdManager.resolveEffectiveConfig = (
  ...
  if (mpd && mpd.keys) {
    const filtered = shaka.util.CmcdManager.filterSupportedKeys_(
        mpd.keys, version);
    if (filtered.length) {
      includeKeys = filtered;
    } else {
      // Keeping the empty list would expand to every key for the version in
      // toReporterConfig_, which inverts Table K.8: keys the player does not
      // support are ignored, they do not select everything.
      shaka.log.warnOnce('CMCD_NO_SUPPORTED_MANIFEST_KEYS',
          'CMCD: the manifest lists no key this player supports at version ' +
          version + '; using the application keys instead.');
    }
  }
