...
shaka.util.CmcdManager = class {
  ...
  reportEvent_(eventType, extraData = {}) {
    ...
    const eventTargets = targets.filter(
        (target) => target.mode === 'event',
    );

    for (const target of eventTargets) {
      const baseURL = target.url;
      const queryString = shaka.util.CmcdManager.toQuery(data);

      const uri = shaka.util.CmcdManager.appendQueryToUri(
          baseURL,
          queryString,
      );
