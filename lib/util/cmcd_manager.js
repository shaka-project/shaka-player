...
shaka.util.CmcdManager = class {
  ...
  applyResponse_(response, data = {}, useHeaders = this.config_.useHeaders) {
    shaka.log.debug(`Original Response: ${JSON.stringify(response)}`);
    if (!this.config_.enabled) {
      return;
    }
    ...
    if (useHeaders) {
      ...
      shaka.util.CmcdManager.appendQueryToUri(response.uri, query);
      shaka.log.debug(`Updated Response: ${JSON.stringify(response)}`);
    }
  }
