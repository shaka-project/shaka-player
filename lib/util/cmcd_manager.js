...
goog.require('shaka.util.CmcdBatchingManager');
...
shaka.util.CmcdManager = class {
  ...
  constructor(playerInterface, config) {
    ...
    /**
     * @private {?shaka.util.CmcdBatchingManager}
     */

    this.batchingManager_ = new shaka.util.CmcdBatchingManager({
      getNetworkingEngine: () => this.playerInterface_.getNetworkingEngine(),
    });
  }
  ...
  reset() {
    ...
    if (this.batchingManager_) {
      this.batchingManager_.reset();
    }
  }
  ...
  appendTextTrackData(uri) {
    try {
      ...
      const query = shaka.util.CmcdUtils.serialize(data);
  ...
  sendCmcdRequest_(cmcdData, target, response) {
    // Check if batching is enabled and target supports batching
    if (this.batchingManager_ &&
        (target.batchSize || target.batchTimer)) {
      this.batchingManager_.addReport(target, cmcdData);
      return;
    }
  ...
  applyCmcdDataToRequest_(output, request, useHeaders) {
    if (useHeaders) {
      ...
      const query = shaka.util.CmcdUtils.serialize(output);
      if (!query) {
        return;
      }
  ...
  static toHeaders(data) {
    ...
    for (let i = 0; i < headerGroups.length; i++) {
      const value = shaka.util.CmcdUtils.serialize(headerGroups[i]);
      if (value) {
   ...
   */
  static toQuery(data) {
    return shaka.util.CmcdUtils.serialize(data);
  }
