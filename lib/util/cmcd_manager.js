...
shaka.util.CmcdManager = class {
  ...
  constructor(playerInterface, config) {
    ...
    this.lastRequestTimestamp_ = null;
  ...
  applyResponseSegmentData(response, context) {
    try {
      ...
      if (this.lastRequestTimestamp_) {
        data.ts = this.lastRequestTimestamp_;
        this.lastRequestTimestamp_ = null;
      } else if (!data.ts) {
        data.ts = Date.now();
      }
  ...
  applyRequestSegmentData(request, context) {
    try {
      ...
      data.ts = Date.now();
      this.lastRequestTimestamp_ = data.ts;
  ...
  applyRequest_(request, data) {
    ...
    const allowedKeysRequestMode = this.checkValidKeys(
        includeKeys,
        allowedKeys,
  ...
  /**
   * Checks and filters the provided CMCD keys to ensure only allowed keys
   * are used for the specified mode.
   *
   * @param {Array<string>} includeKeys - Keys to include.
   * @param {Array<string>} allowedKeys - Allowed keys for the mode.
   * @param {string} mode - CMCD mode ('request' or 'response').
   * @return {Array<string>} - Filtered list of valid keys.
   */
  checkValidKeys(includeKeys, allowedKeys, mode) {
