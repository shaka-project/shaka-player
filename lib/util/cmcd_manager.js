...
shaka.util.CmcdManager = class {
  ...
  applyResponseSegmentData(response, context) {
    try {
      ...
      const data = this.calculateCmcdSegment(context);
      this.applyResponse_(response, data);
    } catch (error) {
      shaka.log.warnOnce(
        'CMCD_SEGMENT_ERROR',
        'Could not generate response segment CMCD data.', error
      );
    }
  }
  ...
  applyRequestSegmentData(request, context) {
    try {
      ...
      var data = this.calculateCmcdSegment(context);
      ...
      if (stream) {
        ...
        if (stream.segmentIndex && segment) {
          const reverse = playbackRate < 0;
          const iterator = stream.segmentIndex.getIteratorForTime(segment.endTime, /* allowNonIndependent= */ true, reverse);
          if (iterator) {
            const nextSegment = iterator.next().value;
            if (nextSegment && nextSegment != segment) {
              if (!shaka.util.ArrayUtils.equal(segment.getUris(), nextSegment.getUris())) {
                data.nor = this.urlToRelativePath_(nextSegment.getUris()[0], request.uris[0]);
              }
            }
          }
        }
  ...
  applyRequest_(request, data = {}, useHeaders = this.config_.useHeaders) {
    ...
    const output = this.getGenericCmcdData(data);
   ...
   * @private
   */
  applyResponse_(response, data = {}, useHeaders = this.config_.useHeaders) {
    if (!this.config_.enabled) {
      return;
    }
    ...
    if (useHeaders) {
      ...
      const encodedQueryString = 'CMCD=' + encodeURIComponent(queryStringRaw);
      response.uri = shaka.util.CmcdManager.appendQueryToUri(response.uri, encodedQueryString);
      ...
      const request = shaka.net.NetworkingEngine.makeRequest([`http://localhost:3003/response-mode?${encodedQueryString}`], retryParams);
      ...
      operation.promise.then( requestResponse => {
        ...
        const text = shaka.util.StringUtils.fromUTF8(requestResponse.data);
