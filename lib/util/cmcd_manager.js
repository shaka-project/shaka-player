...
shaka.util.CmcdManager = class {
  ...
  applyManifestData(request, manifestInfo) {
    try {
      this.sf_ = this.getStreamingFormat_(manifestInfo.format);
  ...
  applySegmentData(request, segmentInfo) {
    try {
      ...
      const type = segmentInfo.stream.type;
      ...
      if (segmentInfo.manifest) {
        data.tb = this.getTopBandwidth_(type, segmentInfo.manifest) / 1000;
      }
   ...
   * @private
   */
  getTopBandwidth_(type, manifest) {
 ...
 * @property {?shaka.extern.Manifest} manifest
 *   The stream manifest
 */
