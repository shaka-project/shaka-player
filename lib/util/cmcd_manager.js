...
shaka.cmcd.CmcdManager = class {
  ...
  apply(request, data = {}) {
    ...
    data.mtp = this.playerInterface_.getBandwidthEstimate() / 1000;
  ...
  applySegmentData(request, segment) {
    const data = {
      ot: this.getObjectType_(segment),
      d: segment.duration * 1000,
      st: this.getStreamType(),
    };

    if (data.ot === 'v' || data.ot === 'a' || data.ot === 'av') {
      data.bl = this.getBufferLength_(segment.stream.type);
    }

    if (segment.stream.bandwidth) {
      data.br = segment.stream.bandwidth / 1000;
    }
