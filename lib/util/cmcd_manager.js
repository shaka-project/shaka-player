...
shaka.cmcd.CmcdManager = class {
   ...
   * @param {!shaka.extern.Request} request The request to apply CMCD data to
   */
  applyManifestData(request, manifest) {
    this.sf_ = this.getStreamingFormat(manifest.format);

    this.apply(request, {
      ot: shaka.cmcd.ObjectType.MANIFEST,
    });
  }

  /**
   * Apply segment level CMCD data to a segment request
   *
   * @param {!shaka.extern.Request} request
   * @param {!shaka.cmcd.Segment} segment
   */
  applySegmentData(request, segment) {
    const ot = this.getObjectType_(segment);
    const isMedia = (ot === 'v' || ot === 'a' || ot === 'av');
    const bl = isMedia ? this.getBufferLength_(segment.stream.type) : undefined;

    this.apply(request, {
      ot: ot,
      d: segment.duration,
      br: segment.stream.bandwidth,
      bl: bl,
   ...
   * Get the stream type
   *
   * @return {shaka.cmcd.StreamType}
   */
  getStreamType() {
    const isLive = this.playerInterface_.isLive();
    if (isLive) {
      return shaka.cmcd.StreamType.LIVE;
    } else {
      return shaka.cmcd.StreamType.VOD;
    }
  }
 ...
 *   getBandwidthEstimate: function():(number|undefined),
 ...
 * @property {function():(number|undefined)} getBandwidthEstimate
