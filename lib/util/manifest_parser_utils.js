...
shaka.util.ManifestParserUtils.resolveUris = function(baseUris, relativeUris) {
  ...
  const relativeAsGoog =
      relativeUris.map((uri) => { return new goog.Uri(uri); });
  // Resolve each URI relative to each base URI, creating an Array of Arrays.
  // Then flatten the Arrays into a single Array.
  return baseUris.map((uri) => { return new goog.Uri(uri); })
    .map((base) => { return relativeAsGoog.map(base.resolve.bind(base)); })
    .reduce(Functional.collapseArrays, [])
    .map((uri) => { return uri.toString(); });
};
