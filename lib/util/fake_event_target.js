 ...
 * @param {boolean} capturing True to listen during the capturing phase, false
 *     to listen during the bubbling phase.  Note that FakeEventTarget does not
 *     support the capturing phase from the standard event model.
 * @override
 */
shaka.util.FakeEventTarget.prototype.addEventListener =
  ...
  shaka.asserts.assert(!capturing);
  if (!capturing) {
 ...
 * @param {boolean} capturing True to remove a listener for the capturing phase,
 *     false to remove a listener for the bubbling phase.  Note that
...
shaka.util.FakeEventTarget.prototype.dispatchEvent = function(event) {
  ...
  Object.defineProperties(event, {
    ...
    'target': { 'value': this },
    // currentTarget will be set many times by recursiveDispatch_(), so make
    // it writable.
