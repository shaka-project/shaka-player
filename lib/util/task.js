 ...
 * be considered an error.  Must be called from within a stage.
 * @throws Error if called from outside a stage.
 */
shaka.util.Task.prototype.end = function() {
  if (!this.executingStage_) {
    throw new Error('Task.end is only usable from inside a stage function!');
  }
...
shaka.util.Task.prototype.startNextStage_ = function(arg) {
  // Flag that we are executing a stage, but ensure we reset that flag even if
  // an exception is thrown.
  this.executingStage_ = true;
  var exception;
  try {
  ...
  // Re-throw any exception thrown by the stage function.
  if (exception) {
    throw exception;
  }
