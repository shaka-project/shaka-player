...
shaka.util.Platform = class {
  ...
  /**
   * Returns true if we need to polyfill Apple legacy MediaKeys
   * implementation
   *
   * @return {boolean}
   */
  static willMediaKeysAppleBePolyfilled() {
  ...
  /**
   * Returns true if we need to polyfill Webkit legacy MediaKeys
   * implementation
   *
   * @return {boolean}
   */
  static willMediaKeysWebkitBePolyfilled() {
    ...
    // eslint-disable-next-line no-restricted-syntax
    return !!(HTMLMediaElement.prototype.webkitGenerateKeyRequest ||
      // eslint-disable-next-line no-restricted-syntax
      HTMLMediaElement.prototype.generateKeyRequest);
  }

  /**
   * Returns true if we need to polyfill MS legacy MediaKeys implementation
   *
   * @return {boolean}
   */
  static willMediaKeysMSBePolyfilled() {
    return window.HTMLVideoElement && window.MSMediaKeys &&
      (!navigator.requestMediaKeySystemAccess ||
       // eslint-disable-next-line no-restricted-syntax
       !MediaKeySystemAccess.prototype.getConfiguration);
  }
  ...
  /**
   * Returns true if any MediaKeys polyfill should have been
   * installed
   *
   * @return {boolean}
   */
  static willMediaKeysBePolyfilled() {
    return shaka.util.Platform.willMediaKeysAppleBePolyfilled() ||
        shaka.util.Platform.willMediaKeysWebkitBePolyfilled() ||
        shaka.util.Platform.willMediaKeysWebkitBePolyfilled();
  }
};
