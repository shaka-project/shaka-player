...
shaka.util.ContentSteeringManager = class {
  ...
  getLocations(streamId) {
    ...
    if (locationsMap.length && this.bannedLocations_.size) {
      const now = Date.now();
      const toDelete = [];
      for (const [uri, bannedUntil] of this.bannedLocations_) {
        if (now > Number(bannedUntil)) {
          toDelete.push(uri);
          continue;
        }
        locationsMap = locationsMap.filter((locationMap) => {
          if (String(uri).includes(locationMap.location)) {
            return false;
          }
          return true;
        });
    ...
    if (locationsMap.length) {
      this.lastPathwayUsed_ = locationsMap[0].pathwayId;
    }

    const locations = locationsMap.map((l) => l.location);
