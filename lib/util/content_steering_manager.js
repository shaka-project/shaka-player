...
shaka.util.ContentSteeringManager = class {
  ...
  addQueryParams_(uri) {
    ...
    if (this.manifestType_ == shaka.media.ManifestParser.DASH) {
      params.set('_DASH_pathway', currentPathwayID);
      params.set('_DASH_throughput', String(currentBandwidth));
    } else if (this.manifestType_ == shaka.media.ManifestParser.HLS) {
      params.set('_HLS_pathway', currentPathwayID);
      params.set('_HLS_throughput', String(currentBandwidth));
    }
  ...
  getLocations(streamId, ignoreBaseUrls = false) {
        ...
        if (clone) {
          ...
          if (cloneLocation) {
            if (clone['URI-REPLACEMENT'].HOST) {
              const updatedUri = shaka.util.URL.setDomain(
                  cloneLocation,
                  clone['URI-REPLACEMENT'].HOST);
    ...
    locationsPathwayIdMap = locationsPathwayIdMap.filter((l) => {
      const domain = shaka.util.URL.getDomain(l.location);
      for (const uri of this.bannedLocations_.keys()) {
    ...
    return shaka.util.URL.resolveUris(this.baseUris_, locations);
  }
};
