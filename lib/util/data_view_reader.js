...
shaka.util.DataViewReader = class {
   ...
   * If the value is larger than Number.MAX_SAFE_INTEGER, a BigInt is returned,
   * when parameter useBigInt is truthy. Otherwise, an error is thrown.
   * @suppress {undefinedVars}
   * @param {boolean} useBigInt Ask for a BigInt when needed, by default don't.
   * @return {number} The integer or BigInt
   * @export
   */
  readUint64(useBigInt = false) {
    /** @type {number} */
    let low;
    /** @type {number} */
    let high;
    ...
    if (high > 0x1FFFFF) {
      if (useBigInt && BigInt) {
        this.position_ += 8;
        console.log('Returning bigint', high, low, BigInt);
        /* eslint-disable new-cap */
        const b1 = BigInt(high);
        const b2 = BigInt(Math.pow(2, 32));
        const b3 = BigInt(low);
        /* eslint-enable new-cap */
        console.log('b1...b3', b1, b2, b3);
        return (b1 * b2) + b3;
      }
