...
shaka.util.DataViewReader = class {
   ...
   * If the value is larger than Number.MAX_SAFE_INTEGER and full64Bit is true,
   * the two 32-bit parts of the 64-bit value are returned as such in an object.
   * If the value is larger than Number.MAX_SAFE_INTEGER but full64Bit is not
   * true, a JS_INTEGER_OVERFLOW error is thrown.
   * @suppress {undefinedVars}
   * @param {boolean} full64Bit Request a high/low response when applicable.
   * @return {number|{high: number, low: number}} The read 64-bit value.
   * @export
   */
  readUint64Ex(full64Bit = false) {
    /** @type {number} */
    let low;
    /** @type {number} */
    let high;
