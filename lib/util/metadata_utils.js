...
goog.provide('shaka.util.Metadata');

goog.require('shaka.util.Id3Utils');
goog.require('shaka.util.IlstUtils');
goog.require('shaka.util.VorbisUtils');


/**
 * @export
 */
shaka.util.Metadata = class {
  /**
  ...
  static getMetadataFrames(data, mimeType) {
    const Metadata = shaka.util.Metadata;

    const frames = [];

    if (Metadata.MIME_TYPES_WITH_ID3_.includes(mimeType)) {
      frames.push(...shaka.util.Id3Utils.getID3Frames(data));
      frames.push(...shaka.util.Id3Utils.getID3v1Frames(data));
    } else if (Metadata.MIME_TYPES_WITH_VORBIS_COMMENTS_.includes(mimeType)) {
      frames.push(...shaka.util.VorbisUtils.getVorbisComments(data));
    } else if (Metadata.MIME_TYPES_WITH_ILST_.includes(mimeType)) {
      frames.push(...shaka.util.IlstUtils.parse(data));
    }
  ...
  static supports(mimeType) {
    return shaka.util.Metadata.SUPPORTED_MIME_TYPES_.includes(mimeType);
  }
};
 ...
 * @private
 */
shaka.util.Metadata.MIME_TYPES_WITH_ID3_ = [
  'audio/mpeg',
  'audio/aac',
];
 ...
 * @private
 */
shaka.util.Metadata.MIME_TYPES_WITH_VORBIS_COMMENTS_ = [
  'audio/flac',
  'audio/ogg',
  'audio/vorbis',
  'audio/opus',
 ...
 * @private
 */
shaka.util.Metadata.MIME_TYPES_WITH_ILST_ = [
  'audio/mp4',
];
 ...
 * @private
 */
shaka.util.Metadata.SUPPORTED_MIME_TYPES_ = [
  ...shaka.util.Metadata.MIME_TYPES_WITH_ID3_,
  ...shaka.util.Metadata.MIME_TYPES_WITH_VORBIS_COMMENTS_,
  ...shaka.util.Metadata.MIME_TYPES_WITH_ILST_,
];
