...
shaka.util.Error.Code = {
   ...
   */
  'STREAMING_INCONSISTENT_BUFFER_STATE': 34,
   ...
   */
  'STREAMING_INVALID_SEGMENT_INDEX': 35,
   ...
   * repositioning the playhead and then invoking StreamingEngine.seeked().
   *
   * <br> error.data[0] is the type of content which caused the error.
   * <br> error.data[1] is the index of the Period.
   * <br> error.data[2] is the timestamp needed.
   */
  'STREAMING_SEGMENT_DOES_NOT_EXIST': 36,

  /**
   * The StreamingEngine cannot append the next segment because doing so
   * would exceed the byte limit.
   *
   * This is a recoverable error; recovery may be attempted by adjusting the
   * byte limit and then invoking StreamingEngine.switch().
   *
   * <br> error.data[0] is the type of content which caused the error.
   */
  'STREAMING_CANNOT_SATISFY_BYTE_LIMIT': 37,
   ...
   * by invoking StreamingEngine.switch().
   *
   * <br> error.data[0] is the type of content which caused the error.
   */
  'STREAMING_BAD_SEGMENT': 38,
   ...
   */
  'UNABLE_TO_GUESS_MANIFEST_TYPE': 39,

  /**
   * The manifest indicated protected content, but the manifest parser was
   * unable to determine what key systems should be used.
   */
  'NO_RECOGNIZED_KEY_SYSTEMS': 40,

  /** None of the requested key systems are available. */
  'REQUESTED_KEY_SYSTEMS_UNAVAILABLE': 41,
   ...
   * <br> error.data[0] is an error message string from the browser.
   */
  'FAILED_TO_GENERATE_LICENSE_REQUEST': 46,
   ...
   * <br> error.data[0] is an error message string from the browser.
   */
  'LICENSE_RESPONSE_REJECTED': 48,

  /**
   * No license server was specified by the application.
   * To fix, set the Player configuration key drm.servers.
   * <br> error.data[0] is the key system identifier.
   */
  'NO_LICENSE_SERVER_SPECIFIED': 49,
   ...
   */
  'ENCRYPTED_CONTENT_WITHOUT_DRM_INFO': 50
};
