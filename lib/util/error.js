...
shaka.util.Error = class {
   ...
   * @return {?shaka.util.Error.MediaElementErrorDetails}
   */
  static getMediaElementErrorDetails(error) {
    const code = error.code;

    // Extra error information from MS Edge:
    let extended = error.msExtendedCode;
    if (extended) {
      // Convert to unsigned:
      if (extended < 0) {
...
shaka.util.Error.Code = {
  ...
  /**
   * An external track failed to load in src= mode.
   */
  'ERROR_LOADING_EXTERNAL_TRACK_TO_SRC_EQUALS': 2018,
   ...
   * <br> error.data[1] is object containing details from media element error
   *   or string with brief explanation.
  ...
  // RETIRED: 'MSS_MISSING_DATA_FOR_TRANSMUXING': 3020,

  // RETIRED: 'MSS_TRANSMUXING_CODEC_UNKNOWN': 3021,

  // RETIRED: 'MSS_TRANSMUXING_FAILED': 3022,
  ...
  /**
   * Some component tried to write past the end of a buffer. The data may be
   * malformed.
   */
  ...
  // RETIRED: 'MSS_INVALID_XML': 4046,

  // RETIRED: 'MSS_LIVE_CONTENT_NOT_SUPPORTED': 4047,
  ...
  /**
   * WebTransport API is not available.
   */
  'WEBTRANSPORT_NOT_AVAILABLE': 4056,

  /**
   * WebTransport initialization failed
   * <br> error.data[0] is the original error.
   */
  'WEBTRANSPORT_INITIALIZATION_FAILED': 4057,

  /**
   * MSF parser encountered a vod playlist.
   */
  ...
  /**
   * Invalid AES-256-GCM segment format (missing IV or authentication tag).
   */
  'HLS_INVALID_GCM_SEGMENT': 4060,

  /**
   * The DASH Manifest contained invalid JSON.
   ...
   * element.  The video element must be provided to Player.attach() before
   * Player.load() is called.
   */
  ...
  // RETIRED: 'CS_AD_MANAGER_NOT_INITIALIZED': 10001,
  ...
  // RETIRED: 'SS_AD_MANAGER_NOT_INITIALIZED': 10003,
  ...
  // RETIRED: 'MT_AD_MANAGER_NOT_INITIALIZED': 10005,

  // RETIRED: 'INTERSTITIAL_AD_MANAGER_NOT_INITIALIZED': 10006,
  ...
  /**
   * CS Ad container, required for ad insertion, has not been configured.
   */
  'CS_AD_CONTAINER_MISSING': 10008,

  /**
   * SS Ad container, required for ad insertion, has not been configured.
   */
  'SS_AD_CONTAINER_MISSING': 10009,

  /**
   * MediaTailor request returned an error and there was no backup asset uri
   * provided.
   */
 ...
 * @typedef {{
 *   code: number,
 *   extended: (number|string|undefined),
 *   message: string,
 * }}
 */
shaka.util.Error.MediaElementErrorDetails;
