...
shaka.util.TsParser = class {
  constructor() {
    ...
    /**
     * The first three bytes of the AV1_video_descriptor, which the AV1 in
     * MPEG-2 TS spec requires to describe the whole stream.
     *
     * @private {?Uint8Array}
     */
    this.av1Metadata_ = null;
          ...
          if (esInfoLength > 0) {
            ...
            while (remaining > 2) {
              ...
              switch (descriptorId) {
                ...
                case 0x80:
                  if (result.videoCodec == 'av1') {
                    // Its first three bytes (marker and version, seq_profile
                    // and seq_level_idx_0, then seq_tier_0 through
                    // chroma_sample_position) are laid out exactly like the
                    // first three of an AV1CodecConfigurationRecord, so keep
                    // them as they are and let the av1C parser read them.
                    if (descriptorLen >= 5) {
                      this.av1Metadata_ =
                          data.subarray(parsePos + 2, parsePos + 5);
                    }
                  }
                  break;
  ...
  getVideoData(naluProcessing = true) {
    if (this.videoDataPesIndices_.length && !this.videoPes_.length) {
      ...
      // AV1 has no NAL units: its access units are carried as start code
      // delimited OBUs, which only the transmuxer needs to look inside.
      if (naluProcessing && this.videoCodec_ != 'av1') {
        const lastInfo = {
          nalu: null,
          state: null,
        };
  ...
  getVideoInfo() {
    ...
    if (this.videoCodec_ == 'av1') {
      return this.getAv1Info_();
    }
  ...
  /**
   * Return the video information for AV1
   *
   * The dimensions live in the sequence header, inside the elementary stream,
   * and reading them means parsing OBUs, which is the transmuxer's job.  What
   * is available here is the AV1 video descriptor, which the spec requires to
   * signal the profile, tier and highest level of the whole stream.
   *
   * @return {{
   *   height: ?string,
   *   width: ?string,
  ...
  getAv1Info_() {
    const videoInfo = {
      ...
      width: null,
      codec: null,
      frameRate: this.getFrameRate_(),
    };
    if (!this.av1Metadata_) {
      return videoInfo;
    }
    // The av1C parser reads a fourth byte, which only carries the initial
    // presentation delay; that is not part of the codec string.
    const record = new Uint8Array(4);
    record.set(this.av1Metadata_);
    const reader = new shaka.util.DataViewReader(
        record, shaka.util.DataViewReader.Endianness.BIG_ENDIAN);
    videoInfo.codec = shaka.util.Mp4BoxParsers.parseAV1C('av01', reader).codec;
    return videoInfo;
  }
