...
shaka.util.TsParser = class {
      ...
      if (data[start] == 0x47) {
        ...
        switch (pid) {
          ...
          case this.videoPid_: {
            ...
            const pes = this.parsePES_(videoData, /* isVideo= */ true);
            if (pes && pes.pts != null) {
  ...
  parseAvcNalus(pes, nextPes) {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      if (!value) {
        numZeros++;
      } else if (numZeros >= 2 && tryToFinishLastNalu) {
        infoOfLastNalu.data = shaka.util.Uint8ArrayUtils.concat(
            infoOfLastNalu.data, data.subarray(0, i - 2));
        infoOfLastNalu.fullData = shaka.util.Uint8ArrayUtils.concat(
            infoOfLastNalu.fullData, data.subarray(0, i - 2));
        break;
  ...
  getAudioData() {
    const Uint8ArrayUtils = shaka.util.Uint8ArrayUtils;
    const audio = [];
    let prevAudioData = new Uint8Array(0);
    // parsePES_() only works if the data begins on a PES boundary.
    // Try the last data blob first, and if it doesn't begin on a
    // PES boundary, prepend the previous blob and try again.
    // This way, a successful parse will always begin and end on
    // the correct boundary, and no data will be skipped.
    for (let i = this.audioData_.length - 1; i >= 0; i--) {
      const data = this.audioData_[i];
      goog.asserts.assert(data, 'We should have a data');
      const audioData = Uint8ArrayUtils.concat(data, prevAudioData);
      const pes = this.parsePES_(audioData);
      if (pes) {
        audio.unshift(pes);
        prevAudioData = new Uint8Array(0);
      } else {
