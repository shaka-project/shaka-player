...
shaka.util.TsParser = class {
        ...
        switch (pid) {
          ...
          case this.videoPid_: {
            ...
            if (this.videoStartTime_ == null) {
              ...
              if (pes && pes.pts != null) {
                // TODO: Ideally we should get the minimum of all PTS from this
                // segment, instead of the first PTS.  With B-frames, this
                // isn't the same thing.  As a cheap fix, we can take the
                // minimum of the first PTS and first DTS.
                // Using the wrong start time creates a larger timestamp
                // offset, which can result in CEA captions or even the first
                // GOP being discarded.
                this.videoStartTime_ = Math.min(pes.dts, pes.pts) / timescale;
              }
            }
  ...
  parseAvcNaLus(pes) {
    ...
    // We will use the state variable to find the correct NALU start.
    let state = 0;
    ...
    while (i < len) {
      ...
      if (!value) {
        ...
        if (lastNaluStart >= 0) {
          ...
          const nalu = {
            data: data.subarray(lastNaluStart + 1, i - state - 1),
            type: lastNaluType,
            time: time,
          };
          nalus.push(nalu);
    ...
    if (lastNaluStart >= 0 && state >= 0) {
      /** @type {shaka.util.TsParser.AvcNalu} */
      const nalu = {
        data: data.subarray(lastNaluStart + 1, len),
        type: lastNaluType,
        time: time,
      };
      nalus.push(nalu);
