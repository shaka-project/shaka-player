...
shaka.util.TsParser = class {
  ...
  parsePES_(data) {
    ...
    if (ptsDtsFlags & 0xC0) {
      ...
      pes.pts = this.handleRollover(pts, this.referencePts_);
      ...
      if (ptsDtsFlags & 0x40) {
        ...
        if (pes.pts != pts) {
          pes.dts = this.handleRollover(dts, this.referenceDts_);
        } else {
          pes.dts = dts;
        }
      }
  ...
  getFrameRate_() {
    ...
    if (videoData.length > 1) {
      ...
      if (!isNaN(secondPts - firstPts)) {
        return String(Math.abs(1 / (secondPts - firstPts) * timescale));
      }
    }
    return null;
  }
   ...
   * @return {number}
   */
  handleRollover(value, reference) {
