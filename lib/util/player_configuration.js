...
shaka.util.PlayerConfiguration = class {
  ...
  static addDeprecatedConfigFields(config) {
    ...
    config.language = (audio0 && audio0.language) || '';
    config.role = (audio0 && audio0.role) || '';
    config.channelCount = (audio0 && audio0.channelCount) || 0;
    config.spatialAudio = (audio0 && audio0.spatialAudio) || false;
    config.audioLabel = (audio0 && audio0.label) || '';

    config.videoRole = (video0 && video0.role) || '';
    config.hdrLevel = (video0 && video0.hdrLevel) || '';
    config.videoLayout = (video0 && video0.layout) || '';
    config.videoLabel = (video0 && video0.label) || '';

    config.preferredAudioCodecs = config.preferredAudio
        .map((p) => p.codec)
        .filter((c) => !!c);
    config.preferredAudioChannelCount =
        (audio0 && audio0.channelCount) || 0;
  }
