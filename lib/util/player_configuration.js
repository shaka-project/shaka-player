...
shaka.util.PlayerConfiguration = class {
  ...
  /**
   * Default factory for the MSF transport. Returns a stock WebTransport so
   * existing setups behave identically.
   *
   * Override `msf.transportFactory` with any
   * `shaka.extern.MsfTransportFactory` implementation that returns an object
   * implementing the WebTransport subset MSF actually uses (see the
   * `MsfManifestConfiguration.transportFactory` jsdoc for the full contract).
   * Custom factories bypass MSF's `window.WebTransport` guard, so this is
   * the integration point for non-WebTransport transports (Direct Sockets,
   * AMT tunnels, in-process test fakes).
   *
   * @param {string} uri
   * @param {!WebTransportOptions} options
   * @return {!WebTransport}
   */
  static defaultTransportFactory(uri, options) {
    return new WebTransport(uri, options);
  }
};
