...
shaka.util.PeriodCombiner = class {
  constructor() {
    ...
    this.streamCollections_ = new Map()
        .set(ContentType.AUDIO, this.audioStreams_)
        .set(ContentType.VIDEO, this.videoStreams_)
        .set(ContentType.TEXT, this.textStreams_)
        .set(ContentType.IMAGE, this.imageStreams_);
  }
  ...
  deleteStream(stream, periodId) {
    ...
    const streams = this.streamCollections_.get(stream.type) || [];
    for (const parent of streams) {
      shaka.util.PeriodCombiner.removeFromMatchedStreams_(parent, stream);
      const optionalStreams =
          shaka.util.PeriodCombiner.getOptionalStreams(parent);
      for (const optional of optionalStreams) {
        shaka.util.PeriodCombiner.removeFromMatchedStreams_(optional, stream);
      }
    }
    if (stream.segmentIndex) {
  ...
  /**
   * Removes a stream from the matchedStreams array, if present.
   *
   * @param {!shaka.extern.Stream} target
   * @param {!shaka.extern.Stream} stream
   * @private
   */
  static removeFromMatchedStreams_(target, stream) {
    target.matchedStreams = target.matchedStreams?.filter((s) => s !== stream);
  }

  /**
   * Builds a Map from an array of streams, keyed by the given key function.
   *
   ...
   * @private
   */
  static toStreamMap_(streams, keyFn) {
    return new Map(streams.map((s) => [keyFn(s), s]));
  }
  ...
  getStreamsPerPeriod_(periods, addDummy) {
    ...
    for (const period of periods) {
      const audioMap = PeriodCombiner.toStreamMap_(
          period.audioStreams, PeriodCombiner.generateAudioKey_);
      const videoMap = PeriodCombiner.toStreamMap_(
          period.videoStreams, PeriodCombiner.generateVideoKey_);
      const textMap = PeriodCombiner.toStreamMap_(
          period.textStreams, PeriodCombiner.generateTextKey_);
      const imageMap = PeriodCombiner.toStreamMap_(
          period.imageStreams, PeriodCombiner.generateImageKey_);
  ...
  async combinePeriods(periods, isDynamic, isPatchUpdate = false) {
    ...
    if (!isDynamic && periods.length == 1) {
      ...
      if (firstNewPeriodIndex == -1) {
        await this.extendRecreatedStreams_(periods);
        return;
      }
  ...
  /**
   * Extend output streams when a previously used Period ID reappears with new
   * Stream objects.
   *
   * @param {!Array<shaka.extern.Period>} periods
   * @return {!Promise}
   * @private
   */
  async extendRecreatedStreams_(periods) {
    const streamsByType = this.getStreamsPerPeriod_(
        periods, /* addDummy= */ false);
    const updates = [
      {
        outputStreams: this.audioStreams_,
        streamsPerPeriod: streamsByType.audioStreamsPerPeriod,
      },
      {
        outputStreams: this.videoStreams_,
        streamsPerPeriod: streamsByType.videoStreamsPerPeriod,
      },
      {
        outputStreams: this.textStreams_,
        streamsPerPeriod: streamsByType.textStreamsPerPeriod,
      },
      {
        outputStreams: this.imageStreams_,
        streamsPerPeriod: streamsByType.imageStreamsPerPeriod,
      },
    ];

    for (const {outputStreams, streamsPerPeriod} of updates) {
      for (const output of outputStreams) {
        const matches = output.matchedStreams;
        if (!matches) {
          continue;
        }
        for (const streams of streamsPerPeriod) {
          const match = this.findBestMatchInPeriod_(streams, output);
          if (!match || matches.includes(match)) {
            continue;
          }
          const firstNewStream = matches.length;
          matches.push(match);
          try {
            if (output.segmentIndex) {
              // eslint-disable-next-line no-await-in-loop
              await shaka.util.PeriodCombiner.extendOutputSegmentIndex_(
                  output, firstNewStream);
            }
          } catch (error) {
            matches.pop();
            throw error;
          }
          shaka.util.PeriodCombiner.concatenateStreams_(output, match);
        }
      }
    }
  }
  ...
  static async combineDbStreams(streamDbsPerPeriod) {
    ...
    const [
      combinedAudioStreamDbs,
      combinedVideoStreamDbs,
      combinedTextStreamDbs,
      combinedImageStreamDbs,
    ] = await Promise.all([
      audioStreamDbsPerPeriod,
      videoStreamDbsPerPeriod,
      textStreamDbsPerPeriod,
      imageStreamDbsPerPeriod,
    ].map((perPeriod) => periodCombiner.combine_(
        /* outputStreams= */ [],
        perPeriod,
  ...
  static async extendOutputSegmentIndex_(outputStream, firstNewPeriodIndex) {
    ...
    for (let i = firstNewPeriodIndex; i < streams.length; i++) {
      ...
      const optionalStreams =
          shaka.util.PeriodCombiner.getOptionalStreams(stream);
      for (const optional of optionalStreams) {
        if (!optional.segmentIndex) {
          operations.push(optional.createSegmentIndex());
        }
      }
    }
  ...
  static extendOutputStream_(
    ...
    for (let i = start; i < matches.length; i++) {
      ...
      if (used) {
        const unusedStreams = unusedStreamsPerPeriod[i - periodsMissing];
        if (unusedStreams) {
          unusedStreams.delete(match);
        }
  ...
  /**
   * Combines two arrays, keeping only unique elements.
   *
   * @param {Array} output
   * @param {!Array} input
   * @return {!Array}
   * @private
   */
  static combineArrays_(output, input) {
    if (!output) {
      output = [];
    }
  ...
  static concatenateStreams_(output, input) {
    ...
    output.roles = shaka.util.PeriodCombiner.combineArrays_(
        output.roles, input.roles);

    if (input.emsgSchemeIdUris) {
      output.emsgSchemeIdUris = shaka.util.PeriodCombiner.combineArrays_(
    ...
    for (const type of shaka.util.PeriodCombiner.OptionalStreamTypes_) {
      shaka.util.PeriodCombiner.combineOptionalStream_(output, input, type);
    }
  }

  /**
   * Combines an optional stream type (e.g. trickModeVideo or dependencyStream)
   * across Periods into a single output stream.
   *
   * @param {shaka.extern.Stream} output
   * @param {shaka.extern.Stream} input
   * @param {string} type
   * @private
   */
  static combineOptionalStream_(output, input, type) {
    /** @type {?shaka.extern.Stream} */
    const inputStream = input[type] || null;

    /** @type {?shaka.extern.Stream} */
    let outputStream = output[type] || null;

    if (inputStream) {
      if (!outputStream) {
        // Create a new output stream based on the input stream
        outputStream = shaka.util.PeriodCombiner.cloneStream_(inputStream);
        outputStream.matchedStreams = [];

        // Lazily build a combined segment index from all matched streams
        outputStream.createSegmentIndex = async () => {
          if (outputStream.segmentIndex) {
            return;
          }
          ...
          goog.asserts.assert(outputStream.matchedStreams,
              `${type} matched streams should exist`);
          for (const stream of outputStream.matchedStreams) {
            if (!stream.segmentIndex) {
              // eslint-disable-next-line no-await-in-loop
              await stream.createSegmentIndex();
            }
            goog.asserts.assert(stream.segmentIndex,
                `${type} segment index should exist`);
            segmentIndex.appendSegmentIndex(stream.segmentIndex);
          }
          outputStream.segmentIndex = segmentIndex;
        };

        output[type] = outputStream;
      }

      // Append the input stream into the output stream
      outputStream.matchedStreams.push(inputStream);
      shaka.util.PeriodCombiner.concatenateStreams_(
          outputStream, inputStream);
    } else if (outputStream) {
      // No input stream of this type in the current Period.
      // Fallback to the main input stream to keep continuity.
      outputStream.matchedStreams.push(input);
      shaka.util.PeriodCombiner.concatenateStreams_(
          outputStream, input);
    }
  }
  ...
  static areTextStreamsCompatible_(outputStream, candidate) {
    ...
    // We will strictly avoid combining text across languages.  A differing
    // "kind" (caption vs subtitle) is allowed, since content can label the
    // same logical track either way from one Period to the next.  A matching
    // kind is preferred in isTextStreamBetterMatch_.
  ...
  static isTextStreamBetterMatch_(outputStream, best, candidate) {
    ...
    // Prefer a matching "kind" (caption vs subtitle).  Combining across kinds
    // is a last resort, when the period offers nothing of the same kind.
    const bestKindMatches = best.kind == outputStream.kind;
    const candidateKindMatches = candidate.kind == outputStream.kind;
    if (candidateKindMatches && !bestKindMatches) {
      return true;
    }
    if (!candidateKindMatches && bestKindMatches) {
      return false;
  ...
  static generateVideoKey_(v) {
    return shaka.util.PeriodCombiner.generateKey_([
      ...
      v.bandwidth,
      v.dependencyStream?.baseOriginalId ?? v.baseOriginalId,
      Array.from(v.keyIds),
    ]);
  }
  ...
  static generateAudioKey_(a) {
    return shaka.util.PeriodCombiner.generateKey_([
      ...
      a.audioSamplingRate,
      a.primary,
      a.dependencyStream?.baseOriginalId ?? a.baseOriginalId,
      Array.from(a.keyIds),
    ]);
  }
   ...
   * @return {!Array<shaka.extern.Stream>}
   */
  static getOptionalStreams(stream) {
    const optionalStreams = [];
    for (const type of shaka.util.PeriodCombiner.OptionalStreamTypes_) {
      const optional = stream[type];
      if (optional) {
        optionalStreams.push(optional);
      }
    }
 ...
 */
shaka.util.PeriodCombiner.OptionalStreamTypes_ = [
  'trickModeVideo',
  'dependencyStream',
];
