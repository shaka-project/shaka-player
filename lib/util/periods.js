  ...
  */
    ...
    for (const period of periods) {
      ...
      period.videoStreams = PeriodCombiner.filterOutStreamDuplicates_(
          period.videoStreams, (v1, v2) => {
            return v1.id !== v2.id &&
              v1.fastSwitching == v2.fastSwitching &&
              v1.width === v2.width &&
      ...
      period.audioStreams = PeriodCombiner.filterOutStreamDuplicates_(
          period.audioStreams, (a1, a2) => {
            return a1.id !== a2.id &&
              a1.fastSwitching == a2.fastSwitching &&
    ...
    if (outputStream.segmentIndex instanceof shaka.media.MetaSegmentIndex) {
      for (let i = firstNewPeriodIndex; i < streams.length; i++) {
        ...
        if (match.segmentIndex) {
          match.segmentIndex.forEachTopLevelReference((ref) => {
            ref.codecs = match.codecs;
            if (ref.initSegmentReference) {
              ref.initSegmentReference.codecs = match.codecs;
            }
          });
  ...
  static areAVStreamsCompatible_(outputStream, candidate) {
    ...
    if (!shaka.util.PeriodCombiner.areAVStreamsExactMatch_(
      ...
      // It's not an exact match. See if we can do multi-codec stream instead,
      // using SourceBuffer.changeType.
      // TODO: add a config value for this (might require making it non-static?)
      // TODO: also require CodecSwitchingStrategy.SMOOTH?
      if (!shaka.media.Capabilities.isChangeTypeSupported()) {
        return false;
      }
      // The mimeType still needs to the be same.
      if (outputStream.mimeType != candidate.mimeType) {
        // TODO: Is this necessary? Can we changeType between mimeTypes?
        return false;
      }
    }
