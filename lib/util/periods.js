  ...
  */
  getStreamsPerPeriod_(periods) {
    ...
    for (const period of periods) {
      ...
      // streams already.
      period.textStreams.push(shaka.util.PeriodCombiner.dummyStream_(
          ContentType.TEXT));
      period.imageStreams.push(shaka.util.PeriodCombiner.dummyStream_(
          ContentType.IMAGE));
    }
    return {
  ...
  async combine_(
    ...
    for (let i = 0; i < firstNewPeriodIndex; i++) {
      // This period's streams have all been used already.
      unusedStreamsPerPeriod.push(new Set());
    }
    for (let i = firstNewPeriodIndex; i < streamsPerPeriod.length; i++) {
      ...
      for (let i = firstNewPeriodIndex; i < streams.length; i++) {
        ...
        if (match.segmentIndex) {
          match.segmentIndex.forEachTopLevelReference((ref) => {
            ...
            ref.mimeType = match.mimeType;
            if (ref.initSegmentReference) {
              ref.initSegmentReference.codecs = match.codecs;
              ref.initSegmentReference.mimeType = match.mimeType;
            }
          });
