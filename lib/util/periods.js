...
shaka.util.PeriodCombiner = class {
  ...
  deleteStream(stream, periodId) {
    ...
    if (stream.type == ContentType.AUDIO) {
      ...
      for (const videoStream of this.videoStreams_) {
        ...
        if (videoStream.dependencyVideo) {
          videoStream.dependencyVideo.matchedStreams =
            videoStream.dependencyVideo.matchedStreams.filter((s) => {
              return s !== stream;
            });
        }
      }
  ...
  async combinePeriods(periods, isDynamic, isPatchUpdate = false) {
    ...
    if (!this.videoStreams_.length || !this.audioStreams_.length) {
      ...
      for (const stream of streams) {
        ...
        if (stream.dependencyVideo) {
          bandwidth += stream.dependencyVideo.bandwidth || 0;
        }
        variants.push({
          id,
      ...
      for (const audio of this.audioStreams_) {
        for (const video of this.videoStreams_) {
          ...
          if (video.dependencyVideo) {
            bandwidth += video.dependencyVideo.bandwidth || 0;
          }
  ...
  static async extendOutputSegmentIndex_(outputStream, firstNewPeriodIndex) {
    ...
    for (let i = firstNewPeriodIndex; i < streams.length; i++) {
      ...
      if (stream.dependencyVideo && !stream.dependencyVideo.segmentIndex) {
        operations.push(stream.dependencyVideo.createSegmentIndex());
      }
    }
  ...
  static concatenateStreams_(output, input) {
    ...
    // Combine dependency video streams, if present.
    if (input.dependencyVideo) {
      if (!output.dependencyVideo) {
        // Create a fresh output stream for trick-mode playback.
        output.dependencyVideo = shaka.util.PeriodCombiner.cloneStream_(
            input.dependencyVideo);
        output.dependencyVideo.matchedStreams = [];
        output.dependencyVideo.createSegmentIndex = () => {
          if (output.dependencyVideo.segmentIndex) {
            return Promise.resolve();
          }
          const segmentIndex = new shaka.media.MetaSegmentIndex();
          goog.asserts.assert(output.dependencyVideo.matchedStreams,
              'dependency video matched streams should exist');
          for (const stream of output.dependencyVideo.matchedStreams) {
            goog.asserts.assert(stream.segmentIndex,
                'dependency video segment index should exist');
            segmentIndex.appendSegmentIndex(stream.segmentIndex);
          }
          output.dependencyVideo.segmentIndex = segmentIndex;

          return Promise.resolve();
        };
      }

      // Concatenate the dependency video input onto the dependency video
      // output.
      output.dependencyVideo.matchedStreams.push(input.dependencyVideo);
      shaka.util.PeriodCombiner.concatenateStreams_(
          output.dependencyVideo, input.dependencyVideo);
    } else if (output.dependencyVideo) {
      // We have a dependency video output, but no input from this Period.
      // Fill it in from the standard input Stream.
      output.dependencyVideo.matchedStreams.push(input);
      shaka.util.PeriodCombiner.concatenateStreams_(
          output.dependencyVideo, input);
    }
  }
  ...
  static generateVideoKey_(v) {
    return shaka.util.PeriodCombiner.generateKey_([
      ...
      v.bandwidth,
      v.dependencyVideo ? v.dependencyVideo.baseOriginalId : null,
      Array.from(v.keyIds),
    ]);
  }
