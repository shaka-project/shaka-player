...
shaka.util.Mp4Generator = class {
  ...
  pasp_(streamInfo) {
    ...
    this.writeUint32_(bytes, hSpacing, 0);
    this.writeUint32_(bytes, vSpacing, 4);
  ...
  frma_(codecs) {
    ...
    this.writeUint32_(bytes, this.stringToCharCode_(codec), 0);
  ...
  mfhd_(streamInfo) {
    ...
    // bytes[0] = version 0
    // bytes[1-3] = flags 0
    this.writeUint32_(bytes, sequenceNumber, 4);
  ...
  tfdt_(streamInfo) {
    ...
    // bytes[1-3] = flags 0
    this.writeUint32_(bytes, upperWordBaseMediaDecodeTime, 4);
    this.writeUint32_(bytes, lowerWordBaseMediaDecodeTime, 8);
  ...
  trun_(streamInfo, offset) {
    ...
    // Header del TRUN
    // version
    bytes[0] = isVideo ? 0x01 : 0x00;
    // flags
    bytes[1] = 0x00;
    bytes[2] = 0x0f;
    bytes[3] = 0x01;
    // sample_count
    this.writeUint32_(bytes, samplesLength, 4);
    // data_offset
    this.writeUint32_(bytes, offset, 8);
    ...
    for (let i = 0; i < samplesLength; i++) {
      ...
      const base = 12 + 16 * i;
      // sample_duration
      this.writeUint32_(bytes, sample.duration, base);
      // sample_size
      this.writeUint32_(bytes, sample.size, base + 4);
      // sample_flags
      bytes[base + 8] = (flags.isLeading << 2) | flags.dependsOn;
      bytes[base + 9] = (flags.isDependedOn << 6) |
          (flags.hasRedundancy << 4) | flags.isNonSync;
      bytes[base + 10] = flags.degradPrio & (0xf0 << 8);
      bytes[base + 11] = flags.degradPrio & 0x0f;
      // sample_composition_time_offset
      this.writeUint32_(bytes, sample.cts, base + 12);
    }
