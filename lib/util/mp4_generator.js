...
shaka.util.Mp4Generator = class {
  ...
  mvhd_() {
    ...
    const bytes = new Uint8Array([
      ...
      (this.timescale_ >> 24) & 0xff,
      (this.timescale_ >> 16) & 0xff,
      (this.timescale_ >> 8) & 0xff,
      (this.timescale_) & 0xff, // timescale
      ...
      (lowerWordDuration) & 0xff, // duration
  ...
  mdhd_() {
    ...
    const bytes = new Uint8Array([
      ...
      (lowerWordDuration >> 8) & 0xff,
      (lowerWordDuration) & 0xff, // duration
      (languageNumber >> 8) & 0xff,
      (languageNumber) & 0xff, // language
      0x00, 0x00,
    ]);
  ...
  mp3_() {
    ...
    const bytes = new Uint8Array([
      ...
      (audioSamplingRate >> 8) & 0xff,
      (audioSamplingRate) & 0xff, // Sample Rate
      0x00, 0x00,
    ]);
    return Mp4Generator.box('.mp3', bytes);
  }
  ...
  mp4a_() {
    ...
    return Mp4Generator.box(boxName, bytes, esdsBox, sinfBox);
  }
  ...
  esds_() {
    ...
    return new Uint8Array([
      ...
      (bandwidth & 0xFF000000) >> 24,
      (bandwidth & 0x00FF0000) >> 16,
      (bandwidth & 0x0000FF00) >> 8,
      (bandwidth & 0x000000FF), // maxBitrate
      (bandwidth & 0xFF000000) >> 24,
      (bandwidth & 0x00FF0000) >> 16,
      (bandwidth & 0x0000FF00) >> 8,
      (bandwidth & 0x000000FF), // avgBitrate
      // DecoderSpecificInfo
  ...
  static box(boxName, ...payload) {
    ...
    const result = new Uint8Array(pointer);
    result[0] = (pointer >> 24) & 0xff;
    result[1] = (pointer >> 16) & 0xff;
    result[2] = (pointer >> 8) & 0xff;
    result[3] = pointer & 0xff;
    result.set(type, 4);
    // copy the payload into the result
    for (i = 0, pointer = 8; i < len; i++) {
