...
shaka.util.LanguageUtils = class {
  ...
  static normalize(lang) {
    ...
    if (splitAt >= 0) {
      ...
      minor = lang.substring(splitAt + 1);
    } else {
      major = lang;
      minor = '';
    }
    ...
    // Convert the minor code to upper case. It is standard for the minor code
    // to be in upper case. If there is no upper code, then it will be an empty
    // string and this will be a no-op.
