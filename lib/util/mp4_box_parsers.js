...
shaka.util.Mp4BoxParsers = class {
  ...
  static parseIACB(reader) {
    ...
    const rest = reader.readBytes(
        reader.getLength() - reader.getPosition(),
        // Don't clone.  The payload is temporary, and is parsed immediately.
        /* clone= */ false);
    const configOBUsSize = Obu.readLeb128(rest, 0);
    if (!configOBUsSize) {
      return {codec: null};
    }
    ...
    const obus = Obu.parseIamf(rest.subarray(
        configOBUsSize.size, configOBUsSize.size + configOBUsSize.value));
    ...
    for (const obu of obus) {
      switch (obu.type) {
        case 31: // OBU_IA_Sequence_Header
          if (obu.data.byteLength >= 6) {
            // The first four bytes are "ia_code", then the two profiles.
            profiles = Mp4BoxParsers.addLeadingZeros_(obu.data[4], 3) + '.' +
                Mp4BoxParsers.addLeadingZeros_(obu.data[5], 3);
          }
          break;
        case 0: { // OBU_IA_Codec_Config
          const codecConfigId = Obu.readLeb128(obu.data, 0);
          if (codecConfigId &&
              obu.data.byteLength >= codecConfigId.size + 4) {
            const i = codecConfigId.size;
            codecId = String.fromCharCode(obu.data[i], obu.data[i + 1],
                obu.data[i + 2], obu.data[i + 3]);
          }
          break;
        }
      }
