...
shaka.util.Mp4BoxParsers = class {
   ...
   */
  static parseAVCC(codecBase, reader) {
   ...
   */
  static parseHVCC(codecBase, reader) {
    const Mp4BoxParsers = shaka.util.Mp4BoxParsers;
    reader.skip(1); // Skip "configurationVersion"
   ...
   */
  static parseDVCC(codecBase, reader) {
    const Mp4BoxParsers = shaka.util.Mp4BoxParsers;
   ...
   */
  static parseDVVC(codecBase, reader) {
    const Mp4BoxParsers = shaka.util.Mp4BoxParsers;
    reader.skip(2); // Skip "dv_version_major" and "dv_version_minor"
    const thirdByte = reader.readUint8();
    const fourthByte = reader.readUint8();
   ...
   */
  static parseVPCC(codecBase, reader) {
    const Mp4BoxParsers = shaka.util.Mp4BoxParsers;
   ...
   */
  static parseAV1C(codecBase, reader) {
    const Mp4BoxParsers = shaka.util.Mp4BoxParsers;
  ...
  /**
   * Parses an IACB (IA Configuration) box, which carries the IAMF
   * Descriptors needed to build the codecs parameter string.
   *
   * @param {!shaka.util.DataViewReader} reader
   * @return {!shaka.util.ParsedIACBBox}
   */
  static parseIACB(reader) {
    const Mp4BoxParsers = shaka.util.Mp4BoxParsers;
    // More info https://aomediacodec.github.io/iamf/#iaconfigurationbox
    const configurationVersion = reader.readUint8();
    if (configurationVersion != 1) {
      // Parsers compliant with this version of the spec ignore the box.
      return {codec: null};
    }
    const configOBUsSize = Mp4BoxParsers.readLeb128_(reader);
    const end = Math.min(reader.getPosition() + configOBUsSize,
        reader.getLength());
    ...
    // configOBUs holds the Descriptors: an IA Sequence Header OBU, a Codec
    // Config OBU, and then the Audio Element and Mix Presentation OBUs, which
    // the codecs parameter string doesn't need.
    while (reader.getPosition() < end && (!profiles || !codecId)) {
      const header = reader.readUint8();
      const obuType = header >> 3;
      const trimmingStatusFlag = (header & 0x02) != 0;
      const extensionFlag = (header & 0x01) != 0;
      // obu_size covers the optional header fields below, too.
      const obuSize = Mp4BoxParsers.readLeb128_(reader);
      const obuEnd = reader.getPosition() + obuSize;
      if (obuEnd > end) {
        break;
      }
      if (trimmingStatusFlag) {
        Mp4BoxParsers.readLeb128_(reader); // num_samples_to_trim_at_end
        Mp4BoxParsers.readLeb128_(reader); // num_samples_to_trim_at_start
      }
      if (extensionFlag) {
        const extensionHeaderSize = Mp4BoxParsers.readLeb128_(reader);
        if (reader.getPosition() + extensionHeaderSize > obuEnd) {
          break;
        }
        reader.skip(extensionHeaderSize);
      }
      switch (obuType) {
        case 31: { // OBU_IA_Sequence_Header
          if (obuEnd - reader.getPosition() < 6) {
            break;
          }
          reader.skip(4); // Skip "ia_code"
          const primaryProfile = reader.readUint8();
          const additionalProfile = reader.readUint8();
          profiles = Mp4BoxParsers.addLeadingZeros_(primaryProfile, 3) + '.' +
              Mp4BoxParsers.addLeadingZeros_(additionalProfile, 3);
          break;
        }
        case 0: { // OBU_IA_Codec_Config
          Mp4BoxParsers.readLeb128_(reader); // Skip "codec_config_id"
          if (obuEnd - reader.getPosition() < 4) {
            break;
          }
          codecId = shaka.util.Mp4Parser.typeToString(reader.readUint32());
          break;
        }
      }
      reader.seek(obuEnd);
    ...
    // The fourth element is the codecs parameter string that the coded audio
    // substreams would use in their own track.  IAMF only allows AAC-LC, so
    // mp4a always maps to mp4a.40.2.
    const subCodec = codecId == 'mp4a' ? 'mp4a.40.2' : codecId;
    return {codec: 'iamf.' + profiles + '.' + subCodec};
  }
  ...
  static parseHDLR(reader) {
    reader.skip(4); // Skip "pre_defined"

    // Read "handler_type" (4 bytes, e.g. 'soun', 'vide').
    // Use readBytes instead of readTerminatedString for Apple QuickTime.
    const data = reader.readBytes(4, /* clone= */ false);
    const handlerType = String.fromCharCode(data[0], data[1], data[2], data[3]);
    return {handlerType};
  }
   ...
   * Parses a SCHM box.
   * @param {!shaka.util.DataViewReader} reader
   * @return {!shaka.util.ParsedSCHMBox}
   */
  static parseSCHM(reader) {
    const Mp4Parser = shaka.util.Mp4Parser;
    const encryptionScheme = Mp4Parser.typeToString(reader.readUint32());
  ...
  static addLeadingZero_(x) {
    return shaka.util.Mp4BoxParsers.addLeadingZeros_(x, 2);
  }
   ...
   * @private
   */
  static addLeadingZeros_(x, digits) {
    let result = String(x);
  ...
  /**
   * Reads an unsigned leb128() as defined by the IAMF and AV1 specs.  Values
   * are encoded 7 bits at a time, low-order group first, with the high bit of
   * each byte set while more bytes follow.
   *
   * @param {!shaka.util.DataViewReader} reader
   * @return {number}
   * @private
   */
  static readLeb128_(reader) {
    let value = 0;
    // A leb128() is at most 8 bytes long.
    for (let i = 0; i < 8 && reader.hasMoreData(); i++) {
      const byte = reader.readUint8();
      // Use multiplication rather than a shift, since the value can exceed
      // what a 32-bit shift can represent.
 ...
 *
 * @property {?string} codec
 *   The IAMF codecs parameter string, or null if the box could not be
 *   understood.
 *
 * @exportDoc
 */
shaka.util.ParsedIACBBox;
