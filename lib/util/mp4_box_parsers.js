...
shaka.util.Mp4BoxParsers = class {
  ...
  static parseSENC(reader, flags, perSampleIVSize, defaultConstantIV) {
    ...
    for (let i = 0; i < sampleCount; i++) {
      ...
      if (ivSize > 0) {
        const ivRaw = reader.readBytes(ivSize, /* clone= */ true);
        iv = new Uint8Array(16);
        iv.set(ivRaw.subarray(0, Math.min(ivRaw.length, 16)), 0);
      } else {
        if (!constantIV) {
          throw new Error(
        ...
        iv = new Uint8Array(16);
        iv.set(constantIV.subarray(0, Math.min(constantIV.length, 16)), 0);
      }
      let subsamples = null;
      if (hasSubsamples) {
