 ...
 *
 * @param {?BufferSource} data
 * @param {boolean} littleEndian true to read in little-endian format, false to
 ...
 *
 * @param {string} str
 * @param {boolean} littleEndian true to write in little-endian format, false to
 *     write in big-endian format.
 * @return {!ArrayBuffer}
 * @export
 */
shaka.util.StringUtils.toUTF16 = function(str, littleEndian) {
  var result = new ArrayBuffer(str.length * 2);
  var dataView = new DataView(result);
  for (var i = 0; i < str.length; ++i) {
    dataView.setUint16(i * 2, str.charCodeAt(i), littleEndian);
  }
  return result;
};
