...
/**
 * The nested container box types on the path from the Movie Box down to a
 * track's Sample Table: 'moov' > 'trak' > 'mdia' > 'minf' > 'stbl'.  These are
 * all simple container boxes and are frequently declared together.
 *
 * @const {!Array<string>}
 */
shaka.util.Mp4Parser.SAMPLE_TABLE_PATH =
    ['moov', 'trak', 'mdia', 'minf', 'stbl'];


/**
 * The nested container box types of a movie fragment: 'moof' > 'traf'.
 *
 * @const {!Array<string>}
 */
shaka.util.Mp4Parser.FRAGMENT_PATH = ['moof', 'traf'];


/**
 * The AVC (H.264) video sample entry box types, together with their Dolby
 * Vision variants ('dvav'/'dva1').
 *
 * @const {!Array<string>}
 * @private
 */
shaka.util.Mp4Parser.AVC_DV_ = ['avc1', 'avc3', 'dvav', 'dva1'];


/**
 * The HEVC (H.265) video sample entry box types, together with their Dolby
 * Vision variants ('dvh1'/'dvhe').
 *
 * @const {!Array<string>}
 * @private
 */
shaka.util.Mp4Parser.HEVC_DV_ = ['hev1', 'hvc1', 'dvh1', 'dvhe'];


/**
 * The VVC (H.266) video sample entry box types, together with their Dolby
 * Vision variants ('dvc1'/'dvi1').
 *
 * @const {!Array<string>}
 * @private
 */
shaka.util.Mp4Parser.VVC_DV_ = ['vvc1', 'vvi1', 'dvc1', 'dvi1'];


/**
 * The AV1 video sample entry box types, together with the Dolby Vision variant
 * ('dav1'), which is also an AV1 bitstream.
 *
 * @const {!Array<string>}
 */
shaka.util.Mp4Parser.AV1_DV = ['av01', 'dav1'];


/**
 * The AVC and HEVC video sample entry box types and their Dolby Vision
 * variants.
 *
 * @const {!Array<string>}
 */
shaka.util.Mp4Parser.AVC_HEVC_DV =
    [...shaka.util.Mp4Parser.AVC_DV_, ...shaka.util.Mp4Parser.HEVC_DV_];


/**
 * The AVC, HEVC and VVC video sample entry box types and their Dolby Vision
 * variants.
 *
 * @const {!Array<string>}
 */
shaka.util.Mp4Parser.AVC_HEVC_VVC_DV =
    [...shaka.util.Mp4Parser.AVC_HEVC_DV, ...shaka.util.Mp4Parser.VVC_DV_];


/**
 * The audio sample entry box types that are parsed with a generic handler,
 * i.e. excluding 'mp4a' (AAC), which is parsed separately: Dolby ('ac-3',
 * 'ec-3', 'ac-4'), Opus, FLAC, APAC and IAMF.
 *
 * @const {!Array<string>}
 */
shaka.util.Mp4Parser.GENERIC_AUDIO_SAMPLE_ENTRIES =
    ['ac-3', 'ec-3', 'ac-4', 'Opus', 'fLaC', 'apac', 'iamf'];
