...
shaka.util.Mp4Parser = class {
  ...
  printPsshKeyIds_(box) {
    ...
    shaka.log.v2(`[APPLE]: PSSH system ID: ` +
    ...
    if (box.version > 0) {
      ...
      for (let i = 0; i < numKeyIds; i++) {
        const keyId =
            shaka.util.Uint8ArrayUtils.toHex(box.reader.readBytes(16));
        shaka.log.v2(`[APPLE]: PSSH key ID: ${keyId}`);
      }
    }
  }
  ...
  parseNext(absStart, reader, partialOkay, stopOnPartial) {
    ...
    if (boxDefinition) {
      ...
      if (name === 'pssh') {
        this.printPsshKeyIds_(box);
      }

      boxDefinition(box);
    } else {
      shaka.log.v2(`[APPLE]: The box type: ${name} has no box definition; ` +
          `not parsing this.`);
