...
shaka.util.CmcdBatchingManager = class {
  /**
   * @param {shaka.util.CmcdBatchingManager.PlayerInterface} playerInterface
   */
  constructor(playerInterface) {
    /** @private {shaka.util.CmcdBatchingManager.PlayerInterface} */
    ...
    this.retryDelays_ = [1000, 5000, 1000, 3000, 5000]; // ms
  ...
  addReport(target, cmcdData) {
    ...
    if (target.batchTimer && !this.timers_.has(key)) {
      ...
      timer.tickAfter(target.batchTimer);
  ...
  async flushByTargetKey_(key) {
    ...
    request.headers['Content-Type'] = 'application/cmcd+json';

    try {
      const response = await this.sendBatchReport_(request);
      if (response && response.status === 410) {
        ...
        if (this.retryQueue_.length === 1) {
          ...
          timer.tickAfter(this.retryDelays_[0]);
    ...
    if (this.timers_.has(key)) {
      this.timers_.get(key).stop();
      this.timers_.delete(key);
    }
  }
  ...
  async processRetryQueue_() {
    ...
    this.retryQueue_ = this.retryQueue_.filter(
        (report) => report.sendTime > now);
 ...
 */
shaka.util.CmcdBatchingManager.PlayerInterface;
