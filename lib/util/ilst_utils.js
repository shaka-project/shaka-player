goog.provide('shaka.util.IlstUtils');
 ...
 * @export
 */
shaka.util.IlstUtils = class {
  /**
   * Parses ILST metadata frames from MP4 data.
   *
    ...
    new shaka.util.Mp4Parser()
        ...
        .box('ilst', (box) => {
          while (box.reader.hasMoreData()) {
            ...
            const frame = (type === '----') ?
                shaka.util.IlstUtils.parseFreeform_(payload) :
                shaka.util.IlstUtils.parseStandard_(type, payload);
  ...
  static parseFreeform_(data) {
    ...
    new shaka.util.Mp4Parser()
        ...
        .fullBox('data', (box) => {
          value = shaka.util.IlstUtils.parseDataBox_(box);
        })
        .parse(data);
   ...
   * @private
   */
  static parseStandard_(key, data) {
    const mappedKey = shaka.util.IlstUtils.ILST_TO_ID3_.get(key) || key;
    ...
    new shaka.util.Mp4Parser()
        .fullBox('data', (box) => {
          const parsed = shaka.util.IlstUtils.parseDataBox_(box, key);
 ...
 * @private
 */
shaka.util.IlstUtils.ILST_TO_ID3_ = new Map()
    .set('©nam', 'TIT2')
    .set('©ART', 'TPE1')
    .set('aART', 'TPE2')
    .set('©alb', 'TALB')
