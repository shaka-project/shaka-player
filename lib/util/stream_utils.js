...
shaka.util.StreamUtils = class {
  ...
  */
  ...
  static async filterManifestByMediaCapabilities(
    ...
    console.log(`[JULIAN]: ${manifest.variants.length} variants before ` +
        `MCap checking.`);
    manifest.variants = manifest.variants.filter((variant) => {
      ...
      if (video) {
        ...
        if (video.codecs.includes(',')) {
          ...
          if (!MediaSource.isTypeSupported(audioFullType)) {
            console.log(`[JULIAN]: ${audioFullType} not supported for muxed`);
            return false;
          }
        }
        ...
        if (!MediaSource.isTypeSupported(fullType)) {
          console.log(`[JULIAN]: ${fullType} not supported for video`);
          return false;
        }
      }
      ...
      if (audio) {
        ...
        if (!MediaSource.isTypeSupported(fullType)) {
          console.log(`[JULIAN]: ${fullType} not supported for audio`);
          return false;
        }
      }
    ...
    console.log(`[JULIAN]: ${manifest.variants.length} variants remaining ` +
        `after MCap checking.`);
  }
