...
shaka.util.StreamUtils = class {
    ...
    manifest.variants = manifest.variants.filter((variant) => {
      ...
      if (!supported.supported) {
        shaka.log.debug('Dropping variant - not compatible with platform',
            StreamUtils.getVariantSummaryString(variant), '; reasons=' + supported.reasons);
      }
   ...
   * @param {!Object<string, string>} keySystemsMapping
   * @return {{supported: boolean, reasons: string}}
   * @private
   */
  static checkVariantSupported_(variant, keySystem, keySystemsMapping) {
    console.log(`[APPLE]: checkVariantSupported_: ${shaka.util.StreamUtils.getVariantSummaryString(variant)}`);
    let unsupportedReasons = [];
    const variantSupported = variant.decodingInfos.some((decodingInfo) => {
      if (!decodingInfo.supported) {
        unsupportedReasons.push('decodingInfo.supported = false');
        console.log(`Unsupported! decodingInfo.supported = false`);
        return false;
      }
      if (keySystem) {
        ...
        if (keySystemAccess) {
          ...
          if (currentKeySystem != keySystem) {
            unsupportedReasons.push(`${currentKeySystem} != ${keySystem}`);
            console.log(`Unsupported! keySystem of ${currentKeySystem} != expected keySystem of ${keySystem}`);
            return false;
          }
        }
      }
    ...
    if (!variantSupported) {
      const reason = [...new Set(unsupportedReasons)].toString();
      console.log(`Unsupported!: ${reason}`);
      return {supported: false, reasons: 'decodingInfo: ' + reason};
    }
    ...
    if (isXboxOne && video && (videoWidth > 1920 || videoHeight > 1080) &&
      ...
      return {supported: false, reasons: 'xbox'};
    }
    ...
    if (videoDependencyStream &&
      ...
      console.log(`Unsupported! dependency stream video`);
      return {supported: false, reasons: 'dependency stream video'};
    }
    ...
    if (isFirefoxAndroid && audio && audio.encrypted &&
      ...
      return {supported: false, reasons: 'firefox encrypted opus'};
    }

    const audioDependencyStream = audio && audio.dependencyStream;
    if (audioDependencyStream) {
      console.log(`Unsupported! dependency stream audio`);
      return {supported: false, reasons: 'dependency stream audio'};
    }

    return {supported: true, reasons: 'none'};
  }
  ...
  static async getDecodingInfosForVariant_(variant, decodingConfigs) {
    ...
    const StreamUtils = shaka.util.StreamUtils;
    console.log(`[APPLE]: getDecodingInfosForVariant_ for variant: ${shaka.util.StreamUtils.getVariantSummaryString(variant)}`);
    ...
    for (const decodingConfig of decodingConfigs) {
      ...
      if (cache.has(cacheKey)) {
        shaka.log.v2('[APPLE, StreamUtils]: Using cached results of mediaCapabilities.decodingInfo',
            'for key', cacheKey, ';', 'variant=', shaka.util.StreamUtils.getVariantSummaryString(variant));
        ...
        console.log(`[APPLE, StreamUtils]: decodingInfo for: videoId=${variant.video.id}, language=${variant.audio.language};label=${variant.audio.label}: ${JSON.stringify(decodingConfig)}`);
    ...
    if (finalResult) {
      console.log(`[APPLE, StreamUtils]: decodingInfo result: ${JSON.stringify(finalResult)}`);
  ...
  static async getDecodingInfosForVariants(variants, usePersistentLicenses,
    ...
    console.log(`[APPLE, StreamUtils]: getDecodingInfosForVariants: preferredKeySystems: ${preferredKeySystems.toString()}`);
    let alreadySatisfiedVariants = [];
    for (const preferredKeySystem of preferredKeySystems) {
      let keySystemSatisfied = false;
      for (const variant of variants) {
        console.log(`[APPLE]: getDecodingConfigs_ for variant: ${shaka.util.StreamUtils.getVariantSummaryString(variant)}`);
        ...
        const decodingConfigs = shaka.util.StreamUtils.getDecodingConfigs_(
            ...
            .filter((configs) => {
              ...
              console.log(`keySystemConfigExists=${config.keySystemConfiguration && config.keySystemConfiguration.keySystem}, preferredKeySystem=${preferredKeySystem}`);
        ...
        if (decodingConfigs.length === 0) {
          console.log(`[APPLE]: FAILED: getDecodingConfigs_ for variant: ${shaka.util.StreamUtils.getVariantSummaryString(variant)}`);
        }
        if (variant.decodingInfos.some((d) => d.supported)) {
          alreadySatisfiedVariants.push(shaka.util.StreamUtils.getVariantSummaryString(variant));
          keySystemSatisfied = true;
        }
      } // for (const variant of variants)
      if (keySystemSatisfied) {
        console.log(`[APPLE, StreamUtils]: keySystemSatisfied already, no need to get decoding info for variants: ${alreadySatisfiedVariants.toString()}`);
    ...
    for (const variant of variants) {
      ...
      if (decodingConfigs.length === 0) {
        console.log(`[APPLE]: FAILED not getting decodingInfos, non-preferredKeySystem loop: getDecodingConfigs_ for variant: ${shaka.util.StreamUtils.getVariantSummaryString(variant)}`);
      }
  ...
  static getDecodingConfigs_(variant, usePersistentLicenses, srcEquals) {
    ...
    if (!allDrmInfos.length) {
      console.log(`[APPLE, StreamUtils]: no drmInfo for variant: ${shaka.util.StreamUtils.getVariantSummaryString(variant)}`);
      return [mediaDecodingConfigBatch];
    } else {
      console.log(`[APPLE, StreamUtils]: found ${allDrmInfos.length} drmInfos for variant: ${shaka.util.StreamUtils.getVariantSummaryString(variant)}`);
    }
   ...
   * @return {string}
   */
  static getVariantSummaryString(variant) {
    const summaries = [];
    if (variant.audio) {
