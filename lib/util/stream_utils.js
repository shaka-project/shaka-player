...
/**
 * Chooses variants according to the user's preferences.
 *
 * @param {!Array.<shaka.extern.Variant>} variants
 ...
 * @return {!Array.<!shaka.extern.Variant>}
 */
shaka.util.StreamUtils.filterVariantsByPreference = function(
...
shaka.util.StreamUtils.filterVariantsByRole_ =
  ...
  return variants.filter(function(variant) {
    return (variant.audio && variant.audio.roles.includes(preferredRole)) ||
           (variant.video && variant.video.roles.includes(preferredRole));
  });
};
...
/**
 * Get all unique streams in the given manifest.
 *
 * @param {shaka.extern.Manifest} manifest
 * @return {!Array.<shaka.extern.Stream>}
 */
shaka.util.StreamUtils.getAllStreams = function(manifest) {
  const streams = [];
  ...
  return shaka.util.ArrayUtils.removeDuplicates(streams);
};
