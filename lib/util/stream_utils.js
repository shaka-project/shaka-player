...
shaka.util.StreamUtils = class {
   ...
   */
  static filterManifestByHDRLevel(manifest, preferredVideoHdrLevel) {
    manifest.variants = manifest.variants.filter((variant) => {
      if (variant.video && variant.video.hdr != preferredVideoHdrLevel) {
        shaka.log.debug('Dropping Variant (incorrect HDR level)', variant);
        return false;
      }
      return true;
    });
