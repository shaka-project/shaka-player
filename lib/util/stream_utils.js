...
shaka.util.StreamUtils = class {
  ...
  static async getDecodingInfosForVariant_(variant, decodingConfig) {
    ...
    try {
      ...
      if (cache[cacheKey]) {
        ...
        const results = await Promise.all(shaka.util.StreamUtils
        ...
        for (const result of results) {
          if (!overallResult) {
            ...
            // Sadly, out compiler is out of date and does not recognize the
            // perfectly valid JS operator "&&=", so this much less elegant
            // code is required.
        ...
        goog.asserts.assert(overallResult, 'results should be found');
  ...
  /**
   * @param {!MediaDecodingConfiguration} decodingConfig
   * @return {!Array.<!Promise.<!MediaCapabilitiesDecodingInfo>>}
   * @private
   */
