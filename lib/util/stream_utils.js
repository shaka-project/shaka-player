...
shaka.util.StreamUtils = class {
  ...
  static chooseCodecsAndFilterManifest(manifest, preferredVideoCodecs,
    ...
    shaka.log.debug('--------', manifest.variants.length);
    ...
    if (preferredDecodingAttributes.length) {
      ...
      for (const variant of variants) {
        shaka.log.debug('-----w', variant.video.width);
        variantsByResolutionMap
            .push(String(variant.video.width || 0), variant);
      }
      shaka.log.debug('-----ppp', variantsByResolutionMap.size());
      const bestVariants = [];
      variantsByResolutionMap.forEach((width, variantsByResolution) => {
        ...
        for (const variant of variantsByResolution) {
          ...
          shaka.log.debug('pp', width, matchCount);
        ...
        shaka.log.debug('--y', matchingVariants.length);
        bestVariants.push(...matchingVariants);
      });
      variants = bestVariants;
    }

    if (preferredAudioChannelCount) {
    ...
    for (const variant of variants) {
      ...
      if (variant.audio) {
        key = `${
          variant.audio.language
        }-${
          variant.audio.channelsCount
        }-${
          variant.audio.bandwidth
        }-${
          variant.audio.roles.join('') + '/' + variant.audio.label
        }-${
    ...
    shaka.log.debug('ll', validAudioIds, validVideoIds);
