  ...
  static async filterImageStreams_(manifest) {
    ...
    for (const stream of manifest.imageStreams) {
      ...
      if (!shaka.util.StreamUtils.supportedImageMimeTypes_.has(mimeType) &&
          !shaka.util.StreamUtils.unsupportedImageMimeTypes_.has(mimeType)) {
        ...
        if (minImage) {
          ...
          if (res) {
            shaka.util.StreamUtils.supportedImageMimeTypes_.add(mimeType);
          } else {
            shaka.util.StreamUtils.unsupportedImageMimeTypes_.add(mimeType);
          }
        } else {
          shaka.util.StreamUtils.unsupportedImageMimeTypes_.add(mimeType);
        }
      ...
      const keep = shaka.util.StreamUtils.supportedImageMimeTypes_.has(mimeType);
 ...
 * @private {!Set.<string>}
 */
shaka.util.StreamUtils.supportedImageMimeTypes_ = new Set()
    .add('image/svg+xml')
    .add('image/png')
    .add('image/jpeg');
 ...
 * @private {!Set.<string>}
 */
shaka.util.StreamUtils.unsupportedImageMimeTypes_ = new Set();
