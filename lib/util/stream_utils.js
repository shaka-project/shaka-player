...
shaka.util.StreamUtils = class {
  ...
  static variantToTrack(variant) {
    ...
    if (video) {
      ...
      track.videoLanguage = video.language || null;
    ...
    if (audio) {
      ...
      track.audioLanguage = audio.language || null;
  ...
  static html5TrackToShakaTrack(audioTrack, videoTrack) {
    ...
    const track = {
      ...
      audioRoles: null,
      videoRoles: null,
      audioLanguage: audioTrack ?
          LanguageUtils.normalize(audioTrack.language || 'und') : null,
      videoLanguage: videoTrack ?
          LanguageUtils.normalize(videoTrack.language || 'und') : null,
      videoId: null,
      audioId: null,
      audioGroupId: null,
      channelsCount: null,
  ...
  static createStream(overrides) {
    ...
    // Derive fullMimeTypes from the mimeType and codecs.  Streams without a
    // mimeType (e.g. dummy streams) keep the default empty set.
    if (stream.mimeType) {
