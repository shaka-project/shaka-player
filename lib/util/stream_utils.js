...
shaka.util.StreamUtils.filterNewPeriod = function(
  ...
  for (var i = 0; i < period.variants.length; ++i) {
    ...
    if (!StreamUtils.isVariantCompatible_(
...
shaka.util.StreamUtils.isVariantCompatible_ =
  ...
  return StreamUtils.isStreamCompatible_(variant.audio,
                                         drmEngine,
                                         activeAudio) &&
         StreamUtils.isStreamCompatible_(variant.video, drmEngine, activeVideo);
};
