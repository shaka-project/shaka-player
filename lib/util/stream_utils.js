 ...
 */
// TODO: Split up variant and text track APIs
shaka.util.StreamUtils.getTracks = function(period, activeStreams) {
  ...
  var tracks = variants.map(function(variant) {
    ...
    if (variant.video && variant.audio) {
      isActive = activeStreams['video'] == variant.video &&
                 activeStreams['audio'] == variant.audio;
    } else {
      isActive = variant.video && activeStreams['video'] == variant.video ||
                 variant.audio && activeStreams['audio'] == variant.audio;
    }
    var codecs = '';
  ...
  var textTracks = period.textStreams
                  .map(function(stream) {
                     return {
                       id: stream.id,
                       active: activeStreams['text'] == stream,
                       type: 'text',
...
shaka.util.StreamUtils.filterVariantsByConfig = function(
  ...
  var foundPrimaryVariants = false;
  variants.forEach(function(variant) {
    if (variant.primary && !foundPrimaryVariants) {
      chosen = [variant];
      foundPrimaryVariants = true;
  ...
  var pref = config.preferredAudioLanguage;
  if (pref) {
 ...
 */
shaka.util.StreamUtils.filterTextStreamsByConfig = function(
    period, config, opt_languageMatches) {
  var LanguageUtils = shaka.util.LanguageUtils;
  ...
  var foundPrimaryStreams = false;
  streams.forEach(function(stream) {
    if (stream.primary && !foundPrimaryStreams) {
      chosen = [stream];
      foundPrimaryStreams = true;
    } else if (stream.primary) {
      chosen.push(stream);
    }
  });
  ...
  // base matches, finally different subtags.  Execute in reverse order so
  // the later steps override the previous ones.
  var pref = config.preferredTextLanguage;
  if (pref) {
    // TODO: Can we refactor this into a separate method used by both
    // variants and text in a clear non-confusing way?
    pref = LanguageUtils.normalize(pref);
    [LanguageUtils.MatchType.OTHER_SUB_LANGUAGE_OKAY,
     LanguageUtils.MatchType.BASE_LANGUAGE_OKAY,
     LanguageUtils.MatchType.EXACT]
