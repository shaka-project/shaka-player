...
shaka.util.StreamUtils = class {
  ...
  static chooseCodecsAndFilterManifest(manifest, preferredVideoCodecs,
    ...
    const getVideoGroupId = (stream) => {
      let id = Math.round(stream.frameRate || 0) + (stream.hdr || '') +
          stream.fastSwitching;
      if (stream.dependencyVideo) {
        id += stream.dependencyVideo.baseOriginalId || '';
      }
      return id;
    };
  ...
  static checkVariantSupported_(variant, keySystem, keySystemsMapping) {
    ...
    if (dependencyVideo &&
        !shaka.lcevc.Dec.isStreamSupported(dependencyVideo)) {
      return false;
    }
  ...
  static variantToTrack(variant) {
    ...
    if (video) {
      ...
      const dependencyVideo = video.dependencyVideo;
      if (dependencyVideo) {
        track.width = dependencyVideo.width || track.width;
        track.height = dependencyVideo.height || track.height;
        track.videoCodec = dependencyVideo.codecs || track.videoCodec;
      }
