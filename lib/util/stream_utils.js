...
shaka.util.StreamUtils = class {
  ...
  static meetsRestrictions(variant, restrictions, maxHwRes) {
    ...
    if (variant && variant.video && variant.video.codecs) {
      if (restrictions.restrictedVideoCodecs.includes(variant.video.codecs)) {
        return false;
      }
    }

    if (variant && variant.audio && variant.audio.codecs) {
      if (restrictions.restrictedAudioCodecs.includes(variant.audio.codecs)) {
        return false;
      }
    }
