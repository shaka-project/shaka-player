...
shaka.util.StreamUtils = class {
   ...
   * @param {shaka.extern.TrackComparator=} trackComparator
   */
  static chooseCodecsAndFilterManifest(manifest, preferredVideoCodecs,
    ...
    for (const stream of audioStreams) {
      ...
      if (!audioStreamsMap[id]) {
        audioStreamsMap[id] = [stream];
      } else {
        const includedStream = audioStreamsMap[id][0];
        if (includedStream.codecs === stream.codecs &&
          includedStream.bandwidth !== stream.bandwidth) {
          audioStreamsMap[id].push(stream);
        } else if (includedStream.bandwidth > stream.bandwidth) {
          audioStreamsMap[id] = [stream];
        }
      }
    }
    const validAudioStreams = [];
    for (const id in audioStreamsMap) {
      validAudioStreams.push(...audioStreamsMap[id]);
    }
        ...
        if (stream.width > previousStream.width ||
          ...
          if (previousCodec == currentCodec) {
            if (stream.bandwidth > previousStream.bandwidth) {
              validVideoStreams.push(stream);
            } else if (!stream.bandwidth && !previousStream.bandwidth) {
              validVideoStreams.push(stream);
            }
          }
        }
  ...
  /**
  * A function for filtering audio tracks for codec switching.
  * Tracks are filtered by unique language and channel count.
  * If tracks share the same language and channel count the one
  * with the least bandwidth is chosen, if there is parity between the
  * bandwidths then one which is within the `preferredAudioCodecs`
  * will be selected. Alternatively you can supply a comparator function
  * that will allow the user to select which language/channel count
  * variant should be preferred.
  * @suppress {checkTypes}
  * @param {!Array<shaka.extern.Variant>} variants
  * @param {!Array.<string>} preferredAudioCodecs
  * @param {shaka.extern.TrackComparator=} comparator
  * @return {!Array<shaka.extern.Variant>}
  * @private
  */
  static filterAudioTracksForCodecSwitching_(variants,
      preferredAudioCodecs,
      comparator) {
    const variantMap = new Map();

    for (const variant of variants) {
      const lang = variant.audio.language;
      const bandwidth = variant.audio.bandwidth || 0;
      const channelCount = variant.audio.channelsCount;
      const codecs = variant.audio.codecs;
      const width = variant.video.width;
      const videoCodecs = variant.video.codecs;
      // add codecs and width to key
      const trackKey =
        `${lang}-${channelCount}-${width}-${videoCodecs}`;
      if (!variantMap.has(trackKey)) {
        variantMap.set(trackKey, variant);
      } else {
        const currentVariant = variantMap.get(trackKey);
        if (comparator) {
          if (comparator(currentVariant.audio, variant.audio, 'audio') < 0) {
            variantMap.set(trackKey, variant);
          }
        } else {
          if (bandwidth < (currentVariant.audio.bandwidth || 0)) {
            variantMap.set(trackKey, variant);
          } else if (bandwidth == currentVariant.audio.bandwidth) {
            const currentCodecIndex = preferredAudioCodecs.findIndex(
                (codec) => currentVariant.audio.codecs.startsWith(codec));
            const nextCodecIndex = preferredAudioCodecs.findIndex(
                (codec) => codecs.startsWith(codec));
            if (nextCodecIndex < currentCodecIndex) {
              variantMap.set(trackKey, variant);
            }
          }
        }
  ...
  /**
  * A function for filtering video tracks for codec switching.
  * Tracks are filtered by unique language and channel count.
  * If tracks share the same language and channel count the one
  * with the least bandwidth is chosen, if there is parity between the
  * bandwidths then one which is within the `preferredVideoCodecs`
  * will be selected. Alternatively you can supply a comparator function
  * that will allow the user to select which variant should be preferred.
  * @suppress {checkTypes}
  * @param {!Array<shaka.extern.Variant>} variants
  * @param {!Array.<string>} preferredVideoCodecs
  * @param {shaka.extern.TrackComparator=} comparator
  ...
  * @private
  */
  static filterVideoTracksForCodecSwitching_(variants,
      preferredVideoCodecs,
      comparator) {
    const variantMap = new Map();

    for (const variant of variants) {
      const audiolang = variant.audio.language;
      const videoBandwidth = variant.video.bandwidth || 0;
      const channelCount = variant.audio.channelsCount;
      const codecs = variant.audio.codecs;
      const width = variant.video.width;
      const trackKey =
        `${audiolang}-${channelCount}-${width}`;
      if (!variantMap.has(trackKey)) {
        variantMap.set(trackKey, variant);
      } else {
        const currentVariant = variantMap.get(trackKey);
        if (comparator) {
          if (comparator(currentVariant.video, variant.video, 'video') < 0) {
            variantMap.set(trackKey, variant);
          }
        } else {
          if (videoBandwidth < (currentVariant.video.bandwidth || 0)) {
            variantMap.set(trackKey, variant);
          } else if (videoBandwidth == currentVariant.video.bandwidth) {
            const currentCodecIndex = preferredVideoCodecs.findIndex(
                (codec) => currentVariant.video.codecs.startsWith(codec));
            const nextCodecIndex = preferredVideoCodecs.findIndex(
                (codec) => codecs.startsWith(codec));
            if (nextCodecIndex < currentCodecIndex) {
              variantMap.set(trackKey, variant);
            }
  ...
  */
  ...
  static async filterManifest(drmEngine, currentVariant, manifest,
    ...
    // const Strategy = shaka.config.CodecSwitchingStrategy;
    ...
    // } else {
    shaka.log.debug('Skipping filtration due to codec switching strategy');
    // }
