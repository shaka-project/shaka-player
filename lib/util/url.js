...
shaka.util.URL = class {
  ...
  static getScheme(uri) {
    ...
    try {
      ...
      const scheme = new URL(normalizedUri).protocol.slice(0, -1);
      // Guard against environments where new URL('') returns about:blank
      // instead of throwing (e.g. old WebKit / Tizen 3).
      if (scheme && normalizedUri.toLowerCase().startsWith(scheme + ':')) {
        return scheme;
      }
    } catch (e) {
      // Ignore errors
    ...
    const match = normalizedUri.match(/^([a-zA-Z][a-zA-Z0-9+.-]*):/);
