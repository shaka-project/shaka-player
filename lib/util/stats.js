...
shaka.util.Stats = class {
  constructor() {
    ...
    /** @private {number} */
    this.timeToFirstFrameSeconds_ = NaN;
   ...
   * and the media element loading its metadata.
   *
  ...
  /**
   * Record the time it took between the user signalling "I want to play this"
   * to the first video frame being available for rendering.
   *
   * @param {number} seconds
   */
  setTimeToFirstFrame(seconds) {
    if (seconds >= 0) {
      this.timeToFirstFrameSeconds_ = seconds;
    }
  }
  ...
  getBlob() {
    return {
      ...
      timeToFirstFrame: this.timeToFirstFrameSeconds_,
