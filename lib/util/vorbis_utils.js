...
goog.provide('shaka.util.VorbisUtils');
...
shaka.util.VorbisUtils = class {
  ...
  static getVorbisComments(data) {
    ...
    if (shaka.util.VorbisUtils.isFlac_(data)) {
      frames.push(...shaka.util.VorbisUtils.parseFlac_(data));
    } else if (shaka.util.VorbisUtils.isOgg_(data)) {
      frames.push(...shaka.util.VorbisUtils.parseOgg_(data));
    }
  ...
  static parseFlac_(data) {
    ...
    while (offset + 4 <= data.length) {
      ...
      if (blockType == 4) {
        frames.push(
            ...shaka.util.VorbisUtils.parseVorbisCommentBlock_(block));
      }

      // PICTURE block
      if (blockType == 6) {
        const frame = shaka.util.VorbisUtils.parseFlacPicture_(block);
        if (frame) {
          frames.push(frame);
        }
      }
  ...
  static parseOgg_(data) {
    ...
    frames.push(
        ...shaka.util.VorbisUtils.parseVorbisCommentBlock_(slice));
  ...
  static parseVorbisCommentBlock_(data) {
    ...
    for (let i = 0; i < commentCount; i++) {
      ...
      frames.push({
        key: shaka.util.VorbisUtils.VORBIS_TO_ID3_MAP_[vorbisKey] || vorbisKey,
        data: value,
        description: '',
        mimeType: null,
        pictureType: null,
 ...
 * @private
 */
shaka.util.VorbisUtils.VORBIS_TO_ID3_MAP_ = {
  'TITLE': 'TIT2',
  'ARTIST': 'TPE1',
  'ALBUM': 'TALB',
  'ALBUMARTIST': 'TPE2',
