 ...
 *
 * @fileoverview Implements a content database writer.
 */
...
shaka.util.ContentDatabaseWriter = function(estimator, parent) {
  ...
  /** @private {number} */
  this.rangeRequestTimeout_ = shaka.player.Defaults.RANGE_REQUEST_TIMEOUT;
};
 ...
 *
 * @param {number} timeout The range request timeout in seconds.
 */
shaka.util.ContentDatabaseWriter.prototype.setRangeRequestTimeout =
    function(timeout) {
  shaka.asserts.assert(!isNaN(timeout));
  this.rangeRequestTimeout_ = timeout;
};
...
shaka.util.ContentDatabaseWriter.prototype.insertGroup = function(
    streamInfos, sessionIds, duration, drmScheme) {
...
shaka.util.ContentDatabaseWriter.prototype.requestSegment_ = function(
  ...
  params.requestTimeoutMs = this.rangeRequestTimeout_;
