...
shaka.lcevc.Dec = class {
  ...
  /**
   * Check for fullscreen support provided by LCEVCdec on webkit browsers.
  ...
  webkitSupportsFullscreen(fullScreenElement) {
    if (this.dec_ && this.dec_.webkitSupportsFullscreen) {
      return this.dec_.webkitSupportsFullscreen(fullScreenElement);
    }
    return false;
  }

  /**
   * Enter LCEVCdec fullscreen on webkit browsers.
   * @param {HTMLDivElement} fullScreenElement element to display in fullscreen
   */
  webkitEnterFullscreen(fullScreenElement) {
    if (this.dec_ && this.dec_.webkitEnterFullscreen) {
      this.dec_.webkitEnterFullscreen(fullScreenElement);
    }
  }

  /**
   * Exit LCEVCdec fullscreen on webkit browsers.
   * @param {HTMLDivElement} fullScreenElement exit fullscreen for this element
   */
  webkitExitFullscreen(fullScreenElement) {
    if (this.dec_ && this.dec_.webkitExitFullscreen) {
      this.dec_.webkitExitFullscreen(fullScreenElement);
    }
  }

  /**
   * Check if LCEVCdec is displaying in fullscreen on webkit browsers.
   * @return {boolean} true if currently displaying in fullscreen
   */
  webkitDisplayingFullscreen() {
    if (this.dec_ && this.dec_.webkitDisplayingFullscreen) {
      return this.dec_.webkitDisplayingFullscreen();
    }
    return false;
  }

  /**
   * Check if LCEVCdec handles device rotation events on webkit browsers.
   * @memberof Fullscreen
   * @return {boolean} true if LCEVCdec will handle device rotation events
   * @public
   */
  webkitHandlesOnRotationEvents() {
    if (this.dec_ && this.dec_.webkitHandlesOnRotationEvents) {
      return this.dec_.webkitHandlesOnRotationEvents();
    }
    return false;
  }
};
