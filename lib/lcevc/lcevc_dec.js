...
shaka.lcevc.Dec = class {
  ...
  appendBuffer(data, timestampOffset, stream) {
    ...
    // LCEVC only supports VIDEO, if the stream is dual track, the enhancement
    // stream should have no dependency.
    if (stream.type !== ContentType.VIDEO || stream.dependencyVideo) {
      return;
    }
    if (this.dec_) {
