...
shaka.cea.CeaDecoder = class {
  constructor() {
     ...
     * are up to 63 services, we create them dynamically only when we need them.
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (ccValid) {
        ...
        if (ccType === shaka.cea.CeaDecoder.NTSC_CC_FIELD_1 ||
          ...
          // The second byte should always be considered just data. i.e.
          // If this byte pair was a DTVCC packet start, only the first byte
          // contains header info, so only the first byte should be labelled as
          // DTVCC packet start.

          // TODO OR

          // The second byte should always be labelled as regular data,
          // regardless of whether this byte pair is DTVCC packet data or a DTVCC
          // packet start, since only the first byte contains header info.
          this.cea708DataArray_.push({
            byte: ccData2,
            pts,
  ...
  decode() {
    ...
    // algorithm isn't stable. This comparator breaks ties on receive order.
    ...
    // CEA-708 packets are DTVCC packets composed of many bytes.
    // Build, process, and clear them.
    ...
    shaka.log.info('got packet!');
    shaka.log.info(dtvccPackets);
    for (const dtvccPacket of dtvccPackets) {
      ...
      if (closedCaptions.length) {
        shaka.log.info('708 cues');
        shaka.log.info(...closedCaptions);
      }
    }
  ...
  decodeCea708_(dtvccPacket) {
    ...
    try {
      while (dtvccPacket.hasMoreData()) {
        ...
        const serviceBlockHeader = dtvccPacket.readBlock().byte;
        ...
        if (serviceNumber == /* b111 */ 0x07 && blockSize != 0) {
          ...
          const extendedServiceBlockHeader = dtvccPacket.readBlock().byte;
      ...
      error.code == shaka.util.Error.Code.BUFFER_READ_OUT_OF_BOUNDS) {
        shaka.log.warnOnce('CEA-708 INVALID DATA',
