...
shaka.cea.CeaDecoder = class {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (ccValid) {
        ...
        if (ccType === shaka.cea.CeaDecoder.NTSC_CC_FIELD_1 ||
          ...
          this.dtvccPacketBuilder_.addBytePair(cc1, cc2, pts, ccType);
        }
      }
    }
  }
  ...
  decode() {
    ...
    // algorithm isn't stable. This comparator sorts on pts, and breaks
    // ties based on receive order.
    const stableComparator = (data1, data2) => {
      const diff = data1.pts - data2.pts;
      const isEqual = diff === 0;
      return isEqual ? data1.order - data2.order : diff;
    };
    ...
    for (const dtvccPacket of dtvccPackets) {
      this.decodeCea708_(dtvccPacket);
    }
    this.dtvccPacketBuilder_.clearParsedPackets();

    this.cea608DataArray_.length = 0;
    return this.parsedClosedCaptions_;
  }
    ...
    while (dtvccPacket.hasMoreData()) {
      ...
      if (serviceNumber != 0) {
        ...
        if (!this.serviceNumberToService_.has(serviceNumber)) {
          const service = new shaka.cea.Cea708Service(this, serviceNumber);
