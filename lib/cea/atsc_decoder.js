...
shaka.cea.AtscDecoder = class {
  constructor() {
    ...
    this.cea708decoder_ = new shaka.cea.Cea708Decoder();
  ...
  /**
   * Decodes CEA-708 closed caption packets.
   * @param {shaka.cea.AtscDecoder.CcData} ccPacket
   * @private
   */
  decodeCea708_(ccPacket) {
    // A new DTVCC series is starting, so process and clear the old one.
    if (ccPacket.type === shaka.cea.AtscDecoder.DTVCC_PACKET_START) {
      this.cea708decoder_.processDtvccData();
      this.cea708decoder_.clearPacket();
    }

    // This is DTVCC data, so push the bytes.
    this.cea708decoder_.addBytes(ccPacket.cc1, ccPacket.cc2);
  }
 ...
 * @private @const {!number}
 */
shaka.cea.AtscDecoder.DTVCC_PACKET_START = 3;
