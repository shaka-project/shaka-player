...
shaka.cea.CeaDecoderUtils = class {
  ...
  static getParsedCaption(topLevelCue, stream, memory, startTime, endTime) {
    ...
    shaka.log.info('first non empty row: '+firstNonEmptyRow+
    ', last non empty row: '+lastNonEmptyRow);
    ...
    let currentTextColor = shaka.cea.CeaDecoderUtils.DEFAULT_TXT_COLOR;
    let currentBackgroundColor = shaka.cea.CeaDecoderUtils.DEFAULT_BG_COLOR;
    ...
    for (let i = firstNonEmptyRow; i <= lastNonEmptyRow; i++) {
      ...
      if (firstNonEmptyCol === -1 || lastNonEmptyCol === -1) {
        const linebreakCue = shaka.cea.CeaDecoderUtils
    ...
    if (topLevelCue.nestedCues.length) {
      shaka.log.info('emitting caption: '+topLevelCue.nestedCues);
      return {
        cue: topLevelCue,
        stream,
      };
 ...
 * @const {!string}
 */
shaka.cea.CeaDecoderUtils.DEFAULT_BG_COLOR = 'black';

/**
 * Default text color.
 * @const {!string}
 */
shaka.cea.CeaDecoderUtils.DEFAULT_TXT_COLOR = 'white';
