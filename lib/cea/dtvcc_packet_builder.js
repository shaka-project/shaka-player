...
shaka.cea.DtvccPacketBuilder = class {
  constructor() {
     ...
     * @private {?Array<!shaka.cea.Cea708Service.Cea708Block>}
     */
    this.currentPacket_ = null;

    /**
     * Stores the unsorted data for a particular series of extracts.
     */
    this.extracted708Data_ = [];
  ...
  /**
   * A single call to build() should be issued after calls to addByte().
   */
  build() {
    // Just like in CEA-608, we have to sort 708 data based on receive time.
    // Since built-in browser sorts are not guaranteed to be stable, we have
    // our own stable sort which breaks ties based on receive order.
    this.extracted708Data_.sort(
    /**
     * Stable sorting function.
     * @param {!shaka.cea.Cea708Service.Cea708Block} ccPacket1
     * @param {!shaka.cea.Cea708Service.Cea708Block} ccPacket2
     * @return {!number}
     */
        (ccPacket1, ccPacket2) => {
    ...
    for (const cea708Byte of this.extracted708Data_) {
   ...
   * @param {!number} type
   */
  addBytePair(byte1, byte2, pts, type) {
    const cea708Byte1 = {
      byte: byte1,
      pts,
      type,
      order: this.extracted708Data_.length,
    };

    // In the case of any pair of bytes, the second byte should never
    // be considered anything other than just data. i.e. If this byte pair was
    // a DTVCC packet start, only the first byte actually contains header info,
    // And so only the first byte should be labelled the DTVCC packet start.
    const cea708Byte2 = {
      byte: byte2,
      pts,
    ...
    this.extracted708Data_.push(cea708Byte1, cea708Byte2);
  }
...
shaka.cea.DtvccPacket = class {
  /**
   * @param {!Array<!shaka.cea.Cea708Service.Cea708Block>} packetData
   */
   ...
   * @return {!shaka.cea.Cea708Service.Cea708Block}
