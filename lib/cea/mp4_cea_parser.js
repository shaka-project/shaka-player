...
shaka.cea.Mp4CeaParser = class {
  ...
  parse(data, on708Data) {
    ...
    let timescale = shaka.cea.Parser.DEFAULT_TIMESCALE_VALUE;
  ...
  parseMdat_(reader, time, timescale, defaultSampleDuration,
    ...
    while (reader.hasMoreData()) {
      ...
      if (naluType == shaka.cea.Parser.NALU_TYPE_SEI) {
        const timeOffset = sampleData.length > 0 ?
      ...
      if (sampleSize == 0) {
        time += sampleData.length > 0 ? sampleData[sampleIndex].sampleDuration
          || defaultSampleDuration: defaultSampleDuration;

        sampleSize = sampleData.length > 0 ? sampleData[sampleIndex].sampleSize
          || defaultSampleSize : defaultSampleSize;
        sampleIndex++;
      }
    }
