...
shaka.cea.Mp4CeaParser = class {
  ...
  init(initSegment) {
    ...
    new Mp4Parser()
        ...
        .boxes(Mp4Parser.AV1_DV, codecBoxParser)
  ...
  parseMdat_(reader, time, timescale, defaultSampleDuration,
    ...
    for (const parsedTRUN of parsedTRUNs) {
      ...
      for (const sample of parsedTRUN.sampleData) {
        ...
        // AV1 has no NAL units: one sample is a bare sequence of OBUs, so it
        // is read whole and walked separately.
        if (this.bitstreamFormat_ == BitstreamFormat.AV1) {
          try {
            const sampleData = reader.readBytes(
                sampleSize,
                // Don't clone.  The sample is temporary and is not stored.
                /* clone= */ false);
            this.parseAv1Sample_(sampleData, pts, captionPackets);
          } catch (e) {
   ...
   * @private
   */
  parseAv1Sample_(sample, pts, captionPackets) {
    const Mp4CeaParser = shaka.cea.Mp4CeaParser;
    let offset = 0;
    ...
    while (offset < sample.length) {
      ...
      const obuHeader = sample[offset++];
      const obuType = (obuHeader >> 3) & 0x0f;
      const obuExtensionFlag = (obuHeader >> 2) & 0x01;
      const obuHasSizeField = (obuHeader >> 1) & 0x01;
      ...
      let obuEnd;
      if (obuHasSizeField) {
        const obuSize = Mp4CeaParser.readLeb128_(sample, offset);
        if (!obuSize) {
          return;
        }
        offset += obuSize.size;
        obuEnd = offset + obuSize.value;
      } else {
        // Without a size field the OBU extends to the end of the sample, so it
        // is necessarily the last one.
        obuEnd = sample.length;
      }

      if (obuEnd > sample.length) {
        // Malformed sample; anything past this point would be misread.
        return;
      }

      if (obuType == Mp4CeaParser.OBU_TYPE_METADATA_) {
        const metadataType = Mp4CeaParser.readLeb128_(sample, offset);
        if (metadataType &&
            metadataType.value == Mp4CeaParser.METADATA_TYPE_ITUT_T35_) {
          // What follows is itu_t_t35(), the same payload an SEI message of
          // type user_data_registered_itu_t_t35 carries. The trailing bits
          // byte at the end of the OBU is harmless: the decoder reads the
          // caption constructs by count.
          captionPackets.push({
            packet: sample.subarray(offset + metadataType.size, obuEnd),
            pts,
            format: shaka.media.ClosedCaptionParser.CaptionPacketFormat.SEI,
          });
        }
  ...
  static readLeb128_(data, offset) {
    ...
    // continuation bit. Multiplication is used instead of a shift because the
    ...
    for (let i = 0; i < 8; i++) {
      ...
      value += (byte & 0x7f) * Math.pow(2, 7 * i);
      if (!(byte & 0x80)) {
 ...
 * @private
 */
shaka.cea.Mp4CeaParser.OBU_TYPE_METADATA_ = 5;
 ...
 * @private
 */
shaka.cea.Mp4CeaParser.METADATA_TYPE_ITUT_T35_ = 4;
...
shaka.cea.Mp4CeaParser.CodecBitstreamMap_ = new Map()
    ...
    // AV1
    .set('av01', shaka.cea.Mp4CeaParser.BitstreamFormat.AV1)
    // Dolby Vision based in AV1
    .set('dav1', shaka.cea.Mp4CeaParser.BitstreamFormat.AV1);
