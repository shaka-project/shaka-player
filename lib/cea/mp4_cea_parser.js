...
shaka.cea.Mp4CeaParser = class {
  ...
  parseAv1Sample_(sample, pts, captionPackets) {
    ...
    for (const obu of shaka.util.Obu.parseAv1(sample)) {
      if (obu.type != Mp4CeaParser.OBU_TYPE_METADATA_) {
        continue;
      }
      const metadataType = shaka.util.Obu.readLeb128(obu.data, 0);
      if (!metadataType ||
          metadataType.value != Mp4CeaParser.METADATA_TYPE_ITUT_T35_) {
        continue;
      }
      // What follows is itu_t_t35(), the same payload an SEI message of type
      // user_data_registered_itu_t_t35 carries. The trailing bits byte at the
      // end of the OBU is harmless: the decoder reads the caption constructs
      // by count.
      captionPackets.push({
        packet: obu.data.subarray(metadataType.size),
        pts,
        format: shaka.media.ClosedCaptionParser.CaptionPacketFormat.SEI,
      });
    }
