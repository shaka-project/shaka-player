...
shaka.cea.Mp4CeaParser = class {
  ...
  parseMdat_(reader, time, timescale, defaultSampleDuration,
    ...
    let sampleIndex = 0;
    let sampleSize = sampleData.length ? sampleData[0].sampleSize
    ...
    while (reader.hasMoreData()) {
      ...
      if (naluType == shaka.cea.CeaParser.NALU_TYPE_SEI) {
        const timeOffset = sampleData.length > sampleIndex ?
            (sampleData[sampleIndex].sampleCompositionTimeOffset || 0)/timescale
             : 0;

        const seiProcessingTime = time/timescale + timeOffset/timescale;
        this.seiProcessor_.append(reader.readBytes(naluSize - 1));
        this.seiProcessor_.process(seiProcessingTime, on708Data);
      } else {
