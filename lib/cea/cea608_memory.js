...
shaka.cea.Cea608Memory = class {
  ...
  forceEmit(startTime, endTime) {
    ...
    // The line is interpreted as the top edge of the caption (lineAlign
    // defaults to START), and the rows grow downward from there. Anchor it to
    // the first row that actually contains text, instead of the cursor row, so
    // that every row lands on its grid line and multi-line captions don't
    // overflow off the bottom of the screen.
    const firstNonEmptyRow = this.rows_.findIndex(
        (row) => row.some((c) => c.getChar().trim() != ''));
    const line = Cea608Memory.ROW_TO_LINE_CONVERSION_.get(firstNonEmptyRow);
    if (line) {
