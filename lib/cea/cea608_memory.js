...
shaka.cea.Cea608Memory = class {
  ...
  forceEmit(startTime, endTime) {
    ...
    // Keeps track of the current for a cue being emitted. Default values.
    ...
    let currentCue = new shaka.text.Cue(startTime, endTime, '');
    currentCue.color = currentTextColor;
    currentCue.backgroundColor = currentBackgroundColor;

    // Logic: Reduce rows into a single top level cue containing nested cues.
    // Each nested cue corresponds to a style change. \n is used for new lines.
    const topLevelCue = new shaka.text.Cue(startTime, endTime, '');
    let seenNonEmptyRow = false;
    for (let i = 0; i < this.rows_.length; i++) {
      for (const styledChar of this.rows_[i]) {
        seenNonEmptyRow = true;
        ...
        if (underline != currentUnderline || italics != currentItalics ||
          ...
          if (currentCue.payload) {
            // Remove only spaces (' ') from front end and back of the payload.
            currentCue.payload = currentCue.payload.replace(/^ +| +$/g, '');
            topLevelCue.nestedCues.push(currentCue);
          }
          currentCue = new shaka.text.Cue(startTime, endTime, '');

          if (underline) {
            currentCue.textDecoration.push(
                shaka.text.Cue.textDecoration.UNDERLINE);
          }

          if (italics) {
            currentCue.fontStyle = shaka.text.Cue.fontStyle.ITALIC;
          }

          currentCue.color = textColor;
          currentCue.backgroundColor = backgroundColor;
    ...
    // Trim all \n and spaces after and including the last non-empty line.
    for (let i = topLevelCue.nestedCues.length-1; i >= 0; i--) {
      const nestedCue = topLevelCue.nestedCues[i];

      // Following line is equal to trimRight(), but works for all browsers.
      nestedCue.payload = nestedCue.payload.replace(/\s+$/, '');
      if (nestedCue.payload.length) {
        break;
      }
      topLevelCue.nestedCues.pop();
    }

    if (topLevelCue.nestedCues.length) {
      this.decoder_.addParsedClosedCaption({
        cue: topLevelCue,
        stream,
      });
    }
