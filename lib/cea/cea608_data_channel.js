...
shaka.cea.Cea608DataChannel = class {
  ...
  controlPac_(b1, b2) {
    ...
    this.curbuf.setUnderline(underline);
    this.curbuf.setItalics(italics);
    this.curbuf.setTextColor(textColor);

    // Clear the background color, since new row (PAC) should reset ALL styles.
    this.curbuf.setBackgroundColor(shaka.cea.Cea608Memory.DEFAULT_BG_COLOR);
  }
  ...
  controlMidrow_(b2) {
    ...
    this.curbuf.setUnderline(false);
    this.curbuf.setItalics(false);
    this.curbuf.setTextColor(shaka.cea.Cea608Memory.DEFAULT_TXT_COLOR);
  ...
  handleControlCode(ccPacket) {
    ...
    if (this.isPAC_(b1, b2)) {
      ...
      this.controlMiscellaneous_(ccPacket);
    }
  }
