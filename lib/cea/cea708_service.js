...
shaka.cea.Cea708Service = class {
  ...
  setPenColor_(dtvccPacket) {
    ...
    const foregroundColor = this.rgbColorToCssColor_(
        foregroundRed, foregroundGreen, foregroundBlue, foregroundByte >> 6);

    const backgroundColor = this.rgbColorToCssColor_(
        backgroundRed, backgroundGreen, backgroundBlue, backgroundByte >> 6);
   ...
   * @param {number=} opacity CEA-708 two-bit opacity
   * @return {string}
   * @private
   */
  rgbColorToCssColor_(red, green, blue, opacity = 0) {
    // CEA-708 opacity: solid=0, flash=1, translucent=2, transparent=3.
    // Preserve the existing solid fallback for flash (no animation support).
    if (opacity === 3) {
      return 'rgba(0, 0, 0, 0)';
    }
    if (opacity === 2) {
      return `rgba(${(red >> 1) * 255}, ${(green >> 1) * 255}, ` +
          `${(blue >> 1) * 255}, 0.5)`;
    }
