...
shaka.cea.Cea708Service = class {
  ...
  handleCea708ControlCode(dtvccPacket) {
    ...
    if (controlCode >= 0x00 && controlCode <= 0x1f) {
      ...
      this.handleG2_(controlCode);
    } else if (controlCode >= 0x10a0 && controlCode <= 0x10ff) {
      this.handleG3_(controlCode);
    }
  ...
  * getSpecifiedWindowIds_(bitmap) {
  ...
  toggleWindows_(windowsBitmap, pts) {
    ...
    for (const windowId of this.getSpecifiedWindowIds_(windowsBitmap)) {
      ...
      // Regardless of which way we're toggling, we should set a new start time.
      window.setStartTime(pts);
      window.toggle();
    }
    return parsedClosedCaption;
  ...
  reset_(pts) {
    ...
    this.deleteWindows_(allWindowsBitmap, pts);
    this.clear();
  }
  ...
  defineWindow_(dtvccPacket, windowNum, pts) {
    ...
    this.windows_[windowNum].update(visible, verticalAnchor, toggleRelative,
        horAnchor, rowCount, anchorId, colCount);
  ...
  rgbColorToHex_(red, green, blue) {
    shaka.log.info('converting rgb to hex: '+red+' '+green+' '+blue);
    // In CEA-708 R,G,B are each from 0-3. To map them to
    // CSS (0-255), multiply by 85.
    red *= 85;
    green *= 85;
    blue *= 85;
