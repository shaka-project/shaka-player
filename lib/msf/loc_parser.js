...
shaka.msf.LOCParser = class {
   ...
   *   codec, used to read the variable-length integers inside the Object
   *   Properties block. Draft-17 changed that encoding, so the same property
   ...
   */
  constructor(codec, frameDuration, normalizedCodec) {
  ...
  readVi64At_(buffer, offset) {
    ...
    const bytesRead = this.codec_.varIntLength(buffer[offset]);
    if (offset + bytesRead > buffer.length) {
      throw new Error(
          `LOCParser.readVi64At: need ${bytesRead} bytes`);
    }
    return {value: this.codec_.decodeVarInt(
