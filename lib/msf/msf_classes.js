...
shaka.msf.Reader = class {
   ...
   * @param {!shaka.extern.MsfCodec} codec The primitive codec of the negotiated
   *   draft. Integer encoding differs between drafts, so the reader cannot
   *   assume one.
   */
  constructor(buffer, stream, codec) {
    /** @private {!Uint8Array} */
    this.buffer_ = buffer;
  ...
  async u62WithSize() {
    await this.fillTo_(1);
    const bytesRead = this.codec_.varIntLength(this.buffer_[0]);
    await this.fillTo_(bytesRead);
    const value = this.codec_.decodeVarInt(this.slice_(bytesRead));
    return {value, bytesRead};
  }

  /**
   * Reads a count-prefixed list of Key-Value-Pairs whose types are absolute.
   *
   * This is the pre-draft-16 encoding, used only by draft-14; draft-16 and
   * later delta encode the types. See deltaKeyValuePairs().
   *
  ...
  /**
   * Cancels the underlying stream. This is best-effort cleanup: when the
   * session is already gone, cancelling rejects with the same error that
   * tore the session down, and there is nothing left to do about it. Callers
   * fire this from a finally block without awaiting it, so a rejection here
   * would surface as an unhandled rejection.
   *
   * This cancels through the reader rather than releasing the lock first:
   * older browsers (e.g. Chromecast) throw from releaseLock() while a read()
   * is still pending, which is the normal state of a stream being listened
   * on. Cancelling settles any pending read() with done, on every browser.
   *
   * @return {!Promise}
   */
  async close() {
    try {
      await this.reader_.cancel('Reader closed');
    } catch (error) {
      shaka.log.debug('Ignoring error while cancelling a stream:', error);
    }
  }
...
shaka.msf.Writer = class {
  ...
  /**
   * Aborts the underlying stream. Like Reader.close(), this is best-effort
   * cleanup: once the session is gone there is nothing left to abort.
   *
   * @return {!Promise}
   */
  async close() {
    try {
      await this.writer_.abort('Writer closed');
    } catch (error) {
      shaka.log.debug('Ignoring error while aborting a stream:', error);
    }
  }
};
