...
shaka.msf.Reader = class {
  ...
  slice_(size) {
    const result = new Uint8Array(
        this.buffer_.buffer, this.buffer_.byteOffset, size);
...
shaka.msf.ControlStream = class {
  ...
  lock_() {
    ...
    const lock = this.mutex_.then(() => done);

    // Update the mutex
    this.mutex_ = lock.then(() => p);
