 ...
 *
 * Carrying both at once is unusual -- a track with a template SHOULD NOT also
 * have a timeline track (MSF section 5.2.15) -- but it is not forbidden, and
 * a single document MAY combine the two formats (section 7.1). Since it is a
 * SHOULD NOT rather than a MUST NOT, a publisher that does it is served
 * rather than rejected, and the precedence above is what resolves it.
 *
 * @see https://datatracker.ietf.org/doc/draft-ietf-moq-msf/ section 7
 *
 * @final
 */
shaka.msf.MediaTimeline = class {
  ...
  constructor() {
    ...
    /**
     * The same entries, keyed by Location. It keeps a record repeated across
     * an incremental update from being stored twice, and answers a lookup by
     * Location without walking the list.
     *
     * @private {!Map<string, shaka.msf.MediaTimeline.Entry>}
     */
    this.entriesByLocation_ = new Map();
  ...
  addObject(data, independent) {
    ...
    if (discarded && !entries.length && independent) {
      // Nothing survived an Object that is supposed to carry the whole
      // timeline, which is a publisher-side problem rather than content
      // ageing out. Keeping the records this Object replaces is not the safer
      // answer: it would leave the player offering a seek range built from a
      // document the publisher has already superseded, and the media behind
      // it may be gone. Emptying the timeline costs the DVR window and
      // nothing else -- playback carries on at the live edge.
      shaka.log.warning(
          'A media timeline object carried no usable record at all; ' +
          'the presentation has no seek range until a usable one arrives');
    }

    if (independent) {
      this.entries_ = [];
      this.entriesByLocation_.clear();
    ...
    for (const entry of entries) {
      ...
      if (this.entriesByLocation_.has(key)) {
        continue;
      }
      this.entriesByLocation_.set(key, entry);
  ...
  timeForLocation(location) {
    const entry = this.entriesByLocation_.get(
        shaka.msf.MediaTimeline.key_(location.group, location.object));
    if (entry) {
      return entry.time;
    }
  ...
  /**
   * Expands a template at one index, per the formulas in MSF section 7.4.1.
   *
  ...
  static parseEntry_(record) {
    // Only the three items the draft defines are read, and a record carrying
    // more is taken as one a later draft extended: MSF tells a parser to
    // ignore what it does not understand, so extra items are left alone
    // rather than making the record invalid.
