...
shaka.msf.MSFTransport = class {
  ...
  async connect(uri, fingerprint, authorizationToken) {
    // Only require `window.WebTransport` when the default factory will run.
    // Custom factories (Direct Sockets, AMT tunnel, in-process test fakes)
    // are the whole reason `transportFactory` exists — gating them on the
    // global being present would defeat the integration point. The default
    // factory itself constructs `new WebTransport(...)` and would throw
    // here without the guard, so the contract is preserved for default
    // callers.
    const usingDefaultFactory = this.config_.transportFactory ===
        shaka.util.PlayerConfiguration.defaultTransportFactory;
    if (usingDefaultFactory && !window.WebTransport) {
    ...
    // Use the configured transport factory so applications can swap in a
    // non-WebTransport transport (Direct Sockets, AMT tunnel, test fake) by
    // setting `msf.transportFactory` on the player config. The default
    // factory returns `new WebTransport(uri, options)`, preserving existing
    // behavior. The factory is allowed to return either a WebTransport or
    // a Promise resolving to one.
    this.webTransport_ = await this.config_.transportFactory(uri, options);
