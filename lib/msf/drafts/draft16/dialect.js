 ...
 *
 * Deprecated: draft-17 replaced all three of those, so draft-16 is a separate
 * wire protocol from everything after it rather than an older revision of one,
 * and keeping it means carrying a second control plane and a second var int
 * encoding in every build. It will be removed in v6.
 *
...
shaka.msf.draft16.Dialect = class {
  ...
  async connect(webTransport, config, authorizationToken) {
    shaka.Deprecate.deprecateFeature(6,
        'MoQT draft-16 support',
        'Draft-16 is superseded by draft-20 and draft-21, and will be ' +
        'removed in v6. Set manifest.msf.version to ' +
        'shaka.config.MsfVersion.DRAFT_21 (or leave it as AUTO) and use a ' +
