...
shaka.msf.TracksManager = class {
  ...
  async handleIncomingStream_(stream) {
    ...
    try {
      ...
      const isSubgroupHeader =
        (streamType >=
            shaka.msf.TracksManager
                .SUBGROUP_HEADER_WITHOUT_EOG_START_BIGINT &&
         streamType <=
            shaka.msf.TracksManager
                .SUBGROUP_HEADER_WITHOUT_EOG_END_BIGINT) ||
        (streamType >=
            shaka.msf.TracksManager
                .SUBGROUP_HEADER_WITH_EOG_START_BIGINT &&
         streamType <=
            shaka.msf.TracksManager
                .SUBGROUP_HEADER_WITH_EOG_END_BIGINT);
      if (isSubgroupHeader) {
  ...
  async handleSubgroupStream_(reader, streamType) {
    try {
      ...
      } else if (streamType === 0x14 || streamType === 0x15 ||
 ...
 * @private @const {number}
 */
shaka.msf.TracksManager.SUBGROUP_HEADER_WITHOUT_EOG_START_BIGINT = 0x10;

/**
 * @private @const {number}
 */
shaka.msf.TracksManager.SUBGROUP_HEADER_WITHOUT_EOG_END_BIGINT = 0x15;

/**
 * @private @const {number}
 */
shaka.msf.TracksManager.SUBGROUP_HEADER_WITH_EOG_START_BIGINT = 0x18;

/**
 * @private @const {number}
 */
shaka.msf.TracksManager.SUBGROUP_HEADER_WITH_EOG_END_BIGINT = 0x1D;
