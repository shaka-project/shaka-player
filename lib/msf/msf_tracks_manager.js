      ...
      const isSubgroupHeader =
        (streamType >=
            shaka.msf.TracksManager
                .SUBGROUP_HEADER_WITHOUT_EOG_START &&
         streamType <=
            shaka.msf.TracksManager
                .SUBGROUP_HEADER_WITHOUT_EOG_END) ||
        (streamType >=
            shaka.msf.TracksManager
                .SUBGROUP_HEADER_WITH_EOG_START &&
         streamType <=
            shaka.msf.TracksManager
                .SUBGROUP_HEADER_WITH_EOG_END);
      if (isSubgroupHeader) {
        await this.handleSubgroupStream_(reader, streamType);
      } else if (streamType === shaka.msf.TracksManager.FETCH_HEADER) {
