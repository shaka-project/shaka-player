...
shaka.msf.MSFParser = class {
  ...
  processCatalog_(catalog) {
    ...
    const willPreprocess = catalogPreprocessor != defaultCatalogPreprocessor;

    // A preprocessor returns nothing, so the only way it can do anything is by
    // mutating the catalog it was handed. A console keeps a logged object by
    // reference and renders it when it is expanded rather than when it was
    // logged, so logging the same object on both lines below would show the
    // processed catalog under both labels -- and the line that is supposed to
    // show what arrived on the wire would be the one lie in the pair. The
    // unprocessed line therefore logs a copy.
    //
    // The copy is made only when a preprocessor will actually run: nothing
    // else here mutates the catalog, and a silenced shaka.log.info is an empty
    // function rather than a call the compiler removes, so its arguments are
    // evaluated in every build.
    shaka.log.info('MSF Catalog:',
        willPreprocess ? shaka.msf.MSFParser.copyCatalog_(catalog) : catalog);
    if (willPreprocess) {
  ...
  /**
   * Copies a catalog, so that a log line holds what the catalog was rather
   * than a reference to what it is about to become.
   *
   * A catalog is the JSON the publisher sent, parsed, so serializing it round
   * trips exactly and cannot meet a value it fails on.
   *
   * @param {msfCatalog.Catalog} catalog
   * @return {msfCatalog.Catalog}
   * @private
   */
  static copyCatalog_(catalog) {
  ...
  processTrack_(track, contentProtections, initDataList) {
    ...
    stream.createSegmentIndex = () => {
      ...
      const segmenter =
          packaging.createSegmenter(this.msfTransport_.getCodec());
      this.subscribeToTrack(track, trackKey, (obj) => {
        ...
        if (obj.data.byteLength) {
          this.recordBandwidthSample_(trackKey, obj.location,
              obj.data.byteLength,
              obj.receiveTimestampMs - obj.payloadReadStartMs);
        }

        for (const segment of segmenter.push(obj)) {
          this.addSegment_(stream, type, segment,
              description.initSegmentReference);
    ...
    stream.closeSegmentIndex = () => {
      ...
      this.bandwidthSamples_.delete(trackKey);
