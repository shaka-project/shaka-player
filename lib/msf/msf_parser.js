  ...
  processTrack_(track, contentProtections) {
    ...
    stream.createSegmentIndex = () => {
      ...
      this.subscribeToTrack(track, trackKey, (obj) => {
        ...
        const deltaTimeMs = obj.receiveTimestampMs - obj.payloadReadStartMs;
        if (deltaTimeMs > 0) {
          this.playerInterface_.onSegmentReceived(
              deltaTimeMs, segmentData.byteLength);
        }
        ...
        if (!this.config_.disableText && !isLoC &&
          ...
          const videoInfo = shaka.media.SegmentUtils.getBasicInfoFromMp4(
              initData, segmentData, /* disableText= */ false);
