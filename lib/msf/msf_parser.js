...
shaka.msf.MSFParser = class {
  ...
  recordBandwidthSample_(trackKey, location, byteLength, readMs) {
    let sample = this.bandwidthSamples_.get(trackKey);
    if (!sample) {
      sample = {bytes: 0, readMs: 0, group: location.group};
      this.bandwidthSamples_.set(trackKey, sample);
    }
  ...
  processTrack_(track, contentProtections, initDataList) {
    ...
    const isLoC = track.codec && track.packaging === 'loc';

    /** @type {?shaka.media.SegmentUtils.BasicInfo} */
    let basicInfo = null;
    const validCmafPackagings = [
      'cmaf',
      'chunk-per-object',
    ];
    if (validCmafPackagings.includes(track.packaging)) {
      basicInfo = shaka.media.SegmentUtils.getBasicInfoFromMp4(
          initData, initData, /* disableText= */ false);
    } else if (isLoC) {
      const mimeType = `moq/loc; codecs="${track.codec}"`;
      basicInfo = shaka.media.SegmentUtils.getBasicInfoFromMimeType(
          mimeType);
    }
    ...
    let locParser = null;
    if (isLoC) {
      // For LOC tracks compute the fixed frame duration from MSF catalog
      // fields so that timing never depends on the optional LOC Timestamp
      // property.
      const frameDuration =
          shaka.msf.LOCParser.frameDurationFromTrack(track);
      if (frameDuration === null) {
        shaka.log.warning(`LOC track "${track.name}" has no usable frame ` +
            'duration; skipping.');
        return;
      }
      locParser = new shaka.msf.LOCParser(frameDuration);
    }
    ...
    stream.createSegmentIndex = () => {
      ...
      this.subscribeToTrack(track, trackKey, (obj) => {
        /** @type {!Uint8Array} */
        let segmentData = obj.data;

        if (!stream.segmentIndex || !segmentData.byteLength) {
          return;
        }
        ...
        if (isLoC) {
          goog.asserts.assert(locParser, 'locParser must be non-null');
          const locResult = locParser.parse(obj);
          duration = locResult.duration;
          startTime = locResult.startTime;
          segmentData = locResult.payload;
        } else {
          ...
          const info = shaka.media.SegmentUtils.getStartTimeAndDurationFromMp4(
              segmentData, timescale);
          startTime = info.startTime;
          duration = info.duration;
        }
        if (!duration || !segmentData.byteLength) {
          return;
        }
        ...
        reference.setSegmentData(segmentData);
