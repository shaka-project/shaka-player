...
goog.requireType('shaka.msf.IControlStream');
...
shaka.msf.RequestIdSession = class {
   ...
   * @param {!shaka.msf.IControlStream} controlStream
  ...
  constructor(webTransport, controlStream, dialect, config) {
    ...
    /** @private {!shaka.msf.IControlStream} */
  ...
  async subscribe(namespace, trackName, callback, startLocation) {
    ...
    const subscribeMsg = {
      ...
      forward: true,
      // A start behind the live edge is what AbsoluteStart says, and both of
      // these drafts carry it the same way: draft-14 as fields of the message
      // after the Filter Type, draft-16 inside the SUBSCRIPTION_FILTER
      // parameter. It is open ended, so it runs from there to the live edge
      // and onwards, and the configured filter does not apply -- that one
      // says where to start when nothing else does.
      filterType: startLocation ?
          shaka.config.MsfFilterType.ABSOLUTE_START :
          this.config_.subscribeFilterType,
