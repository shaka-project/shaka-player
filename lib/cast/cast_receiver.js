...
shaka.cast.CastReceiver = class extends shaka.util.FakeEventTarget {
  ...
  setContentAlbumName(albumName) {
    this.ensureMetadata_();
    this.metadata_['albumName'] = albumName;
    this.metadata_['metadataType'] =
        cast.receiver.media.MetadataType.MUSIC_TRACK;
  }
  ...
  async initState_(initState, appData) {
    ...
    if (initState['manifest']) {
      ...
      try {
        ...
        if (this.player_) {
          this.player_.dispatchEvent(new shaka.event.Error(error));
        }
        return;
      }
    } else {
  ...
  pollAttributes_() {
    ...
    if (this.adManager_) {
      shaka.cast.CastUtils.LargeAdManagerGetterMethods.forEach(
          (frequency, name) => {
            if (this.updateNumber_ % frequency == 0) {
              const update = {'adManager': {}};
              update['adManager'][name] =
                /** @type {Object} */ (this.adManager_)[name]();
