...
shaka.cast.CastUtils = class {
  ...
  static serialize(thing) {
    return JSON.stringify(thing, (key, value) => {
      ...
      const isFakeEvent = value instanceof shaka.util.FakeEvent;
      if (value instanceof Event || isFakeEvent) {
...
shaka.cast.CastUtils.LargePlayerGetterMethods = new Map()
    ...
    .set('getNonDefaultConfiguration', 4)
    ...
    .set('getFetchedPlaybackInfo', 2)
    .set('getAllMetadataRegions', 1)
    .set('getAllEmsgRegions', 1)
    .set('getAllTimelineRegions', 1)
    .set('getAllScte35Events', 1);
...
/**
 * Ad manager getter methods with data large enough to be sent in their own
 * update  messages, to reduce the size of each message.
 * @const {!Map<string, number>}
 */
shaka.cast.CastUtils.LargeAdManagerGetterMethods = new Map()
    .set('getCuePoints', 4)
    .set('getStats', 5);


/**
 * Ad manager methods with no return value that are proxied while casting.
 * @const {!Array<string>}
 */
shaka.cast.CastUtils.AdManagerVoidMethods = [
  'setLocale',
  'configure',
  'requestClientSideAds',
  'updateClientSideAdsRenderingSettings',
  'addMediaTailorTrackingUrl',
  'replaceServerSideAdTagParameters',
  'addCustomInterstitial',
];


/**
 * Ad manager methods returning a Promise that are proxied while casting.
 * @const {!Array<string>}
 */
shaka.cast.CastUtils.AdManagerPromiseMethods = [
  'requestMediaTailorStream',
  'requestServerSideStream',
  'addAdUrlInterstitial',
];
 ...
 *   The playback mimeType, if currently playing.
