...
describe('DrmEngine', () => {
  ...
  describe('MediaKeySessionClosedReason', () => {
    ...
    it('handles session recreation failure due to missing metadata',
        async () => {
          let capturedError = null;
          onErrorSpy.and.callFake((error) => {
            capturedError = error;
          ...
          // Simulate missing metadata by clearing initData
          const activeSessions = drmEngine['activeSessions_'];
          const sessionMetadata = activeSessions.get(session1);
          sessionMetadata.initData = null;
    ...
    it('handles app-initiated close with hardware-context-reset',
        async () => {
          await initAndAttach();
          await sendEncryptedEvent('webm');
          ...
          // Setup session.closed to resolve with hardware-context-reset
          const closedPromise = new shaka.util.PublicPromise();
          session1.closed = closedPromise.then(() => 'hardware-context-reset');
          closedPromise.resolve();

          // Manually close the session (app-initiated)
          await drmEngine['closeSession_'](session1);

          // Should not recreate session for app-initiated close
          // even with hardware-context-reset reason
          expect(mockMediaKeys.createSession).not.toHaveBeenCalled();
        });

    it('handles close timeout without crashing', async () => {
      await initAndAttach();
      await sendEncryptedEvent('webm');

      // Make session.close() hang
      session1.close = jasmine.createSpy('close')
