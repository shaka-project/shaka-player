...
function defineStringUtilTests() {
  ...
  it('parses a 4-byte character with an odd lead byte', () => {
    // U+E0100 (a Plane-14 variation selector) is F3 A0 84 80.  Its lead byte
    // 0xF3 is odd, so masking with 0xf1 instead of 0xf8 would wrongly reject it
    // and produce a replacement character.
    const arr = [0xf3, 0xa0, 0x84, 0x80];
    expect(StringUtils.fromUTF8(new Uint8Array(arr)))
        .toBe('\udb40\udd00');
  });
