...
describe('VTT tag rendering', () => {
  ...
  const renderCue = async (cueText) => {
    ...
    const displayer = new shaka.ui.TextDisplayer(video, container);
  ...
  it('text is rendered into a single span', async () => {
    ...
    expect(container.childNodes[0].textContent).toBe(cueText);
  });
  ...
  it('renders a complex input into nested elements', async () => {
    ...
    cueText += '<b>Hello, </b>';
    cueText += '<i>world! </i>';
    cueText += '<a href="https://www.google.com">This is a </a>';
    cueText += '<i>complex</i> element.';
    cueText += '</u>';
    const container = await renderCue(cueText);
