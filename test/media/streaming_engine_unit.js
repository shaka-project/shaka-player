...
describe('StreamingEngine', () => {
  ...
  describe('handles seeks (VOD)', () => {
    ...
    it('clears buffers when a user seek follows an internal boundary seek',
        async () => {
          streamingEngine.switchVariant(variant);
          streamingEngine.switchTextStream(textStream);
          await streamingEngine.start();
          ...
          for (const type of [
            ...
            mediaSourceEngine.segments[type][0] = true;
          }

          const engine = /** @type {?} */(streamingEngine);
          engine.boundaryTime_ = 20.1;
          engine.crossBoundaryTimer_.tickNow();

          // This is the seek generated by StreamingEngine at the boundary.
          presentationTimeInSeconds = 20.1;
          streamingEngine.seeked();
          expect(mediaSourceEngine.clear).not.toHaveBeenCalled();

          // This user seek arrives before boundary reset processing completes.
