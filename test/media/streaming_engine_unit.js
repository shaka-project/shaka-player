...
describe('StreamingEngine', () => {
   ...
   * @param {HTMLVideoElement=} video Optional media element.
   */
  function createStreamingEngine(config, video) {
    ...
    const playerInterface = {
      ...
      video: video || shaka.test.UiUtils.createVideoElement(),
  ...
  describe('cross-boundary handling', () => {
    ...
    /**
     * Adds boundary metadata to the segment references, starts StreamingEngine
     * just before the boundary, and lets its initial updates complete.
     *
     * @param {string=} secondVideoMimeType The MIME type after the boundary.
     * @return {!Promise}
     */
    async function startNearBoundary(secondVideoMimeType = MIME_AVC) {
      ...
      for (const stream of streams) {
        const originalGet = stream.segmentIndex.get;
        stream.segmentIndex.get = (position) => {
          const reference = originalGet(position);
      ...
      presentationTimeInSeconds = BOUNDARY_TIME - 2;
    ...
    /**
     * Expects exactly one boundary-crossed event from StreamingEngine.
     */
    function expectSingleBoundaryEvent() {
      const boundaryEvents = onEvent.calls.allArgs()
          .map((args) => args[0])
          .filter((event) => event.type === 'boundarycrossed');
      expect(boundaryEvents.length).toBe(1);
    }

    describe('before crossing a boundary with RESET', () => {
      beforeEach(() => {
        const config =
            shaka.util.PlayerConfiguration.createDefault().streaming;
        config.crossBoundaryStrategy =
            shaka.config.CrossBoundaryStrategy.RESET;
        createStreamingEngine(config, video);
      });

      it('does not buffer beyond the boundary', async () => {
        ...
        expect(mediaSourceEngine.segments[ContentType.VIDEO])
            .toEqual([false, true, false, false]);
        expect(mediaSourceEngine.reset).not.toHaveBeenCalled();
      });
    });

    describe('crossing a boundary with RESET', () => {
      beforeEach(() => {
        const config =
            shaka.util.PlayerConfiguration.createDefault().streaming;
        config.crossBoundaryStrategy =
      ...
      it('uses media time when presentation time is clamped', async () => {
        await startNearBoundary();
        ...
        expect(video.currentTime).toBe(BOUNDARY_TIME - 0.5);

        jasmine.clock().tick(1);
        expect(video.currentTime).toBeCloseTo(BOUNDARY_TIME + 0.1);
      });
      ...
      it('resets after a boundary seek with clamped presentation time',
          async () => {
            ...
            // The media element reached the requested boundary time, while
            // the presentation time remains clamped behind it.
            streamingEngine.seeked();
            await Util.fakeEventLoop(5);
      ...
      it('clears buffers when a user seek follows a boundary seek',
          async () => {
            ...
            // Notify StreamingEngine of its automatic boundary seek first.
            streamingEngine.seeked();
            expect(mediaSourceEngine.clear).not.toHaveBeenCalled();

            // A user seek arrives before boundary processing completes.
            presentationTimeInSeconds = 35;
            video.currentTime = presentationTimeInSeconds;
            streamingEngine.seeked();
    ...
    describe('crossing a boundary with KEEP', () => {
      ...
      it('keeps MediaSource for the same codec and container', async () => {
        await startNearBoundary();
      ...
      it('resets MediaSource for a different codec', async () => {
        await startNearBoundary(MIME_HEVC);
