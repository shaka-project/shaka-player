...
describe('Player', () => {
  ...
  describe('EventStream callback', () => {
    const CALLBACK_SCHEME = 'urn:mpeg:dash:event:callback:2015';
     ...
     */
    function makeCallbackRegion(url, startTime = 25, endTime = 30) {
      return {
        schemeIdUri: CALLBACK_SCHEME,
        value: '1',
        startTime,
        endTime,
        id: '1',
        timescale: 1,
        eventNode: /** @type {shaka.extern.xml.Node} */({
          tagName: 'Event',
          attributes: {id: '1'},
          children: [url],
          parent: null,
     ...
     */
    function expectCallbackRequest(url, type) {
      expect(networkingEngine.request).toHaveBeenCalledWith(
          type, jasmine.objectContaining({uris: [url]}));
    }

    // eslint-disable-next-line @stylistic/max-len
    it('sends GET request to the callback URL when entering a callback region', async () => {
      await player.load(fakeManifestUri, 0, fakeMimeType);

      // FakeVideo starts at currentTime=20; initial poll fires during
      // load. Add a region starting at 25 so the playhead is before it.
      const callbackUrl = 'https://example.com/beacon/impression';
      const regionTimeline =
          (/** @type {?} */ (player))['regionTimeline_'];
      regionTimeline.addRegion(makeCallbackRegion(callbackUrl));

      // Move playhead into the region.
      video.currentTime = 27;

      await shaka.test.Util.delay(0.3);

      expectCallbackRequest(
          callbackUrl,
          shaka.net.NetworkingEngine.RequestType.EVENT_CALLBACK);
    });

    it('does not send a request for non-callback scheme regions', async () => {
      await player.load(fakeManifestUri, 0, fakeMimeType);
      ...
      for (const call of networkingEngine.request.calls.all()) {
    ...
    // eslint-disable-next-line @stylistic/max-len
    it('does not send a request for callback regions without a URL', async () => {
      await player.load(fakeManifestUri, 0, fakeMimeType);
    ...
    it('trims whitespace from the callback URL', async () => {
      await player.load(fakeManifestUri, 0, fakeMimeType);

      const callbackUrl = 'https://example.com/beacon/impression';
      // Simulate indented text content as it appears in a real MPD.
      const region =
          makeCallbackRegion('\n        ' + callbackUrl + '\n      ');

      const regionTimeline =
          (/** @type {?} */ (player))['regionTimeline_'];
      regionTimeline.addRegion(region);
  ...
  describe('language methods', () => {
    ...
    describe('getThumbnails', () => {
      ...
      it('returns no thumbnail if exceeds timeline', async () => {
        const uris = () => ['thumbnail'];
        const ref = new shaka.media.SegmentReference(
            0, 30, uris, 0, null, null, 0, 0, Infinity, [],
        );
        ref.trueEndTime = 60;
        const index = new shaka.media.SegmentIndex([ref]);
    ...
    describe('getAllThumbnails', () => {
      ...
      it('returns all thumbnails when duration is clamped', async () => {
        const uris = () => ['thumbnail'];
        const ref = new shaka.media.SegmentReference(
            0, 30, uris, 0, null, null, 0, 0, Infinity, [],
        );
